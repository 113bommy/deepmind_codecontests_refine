...
struct Segment_Tree {
  struct node {
    ...
    node operator+(const node &rhs) const {
      node res;
      if (mn < rhs.mn)
        res.lm = lm, res.rm = std ::max(rm, rhs.mx), res.ans = ans;
      else if (mn > rhs.mn)
        res.lm = std ::max(mx, rhs.lm), res.rm = rhs.rm, res.ans = rhs.ans;
      else
        res.lm = lm, res.rm = rhs.rm,
        res.ans = ans + rhs.ans + std ::max(rm, rhs.lm);
  ...
  int lz[200005 << 2];
  void up(int u) { data[u] = data[u << 1] + data[u << 1 | 1]; }
  void add(int u, int x) { data[u].mn += x, lz[u] += x; }
  void down(int u) {
  ...
  void modify2(int u, int l, int r, int x, int y) {
    ...
    down(u);
    if (x <= md)
      modify2(u << 1, l, md, x, y);
    else
      modify2(u << 1 | 1, md + 1, r, x, y);
    up(u);
  }
  int query() { return data[1].ans + data[1].lm + data[1].rm; }
} T;
void add(int x) {
...
void insert(int x, int y) { del(y), pos[y].insert(x), add(y); }
void erase(int x, int y) { del(y), pos[y].erase(x), add(y); }
int main() {
  n = read(), q = read();
  for (register int i = 1; i <= n; ++i)
    a[i] = read(), pos[a[i]].insert(i), m = std ::max(m, a[i]);
