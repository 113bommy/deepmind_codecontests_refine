...
const int N = 105;
int a[N][N], b[N][N], n, x;
inline long long jue(long long x) { return x > 0 ? x : -x; }
long long get(int li) {
  long long tp = 0;
  for (int i = 1; i < x; i++)
    tp += jue(a[li][i] + (b[li][x] ? -a[li][i + x] : a[li][i + x]) +
              (b[x][i] ? -a[li + x][i] : a[li + x][i]) +
              (b[x][i] ^ b[li + x][x] ? -a[li + x][i + x] : a[li + x][i + x]));
  tp += (b[li][x] ? -a[li][x] : a[li][x]) +
        (b[li + x][x] ? -a[li + x][x] : a[li + x][x]);
  return tp;
}
int main() {
  read(n);
  ...
  for (int i = 0; i <= all; i++) {
    ...
    for (int j = 1; j <= x; j++)
      if (i & (1 << (j - 1)))
        ...
        b[x][j] = 0;
    for (int j = 1; j < x; j++) b[x][x + j] = b[x][j] ^ b[x][x];
    for (int j = 1; j <= n; j++) res += b[x][j] ? -a[x][j] : a[x][j];
    for (int j = 1; j < x; j++) {
      long long tp = -1e15;
