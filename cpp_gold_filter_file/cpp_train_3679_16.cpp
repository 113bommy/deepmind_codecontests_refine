...
mat mult(mat &a, mat &b) {
  ...
  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++) {
      long long t = 0;
      for (int k = 0; k < 4; k++) t += 1ll * a.num[i][k] * b.num[k][j];
      ret.num[i][j] = t % 1000000007;
    }
  return ret;
}
...
int query(long long n, string &q) {
  ...
  if (!n) return f2;
  s = s2.substr(s2.size() - q.size() + 1, q.size() - 1) +
      s2.substr(0, q.size() - 1);
  int t1 = cal(s, q);
  s = s1.substr(s1.size() - q.size() + 1, q.size() - 1) +
      s1.substr(0, q.size() - 1);
  int t2 = cal(s, q);
  ...
  return (1ll * res.num[0][0] * f2 + 1ll * res.num[0][1] * f1 +
          1ll * res.num[0][2] * t1 + 1ll * res.num[0][3] * t2) %
         1000000007;
}
int main() {
  long long n, m;
