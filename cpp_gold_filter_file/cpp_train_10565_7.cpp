...
long double cot(point p, point q) { return 1.0 * (p * q) / (p ^ q); }
...
bool get(point p1, point p2) {
  ...
  for (int i = 1; i <= n; i++) {
    if (((p1 - a[i]) ^ (p2 - a[i])) == 0) {
      ...
      long double k = cot(p1 - a[i], p2 - a[i]);
      if (((p1 - a[i]) ^ (p2 - a[i])) > 0)
        l = max(l, k);
      else
        r = min(r, k);
  ...
  for (int i = 1; i <= m; i++) {
    if (((p1 - b[i]) ^ (p2 - b[i])) == 0) {
      ...
      long double k = cot(p1 - b[i], p2 - b[i]);
      if (((p1 - b[i]) ^ (p2 - b[i])) > 0)
        r = min(r, k);
      else
        l = max(l, k);
...
bool solve(int l, int r) {
  ...
  for (int i = l + 1; i <= r - 1; i++)
    if (dcmp(cot(a[l] - a[i], a[r] - a[i]) - cot(a[l] - a[p], a[r] - a[p])) > 0)
      p = i;
...
bool check(point *p, int _n, point *q, int _m) {
  ...
  int o = 1;
  for (int i = 1; i <= _n; i++) {
    while (n > o && ((a[n - 1] - a[n]) ^ (p[i] - a[n])) <= 0) n--;
    a[++n] = p[i];
  }
  o = n;
...
int main() {
  ...
  puts(_n == 1 || _m == 1 || check(M, _n, S, _m) || check(S, _m, M, _n) ? "YES"
                                                                        : "NO");
}
