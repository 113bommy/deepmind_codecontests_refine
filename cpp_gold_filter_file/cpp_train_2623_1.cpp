...
int64_t subtreeB[NMAX], subtree[NMAX];
...
void dfs1(int node, int prev) {
  ...
  for (int to : G[node]) {
    if (to != prev) {
      expLenDown[node] +=
          (1 + expLenDown[to] + (subtree[node] - 1 - subtree[to])) *
          subtreeB[to];
    }
  }
...
void dfs2(int node, int prev, double dpUp = 0) {
  ...
  if (prev != -1) {
    cdp += (subtreeB[1] - subtreeB[node]) * (dpUp + 1 + subtree[node] - 1);
  }
  ...
  for (int to : G[node]) {
    if (to != prev) {
      double dpd =
          cdp - subtreeB[to] * (1 + expLenDown[to] + n - 1 - subtree[to]);
      dpd -= (subtreeB[1] - valueB[node] - subtreeB[to]) * subtree[to];
