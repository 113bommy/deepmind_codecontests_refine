...
pair<int, int> get(int x, int lo, int hi, int a, int b) {
  if (lo >= a && hi <= b) {
    return t[x];
  }
  if (lo >= b || hi <= a) {
...
long long get_sum(int x, int lo, int hi, int a, int b) {
  prop(x, lo, hi);
  if (lo >= a && hi <= b) {
    return sum[x];
  }
...
void solve() {
  ...
  for (int i = 0; i < q; ++i) {
    R[i]--;
    L[i]--;
    odg_l[R[i]].push_back(i);
    odg_r[L[i]].push_back(i);
  }
  for (int i = 0; i < n; ++i) {
