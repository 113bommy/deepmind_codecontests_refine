...
int n, p, k;
char A[N];
long long f[2][N][2][2], V[N], bit[N];
inline void Add(long long &x, long long y) { x = (x + y) % P; }
long long calc(long long x) {
  if (x < 0) return 0;
  if (x < p) return (1 + x + 1) * (x + 1) >> 1;
  long long tmp = 1ll * p * p;
  ++x;
  if (x <= 2 * p - 2) tmp -= (1 + 2 * p - 1 - x) * (2 * p - x - 1) >> 1;
  return tmp;
}
const int bs = 1e9;
long long calc(int l, int r) { return (calc(r) - calc(l - 1)) % P; }
int main() {
  read(p), read(k);
