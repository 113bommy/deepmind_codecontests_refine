...
void restan(int i) {
  if (n[n.length() - 1 - i] != '0')
    n[n.length() - 1 - i] = char(n[n.length() - 1 - i] - 1);
  else {
    n[n.length() - 1 - i] = '9';
    restan(i + 1);
  }
...
void restab(int i) {
  if (b[b.length() - 1 - i] != '0')
    b[b.length() - 1 - i] = char(b[b.length() - 1 - i] - 1);
  else {
    b[b.length() - 1 - i] = '9';
    restab(i + 1);
  }
...
long long gcd(long long r, long long p) {
  if (p == 0) return r;
  if (r < p)
    return gcd(p, r);
  else
...
long long solve(long long a, string exp, long long mod) {
  ...
  restan(0);
  exp = n;
  if (gcd(g, f) != 1 && g % f != 0 && g % f != 1)
    res = (res * solve(g, n, f)) % f;
  else {
    if (g % f == 0) {
      res = 0;
    } else if (gcd(g, f) == 1) {
