...
void check() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < childs[i].size(); j++) {
      int k = childs[i][j];
      for (typeof(fact[k].begin()) it = fact[k].begin(); it != fact[k].end();
           it++)
    ...
    for (typeof(fact[i].begin()) it = fact[i].begin(); it != fact[i].end();
         it++) {
...
int main() {
  ...
  reverse(a, a + n);
  for (int i = 0; i < n; i++) {
    long long k = 2, m = a[i], x = sqrt(m) + 2;
    while (m > 1 && k <= x) {
      if (m % k == 0) {
        m /= k;
        fact[i][k]++;
