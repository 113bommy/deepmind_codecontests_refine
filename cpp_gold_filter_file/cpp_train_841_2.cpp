...
bool solve(bool fir = 0) {
  ...
  for (int i = 1; i <= n; i++)
    if (can[i]) {
      ...
      while (top1 > 1 && cross(a[sta1[top1 - 1]], a[i], a[sta1[top1]]) >= 0)
        top1--;
      sta1[++top1] = i;
    }
  for (int i = n; i >= 1; i--)
    if (can[i]) {
      ...
      while (top2 > 1 && cross(a[sta2[top2 - 1]], a[i], a[sta2[top2]]) >= 0)
        top2--;
      sta2[++top2] = i;
    }
  ...
  if (fir) {
    ...
    k = 0;
    for (int i = 1; i <= top1; i++) bz[que[++k] = sta1[i]] = 1;
    for (int i = 1; i <= top2; i++)
      if (!bz[sta2[i]]) bz[que[++k] = sta2[i]] = 1;
  }
}
int main() {
  n = get();
