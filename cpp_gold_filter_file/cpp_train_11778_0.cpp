...
#pragma O3
using namespace std;
int n, k, q, sz, tp, a[500000][15], b[15], x, l, r, ans;
...
struct segtree {
  ...
  int query(int beg, int en, int l, int r, int node) {
    ...
    int mid = (l + r) >> 1;
    return max(query(beg, en, l, mid, node << 1),
  ...
  while (q--) {
    ...
    if (tp == 1) {
      ...
      for (int i = 0; i < (1 << (k - 1)); i++) {
        ans = max(ans, st[i].query(l, r, 1, sz, 1) +
                           st[(1 << k) - 1 - i].query(l, r, 1, sz, 1));
      }
      printf("%d\n", ans);
    }
  }
