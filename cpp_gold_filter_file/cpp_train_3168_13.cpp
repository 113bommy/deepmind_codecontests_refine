...
int main() {
  ...
  for (i = 1; i <= w; i++) {
    ...
    if (!op) {
      ...
      for (j = x1; j <= n; j += j & -j) {
        bitx[j] += 1ll * v * (y2 - y1 + 1);
        sx[j] += x1 * 1ll * v * (y2 - y1 + 1);
      }
      for (j = x2 + 1; j <= n; j += j & -j) {
        bitx[j] -= 1ll * v * (y2 - y1 + 1);
        sx[j] -= (x2 + 1) * 1ll * v * (y2 - y1 + 1);
      }
      for (j = y1; j <= m; j += j & -j) {
        bity[j] += 1ll * v * (x2 - x1 + 1);
        sy[j] += y1 * 1ll * v * (x2 - x1 + 1);
      }
      for (j = y2 + 1; j <= m; j += j & -j) {
        bity[j] -= 1ll * v * (x2 - x1 + 1);
        sy[j] -= (y2 + 1) * 1ll * v * (x2 - x1 + 1);
      }
    } else {
