...
struct mint {
  ...
  friend mint mpow(mint a, long long k) {
    mint r = 1;
    while (k) {
      if (k & 1) r = r * a;
      a = a * a;
  ...
  friend mint operator/(mint a, mint b) {
    mint p = a * mint(b.x, (MOD - b.y) % MOD),
         q = mint((1LL * b.x * b.x % MOD + MOD - 5LL * b.y * b.y % MOD) % MOD,
                  0);
...
int k;
mint get(mint a, long long m) {
  if (a.x == 1 && a.y == 0)
    return m % MOD + 1;
  else
    return (mpow(a, m + 1) - 1) / (a - 1);
}
mint solve(long long m) {
  mint ans = 0;
  for (int j = 0; j <= k; j++) {
    mint del = 0;
    for (int p = 0; p <= j; p++) {
