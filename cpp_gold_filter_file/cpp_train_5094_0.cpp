...
int main(){
  ...
  while(q--){
    ...
    if(x == 0){
      ...
      c = tree.split( tree.root, z);     // [0,z) [z,n)
      b = tree.split( c.first , z - 1);  // [0,z - 1) [z-1,z)
      a = tree.split( b.first , y);     //  [0,y) [y,z-1)
 
      tree.root = tree.merge( a.first, b.second); // [0,z) + [z-1,z)
      tree.root = tree.merge( tree.root, a.second); // root + [y,z-1)
      tree.root = tree.merge( tree.root, c.second); // root + [z,n)
