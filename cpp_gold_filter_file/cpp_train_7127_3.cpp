...
int cost[N][N], max_match, lx[N], ly[N], xmatch[N], ymatch[N], slack[N],
    slackx[N], pre[N];
bool S[N], T[N];
...
int hungarian(int n) {
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) lx[i] = max(lx[i], cost[i][j]);
  augment(n);
...
int main() {
  ...
  for (int i = 0; i < n; i++) cost[key[S[i]]][key[E[i]]]++;
  cout << hungarian(k) << "\n";
  for (int i = 0; i < k; i++) cout << val[xmatch[i]];
  return 0;
}
