...
int main() {
  ...
  int now = 0, nxt = 1;
  dp[now][n] = 0;
  for (int i = 0; i < n; i++) sorted_arr[now][i] = i;
  for (int i = 0; i <= 60; i++) {
    la = 0;
    lb = 0;
    for (int j = 0; j < n; j++) {
      int no = sorted_arr[now][j];
      if (arr[no] & (1ll << i)) {
        temp[lb++] = no;
      } else {
        sorted_arr[nxt][la++] = no;
      }
    }
    for (int i = 0; i < lb; i++) sorted_arr[nxt][i + la] = temp[i];
    for (int i = 0; i <= n; i++) dp[nxt][i] = 1ll << 60;
    int rla = 0;
    for (int j = 0; j <= n; j++) {
      int rlb = j - rla;
      dp[nxt][rla] = min(dp[nxt][rla], dp[now][j] + rlb + la - rla);
      dp[nxt][la + rlb] = min(dp[nxt][la + rlb], dp[now][j] + rla + lb - rlb);
      if (j == n) break;
      int no = sorted_arr[now][j];
      if (!(arr[no] & (1ll << i))) {
        rla++;
