...
double polyunion(int n) {
  ...
  for (int i = 0; i < n; ++i)
    for (int ii = 0; ii < po[i].n; ++ii) {
      int tot = 0;
      b[tot++] = make_pair(0.0, 0);
      b[tot++] = make_pair(1.0, 0);
      for (int j = 0; j < n; ++j) {
        if (i == j) continue;
        for (int jj = 0; jj < po[j].n; ++jj) {
          ...
          if (aa == 0 && bb == 0) {
            if (dot(po[i][ii + 1] - po[i][ii], po[j][jj + 1] - po[j][jj]) > 0 &&
                j < i) {
              b[tot++] = make_pair(seg(po[i][ii], po[i][ii + 1], po[j][jj]), 1);
              b[tot++] =
                  make_pair(seg(po[i][ii], po[i][ii + 1], po[j][jj + 1]), -1);
            }
          } else if (aa < 0 && bb >= 0) {
            double s1 = cross(po[i][ii], po[j][jj + 1], po[j][jj]),
                   s2 = s1 + cross(po[j][jj + 1], po[i][ii + 1], po[j][jj]);
            b[tot++] = make_pair(s1 / s2, -1);
          } else if (aa >= 0 && bb < 0) {
            double s1 = cross(po[i][ii], po[j][jj + 1], po[j][jj]),
                   s2 = s1 + cross(po[j][jj + 1], po[i][ii + 1], po[j][jj]);
            b[tot++] = make_pair(s1 / s2, 1);
