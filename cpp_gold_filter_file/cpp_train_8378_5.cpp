...
bool cmp(struct line a, struct line b) { return a.l < b.l; }
long long solve(long long a, int k) {
  for (int i = 0; i < k; i++) a = a * 2 + 1;
  return a;
}
int main() {
  int q, h, k, i, cnt, f;
  long long c1[55], l, r, ans, t;
  for (c1[0] = i = 1; i <= 50; i++) c1[i] = c1[i - 1] << 1;
  while (scanf("%d%d", &h, &q) != EOF) {
    ...
    if (L[0].l > c1[h - 1] + 1)
      ...
      if (c1[h] - 1 > r) t += c1[h] - 1 - r, ans = c1[h] - 1;
      if (t > 1)
        printf("Data not sufficient!\n");
      else if (t == 0)
        printf("Game cheated!\n");
