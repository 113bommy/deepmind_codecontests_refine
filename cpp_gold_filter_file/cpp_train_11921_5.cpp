...
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    int cur = pos[i];
    while (cur != rt) {
      if (cur == pos[i] && have[ac::fail[cur]])
        con[i][have[ac::fail[cur]]] = 1;
      else if (cur != pos[i] && have[cur])
        con[i][have[cur]] = 1;
      cur = fa[cur];
    }
  }
  ...
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= n; j++)
      if (con[i][j]) thor.addedge(i, j + n, inf);
  for (int i = 1; i <= n; i++) {
    thor.addedge(S, i, 1);
  ...
  printf("%d\n", n - thor.maxflow(S, T));
  for (int i = 1; i <= n; i++)
    if (thor.lvl[i] != -1 && thor.lvl[i + n] == -1) printf("%d ", id[i]);
  return 0;
}
