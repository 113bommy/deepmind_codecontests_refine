...
inline void inc(int i) {
  i = e[i];
  pi = 1ll * pi * (a[i] + k - b[i]++) % mo;
}
inline void dec(int i) {
  i = e[i];
  pi = 1ll * pi * inv[a[i] + k - (--b[i])] % mo;
}
int main() {
  for (i = 2, inv[1] = 1; i < N * 2; ++i)
  ...
  for (k = 0; k < N; ++k)
    if (!qu[k].empty()) {
      bl = max(n / sqrt(qu[k].size()), 1.0);
      for (i = 1; i <= n; ++i) bel[i] = (i - 1) / bl + 1;
      sort(qu[k].begin(), qu[k].end(),
           [&](const vector<int>& a, const vector<int>& b) {
             return bel[a[0]] == bel[b[0]]
                        ? (bel[a[0]] & 1 ? a[1] > b[1] : a[1] < b[1])
                        : a[0] < b[0];
           });
      j = 1ll * k * m % mo;
      ssm[0] = 1;
      for (i = 1; i <= n; ++i) ssm[i] = 1ll * ssm[i - 1] * (j + i) % mo;
      l = 1;
      r = 0;
      pi = 1;
      memset(b + 1, 0, m << 2);
      for (vector<int> u : qu[k]) {
        for (; l > u[0]; inc(--l))
          ;
        for (; r < u[1]; inc(++r))
          ;
