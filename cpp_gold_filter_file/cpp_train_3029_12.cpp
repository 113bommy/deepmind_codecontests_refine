...
void dfs(vector<pair<char, int>> A1, vector<pair<char, int>> A2,
  ...
  for (int i = 0; i <= A1.size(); i++) {
    ...
    for (int j = 0; j <= A2.size(); j++) {
      ...
      for (int k = i; k < A1.size(); k++) {
        if (C1.size() >= 1 && C1[C1.size() - 1].first == A1[k].first)
          C1[C1.size() - 1].second += A1[k].second;
        else
          C1.push_back(A1[k]);
      }
      for (int k = j; k < A2.size(); k++) {
        if (C2.size() >= 1 && C2[C2.size() - 1].first == A2[k].first)
          C2[C2.size() - 1].second += A2[k].second;
        else
          C2.push_back(A2[k]);
      }
      vector<pair<int, int>> C3 = S;
      C3.push_back(make_pair(sum1, sum2));
      dfs(C1, C2, C3);
...
vector<pair<int, int>> small_solve(vector<pair<char, int>> A1,
...
vector<pair<int, int>> solve_same(vector<int> A1, vector<int> A2) {
  ...
  for (int i = c1; i < A1.size(); i++) {
    V1.push_back(make_pair('a', LA[i]));
  }
  for (int i = c2; i < A2.size(); i++) {
    V2.push_back(make_pair('b', LB[i]));
  }
...
vector<pair<int, int>> solve_diff(vector<int> A1, vector<int> A2) {
  ...
  if (abs((int)A1.size() - (int)A2.size()) >= 3) {
    ...
    if (d1 < A1.size()) P1[P1.size() - 1] += A1[d1];
    for (int i = d1 + 1; i < A1.size(); i++) P1.push_back(A1[i]);
    if (d2 < A2.size()) P2[P2.size() - 1] += A2[d2];
    for (int i = d2 + 1; i < A2.size(); i++) P2.push_back(A2[i]);
    A1 = P1;
    A2 = P2;
  }
  for (int i = 0; i < A1.size(); i++) LA[i] = A1[i];
  ...
  vector<pair<char, int>> V1, V2;
  for (int i = min((int)A1.size(), (int)A2.size()) - 1; i < A1.size(); i++) {
    V1.push_back(make_pair('a', LA[i]));
  }
  for (int i = min((int)A1.size(), (int)A2.size()) - 1; i < A2.size(); i++) {
    V2.push_back(make_pair('b', LB[i]));
  }
  for (int i = 0; i < V1.size(); i++) {
    if (i % 2 == 0)
