...
int tg[N * 4];
void pst(int k1) { tg[k1] ^= 1, swap(t[k1][0], t[k1][1]); }
void psd(int k1) {
  if (tg[k1]) {
    pst(k1 * 2);
    pst(k1 * 2 + 1);
...
void bud(int k1, int k2, int k3) {
  if (k2 == k3) {
    ...
    if (a[k2] & 1) {
      for (int i = (1); i <= (m); ++i) t[k1][0].w[i] = -1;
      t[k1][0].w[m + 1] = 1;
      for (int i = (1); i <= (m + 1); ++i) t[k1][1].w[i] = -1;
    } else {
      for (int i = (1); i <= (m); ++i) t[k1][1].w[i] = -1;
      t[k1][1].w[m + 1] = 1;
      for (int i = (1); i <= (m + 1); ++i) t[k1][0].w[i] = -1;
    }
    return;
  }
  psd(k1);
