...
int f(int pos) {
  ...
  for (int i = 2; i <= n; i++) {
    if (hor(i - 1, pos))
      ...
      r[i] = 0;
    if (ver(i - 2, pos) && ver(i - 2, pos + 1)) r[i] += r[i - 2];
    if (r[i] >= 1000000007) r[i] -= 1000000007;
  }
  if (n % 2 == 0) {
    int w = 1;
    for (int i = 0; i < n; i++)
      if (t[i][pos] > 6 || t[i][pos + 1] > 6) w = 0;
    if (w) return r[n] - 1;
  }
  return r[n];
}
int main() {
  ...
  for (int i = 2; i <= m; i++) {
    ...
    for (int j = 0; j < n; j++)
      if (t[j][i - 1] > 6) ok = 0;
    if (ok)
      d[i] = (1LL * d[i - 2] * f(i - 2) + d[i - 1]) % 1000000007;
    else
      d[i] = 1LL * d[i - 2] * f(i - 2) % 1000000007;
  }
  printf("%d\n", d[m]);
}
