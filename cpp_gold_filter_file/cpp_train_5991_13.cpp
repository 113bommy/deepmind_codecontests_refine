...
struct cht {
  ...
  int cur = 0;
  cht() { hull.clear(); }
  bool useless(const pair<int, int> left, const pair<int, int> middle,
               const pair<int, int> right) {
    return 1LL * (middle.second - left.second) * (middle.first - right.first) >=
  ...
  void insert(int a, int b) {
    if (hull.empty()) {
      ...
      while (hull.size() >= 2 &&
             useless(hull[hull.size() - 2], hull.back(), make_pair(a, b))) {
        hull.pop_back();
      }
      hull.emplace_back(make_pair(a, b));
    }
  ...
  long long query(int x) {
    ...
    while (((cur + 1) < hull.size()) &&
           ((hull[cur + 1].second - hull[cur].second) <=
            (1LL * x * (hull[cur].first - hull[cur + 1].first)))) {
      ++cur;
    }
