...
inline void ycl(int s, int t) {
  ...
  while (l <= r) {
    int k = que[l++];
    for (int i = son[k]; i; i = e[i].nextt)
      if (!d[e[i].ed]) ++gap[d[e[i].ed] = d[k] + 1], que[++r] = e[i].ed;
  }
}
inline int aug(int x, int s, int t, int mmin) {
  if (x == t) return mmin;
  int flow = 0;
  for (int i = son[x]; i; i = e[i].nextt)
    if (d[x] == d[e[i].ed] + 1) {
