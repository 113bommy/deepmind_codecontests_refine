...
struct Point {
  ...
  Point operator+(Point who) { return {x + who.x, y + who.y}; }
  Point operator-(Point who) { return {x - who.x, y - who.y}; }
  Point operator/(double val) { return {x / val, y / val}; }
  Point operator*(double val) { return {x * val, y * val}; }
  bool operator==(const Point who) const {
    return max(abs(x - who.x), abs(y - who.y)) <= 1e-7;
  }
  bool operator<(const Point who) const {
    if (*this == who) return false;
    return make_pair(x, y) < make_pair(who.x, who.y);
  }
};
struct Line {
  double a, b, c;
  ...
  Line operator||(Point A) { return {a, b, -(a * A.x + b * A.y)}; }
  Line perp() {
    Line ans = {a, b, c};
    swap(ans.a, ans.b);
    ans.a *= -1;
  ...
  bool is_parallel(Line who) { return abs(a * who.b - b * who.a) <= 1e-7; }
};
int n, i, j, ans;
Point P[2017], mid, Origin(0.00, 0.00);
bool used[2017];
Line aux;
Point _P[2017], _mid;
map<Point, int> M;
