...
void update(int x, int y) {
  e[++e_tot] = edge(y, fir[x]);
  fir[x] = e_tot;
  e[++e_tot] = edge(x, fir[y]);
  fir[y] = e_tot;
}
...
again:
  ...
  for (int i = LL, s = 1; i; i = f[i], ++s) dl[i] = s;
  for (int(i) = (1), (_end_) = (18); (i) <= (_end_); ++(i))
    for (int j = LL; j; j = f[j]) {
      g[j][i][0] = g[g[j][i - 1][0]][i - 1][0];
      g[j][i][1] = max(g[j][i - 1][1], g[g[j][i - 1][0]][i - 1][1] +
                                           min(1 << (i - 1), dl[RR] - dl[j]));
      h[j][i][0] = h[h[j][i - 1][0]][i - 1][0];
      h[j][i][1] = max(h[j][i - 1][1], h[h[j][i - 1][0]][i - 1][1] +
                                           min(1 << (i - 1), dl[j] - 1));
    }
...
int work(int L, int R, int type) {
  ...
  if (type == 0) {
    int x = L;
    for (int(i) = (18), (_end_) = (0); (i) >= (_end_); --(i))
...
int main() {
  ...
  for (int(i) = (1), (_end_) = (m); (i) <= (_end_); ++(i)) {
    ...
    if (f[x] == f[y] || f[x] == y || f[y] == x) {
      ...
      else if (sum - dep[x] - f[fa_x] >= -f[fa_y])
        printf("%d\n", dep[x] + max(-f[fa_x] - 1, -f[RR] + f[fa_x]));
      else
        printf("%d\n",
               max(max(dep[x] - f[fa_x] - 1,
                       dep[x] + 1 +
                           work(o[1 - f[fa_x]], o[sum - dep[x] - f[fa_x]], 0)),
                   max(dep[y] + f[fa_y] - f[RR],
                       dep[y] + 1 +
                           work(o[sum - dep[x] - f[fa_x] + 1], o[-f[fa_y] - 1],
                                1))));
    }
  }
  return 0;
