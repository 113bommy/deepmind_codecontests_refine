#include <bits/stdc++.h>
using namespace std;
vector<int> p1[100010 * 2], p2[100010 * 2];
vector<pair<int, int> > A[100010 * 2], B[100010 * 2];
int n, m, nb;
int movex[] = {-1, -1, 1, 1};
int movey[] = {1, -1, 1, -1};
...
int ID(char *a) {
  if (a[0] == 'N' && a[1] == 'E') return 0;
  if (a[0] == 'N' && a[1] == 'W') return 1;
  if (a[0] == 'S' && a[1] == 'E') return 2;
  return 3;
}
void read() {
  scanf("%d %d %d", &n, &m, &nb);
...
void Run(int x, int y, int d) {
  ...
  while (1) {
    ...
    if (lx - ly == rx - ry && lx - ly == sx - sy && lx <= x && x <= rx) ++cur;
    if (lx + ly == rx + ry && lx + ly == sx + sy && lx <= x && x <= rx) ++cur;
    if (cur > 4) break;
  }
}
...
void work() {
  ...
  Run(x, y, ID(str));
  long long ans = 0;
  for (int i = 0; i <= n + m + 2; ++i) ans += calc(A[i]) + calc(B[i]);
  cout << ans << endl;
}
int main() {
  read();
