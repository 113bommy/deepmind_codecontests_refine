...
void doAddInterval(vector<pair<double, bool> > &intervals, double left,
                   double right) {
  ...
  while (left < 2 * PI) {
    intervals.push_back(pair<double, bool>(left + EPS, true));
    intervals.push_back(pair<double, bool>(right - EPS, false));
    left += 2 * PI;
    right += 2 * PI;
  }
}
int main(void) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < n; j++) {
      if (i == j) continue;
      double ang = atan2((pnts[j].first - pnts[i].first) * 1.0,
                         (pnts[j].second - pnts[i].second) * 1.0);
      double dist = sqrt(sqr(pnts[j].first - pnts[i].first) +
                         sqr(pnts[j].second - pnts[i].second));
