...
void backtrack1(int i, int j, int step, int64_t xorsum) {
  if (step == n) {
    ...
    if (i < n - 1) backtrack1(i + 1, j, step + 1, xorsum ^ a[i][j]);
    if (j < m - 1) backtrack1(i, j + 1, step + 1, xorsum ^ a[i][j]);
  }
}
void backtrack2(int i, int j, int step, int64_t xorsum) {
  if (step == m) {
    ...
    if (i > 0) backtrack2(i - 1, j, step + 1, xorsum ^ a[i][j]);
    if (j > 0) backtrack2(i, j - 1, step + 1, xorsum ^ a[i][j]);
  }
}
int main() {
  cin >> n >> m >> k;
