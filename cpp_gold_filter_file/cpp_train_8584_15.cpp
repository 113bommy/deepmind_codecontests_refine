...
void buildSt() {
  ...
  for (int i = 0; i < (int)(off); ++i)
    for (int j = 0; j < (int)(60); ++j)
      st[off + i][j] = pMod(j + 1 + bad[j][a[i]]),
               sv[off + i][j] = 1 + bad[j][a[i]];
...
void updateSt(int pos, int val) {
  ...
  a[pos] = val;
  for (int j = 0; j < (int)(60); ++j)
    st[off + pos][j] = pMod(j + 1 + bad[j][val]),
             sv[off + pos][j] = 1 + bad[j][val];
  for (int i = (off + pos) >> 1; i >= 1; i >>= 1)
    for (int j = 0; j < (int)(60); ++j) {
      st[i][j] = st[(i << 1) | 1][st[i << 1][j]];
      sv[i][j] = sv[i << 1][j] + sv[(i << 1) | 1][st[i << 1][j]];
    }
...
int main() {
  for (int i = 0; i < (int)(60); ++i)
    for (int j = 2; j <= 6; ++j) bad[i][j] = (i % j == 0);
  scanf("%d", &n);
