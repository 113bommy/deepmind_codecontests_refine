...
int main() {
  ...
  for (int i = 1; i + 1 < v.size(); ++i)
    if (v[i].second > 1) poss.push_back(i);
  ...
  for (int i = 1; i < poss.size(); ++i) {
    ...
    if (sir[st] == sir[dr]) {
      for (j = st + 1; j <= dr; ++j) sir[j] = sir[st];
    } else {
      for (j = st; j <= (st + dr) / 2; ++j) sir[j] = sir[st];
      for (j = (st + dr) / 2 + 1; j <= dr; ++j) sir[j] = sir[dr];
    }
  }
  cout << ans << '\n';
