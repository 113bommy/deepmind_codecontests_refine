...
int main() {
  ...
  for (i = 0; i < (1 << n); i++)
    for (j = 0; j <= n * n; j++) a[i][1][j] = a[i][2][j] = 1e9;
  if (xl > xr)
    a[0][1][min(xl - xr, n * n)] = 0;
  else
    a[0][2][min(xr - xl, n * n)] = 0;
      ...
      for (j = 0; j < n; j++) {
        ...
        for (k = 0; k <= n * n; k++) {
          if (a[la][1][k] < 1e9) {
            ...
            if (re == 0)
              a[now][1][min(bl, n * n)] =
                  min(a[now][1][min(bl, n * n)], a[la][1][k] + tot);
            if (bl == 0)
              a[now][2][min(re, n * n)] =
                  min(a[now][2][min(re, n * n)], a[la][1][k] + tot);
          }
          if (a[la][2][k] < 1e9) {
            int nl = k, nr = 0;
            int nred = max(0, b[j + 1].red - l - nl);
            ...
            if (re == 0)
              a[now][1][min(bl, n * n)] =
                  min(a[now][1][min(bl, n * n)], a[la][2][k] + tot);
            if (bl == 0)
              a[now][2][min(re, n * n)] =
                  min(a[now][2][min(re, n * n)], a[la][2][k] + tot);
          }
        }
      }
    }
