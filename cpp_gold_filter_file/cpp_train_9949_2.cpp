...
int n, s, t, cnt, tot, m[N], ans[N], need[N], hed[V], val[E], to[E], nxt[E],
    dep[V], q[V], cur[V];
string d[N], l[N];
...
void solve(int top) {
  ...
  for (int i = 0; i <= n; i++)
    for (int j = 0; j <= n; j++) vec[i][j].clear(), goal[i][j].clear();
  for (int i = 1; i <= n; i++) {
    if (d[i] == "?" && l[i] == "?")
      f.push_back(i);
    else if (d[i] == "?")
      fd[atoi(l[i].c_str())].push_back(i);
    else if (l[i] == "?")
      fl[atoi(d[i].c_str())].push_back(i);
    else
      vec[atoi(l[i].c_str())][atoi(d[i].c_str())].push_back(i);
  }
  for (int i = 1; i <= n; i++) {
    if (fd[i].size()) need[i] = 0;
    for (int j = 0; j <= n; j++)
      if (vec[i][j].size()) need[i] = 0;
  }
  need[top] = 0;
  for (int i = 1; i <= n; i++)
    if (~need[i]) {
      ...
      if (need[i] < i)
        ...
        need[i] = need[i] % i == 0 ? 0 : i - need[i] % i;
    }
  for (int i = 1; i <= n; i++) {
    m[i] = ~need[i] ? 0 : -1;
    for (int j = 0; j <= n; j++)
      if (vec[i][j].size()) m[i] = j;
  }
  int maxt = 0;
  for (int i = 1; i <= n; i++)
    if (fl[i].size()) maxt = i;
  ...
  for (int j = 1; j <= n; j++)
    while (fl[j].size()) move(fl[j], vec[top][j]);
  for (int i = 1; i <= n; i++)
    while (fd[i].size()) move(fd[i], vec[i][1]);
  for (int i = 1; i <= n; i++) {
