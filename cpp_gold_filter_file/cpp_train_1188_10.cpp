...
vector<modular<int> > power_table(modular<int> x, int n) {
  vector<modular<int> > pw(n + 1);
  pw[0] = 1;
  for (int i = 1; i <= n; i++) {
    pw[i] = pw[i - 1] * x;
  }
...
void solve() {
  ...
  function<modular<int>(int, int)> calc = [&](int l, int r) {
    if (r <= l) return modular<int>(1);
    ...
    if (i == j && a[i] == 0) return c.c(r - l - 1, i - l);
    modular<int> res = 0;
    if (a[i] >= a[j]) {
      int exr = a[i] + i;
      if (exr < r && exr >= j) {
