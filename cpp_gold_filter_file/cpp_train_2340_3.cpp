...
inline void upt(int pos, int v) {
  for (seg[pos += L] = v; pos >>= 1;)
    seg[pos] = std::max(seg[pos << 1], seg[pos << 1 | 1]);
}
...
inline void solve(int l, int r) {
  if (l > r) return;
  if (le[l].empty()) return res.push_back(l), solve(l + 1, r);
  if (ri[l].empty()) return solve(l + 1, r), res.push_back(l);
...
int main() {
  ...
  cin >> n >> c;
  for (int i = (1), iend = (n); i <= iend; ++i) minr[i] = n + 1, max[i] = i;
