...
node merge(const node &l, const node &r, int polsz) {
  ...
  ret.__rastl =
      (l.rastl == polsz && l.hr < r.hl) ? polsz + r.__rastl : l.__rastl;
  if (l.__padr == polsz && l.hr > r.hl)
    ret.__rastl = max(ret.__rastl, polsz + r.padl);
  if (l.rastl == polsz && l.hr != r.hl)
    ret.__rastl = max(ret.__rastl, polsz + r.padl);
  ret.__padr = (r.padr == polsz && r.hl < l.hr) ? polsz + l.__padr : r.__padr;
  if (r.__rastl == polsz && r.hl > l.hr)
    ret.__padr = max(ret.__padr, polsz + l.rastr);
  if (r.padr == polsz && l.hr != r.hl)
    ret.__padr = max(ret.__padr, polsz + l.rastr);
