...
int check(int d) {
  ...
  for (int i = 0; i < p; i++) {
    tot = 0;
    for (int j = 0; j < q; j++) {
      if ((Blue[i].a.y <= Red[j].a.y) && (Blue[i].b.y >= Red[j].a.y) &&
          (Red[j].a.x <= Blue[i].a.x) && (Red[j].b.x >= Blue[i].a.x))
        inter[tot++] = j;
    }
    for (int j = 0; j < tot; j++)
      for (int k = j + 1; k < tot; k++)
        if (Map[inter[j]][inter[k]])
          return 1;
        else
