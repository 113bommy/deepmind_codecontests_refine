...
void Insert(int x, int v) {
  int i;
  for (i = 1; i <= k; ++i)
    if (1ll * x * tmpv[i] <= 1ll * v * tmpx[i]) break;
  for (int j = k; j >= i; --j)
...
void dfs(int l, int r) {
  ...
  for (i = 1; i <= k; ++i) {
    ...
    a1[i][0][0] = ans[i][0][0], a1[i][1][0] = ans[i][1][0],
    a1[i][0][1] = ans[i][0][1], a1[i][1][1] = ans[i][1][1];
  }
  int k1 = k;
  dfs(mid + 1, r);
  for (i = 1; i <= k; ++i) {
    tx2[i] = tmpx[i], tv2[i] = tmpv[i];
    a2[i][0][0] = ans[i][0][0], a2[i][1][0] = ans[i][1][0],
    a2[i][0][1] = ans[i][0][1], a2[i][1][1] = ans[i][1][1];
  }
  int k2 = k;
  l = 1, r = 1;
  k = 0;
  ...
  if (v[mid] < v[mid + 1]) Insert(x[mid + 1] - x[mid], v[mid + 1] - v[mid]);
  if (v[mid] > v[mid + 1]) Insert(x[mid + 1] - x[mid], v[mid] - v[mid + 1]);
  ++k;
  tmpx[k] = tmpv[k] = 0;
  for (i = 1; i <= k; ++i)
    ...
    for (int b = 0; b <= 1; ++b)
      for (int c = 0; c <= 1; ++c)
        for (int d = 0; d <= 1; ++d) {
          ...
          if (c == 1 && d == 0)
            for (i = 1; i <= k; ++i)
              if (x[mid + 1] - x[mid] == tmpx[i] &&
                  v[mid] + v[mid + 1] == tmpv[i] && op[i] == 2)
                id = i;
          if (c == 0 && d == 0)
            if (v[mid + 1] > v[mid])
              for (i = 1; i <= k; ++i)
                if (x[mid + 1] - x[mid] == tmpx[i] &&
                    v[mid + 1] - v[mid] == tmpv[i] && op[i] == 2)
                  id = i;
          if (c == 1 && d == 1)
            if (v[mid + 1] < v[mid])
              for (i = 1; i <= k; ++i)
...
int main() {
  ...
  dfs(1, n);
  for (i = 1; i < k; ++i)
    s = (s + (ans[i][0][0] + ans[i][0][1] + ans[i][1][0] + ans[i][1][1]) % M *
                 qpow(tmpv[i], M - 2) % M * tmpx[i]) %
        M;
  cout << (s % M + M) % M;
}
