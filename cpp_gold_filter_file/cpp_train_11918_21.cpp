...
long long d[200005];
struct node {
  long long sm2, sm1, mx;
} t[800005];
inline node operator+(node a, node b) {
  node r;
  r.mx = max(max(a.mx, b.mx), a.sm1 + b.sm2);
  return r.sm2 = max(a.sm2, b.sm2), r.sm1 = max(a.sm1, b.sm1), r;
}
inline void build(int x, int l, int r) {
  if (l == r)
    return t[x].sm2 = h[l] + d[l], t[x].sm1 = h[l] - d[l], t[x].mx = -I, void();
