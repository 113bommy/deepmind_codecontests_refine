...
void rec(long long tl, long long tr, long long l, long long r, int cfree = 60) {
  if (tr < l || r < tl) {
    return;
  }
  if (l <= tl && tr <= r) {
...
vector<seg> split(long long l, long long r) {
  ret_split.clear();
  rec(0, (1LL << 60) - 1, l, r);
  return ret_split;
}
seg operator+(seg a, seg b) {
  long long pre1 = a.l, pre2 = b.l;
  int len = max(a.free, b.free);
  long long pre = (pre1 ^ pre2) / (1LL << len) * (1LL << len);
  return {pre, len};
}
const int N = 100 + 7;
const int i2 = dv(1, 2);
int n, m;
long long l1[N], r1[N], l2[N], r2[N];
...
node* root = new node;
int type;
vector<long long> la[3][100];
void baga2(node* now, long long tl, long long tr, long long l, long long r,
           int kek = 60) {
  if (now->tot) {
    return;
  }
...
void baga(node* now, long long tl, long long tr, long long l, long long r) {
  if (now->tot) {
    return;
  }
  if (l <= tl && tr <= r) {
...
int scoate(node* now, long long tl, long long tr) {
  if (now->tot) {
    return gauss(tl, tr);
  }
  long long tm = (tl + tr) / 2;
