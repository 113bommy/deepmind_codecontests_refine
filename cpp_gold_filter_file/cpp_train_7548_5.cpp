...
class IO {
 ...
 public:
  ...
  inline ~IO() { fwrite(ob, 1, op - ob, out); }
  inline char getchar() {
...
class IOX : public IO {
 ...
 public:
  ...
  inline void put(T first, Args... rest) {
    put(first), put(rest...);
  }
...
const int N = 65536;
inline void trim(int &x) { x -= 1000000007, x += (x >> 31) & 1000000007; }
inline void up(int &x, int y) { trim(x += y); }
inline int fuck(int x, int y) {
  int z = 1;
  for (; y; y >>= 1) {
    if (y & 1) z = 1LL * z * x % 1000000007;
...
inline void NTT(int *a, int *b, int *c) {
  ...
  for (int i = 0; i < N; i++)
    up(c[i],
       ((long long)std::round(t[i].real / (fl)N) % 1000000007 + 1000000007) %
           1000000007);
  for (int i = 0; i < N; i++) t[i] = a1[i] * b2[i];
  FFT(t, N, -1);
  for (int i = 0; i < N; i++)
    up(c[i],
       ((long long)std::round(t[i].real / (fl)N) % 1000000007 * N % 1000000007 +
        1000000007) %
           1000000007);
  ...
  FFT(t, N, -1);
  for (int i = 0; i < N; i++)
    up(c[i], ((long long)std::round(t[i].real / (fl)N) % 1000000007 * N %
...
inline int C(int n, int m) {
  return 1LL * frc[n] * inv[m] % 1000000007 * inv[n - m] % 1000000007;
}
void dfs(int x) {
  if (x == 1) return;
  if (x & 1) {
...
int main() {
  ...
  dfs((int)tmp);
  frc[0] = 1;
  for (int i = 1; i < N; i++) frc[i] = 1LL * frc[i - 1] * i % 1000000007;
  inv[N - 1] = fuck(frc[N - 1], 1000000007 - 2);
  for (int i = N - 1; i; i--) inv[i - 1] = 1LL * inv[i] * i % 1000000007;
  bin[0] = 1;
  ...
  for (int i = n, now = 1; i >= 1; i--) {
    if (path[i]) {
      for (int j = 0; j <= k; j++)
        g[j] = 1LL * f[j] * bin[j] % 1000000007 * inv[j] % 1000000007;
      for (int j = 1; j <= k; j++) h[j] = inv[j];
      NTTImpl::NTT(h, g, f);
      for (int j = 0; j <= k; j++) f[j] = 1LL * f[j] * frc[j] % 1000000007;
      for (int j = k + 1; j < N; j++) f[j] = 0;
      now++;
    } else {
      for (int j = 0; j <= k; j++)
        g[j] =
            1LL * f[j] * fuck(bin[j], now) % 1000000007 * inv[j] % 1000000007;
      for (int j = 0; j <= k; j++) h[j] = 1LL * f[j] * inv[j] % 1000000007;
      NTTImpl::NTT(h, g, f);
      for (int j = 0; j <= k; j++) f[j] = 1LL * f[j] * frc[j] % 1000000007;
      for (int j = k + 1; j < N; j++) f[j] = 0;
      now *= 2;
  ...
  for (int i = 0; i <= k; i++) up(ans, 1LL * f[i] * C(k, i) % 1000000007);
  io.put(ans, '\n');
  return 0;
}
