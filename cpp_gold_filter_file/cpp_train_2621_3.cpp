...
struct Point3 {
  ...
  Point3 normalize() { return (*this) * (1 / length()); }
  ...
  double dist(Point3 other) { return (*this - other).length(); }
  double operator^(Point3 other) {
    return x * other.x + y * other.y + z * other.z;
  }
  Point3 operator*(Point3 other) {
    return Point3(y * other.z - z * other.y, z * other.x - x * other.z,
                  x * other.y - y * other.x);
  }
  void display() { printf("Point3 (%lf , %lf , %lf)\n", x, y, z); }
};
...
struct Point2 {
  ...
  Point2 operator*(double k) { return Point2(k * x, k * y); }
  Point2 normalize() { return (*this) * (1 / length()); }
  Point2 operator+(Point2 other) { return Point2(x + other.x, y + other.y); }
  Point2 operator-(Point2 other) { return Point2(x - other.x, y - other.y); }
  double dist(Point2 other) { return (*this - other).length(); }
  double operator^(Point2 other) { return x * other.x + y * other.y; }
  double operator*(Point2 other) { return x * other.y - y * other.x; }
  void display() { printf("Point2 (%lf , %lf)\n", x, y); }
};
Point2 operator*(double k, Point2 p) { return Point2(k * p.x, k * p.y); }
struct Plane {
  double a, b, c, d;
  Point3 A, B, C;
  Point3 N, O, X, Y;
  ...
  Point3 project(Point3 P) { return (P - (P ^ N) * N) + O; }
  Point2 project2D(Point3 P) {
    P = project(P);
    return Point2(P ^ X, P ^ Y);
  }
  double distance(Point3 p) { return abs(a * p.x + b * p.y + c * p.z + d); }
  void display() {
...
pair<double, double> doTernaryY(double x) {
  ...
  double ans = (lo + hi) / 2;
  return make_pair(ans, eval(x, ans));
}
pair<double, double> doTernaryX() {
  ...
  double ans = (lo + hi) / 2;
  return make_pair(ans, doTernaryY(ans).first);
}
void solve() {
  p2.resize(n);
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) p2[j] = plns[i].project2D(p3[j]);
