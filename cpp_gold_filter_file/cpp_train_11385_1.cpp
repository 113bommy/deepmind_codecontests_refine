  ...
  for (i = 0; i < G[u].size(); i++)
    if (!vis[v = G[u][i]]) {
      ...
      for (j = 0; j < G[v].size(); j++)
        if (G[v][j] == u) f[v][j] = -f[u][i];
    }
}
int main() {
  int i, j, t, k, p = 1, u, v, q, ans;
  std::vector<int> vec;
  read(n), read(m);
  for (i = 1; i <= m; i++)
  ...
  for (i = 0; i <= n; i++) {
    ...
    for (j = 0; j < e.size(); j++)
      e[j] = std::make_pair(a[G[i][j]] - a[i], G[i][j]);
    for (std::sort(e.begin(), e.end()), j = 0; j < e.size(); j++)
      G[i][j] = e[j].second, F[i][G[i][j]] = j;
  }
  dfs(0);
  for (i = 0; i <= n; i++)
    for (s[i].resize(G[i].size()), s[i][0] = f[i][0], j = 1; j < G[i].size();
         j++)
      s[i][j] = s[i][j - 1] + f[i][j];
  for (read(q), t = 1; t <= q; t++) {
    for (read(k), vec.resize(k), i = 0; i < k; i++) read(vec[i]);
    for (p = 0, i = 0; i < k; i++)
      if (a[vec[i]].x < a[vec[p]].x ||
          (a[vec[i]].x == a[vec[p]].x && a[vec[i]].y < a[vec[p]].y))
        p = i;
    if ((a[vec[(p + 1) % k]] - a[vec[p]]) *
            (a[vec[p]] - a[vec[(p + k - 1) % k]]) >
        0)
      std::reverse(vec.begin(), vec.end());
    ans =
        calc(vec[0], vec[k - 1], vec[1]) + calc(vec[k - 1], vec[k - 2], vec[0]);
    for (j = 1; j + 1 < vec.size(); j++)
      ans += calc(vec[j], vec[j - 1], vec[j + 1]);
    printf("%d\n", -ans);
  }
}
