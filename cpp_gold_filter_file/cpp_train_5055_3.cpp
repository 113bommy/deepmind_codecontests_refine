...
struct lazy_propagation {
  ...
  long long seg_query(int nod, int l, int r, int s, int e) {
    ...
    seg_relax(nod, l, r);
    int m = (l + r) >> 1;
    return min(seg_query(nod << 1, l, m, s, e),
...
int main() {
  ...
  while (q--) {
    ...
    scanf("%d%d%d", &a, &b, &c);
    if (a == 2) {
      long long diff = b == 0 ? 0
                              : stree[(b & 1) ^ 1].seg_query(
                                    1, 0, TSIZE - 1, (b - 1) / 2, (b - 1) / 2) -
                                    1;
      if (b & 1) diff *= -1;
      if (stree[0].seg_query(1, 0, TSIZE - 1, (b + 1) / 2, (c - 1) / 2) +
                  diff <=
              0 ||
      ...
      if ((b ^ c) & 1) {
        ...
        stree[(b & 1) ^ 1].seg_update(1, 0, TSIZE - 1, (c + 1) / 2, TSIZE - 1,
                                      -k);
      }
    }
  }
