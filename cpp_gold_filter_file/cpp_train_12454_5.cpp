...
void merge(int x) {
  ...
  while (i < items[2 * x].size() && j < items[2 * x + 1].size()) {
    if (items[2 * x][i] + dist[2 * x] <=
        items[2 * x + 1][j] + dist[2 * x + 1]) {
...
long long count(int x, int h) {
  if (h <= 0 || x > n) return 0;
  int p =
      lower_bound(items[x].begin(), items[x].end(), h) - items[x].begin() - 1;
  long long ret = 1LL * (p + 1) * h - sum[x][p];
  return ret;
}
int main() {
  int m;
