...
double X[4010], Y[4010], avg1[10010], avg2[10010], Ansx[200000], Ansy[1200000],
    qwq[200000];
...
void Do(double x, double y, double o) {
  V.clear();
  for (int s = 1; s <= k * n; s++) V.push_back(pow(dis(x, y, X[s], Y[s]), o));
  sort(V.begin(), V.end());
...
int main() {
  ...
  for (int i = -50; i <= 50; i++)
    for (int j = -50; j <= 50; j++)
      if (dis(0, 0, i * 20000, j * 20000) <= 1e6)
        Ansx[++cnt] = i * 20000, Ansy[cnt] = j * 20000,
        qwq[cnt] = getval(i * 20000, j * 20000);
  for (int i = 1; i <= k; i++) {
