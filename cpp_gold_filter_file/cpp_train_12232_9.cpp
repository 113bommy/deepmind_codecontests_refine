...
bool bad(pair<int, int> a, pair<int, int> b, pair<int, int> c) {
  long long x1 = a.first, x2 = b.first, x3 = c.first;
  long long y1 = a.second, y2 = b.second, y3 = c.second;
...
bool check(pair<int, int> x) {
  if (x.first >= hull[nhull - 1].first) return false;
  while (hull[cloc + 1].first < x.first) cloc++;
  return !bad(hull[cloc], hull[cloc + 1], x);
}
int main() {
  ios_base::sync_with_stdio(0);
  cin >> N >> M >> X >> Y;
  for (int i = 0; i < N; i++) cin >> hi[i].first >> hi[i].second;
  for (int i = 0; i < M; i++) cin >> lo[i].first >> lo[i].second;
  sort(hi, hi + N);
  sort(lo, lo + M);
  nhull = 1;
  ...
  for (int i = 0; i < M; i++) {
    if (i == M - 1 || (lo[i].first != lo[i + 1].first)) add(lo[i]);
  }
  cloc = 0;
  bool good = true;
  for (int i = 0; i < N; i++)
    if (!check(hi[i])) good = false;
  if (good)
    cout << "Min\n";
  else
    cout << "Max\n";
