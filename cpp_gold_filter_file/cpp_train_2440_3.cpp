...
pii make_pair(int a, int b) { return (pii){a, b}; }
const int N = 250000;
int n, x, t, st[N], w, xb, y, dfn[N], id[N], b[N], m, j, dep[N], li[N], ri[N],
    k, o, q, i, xx, yy, zz;
...
inline pii max(pii a, pii b) { return a > b ? a : b; }
inline pii min(pii a, pii b) { return a < b ? a : b; }
struct node {
...
struct list {
  ...
  inline int size() { return a[rt].size; }
  ...
  int query(int i, int y) {
    ...
    pushdown(a[i].r);
    pushdown(a[i].l);
    if (a[a[i].r].ma.first >= y && a[a[i].r].mi.first <= y)
      return query(a[i].r, y);
