...
void solveA(int stop) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    if (stop != -1) ans[tmp[i - 1].second]++;
    if (stop == i) break;
  }
}
void solveB(int stop) {
  if (stop == 0) return;
  if (stop == -1) sort(B, B + nB, cmp2());
  set<level, cmp> min1;
  set<level, cmp2> min2;
  for (int i = 0; i < nB; i++) {
    min1.insert(B[i]);
  ...
  for (int i = 0; i < nB; i++) {
    ...
    if (i > 0)
      bestB[i * 2 + 1] =
          min(bestB[i * 2 + 1], sum + mind.begin()->first + min2.begin()->a2);
    if (stop == i * 2 + 1) {
      for (int j = 0; j < i; j++) ans[B[j].i] = '2';
...
int main() {
  ...
  solveA(-1);
  solveB(-1);
  pair<long long, pair<int, int>> rans =
      min(make_pair(bestA[W], make_pair(W, 0)),
          make_pair(bestB[W], make_pair(W, 1)));
  for (int i = 0; i <= W; i++)
    rans = min(rans, {bestA[i] + bestB[W - i], {i, 2}});
  memset(ans, '0', sizeof ans);
  if (rans.second.second == 0 || rans.second.second == 2)
    solveA(rans.second.first);
  if (rans.second.second == 1) solveB(rans.second.first);
  if (rans.second.second == 2) solveB(W - rans.second.first);
  printf("%lld\n", rans.first);
  printf("%.*s\n", N, ans);
  return 0;
}
