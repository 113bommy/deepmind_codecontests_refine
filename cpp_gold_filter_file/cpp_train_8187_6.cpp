#include <bits/stdc++.h>
using namespace std;
int head[4010], to[80010], nxt[80010], val[80010], tot = 1, n1, n2, m, S, T,
                                                   dis[4010], deg[4010], t1, t2;
vector<int> ansl[4010];
...
int dinic(int u, int minv) {
  ...
  for (int i = head[u]; i; i = nxt[i])
    if (val[i] && dis[to[i]] == dis[u] + 1 &&
        (t = dinic(to[i], min(minv, val[i])))) {
      val[i] -= t;
      val[i ^ 1] += t;
      minv -= t;
      ans += t;
...
int main() {
  ...
  int mind = 0x3f3f3f3f;
  for (int i = 1; i <= n1 + n2; ++i) mind = min(mind, deg[i]);
  ...
  for (int i = mind; i >= 1; --i) {
    ...
    for (int j = 2; j < (1 + m << 1); j += 2)
      if (val[j]) ansl[i].push_back(j >> 1);
    for (int j = 1 + m << 1; j <= tot; j += 2) ++val[j];
  }
  for (int i = 0; i <= mind; ++i) {
    printf("%d ", ansl[i].size());
    for (int j = 0; j < ansl[i].size(); ++j) printf("%d ", ansl[i][j]);
    putchar(10);
  }
  return 0;
}
