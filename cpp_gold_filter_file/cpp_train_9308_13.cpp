...
struct line {
  ...
  inline int k() { return x1 == x2 ? 1 : (y2 - y1) / (x2 - x1); }
  inline int at(int x) { return y1 + (x - x1) * k(); }
} l1[N], l2[N];
int n1, n2, l[N];
...
bool check(int t) {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    for (int j = 1; !flag && j <= n1; ++j) {
      if (conj(l1[j], cur)) l1[j] = merge(l1[j], cur), flag = 1;
    }
    if (!flag) l1[++n1] = cur;
  }
  n2 = 0;
  for (int i = 1; i <= m; ++i) {
    ...
    for (int j = 1; !flag && j <= n2; ++j) {
      if (conj(l2[j], cur)) l2[j] = merge(l2[j], cur), flag = 1;
    }
    if (!flag) l2[++n2] = cur;
  }
  int ln;
  for (int i = 1; i <= n2; ++i)
    for (int j = 1; j <= n2; ++j) vis[i][j] = 0;
  for (int i = 1; i <= n1; ++i) {
    ln = 0;
    for (int j = 1; j <= n2; ++j)
      if (inter(l1[i], l2[j])) l[++ln] = j;
    for (int j = 1; j <= ln; ++j)
