...
int get(int id, int L, int R, int l, int r) {
  ...
  int mid = (L + R) / 2;
  int ret = 0;
  if (mid > l) ret = max(ret, get(2 * id + 0, L, mid, l, min(mid, r)));
  if (mid < r) ret = max(ret, get(2 * id + 1, mid, R, max(l, mid), r));
  return ret;
}
...
int main() {
  ...
  for (int i = n; i >= 1; i--) {
    int idx = upper_bound(
                  p + 1, p + n + 1,
                  make_pair(make_pair(p[i].first.first + p[i].first.second, -1),
                            -1)) -
              p;
    dp[i] = i;
  ...
  for (int i = 1; i <= n; i++) ans[p[i].second] = dp[i] - i + 1;
