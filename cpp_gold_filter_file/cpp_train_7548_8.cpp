#include <bits/stdc++.h>
using namespace std;
int N, E, n, K, mod, inv[100009], fac[100009], put[100009], dp[100009],
    baza[100009], rev[100009];
...
comp operator*(comp c1, comp c2) {
  return comp(c1.a * c2.a - c1.b * c2.b, c1.a * c2.b + c1.b * c2.a);
}
comp operator-(comp c1, comp c2) { return comp(c1.a - c2.a, c1.b - c2.b); }
...
void FFT(comp *A, bool inv) {
  ...
  if (inv) {
    for (int i = 0; i < N; i++) A[i] = A[i] * comp(1.0 / N);
  }
}
int pow(int a, int b) {
  int p = 1;
...
void Multiply(int a[], int b[], int fin_ans[], int l) {
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < 3; j++)
      A[j][i] = comp((double)(v1 % 1000)), v1 /= 1000,
      B[j][i] = comp((double)(v2 % 1000)), v2 /= 1000;
    fin_ans[i] = 0;
  }
  for (int i = 0; i < 3; i++) FFT(A[i], 0), FFT(B[i], 0);
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) {
      for (int k = 0; k < N; k++) aux[k] = A[i][k] * B[j][k];
      FFT(aux, 1);
      int coef = 1, auxx = i + j;
      while (auxx--) coef = (1LL * coef * 1000) % mod;
      for (int k = 0; k < N; k++)
        fin_ans[k] =
            ((long long)fin_ans[k] + 1LL * coef * aux[k].get_val()) % mod;
    }
  for (int i = 0; i < N; i++) fin_ans[i] = (1LL * fin_ans[i] * fac[i]) % mod;
}
...
int main() {
  ...
  while (N <= 2 * K) N += N, E++;
  for (int i = 0; i < N; i++)
    for (int k = 0; k < E; k++) rev[i] = (rev[i] << 1) | ((i >> k) & 1);
  for (int i = 1; i <= K; i++) baza[i] = 1, dp[i] = n & 1;
  if (n % 2 == 0) dp[0] = 1;
  for (int i = 1; (1 << i) <= n; i++) {
