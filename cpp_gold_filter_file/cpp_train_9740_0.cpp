...
int n, tree_ct, vis[Maxn], ord[Maxn], a[Maxn], Head[Maxn], P[Maxn], rk[Maxn],
    nxt_pos[Maxn], Mini[Maxn];
int rev_tree_ct, rev_a[Maxn], rev_Head[Maxn], rev_rk[Maxn], rev_ord[Maxn];
long long k, ans, maxi = -0x7fffffffffffffffLL, sum[Maxn], tmp[Maxn],
                  las_ans[Maxn], suf_maxi[Maxn], pre_maxi[Maxn], rev_sum[Maxn];
set<int> Se;
struct Tree {
...
void rev_build(int now, int las, int lt, int rt, int pos) {
  if (lt + 1 == rt) {
    ...
    rev_tree[now].cnt =
        rev_tree[rev_tree[now].lson].cnt + rev_tree[rev_tree[now].rson].cnt;
    rev_tree[now].sum =
        rev_tree[rev_tree[now].lson].sum + rev_tree[rev_tree[now].rson].sum;
    rev_tree[now].mini = min(rev_tree[rev_tree[now].lson].mini,
                             rev_tree[rev_tree[now].rson].mini);
    rev_tree[now].maxi = max(rev_tree[rev_tree[now].lson].maxi,
                             rev_tree[rev_tree[now].rson].maxi);
  }
}
int ask_cnt(int root, int lt, int rt, int L, int R) {
  ...
  if (lt == L && rt == R)
    ...
    int mid = (lt + rt) >> 1;
    if (L >= mid)
      return ask_cnt(tree[root].rson, mid, rt, L, R);
    else if (R <= mid)
      return ask_cnt(tree[root].lson, lt, mid, L, R);
    else
      return ask_cnt(tree[root].lson, lt, mid, L, mid) +
             ask_cnt(tree[root].rson, mid, rt, mid, R);
  }
}
long long ask_sum(int root, int lt, int rt, int L, int R) {
  ...
  if (lt == L && rt == R)
    ...
    int mid = (lt + rt) >> 1;
    if (L >= mid)
      return ask_sum(tree[root].rson, mid, rt, L, R);
    else if (R <= mid)
      return ask_sum(tree[root].lson, lt, mid, L, R);
    else
      return ask_sum(tree[root].lson, lt, mid, L, mid) +
             ask_sum(tree[root].rson, mid, rt, mid, R);
  }
}
int ask_mini(int root, int lt, int rt, int L, int R) {
  ...
  if (lt == L && rt == R)
    ...
    int mid = (lt + rt) >> 1;
    if (L >= mid)
      return ask_mini(tree[root].rson, mid, rt, L, R);
    else if (R <= mid)
      return ask_mini(tree[root].lson, lt, mid, L, R);
    else
      return min(ask_mini(tree[root].lson, lt, mid, L, mid),
                 ask_mini(tree[root].rson, mid, rt, mid, R));
  }
}
int get_pos(int root, int lt, int rt, int val) {
  if (lt + 1 == rt)
    ...
    int mid = (lt + rt) >> 1;
    if (tree[tree[root].lson].cnt < val)
      return get_pos(tree[root].rson, mid, rt, val - tree[tree[root].lson].cnt);
    else
      return get_pos(tree[root].lson, lt, mid, val);
  }
}
...
int get_first(Tree tree[], int root, int lt, int rt, int val) {
  if (lt + 1 == rt)
    ...
    int mid = (lt + rt) >> 1;
    if (tree[tree[root].lson].maxi >= val)
      return get_first(tree, tree[root].lson, lt, mid, val);
    return get_first(tree, tree[root].rson, mid, rt, val);
  }
}
long long cost(pair<int, int> x) {
  return (sum[x.second] -
          sum[ord[get_first(tree, Head[x.second + 1], 0, n, x.first - 1)]]) +
         (rev_sum[n - x.first + 1] -
          rev_sum[rev_ord[get_first(rev_tree, rev_Head[n - x.first + 2], 0, n,
                                    n - x.second)]]);
}
...
struct sta {
  pair<int, int> x;
  long long cost;
  sta(const pair<int, int> _x) { x = _x, cost = cost2(_x); }
...
void work(long long t) {
  ...
  while (l + 1 <= r) {
    ...
    for (int i = 1; i <= n; i++) now += cal(i, mid);
    if (now >= t)
      l = mid;
    else
      r = mid - 1;
  ...
  for (int i = 1; i <= n; i++)
    if (t && cal(i, l + 1)) {
      ...
      ans += (las_ans[i] = P[i] * sum[i] -
                           ask_sum(Head[i], 0, n, 0, min(pos1, pos2 + 1)));
    }
  for (int i = 1; i <= n; i++)
    if (t && cal(i, l)) {
  ...
  for (set<int>::iterator it = Se.begin(); it != Se.end(); it++) {
    ...
    it2++;
    if (it2 != Se.end())
      Out.insert(make_pair(*it + 1, *it2 - 1)),
          ans += cost(make_pair(*it + 1, *it2 - 1));
  }
  for (int i = 1; i <= n; i++)
    if (P[i] < i) {
...
void assign(int lt, int rt) {
  ...
  for (set<int>::iterator it = it_l; it != it_r; it++) {
    ...
    set<int>::iterator it2 = it;
    it2++;
    if (it2 != Se.end()) erase(make_pair(*it + 1, *it2 - 1));
  }
...
int main() {
  scanf("%d%lld", &n, &k);
  for (int i = 1; i <= n; i++) scanf("%d", &a[i]), rev_a[i] = a[i];
  reverse(rev_a + 1, rev_a + 1 + n);
  for (int i = 1; i <= n; i++)
    sum[i] = sum[i - 1] + a[i], rev_sum[i] = rev_sum[i - 1] + rev_a[i],
    pre_maxi[i] = max(pre_maxi[i - 1] + a[i], 0LL);
  for (int i = n; i >= 1; i--) suf_maxi[i] = max(suf_maxi[i + 1] + a[i], 0LL);
  for (int i = 0; i < n; i++) tmp[i] = sum[i], ord[i] = rev_ord[i] = i;
  sort(tmp, tmp + n);
  sort(ord, ord + n, [](int x, int y) { return sum[x] < sum[y]; });
  sort(rev_ord, rev_ord + n,
       [](int x, int y) { return rev_sum[x] < rev_sum[y]; });
  ...
  for (long long i = t; i <= k; i++) {
    if ((bool)Se.size() + i <= k && (int)Se.size() + i >= k) {
      ...
      maxi = max(maxi, ans + (Se.size() ? (max(0LL, pre_maxi[*Se.begin() - 1]) +
                                           max(0LL, suf_maxi[*Se.rbegin() + 1]))
                                        : 0));
    }
    add_top();
  }
