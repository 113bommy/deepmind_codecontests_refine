...
double solve(int s) {
  sort(a, a + s, [&](const pair<int, int> &p, const pair<int, int> &q) {
    return ccw(a[s], p, q) > 0;
  });
  memset(has, 0, sizeof(has));
  for (int i = 0; i < s - 1; i++) {
    pair<int, int> w = a[i + 1];
    for (int j = i + 2; j < s; j++) {
      if (ccw(a[i], a[j], w) < 0) {
        w = a[j];
      } else
  ...
  for (int i = 0; i < s; i++) {
    ...
    sort(lo[i], lo[i] + ls[i],
         [&](const int &p, const int &q) { return ccw(a[i], a[p], a[q]) < 0; });
    sort(hi[i], hi[i] + hs[i],
         [&](const int &p, const int &q) { return ccw(a[i], a[p], a[q]) < 0; });
  }
  for (int i = 0; i < s; i++) {
    for (int j = i + 1; j < s; j++) {
      if (has[i][j])
        ...
        dp[i][j] = ccw(a[s], a[i], a[j]);
    }
  }
  for (int i = 1; i <= k - 3; i++) {
    long long nxt[MAXN][MAXN];
    for (int j = s - 1; j >= 0; j--) {
      long long curmx = -1e18;
      int pos = 0;
