...
vector<int> v[MAXN];
int root[MAXN], ls[MAXN * 42], rs[MAXN * 42], add[MAXN * 42], tot;
...
int main() {
  int n, k;
  scanf("%d%d", &n, &k);
  build(1, 100000, root[0]);
  for (int i = 1; i <= n; i++) {
    int x;
    scanf("%d", &x);
    if (v[x].size() >= k)
      insert(1, 100000, v[x][v[x].size() - k] + 1, i, root[i], root[i - 1]);
    else
      insert(1, 100000, 1, i, root[i], root[i - 1]);
    v[x].push_back(i);
  }
  int lst = 0, m;
  scanf("%d", &m);
