...
int l;
bool operator<(const fix &a, const fix &b) { return a.t % l < b.t % l; }
...
int main() {
  ...
  l = read();
  for (int i = 1; i <= n; i++)
    f[i].t = read(), f[i].x = read(), f[i].y = read();
  sort(f + 1, f + n + 1);
  for (int i = 1; i <= n; i++) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    if (pa == pb)
      if (!(f[i].x >= f[i - 1].x && f[i].x - f[i - 1].x <= qb - qa &&
            f[i].y >= f[i - 1].y && f[i].y - f[i - 1].y <= qb - qa)) {
        puts("NO");
        return 0;
      }
    if (pb > pa) {
      ...
      if (f[i].x - f[i - 1].x > qb - qa)
        s1l = max(s1l, (f[i].x - f[i - 1].x - (qb - qa) - 1) / (pb - pa) + 1);
      s1r = min(s1r, (f[i].x - f[i - 1].x) / (pb - pa));
      if (f[i].y - f[i - 1].y > qb - qa)
        s2l = max(s2l, (f[i].y - f[i - 1].y - (qb - qa) - 1) / (pb - pa) + 1);
    ...
    if (pb < pa) {
      ...
      if (f[i - 1].x > f[i].x)
        s1l = max(s1l, (f[i - 1].x - f[i].x - 1) / (pa - pb) + 1);
      s1r = min(s1r, (qb - qa + f[i - 1].x - f[i].x) / (pa - pb));
      if (f[i - 1].y > f[i].y)
        s2l = max(s2l, (f[i - 1].y - f[i].y - 1) / (pa - pb) + 1);
  ...
  for (int i = 1; i <= n; i++)
    if (f[i].t % l > f[i - 1].t % l) {
      ...
      for (int j = f[i - 1].t % l + 1; j <= f[i - 1].t % l + dx; j++)
        ans1[j] = 1;
      int dy = (f[i].y - f[i - 1].y) - (f[i].t / l - f[i - 1].t / l) * s2;
      for (int j = f[i - 1].t % l + 1; j <= f[i - 1].t % l + dy; j++)
        ans2[j] = 1;
    }
  int dx = s1 - (f[n].x - f[n].t / l * s1),
      dy = s2 - (f[n].y - f[n].t / l * s2);
