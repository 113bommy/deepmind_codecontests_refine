...
long long dis1[1000005], dis2[1000005], f1[1000005], f2[1000005], s1[1000005],
    s2[1000005];
int main() {
  scanf("%d%d", &n, &q);
  for (int i = 1; i <= n; i++) {
  ...
  for (int i = 1; i <= lg2[n]; i++)
    for (int j = 1; j <= n; j++)
      maxp[i][j] = a[maxp[i - 1][j]] > a[maxp[i - 1][j + (1 << (i - 1))]]
                       ? maxp[i - 1][j]
                       : maxp[i - 1][j + (1 << (i - 1))];
  for (int i = 1; i <= n; i++) {
  ...
  for (int i = 1; i <= q; i++) {
    int d = lg2[r[i] - l[i] + 1];
    int p = a[maxp[d][l[i]]] > a[maxp[d][r[i] - (1 << d) + 1]]
                ? maxp[d][l[i]]
                : maxp[d][r[i] - (1 << d) + 1];
    printf("%lld ", 1 + s1[r[i]] - s1[p] - f1[p] * (r[i] - p) + s2[l[i]] -
                        s2[p] - f2[p] * (p - l[i]));
  }
  printf("\n");
  return 0;
