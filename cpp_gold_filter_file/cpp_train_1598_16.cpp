...
long double go(long long i, long long wins, long long bags) {
  ...
  if (bgs[i] == -1) {
    ...
    ans += (p[i] * go(i + 1, wins + 1, min(202ll, 1 + bags + bgs[i])));
  }
...
void solve() {
  ...
  bgs.resize(n);
  read(bgs);
  for (long long i = int(0); i < int(204); i++) {
    for (long long j = int(0); j < int(204); j++)
      for (long long k = int(0); k < int(204); k++) dp[i][j][k] = -1.0;
  }
