#include <bits/stdc++.h>
int n, mid, size[250 + 5], val[250 + 5], l[250 + 5], r[250 + 5];
bool dp[250 + 5][2][3], vis[250 + 5][2][3];
  ...
  if (skip) {
    ...
    if (size[pos] & 1) {
      if (size[l[pos]] & 1) {
        ans = merge(
            ans,
            merge(getdp(l[pos], who, 0),
                  getdp(r[pos], !who, 1 + (size[l[pos]] == 1 && skip == 1)),
                  !who),
            who);
        ans = merge(
            ans,
            merge(getdp(r[pos], who, 0),
                  getdp(l[pos], !who, 1 + (size[r[pos]] == 1 && skip == 1)),
                  !who),
            who);
      } else {
        if (size[l[pos]]) {
          ans = merge(
              ans,
              merge(getdp(l[pos], who, 1 + (size[r[pos]] == 0 && skip == 1)),
                    getdp(r[pos], !who, 0), !who),
              who);
        }
        if (size[r[pos]]) {
          ans = merge(
              ans,
              merge(getdp(r[pos], who, 1 + (size[l[pos]] == 0 && skip == 1)),
                    getdp(l[pos], !who, 0), !who),
              who);
        }
      }
    } else {
    ...
    if (size[pos] & 1) {
      ...
      ans = merge(merge(getdp(l[pos], who, 0), getdp(r[pos], !who, 0), !who),
                  merge(getdp(r[pos], who, 1), getdp(l[pos], !who, 1), !who),
                  who);
    }
  }
  return ans;
