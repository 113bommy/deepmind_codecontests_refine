...
struct segment_tree {
  ...
  segment_tree() {
    ...
    for (int i = 0; i < (nax * 4); ++i) sg[i] = node();
  }
  ...
  void add(int v, int x, int y, int pos, int sgn) {
    ...
    int mid = x + y >> 1;
    if (pos <= mid)
      add(v << 1, x, mid, pos, sgn);
    else
      add(v << 1 ^ 1, mid + 1, y, pos, sgn);
  ...
  void ins(int pos) { add(1, 1, lim, pos, 1); }
  void era(int pos) { add(1, 1, lim, pos, -1); }
...
vector<int> pref_sum(vector<int> &v) {
  ...
  res[0] = v[0];
  for (int i = 1; i < (int)v.size(); ++i) res[i] = res[i - 1] + v[i];
  return res;
}
int main() {
  ...
  for (int i = 0; i <= (int)both.size(); ++i) {
    ...
    if (min(pt_a, pt_b) + 1 >= need) {
      if (need) c += pref_a[need - 1] + pref_b[need - 1];
      int want = m - i - need * 2;
      if (want >= 0) {
