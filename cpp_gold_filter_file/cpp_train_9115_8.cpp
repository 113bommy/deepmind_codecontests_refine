...
struct state {
  ...
  state(int _l, int _r, int _v, int _c, int _w)
      : l(_l), r(_r), v(_v), c(_c), w(_w) {}
};
state plus(state a, state b) {
  return state(-1, -1, (1ll * a.v * qpow(10, b.c) + b.v) % P,
               (a.c + b.c) % (P - 1), (a.w + b.w) % P);
}
int as(int n) { return 1ll * n * (n - 1) % P * qpow(2, P - 2) % P; }
...
int ags(int a, int n, int c) {
  if (a == 1) {
    ...
    return (1ll * c * an + 1ll * (P - a) * (an + P - 1) % P * inv) % P * inv %
           P;
  }
}
state solve(std::string x) {
  ...
  for (int i = 1; i < n; ++i) {
    ...
    tmp.v = 1ll * qpow(10ll * w % P, (P + 10ll * c - 2) % (P - 1)) *
            ags(qpow(10ll * w % P, P - 2), 10ll * c % (P - 1), 10ll * w % P) %
            P;
    tmp.v = (tmp.v + 1ll * qpow(10ll * w % P, (P + 10ll * c - 2) % (P - 1)) *
                         (P - ags(qpow(10ll * w % P, P - 2), c, w)) % P) %
            P;
    res = plus(res, tmp);
    c = 10ll * c % (P - 1);
    w = 10ll * w % P;
  ...
  state tmp(-1, -1, 0, 1ll * (xc + P - 1 - c) * n % (P - 1),
            1ll * (xw + P - w) % P * n % P);
  tmp.v = 1ll * qpow(10ll * w % P, (P + xc - 2) % (P - 1)) *
          ags(qpow(10ll * w % P, P - 2), xc, xw) % P;
  tmp.v = (tmp.v + 1ll * qpow(10ll * w % P, (P + xc - 2) % (P - 1)) *
                       (P - ags(qpow(10ll * w % P, P - 2), c, w)) % P) %
          P;
  res = plus(res, tmp);
  return res;
...
state minus(std::string a, std::string b) {
  ...
  return state(-1, -1, (1ll * (P - sa.v) * qpow(10, c) + sb.v) % P, c,
               (sb.w - sa.w + P) % P);
}
