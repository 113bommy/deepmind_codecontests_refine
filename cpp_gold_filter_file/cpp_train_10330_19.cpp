...
inline bool check(int now) {
  for (int i = 0; i <= 401; i++)
    for (int j = 0; j <= 401; j++) f[i][j] = 100000;
  f[0][now] = nxt[0][t[now] - 'a'];
  for (int i = now + 1; i <= n; i++)
    if (f[0][i - 1] < 100000) f[0][i] = nxt[f[0][i - 1] + 1][t[i] - 'a'];
  f[1][now - 1] = nxt[0][t[1] - 'a'];
  for (int i = 2; i < now; i++)
    if (f[i - 1][now - 1] < 100000)
      f[i][now - 1] = nxt[f[i - 1][now - 1] + 1][t[i] - 'a'];
  for (int i = 1; i < now; i++)
    for (int j = now; j <= n; j++) {
      if (f[i - 1][j] < 100000) {
...
int main() {
  ...
  while (q--) {
    ...
    for (int i = 0; i <= 401; i++)
      for (int j = 0; j < 26; j++) nxt[i][j] = 100000;
    for (int i = m; i >= 0; i--) {
