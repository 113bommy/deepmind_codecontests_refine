...
inline char gc() {
  ...
  return p1 == p2 && (p2 = (p1 = buf) + fread(buf, 1, 1048576, stdin), p1 == p2)
             ? EOF
             : *p1++;
}
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    W[i][i] = w[i];
    for (int j = i + 1; j < N; j++) W[i][j] = merge(W[i][j - 1], w[j]);
  }
  for (int i = 0; i < N; i++) {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < 20; j++)
      for (int k = 0; k < 20; k++) v[i][j].insert(a[i * SIZE + j * 20 + k]);
    for (int j = 0; j < 20; j++) {
      V[i][j][j] = v[i][j];
  ...
  for (int l, r, i = 1; i <= q; i++) {
    ...
    if (L == R) {
      ...
      if (L1 == R1) {
        ...
        for (int j = l; j <= r; j++) res.insert(a[j]);
      } else {
        res = V[L][L1 + 1][R1 - 1];
        for (int j = l; j < L * SIZE + (L1 + 1) * 20; j++) res.insert(a[j]);
        for (int j = R * SIZE + R1 * 20; j <= r; j++) res.insert(a[j]);
      }
    } else
      res = merge(merge(suf[L][l - L * SIZE], W[L + 1][R - 1]),
                  pre[R][r - R * SIZE]);
    int ans = 0;
    for (int j = 19; j >= 0; j--) ans = max(ans, ans ^ res.b[j]);
    printf("%d\n", ans);
  }
  return 0;
}
