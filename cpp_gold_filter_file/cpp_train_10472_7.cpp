...
struct Trie {
  ...
  void Reset(int u, int v) { vis[u] = v; }
  int Get(int u) { return vis[u]; }
} trie;
struct Expr {
  string name;
  string exp;
...
int dp(const string& s, int L, int R) {
  ...
  for (long long j = (L); j <= (R); j++) {
    tmp.append(1, s[j]);
    if (!isdigit(s[j]) && !islower(s[j]) && !isupper(s[j])) flag = 0;
  }
  for (long long j = (L); j <= (R); j++) {
    if (!islower(s[j]) && !isupper(s[j])) flag2 = 0;
  }
  if (flag2 && trie.Find(tmp) != -1) {
    return f[L][R] = trie.Get(trie.Find(tmp));
  }
  if (flag) {
    return f[L][R] = 1;
  }
...
int main() {
  ...
  for (long long i = (1); i <= (n); i++) {
    memset(f, 0, sizeof f);
    trie.Reset(trie.Find(expr[i].name),
               dp(expr[i].exp, 0, expr[i].exp.length() - 1));
  }
  memset(f, 0, sizeof f);
