...
ll go(int f, int t, ll x1, ll y1, ll x2, ll y2) {
  if (x1 >= x2 || y1 >= y2 || x2 < 0 || y2 < 0) return 0;
  if (t == -1) return g[x1][y1][f] - g[x2][y1][f] - g[x1][y2][f] + g[x2][y2][f];
  ll nn = (ll)n << t;
  ll mm = (ll)m << t;
  if (x1 == 0 && x2 == 2 * nn) return (ll)(x2 - x1) * (y2 - y1) / 2;
  if (y1 == 0 && y2 == 2 * mm) return (ll)(x2 - x1) * (y2 - y1) / 2;
  return go(f, t - 1, x1, y1, min(x2, nn), min(y2, mm)) +
