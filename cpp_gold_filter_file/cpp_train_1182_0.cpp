...
int main(int argc, char** argv) {
    ...
    for (L[now] = n + 1, d = 1; now <= n;) {
      ...
      if (isdigit(op[now])) {
        ...
        if (d)
          ...
          now = L[now];
        if (now <= n && !isdigit(op[now]))
          R[L[pre]] = R[pre], L[R[pre]] = L[pre];
      }
    }
  }
  for (int i = 1, l, r, le, ri; i <= cases; ++i) {
    ...
    ri = cnt;
    if (r < n)
      if ((tmp = upper_bound(pos[r + 1].begin(), pos[r + 1].end(), le)) !=
          pos[r + 1].end())
        cmin(ri, (*tmp) - 1);
    if (t[l])
      if ((tmp = upper_bound(pos[t[l]].begin(), pos[t[l]].end(), le)) !=
          pos[t[l]].end())
        cmin(ri, (*tmp) - 1);
    if ((tmp = lower_bound(part.begin(), part.end(), le)) != part.end())
      cmin(ri, (*tmp));
    for (int j = 0; j <= 9; ++j)
      printf("%d%c", sum[ri][j] - sum[le - 1][j], j == 9 ? '\n' : ' ');
  }
  return 0;
}
