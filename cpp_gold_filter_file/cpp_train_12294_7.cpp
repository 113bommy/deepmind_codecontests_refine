...
void FFT(int a[], int coef) {
  static unsigned long long f_a[FFTmx];
  static int w[FFTmx];
  ...
  const unsigned long long md2 = 1ull * mod * mod;
  for (int l = 1; l < fsz; l <<= 1) {
    int l2 = l << 1, u = FFTpmx / l2;
    if (coef == 1)
      for (int j = 0, j2 = 0; j < l; j++, j2 += u) w[j] = w_pre[j2];
    else
      for (int j = 0, j2 = FFTpmx; j < l; j++, j2 -= u) w[j] = w_pre[j2];
    for (int i = 0; i < fsz; i += l2) {
      for (int j = 0; j < l; j++) {
        unsigned long long tmp = f_a[i + j + l] % mod * w[j];
        f_a[i + j + l] = f_a[i + j] + md2 - tmp;
        f_a[i + j] += tmp;
      }
    }
    if (l == (1 << 16))
      for (int j = 0; j < fsz; j++) f_a[j] %= mod;
  }
...
vector<int> polymul(const vector<int> &a, const vector<int> &b) {
  ...
  for (int i = 0; i < a.size() + b.size() - 1; i++) ret.push_back(A[i]);
  return ret;
}
vector<int> polyadd(const vector<int> &v1, const vector<int> &v2) {
  vector<int> ret;
  for (int i = 0; i < max(v1.size(), v2.size()); i++)
    ret.push_back(((i < v1.size() ? v1[i] : 0) + (i < v2.size() ? v2[i] : 0)) %
                  mod);
  return ret;
}
...
pair<vector<int>, vector<int> > solve(const vector<int> &v) {
  vector<int> retK, retB;
  if (v.size() == 1) {
    retB.resize(csz[v[0]] + 1);
    retB[csz[v[0]]] = 1;
    return make_pair(G[v[0]], retB);
  }
  int alls = 0;
  ...
  for (int i = 0; i < v.size(); i++)
    if (i <= m)
      ...
      v1.push_back(v[i]);
  pair<vector<int>, vector<int> > L = solve(v0), R = solve(v1);
  vector<int> tmp;
  tmp.resize(curs);
  for (int i = 0; i < R.second.size(); i++) tmp.push_back(R.second[i]);
  tmp.swap(R.second);
  retK = polymul(L.first, R.first);
  retB = polyadd(polymul(R.first, L.second), R.second);
...
void dfs2(int x, int tp) {
  ...
  if (x == tp) {
    ...
    pair<vector<int>, vector<int> > pp = solve(v);
    F[x] = polyadd(pp.first, pp.second);
  }
}
int main() {
  FFTprecalc();
