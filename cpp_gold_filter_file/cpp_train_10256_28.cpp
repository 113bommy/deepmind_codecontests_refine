...
struct Polygon {
  ...
  void convexHull(int opt = 1) {
    ...
    for (int i = 0; i < sze(); ++i) {
      while (top > 0 &&
             sgn((res[top] - res[top - 1]) ^ (p[i] - res[top - 1])) < opt)
        --top;
      res[++top] = p[i];
    }
    int pre = top;
...
void run() {
  ...
  po.add(Point(0.0, 0.0));
  po.add(Point((db)(*max_element(a + 1, a + 1 + n)), 0.0));
  po.add(Point(0.0, (db)(*max_element(b + 1, b + 1 + n))));
  po.convexHull();
  ...
  for (auto &it : li) {
    ...
    if (sgn(now.x) > 0 && sgn(now.y) > 0 && it.pointOnSeg(now)) {
      chmin(ans, min(p * 1.0 / (db)now.x, q * 1.0 / (db)now.y));
    }
  }
  pt(ans);
}
