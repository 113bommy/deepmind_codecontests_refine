...
if len(coord) == 1 and coord_left == []:
    ...
    while 1:
        if current_direction == 'left':
            if coord_left == []:
                break
            apple_left += min(coord_left)[1]
            del coord_left[coord_left.index(min(coord_left))]
    ...
    while 1:
        if current_direction == 'left':
            if dup_coord_left == []:
                break
            apple_right += min(dup_coord_left)[1]
            del dup_coord_left[dup_coord_left.index(min(dup_coord_left))]
            current_direction = reverse(current_direction)
        elif current_direction == 'right':
            if dup_coord_right == []:
                break
