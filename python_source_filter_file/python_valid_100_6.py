def main():
    ...
    for i in range(tc): 
        ...
        if cnt[a[-1]]>=2:
            ...
            # max-1 se kam kitne hai = n-1-cnt[max-1]
            z= n-1-cnt[a[-1]-1]
            galat=0
            for i in range(z+1):
                galat+=nCr(z,i)*fact[i]*fact[n-1-i]
            wi(fact[n]-galat)
...
def rant():
    ...
    #   Max/Min Answer including ith element or Ways to Answer using first i elements?
    # 
    ...
    #   (if i to i+1 formula not making sense make i to i-1 formula)
    #
    ...
    # 
    # 14.Two-Pointer Solution or Optimization??
    # 
    # 15.Sets & Multisets faster and easier to copy in CPP
    # 
    # 16.Critical Observations-Observe carefully and think about effects of something happening given in quesn.
    #   For example-Maybe complex problem for all posns can be broken into easier sub problem for Even positions and Odd positions.
    #   because if observed carefully even and odd positions are getting affected independently then we can combine answer 
    #
    # RANT ENDS
