...
for _ in range(int(input()) if True else 1):
    ...
    available2 = SortedList([])
    cur = 0
    i = 0
    while i <= n:
        if available.__len__():
            ans[i] = min(ans[i], available.__getitem__(0) - i)
        if cur != k and pairs[cur][0] == i:
            available.remove(pairs[cur][0] + pairs[cur][1])
            available2.add(pairs[cur][1] - pairs[cur][0])
            cur += 1
        
        if available2.__len__():
            ans[i] = min(ans[i], available2.__getitem__(0) + i)
        i += 1
    print(*ans[1:])