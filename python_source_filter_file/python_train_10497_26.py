...
for j in range(K):
  for i in range(2*K):
    if i<K:
      ...
      BW_res[j][i] += RANGE_SUM_in_ACC(BW_ACC,0,max(0,j-1),0,max(0,i-1))
      BW_res[j][i] += RANGE_SUM_in_ACC(BW_ACC,0,max(0,j-1),min(2*K,i+K),2*K)
      BW_res[j][i] += RANGE_SUM_in_ACC(BW_ACC,min(2*K,j+K),2*K,0,max(0,i-1))
