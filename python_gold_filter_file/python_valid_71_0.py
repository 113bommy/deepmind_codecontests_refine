...
for _ in range (t):
    ...
    if k==1:
        ...
        def dfs(a,b,curr,prev,ind):
            ...
            m2=max(a,b)
            if prev:
                dfs(a,b,nx+m1,1,ind+1)
            else:
                if int(s[ind])<m1:
                    dfs(a,b,nx+m1,1,ind+1)
                elif int(s[ind])==m1:
                    dfs(a,b,nx+m1,0,ind+1)
                if int(s[ind])<m2:
                    dfs(a,b,nx+m2,1,ind+1)
                elif int(s[ind])==m2:
                    dfs(a,b,nx+m2,0,ind+1)
        dfs(st[0],st[1],st[0],0,1)
        dfs(st[0],0,st[0],0,1)
        if st[1]!=0:
            dfs(st[0],st[1],st[1],0,1)
        if st[1]!=9:
            dfs(st[0],st[1]+1,st[0],0,1)
            dfs(st[0],st[1]+1,st[1]+1,0,1)
        print(mn[0])