...
def get_vecs(point, operation):
    ...
    vec_1 = 'x'.join(list(map(lambda x: str(x),curr_point))) + 'x'.join(list(map(lambda x: str(x),point)))
    vec_2 = 'x'.join(list(map(lambda x: str(x),point))) + 'x'.join(list(map(lambda x: str(x),curr_point)))
