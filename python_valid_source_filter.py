...
def make_python_file(pid, index, code):
    with open(f'./python_source_filter_file/python_valid_{pid}_{index}.py', 'w') as f:
        f.write(code)
    ...
    for index, data in enumerate(input_data_list):
        ...
        try:
            process = subprocess.run(
                ['python', f"./python_source_filter_file/{file}"], input=python_input, text=True, errors="replace", capture_output=True, timeout=2
            )
            try:
...
def compile_and_test(pid_index_codes):
    ...
    script_file_path = f"./python_source_filter_file/python_valid_{pid}_{index}.py"
    ...
    try:
        ...
        with open(f'./python_result/exclude_valid_source.txt', "w", encoding='utf-8') as err_log_file:
            err_log_file.write(f"{script_file_path}\n")
    else:
        if execute_result.returncode != 0:
            return (pid, index, False)

    input_list = python_valid[pid]['test_case']['input']
    output_list = python_valid[pid]['test_case']['output']

    success_check = run_test_cases(f"python_valid_{pid}_{index}.py", input_list, output_list)
    return (pid, index, success_check)

def save_results(pid, results):
    try:
        with open(f'./python_valid_source_result/python_valid_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./python_valid_source_result/python_valid_check_{pid}.json', 'w') as f:
...
if __name__ == "__main__":
    delete_all_files_in_folder('./python_valid_source_result')
    with open('./python_data/python_valid_gold_filtered.json', 'r') as f:
        python_valid = json.load(f)
    # python_valid = concat_valid_case(python_valid)

    tasks = []
    for idx, (pid, value) in enumerate(list(python_valid.items())):
        for index, codes in enumerate(value['code_pair']):
            tasks.append((pid, index, codes[1]))
