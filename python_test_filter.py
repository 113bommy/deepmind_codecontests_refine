...
python_json_path = './python_data/python_test.json'
...
def make_python_file(pid, index, code):
    with open(f'./python_file/python_test_{pid}_{index}.py', 'w') as f:
        f.write(code)
...
def run_test_cases(file, input_data_list, expected_output_list):
    ...
    if len(output_list) != len(input_data_list):
        with open(f'./python_result/exclude_test_gold.txt', "w", encoding='utf-8') as err_log_file:
...
def compile_and_test(pid_index_codes):
    ...
    script_file_path = f"./python_file/python_test_{pid}_{index}.py"
    ...
    input_list = python_test[pid]['test_case']['input']
    output_list = python_test[pid]['test_case']['output']

    success_check = run_test_cases(f"python_test_{pid}_{index}.py", input_list, output_list)
    return (pid, index, success_check)

def save_results(pid, results):
    try:
        with open(f'./python_test_result/python_test_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./python_test_result/python_test_check_{pid}.json', 'w') as f:
...
with open('./python_data/python_test_refine.json', 'r') as f:
    python_test = json.load(f)
python_test = concat_test_case(python_test)

if __name__ == "__main__":
    tasks = []
    for idx, (pid, value) in enumerate(list(python_test.items())):
