import sys
# input = sys.stdin.readline
input = sys.stdin.buffer.readline
import math
from itertools import combinations
EPS = 10**-7


class P2():
    def __init__(self, x: float, y: float):
        self.x = x
        self.y = y
        self.norm = (self.x**2 + self.y**2)**0.5

    def __add__(self, other):
        return P2(self.x + other.x, self.y + other.y)

    def __sub__(self, other):
        return P2(self.x - other.x, self.y - other.y)

    def __mul__(self, c: float):
        return P2(self.x * c, self.y * c)

    def __truediv__(self, c: float):
        return P2(self.x / c, self.y / c)

    def __eq__(self, other):
        return (abs(self.x - other.x) < EPS) and (abs(self.y - other.y) < EPS)

    def dot(self, other) -> float:
        return self.x * other.x + self.y * other.y

    def det(self, other) -> float:
        return self.x * other.y - self.y * other.x

    def rot90(self):
        return P2(-self.y, self.x)


class Circle():
    def __init__(self, p: P2, r: float):
        self.p = p  # center of the circle
        self.r = r  # radius of the circle

    def contains(self, p: P2, allow_on_edge: bool) -> bool:
        if allow_on_edge:
            return (self.p - p).norm < self.r + EPS
        else:
            return (self.p - p).norm < self.r - EPS

    def cct(self, other) -> int:
        # count common tangent
        if self.p == other.p:
            # center is same
            return float('inf') if abs(self.r - other.r) < EPS else 0

        d = other.p - self.p
        if d.norm > self.r + other.r + EPS:
            # 離れてる
            return 4
        elif d.norm > self.r + other.r - EPS:
            # 外接
            return 3
        elif d.norm > abs(self.r - other.r) + EPS:
            # 交点2つ
            return 2
        elif d.norm > abs(self.r - other.r) - EPS:
            # 内接
            return 1
        else:
            # 内包
            return 0

    def intersection_points_with_circle(self, other):
        cct = self.cct(other)
        d = other.p - self.p
        intersections = []
        if cct == 1:
            # 内接
            if self.r > other.r:
                intersections.append(self.p + d * (self.r / d.norm))
            else:
                intersections.append(self.p - d * (self.r / d.norm))
        elif cct == 3:
            # 外接
            intersections.append(self.p + d * (self.r / d.norm))
        elif cct == 2:
            # 交点2個
            x = (self.r**2 - other.r**2 + d.norm**2) / (2 * d.norm)
            h = math.sqrt(max(self.r**2 - x**2, 0.0))
            v = d.rot90() * (h / d.norm)
            dx = self.p + d * (x / d.norm)
            intersections.append(dx + v)
            intersections.append(dx - v)

        return intersections


n, k = map(int, input().split())
p = []
c = []
for _ in range(n):
    x, y, cc = map(float, input().split())
    p.append(P2(x, y))
    c.append(cc)

dist = [[(p[i] - p[j]).norm for j in range(n)] for i in range(n)]


def check(r):
    circles = [Circle(p[i], r / c[i]) for i in range(n)]

    # 半径rの円２つの交点の列挙
    cand = [pi for pi in p]
    for c_i, c_j in combinations(circles, r=2):
        cand_ij = c_i.intersection_points_with_circle(c_j)
        cand.extend(cand_ij)

    for cand_p in cand:
        cnt = 0
        for c_i in circles:
            if (c_i.p - cand_p).norm < c_i.r + EPS:
                cnt += 1
                if cnt >= k:
                    return True

    return False


lb = 0.0  # False
ub = 3000 * 100  # True
while ub - lb > EPS:
    mid = (ub + lb) / 2
    if check(mid):
        ub = mid
    else:
        lb = mid

print(ub)
