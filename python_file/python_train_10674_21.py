...
def tricky_sum():
    ...
    for _ in range(int(input())):
        ...
        while pow_two <= number:
            ...
            pow_two *= 2 #pow_two + int(pow_two << 1)
            print(pow_two)
            #print(pow_two)
        ...
        trick_sum = sum_formula - 2*pow_two+2
        print(trick_sum)
    return
...
def three_indices():
    ...
    for _ in range(int(input())):
        ...
        idx_i, idx_j, idx_k = 1 , 2 , 3
        num_i, num_j, num_k = 0, 0, 0

        for idx in range(1,n-1):
            if(permutation[idx]>permutation[idx-1] and permutation[idx] > permutation[idx+1]):
                print('YES')
...
def three_friends():
    ...
    for test_cases in range(int(input())):
        ...
        coords_neu = sorted(coords)



        if coords_neu[2] - coords_neu[0] <= 2:
            print(0)
        else:
            print(2*(coords_neu[2] - coords_neu[0])-4)
    return
...
def cookies():
    ...
    bag_content = list(map(int, input().split()))

    sum_cookies = sum(bag_content)
