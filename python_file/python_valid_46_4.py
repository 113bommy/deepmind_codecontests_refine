    ...
    for start in range (0,n,2):
        ...
        for end in range (start+1,n):
            if end%2==0:
                ...
                curr-=a[end]
                if curr>=0:
                    if curr<a[start] and curr<=m:
                        #print("A",start,end,min(a[start]-curr,a[end],m-curr),m)
                        ans+=min(a[start]-curr,a[end],m-curr+1)
                        #print(ans)
                    m=min(m,curr)
                    continue
                curr+=a[end]
                #print("B",start,end,min(a[start],m,curr),m)
                ans+=min(a[start],m+1,curr)
                #print(ans)
                break
    print(ans)