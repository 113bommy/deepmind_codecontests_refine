...
def make_cpp_file(pid, index, code):
    with open(f'./cpp_source_filter_file/cpp_test_{pid}_{index}.cpp', 'w') as f:
        f.write(code)
...
def compile_and_test(pid_index_codes):
    ...
    compile_result = subprocess.run(
        ["g++-7", "-std=c++11", "-static", '-O2', "-o", compiled_file_path, f'./cpp_source_filter_file/cpp_test_{pid}_{index}.cpp'],
        capture_output=True
    )

    if compile_result.returncode != 0:
        with open(f'./cpp_result/exclude_test_source.txt', "a", encoding='utf-8') as err_log_file:
...
def save_results(pid, results):
    try:
        with open(f'./cpp_test_source_result/cpp_test_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./cpp_test_source_result/cpp_test_check_{pid}.json', 'w') as f:
...
if __name__ == "__main__":
    delete_all_files_in_folder('./cpp_test_source_result')
    with open('./cpp_data/cpp_test_gold_filtered.json', 'r') as f:
