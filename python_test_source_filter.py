...
def make_python_file(pid, index, code):
    with open(f'./python_source_filter_file/python_test_{pid}_{index}.py', 'w') as f:
        f.write(code)
...
def compile_and_test(pid_index_codes):
    ...
    script_file_path = f"./python_source_filter_file/python_test_{pid}_{index}.py"
    ...
    try:
        ...
        with open(f'./python_result/exclude_test_source.txt', "w", encoding='utf-8') as err_log_file:
...
def save_results(pid, results):
    try:
        with open(f'./python_test_source_result/python_test_check_{pid}.json', 'r+') as f:
