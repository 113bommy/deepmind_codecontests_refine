...
struct SegmentTree {
  ...
  void update(int root) {
    if (a[a[root].lc].tag == a[a[root].rc].tag)
      a[root].tag = a[a[root].lc].tag;
    else
      a[root].tag = -1;
  }
  ...
  void modify(int root, int l, int r, int ql, int qr, int d) {
    if (l == ql && r == qr && a[root].tag != -1) {
      if (a[root].tag != 0) BIT.modify(a[root].tag, -(r - l + 1));
      a[root].tag = d, BIT.modify(a[root].tag, r - l + 1);
      return;
    }
    int mid = (l + r) / 2;
    pushdown(root);
  ...
  void modify(int l, int r, int d) { modify(root, 1, n, l, r, d); }
  int query(int root, int l, int r, int pos) {
    if (a[root].tag != -1) return a[root].tag;
    int mid = (l + r) / 2;
    if (mid >= pos)
      return query(a[root].lc, l, mid, pos);
    else
      return query(a[root].rc, mid + 1, r, pos);
  }
  int query(int pos) { return query(root, 1, n, pos); }
} ST;
vector<int> a[MAXN];
...
int query(int x) {
  ...
  return BIT.query(tmp - 1) + depth[x] + depth[pos[tmp]] -
         2 * depth[lca(x, pos[tmp])] + 1;
}
...
int main() {
  ...
  for (int i = 1; i <= m; i++) {
    ...
    scanf("\n%s%d", s, &x);
    if (s[0] == 'u')
      modify(Max, x, i + n), pos[i + n] = Max, Max = x;
    else if (s[0] == 'w')
      printf("%d\n", query(x));
    else if (s[0] == 'c') {
      read(y);
