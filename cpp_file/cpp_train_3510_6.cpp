...
struct base {
  ...
  inline base operator+(const base& b) { return base(re + b.re, im + b.im); }
  inline void operator+=(const base& b) { re += b.re, im += b.im; }
  ...
  inline base operator/(int b) const { return base(re / b, im / b); }
  inline base conj() const { return base(re, -im); }
};
vector<int> rev;
vector<base> wlen_pw;
void fft(base a[], int n, bool invert) {
  ...
  for (int len = 2; len <= n; len <<= 1) {
    ...
    for (int i = 0; i < n; i += len) {
      base t, *pu = a + i, *pv = a + i + len2, *pu_end = a + i + len2,
              *pw = &wlen_pw[0];
...
vector<long long> multiply(const vector<int>& a, const vector<int>& b) {
  ...
  for (size_t i = 0; i < a.size(); ++i) P[i].re = a[i];
  for (size_t i = 0; i < b.size(); ++i) P[i].im = b[i];
  size_t n = 2;
  ...
  fft(&Q[0], n, true);
  res.resize(n);
  for (size_t i = 0; i != n; ++i) res[i] = llrint(Q[i].re);
  return res;
}
}  // namespace FFT
int main() {
  ...
  vector<long long> V = FFT::multiply(r, d);
  V[n] = n0;
  V.resize(2 * n + 1, 0);
  for (int i = n; i <= 2 * n; ++i) {
    cout << V[i] << " ";
