...
int main() {
  ...
  while (t--) {
    ...
    for (int i = 0; i + 1 < n; i += 2) q0.emplace_back(i, i + 1);
    vector<int> a0 = ask(q0);
    for (int i = 0; i + 1 < n; i += 2) eq[i] = a0[i / 2];
    for (int i = 1; i + 1 < n; i += 2) q1.emplace_back(i, i + 1);
    ...
    vector<int> col[3];
    for (int i = 0; i < n; i++) col[ans[par[i]]].push_back(i + 1);
    cout << "A";
    for (int i = 0; i < 3; i++) cout << ' ' << col[i].size();
    cout << endl;
    for (int i = 0; i < 3; i++) {
