...
vector<int> calc(const string& pre, const string& cur, const vector<int>& dp) {
  ...
  for (int i = 0; i < (int)cur.size(); ++i) {
    ...
    if (i + 1 < (int)pre.size()) upd(val[1 + state], dp[i + 1]);
  }
  if (cmp_up(cur.size() + 1, cur.size()) >= 0) upd(ndp.back(), val[1]);
  upd(ndp.back(), val[2]);
  if (state > 0) {
  ...
  for (int i = (int)cur.size(); i < (int)pre.size(); ++i) upd(val[0], dp[i]);
  for (int i = (int)cur.size() - 1; i >= 0; --i) {
    if (cmp_down(i - 1) > 0) {
      for (int j = 0; j < 3; ++j) upd(ndp[i], val[j]);
    } else if (cmp_down(i - 1) == 0) {
      for (int j = 1; j < 3; ++j) {
        upd(ndp[i], val[j]);
      }
    }
    if (i < (int)pre.size()) upd(val[1 + cmp_up(i + 1, i + 1)], dp[i]);
