  ...
  while (!que.empty()) {
    ...
    for (int i = 0; i < edge[now].size(); i++)
      if (!vis[edge[now][i].first]) {
        que.push(edge[now][i].first);
        vis[edge[now][i].first] = 1;
        dis[x][edge[now][i].first] = dis[x][now] + edge[now][i].second;
      }
  }
}
void dfs(int x, int y) {
  for (int i = 0; i < edge[x].size(); i++)
    if (edge[x][i].first != y) dfs(edge[x][i].first, x);
  for (int i = 1; i <= n; i++)
    if (dis[x][i] <= 1LL * mdis) {
      siz[x] = 0;
      int tmp[2][505], now;
      ...
      for (int j = 0; j < edge[x].size(); j++)
        if (edge[x][j].first != y) {
          for (int k = siz[x]; k >= 0; k--)
            for (int h = siz[edge[x][j].first]; h >= 0; h--) {
              tmp[now ^ 1][k + h] =
                  min(tmp[now ^ 1][k + h],
                      tmp[now][k] + dp[edge[x][j].first][i][h]);
              tmp[now ^ 1][k + h] = min(tmp[now ^ 1][k + h],
                                        tmp[now][k] + res[edge[x][j].first][h]);
            }
          for (int k = 0; k <= n; k++) tmp[now][k] = 1000;
          now ^= 1;
          siz[x] += siz[edge[x][j].first];
        }
      for (int j = 0; j <= sum; j++)
        dp[x][i][j] = tmp[now][j],
        res[x][j + 1] = min(res[x][j + 1], dp[x][i][j] + 1 - col[i]);
    }
  siz[x] += 1;
}
int main() {
