...
int main() {
  ...
  for (i = 1; i <= m; ++i) {
    ...
    for (j = 1; j <= n; ++j)
      tmp[j] = (b[j].x) * (b[j].x) + (b[j].y) * (b[j].y) + (b[j].z) * (b[j].z);
    R = 0;
    for (j = 2; j <= n; ++j)
      if (dist(o, b[j]) > R + 1e-9) {
        o = b[j];
        R = 0;
        for (x = 1; x < j; ++x)
          if (dist(o, b[x]) > R + 1e-9) {
            o.x = (b[j].x + b[x].x) / 2;
            o.y = (b[j].y + b[x].y) / 2;
            o.z = (b[j].z + b[x].z) / 2;
            R = dist(o, b[j]);
            for (y = 1; y < x; ++y)
              if (dist(o, b[y]) > R + 1e-9) {
                double t1 = -v.x / v.z, t2 = -v.y / v.z;
                work(-2 * b[j].x - 2 * b[j].z * t1 + 2 * b[x].x +
                         2 * b[x].z * t1,
                     -2 * b[j].y - 2 * b[j].z * t2 + 2 * b[x].y +
                         2 * b[x].z * t2,
                     tmp[j] - tmp[x],
                     -2 * b[j].x - 2 * b[j].z * t1 + 2 * b[y].x +
                         2 * b[y].z * t1,
                     -2 * b[j].y - 2 * b[j].z * t2 + 2 * b[y].y +
                         2 * b[y].z * t2,
                     tmp[j] - tmp[y]);
                o.z = t1 * o.x + t2 * o.y;
                R = dist(o, b[j]);
