...
ll czyt(int v, int a, int b, int graa, int grab) {
  if (a >= graa && b <= grab) return drz[v];
  if (a > grab || b < graa) return 0;
  return max(czyt((v << 1), a, (a + b) >> 1, graa, grab),
             czyt((v << 1) ^ 1, (a + b + 2) >> 1, b, graa, grab));
}
bool mniej(int a, int b) { return kosz[a] < kosz[b]; }
