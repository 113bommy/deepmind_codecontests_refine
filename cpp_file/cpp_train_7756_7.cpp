...
class ModuloRing {
 ...
 public:
  int number;
  ModuloRing(ll number_ = 0) { number = number_ % modulo; }
  ModuloRing operator+(ModuloRing const &a) const { return number + a.number; }
...
class SegmentTree {
 ...
 public:
  SegmentTree(int n) {
    aint.resize(1 + 4 * n);
    for (int i = 1; i <= 4 * n; i++) aint[i].coef = 1;
  }
...
int main() {
  ...
  for (int i = 1; i <= q; i++) {
    if (queries[i][0] == 0) {
      ...
      grad[far[node]]++;
      aint._update(1, 1, ptr, start[far[node]],
                   {1LL * v[far[node]] * grad[far[node]], grad[far[node]]});
      aint._update(1, 1, ptr, stop[far[node]],
                   {0, ModuloRing(1) / grad[far[node]]});
      grad[node] = 1;
