...
void lim_dinic() {
  ...
  for (int i = oecnt + 1; i <= ecnt; i++) edg[i].vol = 0;
  ans -= dinic(T, S);
  printf("%I64d\n",
         (long long)ans * max(r, b) + (long long)(n - ans) * min(r, b));
  for (int i = 1; i <= n; i++)
...
int main() {
  ...
  cnt = 0;
  for (int i = 1; i <= n; i++)
    scanf("%d %d", &x[i], &y[i]), discx[i] = x[i], discy[i] = y[i];
  sort(discx + 1, discx + n + 1);
  cntx = 1;
  sort(discy + 1, discy + n + 1);
  cnty = 1;
  for (int i = 2; i <= n; i++)
