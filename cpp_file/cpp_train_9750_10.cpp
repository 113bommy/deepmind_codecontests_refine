...
struct node {
  double x, y;
} A[130005 * 4], B[130005 * 4], w[2][130005 * 4];
int i, j, k, l, s, n, m, M, rev[130005 * 4];
int ans[130005 * 4][6][6], Ans, fa[130005];
char c[130005], c1[130005];
inline node operator+(node a, node b) { return (node){a.x + b.x, a.y + b.y}; }
inline node operator-(node a, node b) { return (node){a.x - b.x, a.y - b.y}; }
inline node operator*(node a, node b) {
  return (node){a.x * b.x - a.y * b.y, a.x * b.y + b.x * a.y};
}
inline void pre() {
  for (int i = 0; i < M; i++) {
    int j = i, y = 0;
  ...
  for (int i = 0; i < M; i++)
    w[0][i] = (node){cos(2 * acos(-1.0) * i / M), sin(2 * acos(-1.0) * i / M)},
    w[1][i] = (node){cos(2 * acos(-1.0) * i / M), -sin(2 * acos(-1.0) * i / M)};
}
...
int main() {
  ...
  for (i = 0; i < 6; i++)
    for (j = 0; j < 6; j++) {
      ...
      FFT(A, 1);
      for (k = m; k < M; k++) ans[k - m][i][j] = (A[k].x > 1e-8);
    }
  for (i = 0; i <= n - m; i++) {
    for (j = 0; j < 6; j++) fa[j] = j;
    for (j = 0; j < 6; j++)
      for (k = 0; k < 6; k++)
        if (ans[i][j][k]) fa[get(j)] = get(k);
