...
vector<pair<long long, bool> > b, bb;
...
int main() {
  ...
  for (int i = 0; i < lucky.size(); ++i) lft[i] = INF, rght[i] = INF;
  b.reserve(630000);
  bb.reserve(630000);
  for (int i = 0; i < lucky.size(); ++i) bb.push_back(make_pair(lucky[i], 1));
  b = bb;
  for (int i = 0; i < n; ++i) b.push_back(make_pair(a[i].second, 0));
  ...
  b = bb;
  for (int i = 0; i < n; ++i) b.push_back(make_pair(a[i].first, 0));
  sort(b.begin() + lucky.size(), b.end());
  inplace_merge(b.begin(), b.begin() + lucky.size(), b.end());
  ...
  for (int i = 0; i < lucky.size(); ++i) {
    r = max(i, r);
    while (r < lucky.size() && lucky[r] - lucky[i] <= len &&
           lft[r] + rght[i] <= k)
      ans = max(ans, r - i + 1), r++;
  }
  printf("%d", ans);
