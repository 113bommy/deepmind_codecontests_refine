...
int main() {
  ...
  scanf("%I64d%I64d", &n, &maxk);
  scanf("%s", str);
  dp[n][0][1] = 1;
  for (int i = n - 1; i >= 0; i--) {
    for (int j = 0; j <= maxk; j++) {
      dp[i][j][1] += (dp[i + 1][j][0] + dp[i + 1][j][1]) * (str[i] - 'a') % mod;
      if (dp[i][j][1] > mod) dp[i][j][1] -= mod;
      dp[i][j][1] += dp[i + 1][j][1];
      if (dp[i][j][1] > mod) dp[i][j][1] -= mod;
    }
    for (int j = i; j < n; j++) {
      long long add = 1ll * (n - j) * (j - i + 1);
      for (int k = 0; k <= maxk - add; k++) {
        dp[i][k + add][0] +=
            (dp[j + 1][k][1] + dp[j + 1][k][0]) * ('z' - str[j]) % mod;
        if (dp[i][k + add][0] > mod) dp[i][k + add][0] -= mod;
      }
    }
  }
  cout << (dp[0][maxk][0] + dp[0][maxk][1]) % mod << endl;
  return 0;
}
