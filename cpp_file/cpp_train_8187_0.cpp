...
long long run(int s, long long minE) {
  ...
  for (; px[s] < (int)g[s].size(); px[s]++) {
    ...
    long long tmp = run(v.to, min(minE, v.cap));
...
int main() {
  ...
  int em = ne;
  for (int i = 0; i < n1; i++) addEdge(source, i, in[i] - k - 1);
  int e1 = ne;
  for (int i = 0; i < n2; i++) addEdge(n1 + i, target, in[n1 + i] - k - 1);
  int e2 = ne;
  ...
  for (; k >= 0; k--) {
    ...
    flow();
    vector<int> tmp;
    for (int i = 0; i < em; i += 2)
      if (edge[i].cap) tmp.push_back(i / 2 + 1);
