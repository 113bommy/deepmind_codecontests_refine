...
int n, m, edgenum, w[200005], pos[200005], head[200005], pa[200005],
    size[200005], maxs[200005], cnt[200005], tmp[200005], top, root, ans;
...
void check(int r) {
  ...
  dfs2(r);
  double sum = 0;
  for (int i = 1; i <= top; i++) sum += sqrt(st[i]) * st[i] * w[pos[i]];
...
void solve() {
  ...
  for (int hd = head[root]; hd; hd = e[hd].nxt) {
    ...
    num++;
    sum[num] = 0;
    for (int i = 1; i <= top; i++) sum[num] += sqrt(st[i]) * w[pos[i]];
    tmp[num] = to;
  }
  double s = 0;
  int x = 0;
