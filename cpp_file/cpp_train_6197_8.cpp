...
double dist(TP a, TP b) { return sqrt(sqr(a.x - b.x) + sqr(a.y - b.y)); }
int sgn(double a) {
  if (a > eps) return 1;
  if (a < -eps) return -1;
  return 0;
...
bool inside(TP a, int id) {
  ...
  if (f == 0) return 0;
  if (f != sgn(det(a, p[id][1], p[id][2])) ||
      f != sgn(det(a, p[id][2], p[id][0])))
    return 0;
  return 1;
}
vector<TP> all;
    ...
    for (int x = 0; x < 3; x++) {
      ...
      for (int z = 1; z < all.size(); z++) {
        ...
        for (int id = 0; id < n; id++)
          if (inside(t, id)) flag = 0;
        if (flag) ans += dist(all[z - 1], all[z]);
      }
    }
  printf("%.10f\n", ans);
}
