  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    switch ((size[l] & 1) | (size[r] & 1) << 1) {
      case 0:
        ...
        f[x][0][1] = f[x][1][0] &&
                     (!size[l] || f[l][0][1 + !size[r]] || f[r][1][0]) &&
                     (!size[r] || f[l][1][0] || f[r][0][1 + !size[l]]),
        f[x][1][1] = f[x][0][0] ||
                     (size[l] && f[l][1][1 + !size[r]] && f[r][0][0]) ||
                     (size[r] && f[l][0][0] && f[r][1][1 + !size[l]]),
        f[x][0][2] = f[x][1][0] && (!size[l] || f[l][0][1] || f[r][1][0]) &&
                     (!size[r] || f[l][1][0] || f[r][0][1]),
        f[x][1][2] = f[x][0][0] || (size[l] && f[l][1][1] && f[r][0][0]) ||
                     (size[r] && f[l][0][0] && f[r][1][1]);
        break;
      case 1:
      ...
      case 3:
        ...
        f[x][0][1] = f[x][1][0] &&
                     (f[l][1][1 + (size[r] == 1)] || f[r][0][0]) &&
                     (f[l][0][0] || f[r][1][1 + (size[l] == 1)]);
        f[x][1][1] = f[x][0][0] ||
                     (f[l][0][1 + (size[r] == 1)] && f[r][1][0]) ||
                     (f[l][1][0] && f[r][0][1 + (size[l] == 1)]);
        f[x][0][2] = f[x][1][0] && (f[l][1][1] || f[r][0][0]) &&
                     (f[l][0][0] || f[r][1][1]);
        f[x][1][2] = f[x][0][0] || (f[l][0][1] && f[r][1][0]) ||
                     (f[l][1][0] && f[r][0][1]);
        break;
    }
  }
...
void work() {
  ...
  for (i = 0; i < n; ++i)
    if (cin >> w[i], !~w[i])
      ...
      lc[i] = rc[i] = -1, down(L, w[i]), up(R, w[i]);
  dfs(0);
