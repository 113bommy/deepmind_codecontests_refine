...
inline bool Cross(pair<long long, long long> &A,
...
int main() {
  ...
  for (int i = 0; i < Union.size(); i++) {
    int X = Segs.size() - 1;
    if (Segs[X].second + 1 >= Union[i].first)
      Segs[X].second = max(Segs[X].second, Union[i].second);
    else
      Segs.push_back(Union[i]);
  }
  pair<long long, long long> CurRes = make_pair(L, R);
  for (int i = 1; i < Segs.size(); i++)
    if (Cross(CurRes, Segs[i])) break;
  if (CurRes.first == CurRes.second)
    printf("%I64d\n", CurRes.first);
  else
    puts("Data not sufficient!");
  return 0;
