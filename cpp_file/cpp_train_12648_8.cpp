...
struct SplayTree {
  ...
  long long lazy[N], val[N];
  int typ(int x) { return x == ch[fa[x]][1]; }
  void PullUp(int x) { sz[x] = sz[ch[x][0]] + sz[ch[x][1]] + 1; }
  void PushDown(int x) {
    val[x] += lazy[x];
    lazy[ch[x][0]] += lazy[x];
    lazy[ch[x][1]] += lazy[x];
  ...
  int fnd(int &pos, int f, int csz, long long v) {
    if (!pos) {
      int ret = pos = ++tot;
  ...
  void OptAns(int pos, long long &pre) {
    if (!pos) return;
    PushDown(pos);
    OptAns(ch[pos][0], pre);
    pre += val[pos];
