...
class graph {
 public:
  ...
  void add_edge(int x, int y, long long c) { v[x].push_back(make_pair(y, c)); }
  void solve(int S) {
    priority_queue<pair<long long, int>, vector<pair<long long, int> >,
                   greater<pair<long long, int> > >
        heap;
    for (int i = 1; i <= N; i++) best[i] = H + 1;
    heap.push(make_pair(1, S));
    while (!heap.empty()) {
      pair<long long, int> X = heap.top();
      heap.pop();
      if (best[X.second] != H + 1) continue;
      best[X.second] = X.first;
      for (vector<pair<int, long long> >::iterator it = v[X.second].begin();
           it != v[X.second].end(); it++)
...
int main() {
  ...
  for (int i = 1; i <= N; i++) scanf("%I64d %d", &Pos[i], &C[i]);
  dinamica.N = K;
  Refresh();
  while (M) {
    M--;
    ...
    if (tip == 1) {
      ...
      scanf("%d %d", &X, &Y);
      C[X] -= Y;
      if (ap[X] == 1) PQ.push(make_pair(C[X], -X));
    } else {
      int answer, where = -1;
      while (!PQ.empty()) {
