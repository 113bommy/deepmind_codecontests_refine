...
void fwt1(int *a) {
  for (int i = 0; i < n; i += 2) a[i + 1] = plus(a[i + 1], a[i]);
  for (int i = 4; i <= n; i <<= 1)
    for (int j = 0; j < n; j += i)
...
void fwt2(int *a) {
  for (int i = 0; i < n; i += 2) a[i + 1] = minus(a[i + 1], a[i]);
  for (int i = 4; i <= n; i <<= 1)
    for (int j = 0; j < n; j += i)
      for (int k = 0; k < i / 2; k += 2) {
...
void gao1(int *a, int *b, int *c) {
  for (int i = 0; i < n; i++) a1[bitcount[i]][i] = a[i];
  int len = bitcount[n - 1];
  for (int i = 0; i <= len; i++) fwt1(a1[i]);
  for (int i = len; i >= 0; i--)
    for (int j = 0; j <= i; j++)
      for (int k = 0; k < n; k++)
        a2[i][k] = (a2[i][k] + (long long)a1[i - j][k] * a1[j][k]) % p;
  for (int i = 0; i <= len; i++) fwt2(a2[i]);
  for (int i = 0; i < n; i++) c[i] = a2[bitcount[i]][i];
}
void gao2(int *a, int *b, int *c) {
  ...
  fwt3(a1);
  fwt3(a2);
  for (int i = 0; i < n; i++) a1[i] = (long long)a1[i] * a2[i] % p;
  fwt4(a1);
...
int main() {
  ...
  gao1(a, a, b);
  gao2(a, a, c);
  for (int i = 0; i < n; i++) a[i] = (long long)a[i] * f[i] % p;
  for (int i = 0; i < n; i++) b[i] = (long long)b[i] * f[i] % p;
  for (int i = 0; i < n; i++) c[i] = (long long)c[i] * f[i] % p;
  gao3(a, b, d);
  gao3(c, d, e);
  int ans = 0;
  for (int i = 1; i < n; i <<= 1) ans = (ans + e[i]) % p;
  ans = (ans + p) % p;
  printf("%d\n", ans);
}
