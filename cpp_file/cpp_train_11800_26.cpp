  ...
  if (cnt == 0) {
    ...
    for (int i = 0; i < 26; ++i) {
      if (p->next[i] != NULL) {
        ...
        for (int j = lim; j >= 0; --j)
          for (int k = 1;
               k < (int)tmp->size() && j + k <= lim + p->next[i]->whole; ++k)
            (*ret)[j + k] =
                max((*ret)[j + k], (*ret)[j] + (*tmp)[k] + (j * k * dep));
        lim += p->next[i]->whole;
      }
    }
    return ret;
...
int dfs(trie *p) {
  p->whole = p->cnt;
  for (int i = 0; i < 26; ++i)
    if (p->next[i] != NULL) p->whole += dfs(p->next[i]);
  return p->whole;
}
int main() {
  char s[512];
