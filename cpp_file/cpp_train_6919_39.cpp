...
struct SegTree {
  ...
  int sum[M << 2];
  vector<int> pre;
  void init(const vector<int> &_pre) { pre = _pre; }
  void update(int idx, int l, int r, int ll, int rr, int val) {
    if (l >= rr || r <= ll) return;
    if (l >= ll && r <= rr) {
      cnt[idx] += val;
      sum[idx] =
          (cnt[idx] == 0 ? l == r - 1 ? 0 : sum[idx * 2 + 1] + sum[idx * 2 + 2]
                         : pre[r] - pre[l]);
      return;
    }
    int mid = (l + r) >> 1;
    ...
    sum[idx] =
        (cnt[idx] == 0 ? sum[idx * 2 + 1] + sum[idx * 2 + 2] : pre[r] - pre[l]);
  }
  void update(int l, int r, int val) { update(0, 0, pre.size(), l, r, val); }
  int query() { return sum[0]; }
} tree;
int main() {
  scanf("%d %d %d", &n, &m, &k);
  for (int i = 0; i < m; ++i) {
  ...
  for (int lg = 0; lg < LG; ++lg) {
    ...
    for (int i = 0; i < m * 2; ++i) {
      ...
      tree.update(
          lower_bound(ys.begin(), ys.end(), y / (1 << lg)) - ys.begin(),
          lower_bound(ys.begin(), ys.end(), yy / (1 << lg)) - ys.begin(), val);
      lst = x;
    }
  }
  for (int i = 0; i < LG; ++i) {
