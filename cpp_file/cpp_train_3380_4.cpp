...
typedef struct Node {
  ...
  Node(int a, int b, int c)
      : key(a), lis(b), lds(c), hi(1), mxis(b), mxds(c), lf(NULL), rt(NULL) {}
  inline void clear() { hi = 1, mxis = lis, mxds = lds, lf = rt = NULL; }
  inline void print() {
    _debug("%d: (%d,%d) mx (%d,%d) hi %d\n", key, lis, lds, mxis, mxds, hi);
  }
} Node;
typedef struct Tree {
  Node *root;
  ...
  inline void _insert(Node *now, Node **par, Node *nd) {
    if (now->key < nd->key) {
      if (now->rt == NULL)
        ...
        _insert(now->rt, &now->rt, nd);
    } else {
      if (now->lf == NULL)
        now->lf = nd;
      else
        _insert(now->lf, &now->lf, nd);
    }
    _debug("\tins %d @ %d h %d %d\n", nd->key, now->key, getH(now->lf),
           getH(now->rt));
...
Node _node[1310720 << 1];
int nNode;
int V[1310720], SIZE[1310720];
short _in[1310720], _vst[1310720];
int T, N;
list<int> E[1310720];
...
inline int mergeQuery(Tree *t1, Tree *t2, int v) {
  int ans = max(t1->queryMxis(v) + t2->queryMxds(v),
                t1->queryMxds(v) + t2->queryMxis(v)) +
            1;
  int head, tail;
  for (head = 0, tail = 1, Q[0] = t2->root; head < tail; ++head) {
    ...
    int t = max(now->lis + t1->queryMxds(now->key),
                now->lds + t1->queryMxis(now->key));
    if (t > ans) ans = t;
  }
