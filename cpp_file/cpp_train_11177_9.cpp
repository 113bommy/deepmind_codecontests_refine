...
int a[MAXN][MAXN];
Node *st;
int cp(int x0, int y0, int x1, int y1) { return x0 * y1 - y0 * x1; }
void push(Node *v) {
  if (v->l == 0)
    v->l = new Node(v->val);
  else {
...
int get(Node *v, int tl, int tr, int l, int r) {
  ...
  if (l < tm) res = min(res, get(v->l, tl, tm, l, min(r, tm)));
  if (r > tm) res = min(res, get(v->r, tm, tr, max(l, tm), r));
  return res;
}
void update(Node *v, int tl, int tr, int l, int r, int val) {
  ...
  push(v);
  int tm = (tl + tr) / 2;
  if (l < tm) update(v->l, tl, tm, l, min(r, tm), val);
  if (r > tm) update(v->r, tm, tr, max(l, tm), r, val);
