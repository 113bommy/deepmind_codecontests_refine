...
double findConeSurfaceDistance(double x1, double y1, double z1, double x2,
                               double y2, double z2) {
...
double sortaBinarySearch(double left, double right, double xHi, double yHi,
                         double zHi, double xLo, double yLo, double zLo) {
...
double findOptimalSingleBrinkPointDistance(double xHi, double yHi, double zHi,
  ...
  minVal = sortaBinarySearch(minDegree - aDegree, minDegree + aDegree, xHi, yHi,
                             zHi, xLo, yLo, zLo);
  minVal = min(minVal, sortaBinarySearch(minDegree - 2 * aDegree,
                                         minDegree + 2 * aDegree, xHi, yHi, zHi,
                                         xLo, yLo, zLo));
  return minVal;
}
double sortaBinarySearch2(double left, double right, double x1, double y1,
...
double findOptimalDoubleBrinkPointDistance(double x1, double y1, double z1,
  ...
  minVal = sortaBinarySearch2(minDegree - aDegree, minDegree + aDegree, x1, y1,
                              z1, x2, y2, z2);
  minVal = min(minVal, sortaBinarySearch2(minDegree - 2 * aDegree,
                                          minDegree + 2 * aDegree, x1, y1, z1,
                                          x2, y2, z2));
  minVal = min(minVal, sortaBinarySearch2(minDegree - 3 * aDegree,
                                          minDegree + 3 * aDegree, x1, y1, z1,
                                          x2, y2, z2));
  return minVal;
}
int main() {
