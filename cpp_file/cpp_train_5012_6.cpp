...
struct fenwick {
  ...
  int operator[](int pos) { return get(pos, pos); }
};
struct segment {
  int l, r, clr;
...
const int lg = 20;
int n, q, timer, ptr;
int sz[mx], tin[mx], tout[mx], h[mx], euler[mx], par[mx], p[2 * mx], _p[2 * mx],
    link[mx], up[mx][lg];
...
int distance(int u, int v) { return h[u] + h[v] - 2 * h[lca(u, v)]; }
...
void hang(int v, int pv) {
  ...
  if (!vp.empty()) {
    int id = max_element(vp.begin(), vp.end()) - vp.begin();
