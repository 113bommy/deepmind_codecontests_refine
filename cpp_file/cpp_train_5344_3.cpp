...
struct T {
  ...
  T operator+(T t) const { return T(max(maxa, t.maxa), min(minr, t.minr)); }
};
T a[MAXN * 2];
int del[MAXN * 2];
...
void init() { fill(a, a + MAXN * 2, T()); }
void put(int t) { printf("%d ", t); }
int main() {
  int n;
  int i, j, k;
  int ans = 0;
  ...
  for (i = 1; i <= n; i++) {
    if (max(ans, l[i]) + t[i] <= r[i]) {
      ...
      if (max(ans - move, l[i]) + t[i] > r[i] ||
          max(ans - move, l[i]) + t[i] >= ans) {
        put(-1);
        continue;
      }
