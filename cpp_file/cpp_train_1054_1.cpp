...
void st_inc(int a, int b, int64_t v) { st_inc(1, 0, N, a, b, v); }
int64_t st_query(int w, int L, int R, int a, int b) {
  ...
  if (a <= L && b >= R) {
    ...
    int M = (L + R) / 2;
    return std::min<int64_t>(st_query(w << 1, L, M, a, b),
                             st_query(w << 1 | 1, M, R, a, b));
  }
}
int64_t st_query(int a, int b) { return st_query(1, 0, N, a, b); }
int64_t ans[500005];
struct Query {
  int id;
  int L, R;
