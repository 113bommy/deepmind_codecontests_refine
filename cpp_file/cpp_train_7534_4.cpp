...
struct segtree {
  ...
  void update(int v, int tl, int tr, int l, int r, int val, int tme) {
    if (l > r) return;
    if (l == tl && tr == r) {
      lazy[v] = val;
      lazytm[v] = tme;
      ...
      ;
      update(lf, tl, tm, l, min(r, tm), val, tme);
      update(rt, tm + 1, tr, max(l, tm + 1), r, val, tme);
    }
  }
  ...
  void u(int l, int r, int val, int tm) { update(1, 0, N - 1, l, r, val, tm); }
} st;
int a[LIM], b[LIM];
int main() {
