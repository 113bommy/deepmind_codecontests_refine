...
int main() {
  ...
  for (i = 2; i * i <= 10000; ++i)
    if (!ciur[i])
      for (j = i * i; j <= 10000; j += i) ciur[j] = 1;
  j = 1;
  k = 0;
  unde[1] = 1;
  for (t = 1; t <= 22; ++t) {
    ...
    for (i = 1; i <= 20; ++i) {
      ...
      q = q * j;
      ++j;
      if (q > 1000000000000000000 || q < 0) break;
      v[t] = q;
    }
    --k;
    unde[t + 1] = k;
    ...
    for (int pz = 1; pz <= 22 && qr > 0; ++pz) {
      ...
      while (qr) {
        for (i = unde[pz]; i <= min(k, unde[pz] + 20); ++i) {
          if (val % prime[i] == 0) break;
        }
        if (i == min(k, unde[pz] + 20) + 1) break;
        val /= prime[i];
