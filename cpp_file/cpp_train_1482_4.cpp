...
const int MAX = 20005;
long long int sum[55][MAX], dp[55][MAX], val[55][MAX], n, m, temp1[MAX],
    temp2[MAX], k;
deque<pair<long long int, long long int> > dq;
long long int maxi[MAX];
long long int backMaxi[2 * MAX];
int main() {
  ...
  for (int i = 3; i <= n + 1; ++i) {
    ...
    for (int j = k; j <= m; ++j) {
      ...
      long long int cur =
          maxi[j - k] + curSum + sum[i - 1][j] - sum[i - 1][j - k];
    ...
    for (int j = m; j >= k; --j) {
      ...
      long long int res = dq.front().second + sum[i - 1][dq.front().first - k] -
                          sum[i - 1][j - k];
      long long int cur =
          backMaxi[j + k] + curSum + sum[i - 1][j] - sum[i - 1][j - k];
      temp2[j] = max(cur, curSum + res);
