...
void push_up(int n) { s[n].v = s[s[n].ls].v + s[s[n].rs].v; }
...
void update(int tl, int tr, int p, int ln, int &n) {
  ...
  int tm = (tl + tr) >> 1;
  if (p < tm)
    update(tl, tm, p, s[ln].ls, s[n].ls);
  else
    update(tm, tr, p, s[ln].rs, s[n].rs);
  push_up(n);
}
...
int main() {
  ...
  for (int i = 2; i <= n + m; i++) {
    ...
    for (int c = 1, tx = x, ty = y; tx <= n && ty >= 1; c++, tx++, ty--) {
      rt[c] = rt[c - 1];
      if (str[tx][ty] == 'z')
        update(1, n + m, r[tx][ty] - c + n, rt[c - 1], rt[c]);
    }
    for (int c = 1, tx = x, ty = y; tx <= n && ty >= 1; c++, tx++, ty--) {
      int len = min(l[tx][ty], d[tx][ty]);
      res += query(1, n + m, n + 1 - c, n + m, rt[c + len - 1]) -
             query(1, n + m, n + 1 - c, n + m, rt[c - 1]);
    }
  }
  printf("%I64d", res);
  return 0;
