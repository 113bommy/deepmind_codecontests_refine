...
inline void build() {
  for (long long i = 1; i <= sz; i++) {
    dp[i] = 0;
    for (long long j = 0; j < 26; j++) tr[i].nex[j] = 0;
  }
  las = sz = 1;
  tr[1].len = 0, tr[1].f = 0;
  return;
...
long long t[A], p[A];
inline void Sort() {
  for (long long i = 0; i <= len; i++) t[i] = 0;
  for (long long i = 1; i <= sz; i++) t[tr[i].len]++;
  for (long long i = 1; i <= len; i++) t[i] += t[i - 1];
  for (long long i = 1; i <= sz; i++) p[t[tr[i].len]--] = i;
  return;
}
signed main() {
  T = Read();
  while (T--) {
    ...
    for (long long i = sz; i; i--) {
      dp[tr[p[i]].f] += dp[p[i]];
      ans += ((tr[p[i]].len - tr[tr[p[i]].f].len) * dp[p[i]] * dp[p[i]]);
    }
    printf("%lld\n", ans);
  }
  return 0;
