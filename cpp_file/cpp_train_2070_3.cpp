...
int cost[200][200], tim[200][200],dist[200];
int x[200][200];
bool vis[200];
int M,N;
...
void dij(int p, int q, int r){
	int i,j,k,l,m,n,mn;
	rep(i,M)rep(j,M)x[i][j]= ((r==0)?(cost[i][j]):(tim[i][j]));
...
int main(){
	...
	while(cin>>N>>M){
		...
		rep(i,M){
			dist[i]=INF; 
		    rep(j,M){
			  if(i==j) {cost[i][j]=tim[i][j]=0; continue;}
		...
		rep(i,N){
			...
			cin>>a>>b>>c>>t;
			a--; b--;
			cost[a][b]=cost[b][a]=c; tim[a][b]=tim[b][a]=t;
		}
		cin>>k;
		rep(i,k){
			int p,q,r;
