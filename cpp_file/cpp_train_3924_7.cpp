...
int sum(int p, int q, int l, int r, int qL, int qR) {
  ...
  if (qL <= l && r <= qR) return seg[q] - seg[p];
  return sum(lc[p], lc[q], l, l + r >> 1, qL, qR) +
         sum(rc[p], rc[q], (l + r >> 1) + 1, r, qL, qR);
}
...
void dfs2(int x, int t = 1) {
  ...
  for (int i = lst[x]; i; i = e[i].nxt)
    if (e[i].to != par[x] && e[i].to != son[x]) dfs2(e[i].to, e[i].to);
  out[x] = dn;
}
int lca(int x, int y) {
  while (top[x] != top[y]) {
...
int far(int x, int y) {
  if (x == y) return 0;
  if (dep[f[x][Logn]] > dep[y]) return -1;
  int ans = 0;
  for (int i = Logn; i >= 0; i--)
    if (dep[f[x][i]] > dep[y]) x = f[x][i], ans += 1 << i;
  return ans + 1;
}
bool sub(int x, int y) { return in[y] <= in[x] && out[x] <= out[y]; }
int query(int u, int v) {
  int w = lca(u, v);
  if (dep[f[u][Logn]] > dep[w]) return -1;
  int len = 0;
  for (int i = Logn; i >= 0; i--)
...
vector<int> t[MaxN];
bool cmp(line a, line b) { return dep[a.w] < dep[b.w]; }
int fat[MaxN];
int find(int x) { return fat[x] == x ? x : fat[x] = find(fat[x]); }
void work(int u, int v) {
  for (u = find(u); dep[u] >= dep[v]; u = find(u)) f[u][0] = v, fat[u] = par[u];
}
int main() {
  n = read();
  for (int i = 2; i <= n; i++) {
  ...
  for (int i = 1; i <= m; i++) {
    ...
    if (l[i].u != l[i].v) {
      t[in[l[i].u]].push_back(in[l[i].v]), t[in[l[i].v]].push_back(in[l[i].u]);
    }
  }
  sort(l + 1, l + m + 1, cmp);
  for (int i = 1; i <= n; i++) f[i][0] = i, fat[i] = i;
  for (int i = 1; i <= m; i++) {
    work(l[i].u, l[i].w);
    work(l[i].v, l[i].w);
  }
  ...
  for (int i = 1; i <= n; i++) {
    rot[i] = rot[i - 1];
    for (int ti = 0; ti < ((int)(t[i]).size()); ti++) {
      int j = t[i][ti];
