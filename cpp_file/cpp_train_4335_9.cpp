...
int qpow(int x, int e) {
  int ret = 1;
  for (; e; e >>= 1, x = 1ll * x * x % p)
    if (e & 1) ret = 1ll * ret * x % p;
  return ret;
}
int fac[MAXN + 5], ifac[MAXN + 5];
int num[MAXN + 5][MAXD + 5];
...
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    for (int j = 1; j <= prcnt; j++)
      while (cur % pr[j] == 0) num[i][j]++, cur /= pr[j];
    fac[i] = 1ll * fac[i - 1] * cur % p;
    ifac[i] = qpow(fac[i], phi - 1);
  }
  int ans = 0;
  for (int i = 0; i <= n - l; i++) {
    int numways = (0ll + binom(n - i, (n - i - l) >> 1) -
                   binom(n - i, (n - i - r - 1) >> 1) + p) %
                  p;
