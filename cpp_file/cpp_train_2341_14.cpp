...
int main() {
  ...
  auto gen = [&](int from, int to, int dx, int dy) {
    ...
    for (int i = 0; i < abs(dy); i++)
      str[from + abs(dx) + i] = (dy < 0 ? 'D' : 'U');
    for (int i = abs(dx) + abs(dy); i < to - from; i++)
      str[i + from] = ((i & 1) ? 'L' : 'R');
  };
  if (!flg) {
    for (int i = 1; i <= N; i++) id[i] = i;
    sort(id + 1, id + N + 1, [&](int i, int j) { return t[i] % L < t[j] % L; });
    id[N + 1] = 0;
  ...
  ans[L] = {X, Y};
  for (int i = 1; i <= N; i++)
    ans[t[i] % L] = make_pair(x[i] - t[i] / L * X, y[i] - t[i] / L * Y);
  int last = -1;
  ...
  for (auto t : ans) {
    ...
    if (abs(t.second.first - seen.first) + abs(t.second.second - seen.second) >
        t.first - last)
      return puts("NO"), 0;
    gen(last, t.first, t.second.first - seen.first,
        t.second.second - seen.second);
    last = t.first;
    seen = t.second;
  }
  puts(str);
