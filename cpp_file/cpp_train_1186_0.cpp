...
vector<int> get(int n, int len) {
  vector<int> ret;
  ret.push_back(n);
  ret.push_back(-n);
  for (int i = 1, p = 10; i < len; ++i, p *= 10) {
    vector<int> a = get(n % p, i);
    vector<int> b = get(n / p, len - i);
...
int main() {
  ...
  for (int i = 0; i < (int)(10000); i++) {
    vector<int> cur = get(i, 4);
    for (int j = 0; j < (int)(cur.size()); j++) {
      if (abs(k - cur[j]) < 10000) {
        sprintf(buf, "%04d%04d", abs(k - cur[j]), i);
        sprintf(buf, "%04d%04d", i, abs(k - cur[j]));
        s.insert(string(buf));
