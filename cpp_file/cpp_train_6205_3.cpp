...
map<pair<int, int>, vector<pair<int, int>>> qs;
vector<array<int, 4>> ops;
pair<int, int> find(int u) {
  if (u == rep[u]) return {u, 0};
  auto p = find(rep[u]);
...
void solve(int l, int r) {
  for (auto q : qs[{l, r}]) join(q.first, q.second);
  if (l == r)
    res[l] = ans;
  else {
    int m = (l + r) / 2;
  ...
  for (auto q : qs[{l, r}]) undo();
}
void decomp(int l, int r, int rl, int rr, pair<int, int> e) {
  if (r < rl || rr < l) return;
  if (rl <= l && r <= rr) {
    qs[{l, r}].push_back(e);
    return;
