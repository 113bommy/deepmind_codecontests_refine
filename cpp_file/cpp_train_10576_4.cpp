...
class MaxFlowMinCost {
 public:
  ...
  inline pair<int, long long int> computeFlow() { return JohnsonDinic(); }
 ...
 private:
  struct Edge {
    ...
    inline int other(int node) { return from ^ to ^ node; }
  };
  int n, m, s, t;
  vector<int> graph[NMAX];
  Edge edges[2 * MMAX];
  const long long int INF = 1E18 + 15;
  long long int dist[NMAX];
  int father[NMAX];
  bool vis[NMAX];
  long long int potentials[NMAX];
  priority_queue<pair<long long int, int> > _queue;
  bool Dijkstra() {
    ...
    for (int i = 1; i <= n; ++i) dist[i] += (potentials[i] - potentials[s]);
    return vis[t];
  }
  pair<int, long long int> JohnsonDinic() {
    memset(potentials, 0, (n + 1) * sizeof(long long int));
    Dijkstra();
    int flow = 0;
    ...
    while (Dijkstra()) {
      ...
      while (node != s) {
        minimum =
            min(minimum, edges[father[node]].cap - edges[father[node]].flow);
...
int main() {
  ...
  for (int i = 1; i <= N; ++i) f.addEdge(2 * N + 2, i, 1, a[i]);
  for (int i = N + 1; i <= 2 * N; ++i) f.addEdge(i, 2 * N + 3, 1, b[i - N]);
  for (int i = 1; i <= N; ++i) f.addEdge(i, N + i, 1, 0);
  for (int i = N + 1; i < 2 * N; ++i) f.addEdge(i, i + 1, K, 0);
  f.setS(2 * N + 1);
  f.setT(2 * N + 3);
  f.setN(2 * N + 3);
