...
int Fdayu(int i, int j, int k) {
  if (n < 1000000) return cnt[i][9][k - 1] - cnt[i][j][k - 1];
...
void get_final(long long int a) {
  long long int i, j;
...
void calc() {
  long long int a, b, c, lk;
  for (long long int i = n; i <= 10000000; i += n) {
    if ((Wavy1[i] || Wavy2[i])) k--;
    if (k == 0) {
      cout << i;
      return;
    }
  ...
  for (a = 1; a <= 10000000; a++) {
    if (!(Wavy1[a] || Wavy2[a])) continue;
    lk = k;
    c = n - (10000000 * a) % n;
    if (c == n) c = 0;
    if (((a % 100) / 10) > (a % 10) || a < 10) k -= Fdayu(c, a % 10, 2);
    if (((a % 100) / 10) < (a % 10) || a < 10) k -= Fxiaoyu(c, a % 10, 1);
    if (k <= 0) k = lk, get_final(a);
  }
  cout << -1;
}
int main() {
