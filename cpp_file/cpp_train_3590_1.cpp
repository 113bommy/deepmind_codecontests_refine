...
inline bool operator<(answer a, answer b) { return a.key < b.key; }
...
void _insert(int v, int a, int b) {
  ...
  if (a >= pa && b <= pb) {
    ...
    best[v].first += sum[v];
    if (prev != best[v].second) tree[v].push_back(answer(pq, best[v].second));
  }
}
...
int main() {
  ...
  delete[] sum;
  delete[] best;
  for (int i = offset; i < offset * 2; i++) items[i] = new int[1];
  for (int i = 0; i < n; i++) items[a[i].second + offset][0] = a[i].first;
  for (int i = offset - 1; i > 0; i--) {
    size[i] = size[i * 2] + 1;
    int s = 1 << size[i * 2];
    items[i] = new int[s * 2];
    merge(items[i * 2], items[i * 2] + s, items[i * 2 + 1],
          items[i * 2 + 1] + s, items[i]);
  }
  int s, ans = 0;
  scanf("%d", &s);
  while (s--) {
