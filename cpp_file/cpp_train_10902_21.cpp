...
vector<ll> to_int(vector<cd>& a) {
  int n = a.size();
  vector<ll> b(n);
  for (int i = 0; i < n; ++i) b[i] = a[i].real() + 0.5;
  return b;
}
void sqr(vector<ll>& a) {
  ...
  fft(b, true);
  a.resize(0);
  for (int i = 0; i < b.size(); ++i) a.emplace_back(ll(b[i].real() + 0.5));
  normalize(a);
  shrink(a);
}
vector<ll> bp(int e) {
