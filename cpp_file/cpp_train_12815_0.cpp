...
int simulate(int q, string s) {
  for (int i = 0; i < int(s.size()); i++) q = sig[q][s[i] - '0'];
  return q;
}
long long int amount[limit][limit][limit];
long long int sol;
int main() {
  int n;
  cin >> n;
  ...
  for (nsig = 0; nsig < int(s.size()); nsig++) {
    ...
    sig[nsig][c] = nsig + 1;
    if (nsig > 0) proper[nsig + 1] = sig[proper[nsig]][c];
  }
  ...
  for (int q0 = 1; q0 < nsig; q0++) {
    for (int lenle = 1; lenle < nsig; lenle++) {
      ...
      if (qpreend == nsig - 1) sol += amount[qafter][n - lenle][q0];
    }
  }
  cout << sol << endl;
}
