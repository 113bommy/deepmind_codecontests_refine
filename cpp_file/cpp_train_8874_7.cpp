...
void update(int i, int d) {
  ...
  while (pos) {
    ...
    if (ex[pos * 2] != -1 && ex[pos * 2 + 1] != -1) {
      ...
      if (len)
        (res[pos] *= (x * dp[len] + (10 - x) * dp[len - 1]) % mod) %= mod;
      else
        (res[pos] *= x) %= mod;
    }
    pre[pos] = pre[pos * 2];
    ...
    ex[pos] = ex[pos * 2];
    if (ex[pos * 2] == -1)
      pre[pos] += pre[pos * 2 + 1], ex[pos] = ex[pos * 2 + 1];
...
int main() {
  ...
  cin.tie(0);
  dp[0] = 1, dp[1] = 2;
  for (int i = 2; i < MX; i++) dp[i] = 2 * (dp[i - 1] + 4 * dp[i - 2]) % mod;
  cin >> n >> q;
  build(1, n, 1);
  for (int i = 1; i <= n; i++) {
    char c;
  ...
  while (q--) {
    ...
    if (x) {
      if (len)
        (r *= (x * dp[len] + (10 - x) * dp[len - 1]) % mod) %= mod;
      else
        (r *= x) %= mod;
      (r *= dp[suf[1]]) %= mod;
    } else {
