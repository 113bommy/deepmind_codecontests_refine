...
multiset<int> s;
int cmp(evt x, evt y) {
  return x.x == y.x ? (x.type == y.type ? x.l > y.l : x.type > y.type)
                    : x.x < y.x;
}
tree *bt(int l, int r) {
  tree *i = new tree(l, r);
...
int get(tree *i, int l, int r) {
  ...
  if (r <= md)
    ...
    return (get(i->lc, l, md) + get(i->rc, md + 1, r)) % mod;
}
void set0(tree *i, int l, int r) {
  ...
  if (r <= md)
    ...
    set0(i->lc, l, md), set0(i->rc, md + 1, r);
  i->sum = (i->lc->sum + i->rc->sum) % mod;
}
void print(tree *i) {
  if (!i) return;
  ...
  for (int i = hh; i <= num && a[i].x <= n; i++) {
    if (a[i].type == 1) {
      if (*s.upper_bound(a[i].r) > a[i].r + 1)
        add(root, a[i].r + 1,
            get(root, *(--s.upper_bound(a[i].r)) + 1, a[i].r));
