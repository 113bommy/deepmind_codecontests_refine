...
vector<long long int> memo[maxN];
...
void merge(int u, int v) {
  ...
  parent[v] = u;
  vector<long long int> aux(memo[u].size() + memo[v].size() - 1);
  aux[1] = isClique(u);
...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    ...
    edges.clear();
    for (int i = 0; i < n; i++)
      for (int j = i + 1; j < n; j++) edges.push_back(Edge{i, j, a[i][j]});
    ...
    vector<long long int> &ans = memo[root(0)];
