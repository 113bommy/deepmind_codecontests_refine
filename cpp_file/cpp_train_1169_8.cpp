...
inline void dfs(int k, int upper, int *f, long long cur = 0) {
  if (k == upper) {
    ++f[__builtin_popcountll(cur)];
    return;
  }
  dfs(k + 1, upper, f, cur);
  if (base[k]) dfs(k + 1, upper, f, cur ^ base[k]);
}
std::map<long long, int> fa[66], fb[66];
inline void dfs2(int k, int upper, int opt, long long cur = 0) {
  if (k == upper) {
    if (opt == 0)
  ...
  if (base[k]) dfs2(k + 1, upper, opt, cur ^ base[k]);
}
int main() {
  pw2[0] = 1;
  read(n), read(m);
        ...
        if (fa[i].size() && fb[j].size()) {
          for (auto u : fa[i])
            for (auto v : fb[j]) {
              ...
              ans[i + j + c] =
                  (ans[i + j + c] + 1LL * u.second * v.second) % mod;
            }
        }
  }
  for (int i = 0; i <= m; ++i) {
