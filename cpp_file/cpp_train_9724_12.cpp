...
bool cmp(const node *u, const node *v) { return u->flag > v->flag; }
node *S, *T, *seq[N];
...
struct data {
  node *u, *v;
  int w;
  friend bool operator<(const data &a, const data &b) { return a.w < b.w; }
} res[N];
...
int main() {
  ...
  newE = E;
  for (int i = 0; i < cnt; i++) addedge(res[i].u, res[i].v, res[i].w);
  print(V + 1);
}
