...
bool thing_cmp(const thing &t1, const thing &t2) { return t1.t < t2.t; }
int n, p, q;
vector<state> pre[T], suf[T];
void init() {
  scanf("%d%d", &n, &p);
  for (int i = 1; i <= n; ++i) scanf("%d%d%d", &t[i].c, &t[i].h, &t[i].t);
...
void query() {
  ...
  for (int i = 1, l = 0, r = 0, v = 0; i <= q; ++i) {
    ...
    for (vector<state>::iterator it = suf[b1].begin(); it != suf[b1].end();
         ++it)
      if (l <= it->t) s1 = &(*it);
    for (vector<state>::iterator it = pre[b2].begin(); it != pre[b2].end();
         ++it)
      if (it->t <= r) s2 = &(*it);
    int ans = 0;
    for (int j = 0; j <= v; ++j) ans = max(ans, s1->f[j] + s2->f[v - j]);
    printf("%d\n", ans);
  }
}
int main() {
