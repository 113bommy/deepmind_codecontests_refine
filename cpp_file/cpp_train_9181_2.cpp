...
int main() {
  ...
  while (T--) {
    ...
    scanf("%d", &n);
    for (int i = 1; i <= n; ++i)
      scanf("%d", &a[i]), dz.push_back(a[i]), Max(mx, a[i]);
    int m;
    scanf("%d", &m);
    for (int i = 1; i <= m; ++i) {
      scanf("%d%d", &p[i], &s[i]), dz.push_back(p[i]);
      if (p[i] >= mx) ans = 0;
    }
    (dz).push_back(-INF), sort((dz).begin(), (dz).end()),
        (dz).erase(unique((dz).begin(), (dz).end()), (dz).end());
    if (ans != -1) {
      for (int i = 1; i <= m; ++i)
        Max(b[(lower_bound(dz.begin(), dz.end(), (p[i])) - dz.begin())], s[i]);
      for (int i = ((int)(dz).size()) - 1; i >= 1; --i) Max(b[i], b[i + 1]);
      int last = 0, mx = 0;
      a[n + 1] = 2e9;
      for (int i = 1; i <= n + 1; ++i) {
        a[i] = (lower_bound(dz.begin(), dz.end(), (a[i])) - dz.begin());
        Max(mx, a[i]);
        if (b[mx] < i - last) ++ans, last = i - 1, mx = a[i];
      }
      for (int i = 0; i <= ((int)(dz).size()) + 5; ++i) b[i] = 0;
    }
    printf("%d\n", ans);
  }
  return 0;
