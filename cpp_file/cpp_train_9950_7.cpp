...
int gete(int g, int x) {
  if (v[g].empty() || v[g].back() <= x) return -1;
  int l = 0, r = v[g].size() - 1;
  while (l < r) {
    int m = (l + r) / 2;
    if (v[g][m] <= x)
      l = m + 1;
...
int main() {
  ...
  int ii = 0, ij = t.size() - 1;
  for (int i = 0; i < t.size(); i++) {
    if (i && l[i - 1] == -1) break;
    int tl = i, tr = t.size() - 1;
    while (tl < tr) {
      int m = (tl + tr) / 2;
  ...
  for (int i = 0; i < ii; i++) cout << t[i];
  for (int i = ij + 1; i < t.size(); i++) cout << t[i];
}
