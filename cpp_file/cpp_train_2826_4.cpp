...
void insertIner(vector<int> &BIT, int ind, int val) {
  ind++;
  for (int i = ind; i > 0; i -= i & -i) BIT[i] += val;
}
int getIner(const vector<int> &BIT, int ind) {
  ind++;
  int res = 0;
  for (int i = ind; i < BIT.size(); i += i & -i) res += BIT[i];
  return res;
}
void attachOuter(int ind, int val) {
  ind++;
  for (int i = ind; i <= un.size(); i += i & -i)
    if (st[i].empty() || st[i].back() != val) st[i].push_back(val);
}
void createOuter() {
  for (int i = 1; i <= un.size(); i++) BIT[i].resize(st[i].size() + 1, 0);
}
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    seq1.push_back(pair<int, int>(A[i] + B[i], i));
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    int bitind =
        lower_bound(un.begin(), un.end(), B[ind] - A[ind]) - un.begin();
  ...
  for (int i = 0; i < m; i++) {
    scanf("%d", &X[i]);
    seq2.push_back(pair<int, int>(X[i] + Y[i], i));
  }
  ...
  for (int i = 0; i < seq2.size(); i++) {
    ...
    int bitind =
        upper_bound(un.begin(), un.end(), Y[ind] - X[ind]) - un.begin() - 1;
    res[ind] = getOuter(bitind, Y[ind]);
  }
  for (int i = 0; i < m; i++) printf("%d%c", res[i], i + 1 < m ? ' ' : '\n');
  return 0;
}
