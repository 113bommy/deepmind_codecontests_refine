...
struct pt {
  ...
  bool operator<(const pt x) const { return r == x.r ? l > x.l : r < x.r; }
} a[100005];
struct edge {
  int v, w, nxt;
} e[2000005];
int L, R, n, m, e2[500005][2], h[500005], t, dis[500005], g[100005], gt,
    vis[500005];
...
int main() {
  ...
  if (L - l >= ll && L - l <= rr)
    ans1 = l, ans2 = L - l;
  else if (R - l >= ll && R - l <= rr)
    ans1 = l, ans2 = R - l;
  else if (l + ll >= L && l + ll <= R)
    ans1 = l, ans2 = ll;
  else if (l + rr >= L && l + rr <= R)
    ans1 = l, ans2 = rr;
  else if (L - r >= ll && L - r <= rr)
    ans1 = r, ans2 = L - r;
  else if (R - r >= ll && R - r <= rr)
    ans1 = r, ans2 = R - r;
  else if (r + ll >= L && r + ll <= R)
    ans1 = r, ans2 = ll;
  else if (r + rr >= L && r + rr <= R)
    ans1 = r, ans2 = rr;
  if (fll == 1) swap(ans1, ans2);
  printf("%d %d\n", ans1, ans2);
  for (int i = 1; i <= n; i++) printf("%d", (b[g[i]] ^ (vis[i] - 2) ^ 1) + 1);
  return 0;
}
