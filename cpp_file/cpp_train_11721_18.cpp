...
struct Counter : std::unordered_map<int, int> {
  void inc(int x) { ++(*this)[x]; }
  void dec(int x) {
    auto iter = this->find(x);
    if (0 == --iter->second) this->erase(iter);
  }
};
int main() {
  ...
  struct Range {
   ...
   public:
    ...
    bool have_new_r() const { return pre[r] < l; }
    void incr() {
      if (have_new_r()) ++size_;
      ++r;
    }
  ...
  for (int i = 1;;) {
    ...
    for (unsigned j = 0;; ++j) {
      ...
      while (true) {
        if (x.r == (int)a.size()) {
          t.erase(begin(t) + j + 1, end(t));
          goto break_outer;
        }
        if ((int)x.size() == i && x.have_new_r()) break;
        x.incr();
      }
      auto& y = t[j + 1];
      if (x.r >= y.r) {
