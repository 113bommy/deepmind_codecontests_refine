...
const int N = 505;
int n, m, k;
long long dp[N][N], sum[N][N], s[2][N][N], d[2][N][N];
vector<pair<int, int> > c[2][N][N];
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    for (int j = 1; j <= m; ++j) {
      ...
      for (pair<int, int> p : c[0][i][j]) {
        d[0][i][j - p.first] += p.second;
      }
      for (pair<int, int> p : c[1][i][j]) {
      ...
      for (int k = 1; k < j; ++k) {
        vx += d[0][i][j - k];
        dp[i][j] =
            min(dp[i][j], dp[i][j - k] + s[0][i][j] - s[0][i][j - k] - vx);
      }
      for (int k = 1; k < i; ++k) {
        vy += d[1][i - k][j];
        dp[i][j] =
            min(dp[i][j], dp[i - k][j] + s[1][i][j] - s[1][i - k][j] - vy);
      }
    }
  }
  printf("%lld\n", dp[n][m]);
