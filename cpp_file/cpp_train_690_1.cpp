    ...
    for (long long i = 1; i <= n; i++) {
      ...
      if (pos + i - 1 < k21) {
        cout << "Yes"
             << "\n"
             << (n - k21 + 1) << ' ' << i << "\n";
        for (int j = k21; j <= n; j++) cout << a[j].second << ' ';
        cout << "\n";
        for (int j = pos; j <= pos + i - 1; j++) cout << a[j].second << ' ';
        return 0;
      }
    }
  }
  if (k22 != 0) {
    for (long long i = 1; i <= n; i++) {
      ...
      if (pos + i - 1 < k22) {
        ...
        for (int j = pos; j <= pos + i - 1; j++) cout << a[j].second << ' ';
        cout << "\n";
        for (int j = k22; j <= n; j++) cout << a[j].second << ' ';
        return 0;
      }
    }
  }
