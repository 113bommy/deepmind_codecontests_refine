...
struct node_t {
  int lc, rc, siz, len, minn, val, tag, rev, key;
} p[N * 5];
int a[N];
int T, n, tot, root, add, naive;
inline int newNode(int len, int val) {
  ++tot;
  p[tot].lc = p[tot].rc = p[tot].rev = 0;
  p[tot].tag = INF;
  p[tot].siz = p[tot].len = len;
  p[tot].val = p[tot].minn = val;
  p[tot].key = (int)R();
  return tot;
}
void update(int u) {
...
pair<int, int> find(int u, int x) {
  if (p[p[u].lc].siz < x && x <= p[p[u].lc].siz + p[u].len)
    return make_pair(u, p[p[u].lc].siz);
  pushdown(u);
  if (x <= p[p[u].lc].siz) return find(p[u].lc, x);
  return find(p[u].rc, x - p[p[u].lc].siz - p[u].len) +
         (p[p[u].lc].siz + p[u].len);
}
...
void changev(int u, int x, int val) {
  ...
  if (x <= p[p[u].lc].siz)
    changev(p[u].lc, x, val);
  else
    changev(p[u].rc, x - p[p[u].lc].siz - p[u].len, val);
  update(u);
}
int main() {
  p[0].minn = INF;
  ...
  while (T--) {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      split(a[i] - 1);
      split(root, a[i] - 1, root, naive);
      add_rev(root);
      ++add;
      add_tag(root, all + 2 - add);
      if (a[i] % 2 == 0) {
