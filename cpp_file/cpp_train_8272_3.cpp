...
char s[maxn];
unsigned long long Seed_Pool[] = {19260817, 91815541};
unsigned long long Mod_Pool[] = {1000000009, 4294967291ull};
int n, k;
struct Hash_1D {
...
bool check(int st1, int st2, int len) {
  ...
  for (int i = 0; i < HASH_CNT; i++) {
    unsigned long long hash_val1 = hasher[i].getHash(st1, st1 + len - 1);
    unsigned long long hash_val2 = hasher[i].getHash(st2, st2 + len - 1);
...
int main() {
  ...
  for (int len = 1; len <= n; len++) {
    int Len = len * k;
    if (Len > n) break;
    unsigned long long hash_val1 = hasher[0].getHash(1, len);
    unsigned long long hash_val2 = hasher[1].getHash(1, len);
    bool succ = true;
    for (int j = 1; j < k; j++) {
      int l = len * j + 1;
      int r = l + len - 1;
      unsigned long long temp_val1 = hasher[0].getHash(l, r);
      unsigned long long temp_val2 = hasher[1].getHash(l, r);
