...
class Automaton {
 public:
  ...
  void mark(const int &lower_bound, const int &upper_bound) {
    ...
    for (Node *k = last_node_; k; k = k->link) k->endings = 1;
    for (int i = nodes_.size() - 1; i >= 0; --i)
      for (auto &now : nodes_[i])
        for (int j = 0; j < 26; ++j)
          if (now->next[j]) now->endings += now->next[j]->endings;
    root_->lower_bound = 1;
    for (int i = 1; i < int(nodes_.size()); ++i)
      for (auto &now : nodes_[i]) {
        if (now->endings >= lower_bound)
          now->upper_bound = now->length;
        else
          now->upper_bound = now->link->upper_bound;
        if (now->endings > upper_bound)
          now->lower_bound = now->length + 1;
        else
          now->lower_bound = now->link->lower_bound;
      }
  }
  pair<vector<int>, vector<int> > match(const string &S) {
    ...
    for (int i = 0; i < int(S.size()); ++i) {
      ...
      if (lower_bound_ == 0) upper_bound[i] = i + 1;
    }
 ...
 private:
  ...
  void extend(const int &edge) {
    ...
    for (; k and k->next[edge] == p; k = k->link) k->next[edge] = clone;
  }
...
int main() {
  ...
  for (int i = 0; i < int(S.size()); ++i) upper_bound[i] = i + 1;
  for (int i = 0; i < N; ++i) {
