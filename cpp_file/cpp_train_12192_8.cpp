...
pair<int, int> h1[1010], h2[1010];
queue<int> q;
int fa[1010], L[1010], R[1010];
int head[2010], cur[2010], level[2010], X[1010], Y[1010], n, vs, vt, t1, t2,
    tot;
bool vis[2010];
inline int rd() {
  int x = 0;
...
int main() {
  n = rd();
  for (int i = 1; i <= n; i++)
    h[i].x = rd(), h[i].y = rd(), h[i].id = i, X[i] = h[i].x, Y[i] = h[i].y;
  t1 = t2 = 0;
  sort(h + 1, h + n + 1, cmp1);
  for (int i = 1, j; i <= n; i = j) {
    for (j = i; j <= n && h[j].x == h[i].x; j++)
      ;
    for (int k = i; k < j - 1; k++)
      h1[++t1] = pair<int, int>(h[k].id, h[k + 1].id);
  }
  sort(h + 1, h + n + 1, cmp2);
  for (int i = 1, j; i <= n; i = j) {
    for (j = i; j <= n && h[j].y == h[i].y; j++)
      ;
    for (int k = i; k < j - 1; k++)
      h2[++t2] = pair<int, int>(h[k].id, h[k + 1].id);
  }
  vs = 0;
  vt = t1 + t2 + 1;
  ...
  for (int i = 1; i <= t1; i++)
    for (int j = 1; j <= t2; j++)
      if (X[h1[i].first] > X[h2[j].first] && X[h1[i].first] < X[h2[j].second] &&
          Y[h2[j].first] > Y[h1[i].first] && Y[h2[j].first] < Y[h1[i].second])
        add(i, j + t1, 2123333333);
  while (bfs()) dfs(vs, 2123333333);
  gao(vs);
  ...
  for (int i = 1; i <= t2; i++)
    if (!vis[t1 + i]) {
      ...
      if (f1 != f2)
        fa[f1] = f2, L[f2] = min(L[f1], L[f2]), R[f2] = max(R[f1], R[f2]);
    }
  int tt = 0;
  for (int i = 1; i <= n; i++)
    if (find(i) == i) tt++;
  ...
  for (int i = 1; i <= n; i++)
    if (find(i) == i)
      print(L[i]), print(Y[i]), print(R[i]), print(Y[i]), putchar('\n');
  ...
  for (int i = 1; i <= n; i++)
    if (find(i) == i)
      print(X[i]), print(L[i]), print(X[i]), print(R[i]), putchar('\n');
  return 0;
}
