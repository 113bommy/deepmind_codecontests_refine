...
int N, M, K, Q, ex[MN], cap[MM * 2], h[MN], hd[MN], nx[MM * 2], to[MM * 2], X,
    cur[MN], tot, mask, ans[1 << MK], sm[1 << MK], vis[MN], V, buffer[(int)3e7],
    *flow = buffer, *buf = buffer;
bool done[1 << MK];
...
int saturate() {
  ...
  auto relabel = [&](int n) {
    ...
    for (int i = hd[n]; ~i; i = nx[i])
      if (cap[i] - flow[i] > 0) ckmin(v, h[to[i]]);
    h[n] = v + 1;
  };
  h[1] = N;
...
int dfs(int n, int f) {
  ...
  for (int& i = cur[n]; ~i; i = nx[i])
    if (cap[i] - flow[i] > 0 && vis[to[i]] < V) {
      ...
      if (v > 0) return flow[i] += v, flow[i ^ 1] -= v, v;
    }
  return 0;
}
int resaturate() {
...
int main() {
  ...
  tot = saturate();
  ans[(1 << K) - 1] = tot;
  for (int i = 0; i < K; ++i) flow[i * 2] = cap[i * 2] = 25;
  ...
  for (int i = 0; i < Q; ++i) {
    for (int j = 0; j < K; ++j) scanf("%d", sm + (1 << j));
    int fv = ans[0];
