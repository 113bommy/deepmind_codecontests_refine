...
double calc(double xa, double ya, double za, double xb, double yb, double zb,
            double x0, double y0, double z0, double L, double R) {
  double res = 1e20;
...
int main() {
  ...
  for (int step = 0; step < 1000; ++step) {
    ...
    for (int i = 0; i < 200; ++i) {
      ...
      double valueL = solve(xa, ya, za, r * cos(L), r * sin(L), 0) +
                      solve(r * cos(L), r * sin(L), 0, xb, yb, zb);
      double valueR = solve(xa, ya, za, r * cos(R), r * sin(R), 0) +
                      solve(r * cos(R), r * sin(R), 0, xb, yb, zb);
      res = min(res, valueL);
      res = min(res, valueR);
      if (valueL < valueR) {
        right_bound = R;
  ...
  for (int stepA = 0; stepA < cnt; ++stepA) {
    for (int stepB = 0; stepB < cnt; ++stepB) {
      ...
      for (int i = 0; i < 60; ++i) {
        ...
        double R = (left_bound + 2.0 * right_bound) / 3.0;
        double valueL =
            calc(xa, ya, za, xb, yb, zb, r * cos(L), r * sin(L), 0.0, lB, rB);
        double valueR =
            calc(xa, ya, za, xb, yb, zb, r * cos(R), r * sin(R), 0.0, lB, rB);
        res = min(res, valueL);
        res = min(res, valueR);
        if (valueL < valueR) {
          right_bound = R;
