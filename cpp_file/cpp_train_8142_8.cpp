#include <bits/stdc++.h>
using namespace std;
const int N = 2 * 1000 * 100 + 10, mod = 1e9 + 21, P = 313;
long long n, pw[N], dv;
string s;
...
struct Node {
  ...
  void merge(Node L, Node R) {
    ...
    if (L.en == 1 && R.st == 1) {
      ...
      hsh = add(add(L.hsh, -mul(pw[L.len - 1], 2)),
                mul(pw[L.len - 1], mul(add(R.hsh, -2), dv)));
      return;
    }
  ...
  bool operator==(Node x) { return hsh == x.hsh; }
  void init(int x) {
    en = st = x, len = 1;
    hsh = x + 1;
  }
