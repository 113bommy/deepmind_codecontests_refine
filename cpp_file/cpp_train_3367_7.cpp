    ...
    if (id == -1) {
    ...
    } else {
      {
        ...
        if (it != mapa.end())
          dist[id] =
              min(dist[id],
                  (absolut(x - it->first) + absolut(y - it->second)) / divi);
      }
      {
        map<int, int>::iterator it = mapa.lower_bound(x);
        if (it != mapa.begin()) {
...
void computadistext(vector<elem> &ve, vector<int> &dist, int divi) {
  ...
  for (int i = 0; i < int(ve.size()); i++) ve[i].y *= -1;
  computadist(ve, dist, divi);
}
const int tope = 1 << 20;
...
void computalinea(vector<elem> &ve, vector<int> &dist) {
  for (int i = 0; i < tope; i++) minimo[i] = infinito;
  map<int, int> posicion;
  for (int i = 0; i < int(ve.size()); i++) {
    posicion[ve[i].x];
    if (ve[i].id != -1) posicion[ve[i].x2];
  }
  {
    int i = 0;
    for (map<int, int>::iterator it = posicion.begin(); it != posicion.end();
         it++, i++)
      it->second = i;
  }
  sort(ve.begin(), ve.end());
  for (int i = 0; i < int(ve.size()); i++) {
    ...
    if (e.id == -1) {
      ...
      dist[e.id] =
          min(dist[e.id], minimoentre(posicion[e.x], posicion[e.x2]) - e.y);
    }
  }
}
...
bool compara(int i1, int i2) { return dd[i1] < dd[i2]; }
void versionfacil() {
...
int main() {
  ...
  vector<elem> vever;
  {
    ...
    for (int i = 0; i < n; i++) dd[i] = 2 * dist[i];
  ...
  for (int i = n - 1; i > 0; i--) {
    ...
    if (tiempo + draya <= dis / 2)
      sol = min(sol, dis / 2);
    else if (draya + dis / 2 <= tiempo)
      sol = min(sol, tiempo);
    else
      sol = min(sol, (tiempo + draya + dis / 2) / 2);
  }
  cout << (sol + 1) / 2 << endl;
}
