...
int main() {
  ...
  for (int i = 1; i <= n; ++i)
    if (deg[i] <= 1)
      for (int j = 1; j <= n; ++j)
        if (G[i][j] && deg[j] <= 1) H[i].push_back(j);
  c[0][0] = 1;
  for (int i = 1; i <= n; ++i) {
    c[i][0] = 1;
    ...
    for (int u = 1; u <= n; ++u) {
      ...
      for (auto v : H[u]) {
        ...
        for (int i = size[u]; i >= 0; --i)
          g[u][i + size[v]] =
              1ll * g[u][i] * f[v][size[v]] % MOD * c[i + size[v]][i] % MOD,
                   f[u][i + size[v]] = 1ll * f[u][i] * f[v][size[v]] % MOD *
                                       c[i + size[v]][i] % MOD;
        for (int i = 0; i < size[v]; ++i) g[u][i] = f[u][i] = 0;
        for (int i = 0; i <= size[u] + size[v] - 1; ++i)
          g[u][i] = dmy(g[u][i] + temp[i]);
        size[u] += size[v];
      }
      if (!deg[u])
        for (int j = 0; j <= size[u]; ++j)
          sum[j] = dmy(sum[j] + dmy(g[u][j] + f[u][j]));
      else
        for (int j = 0; j <= size[u]; ++j) sum[j] = dmy(sum[j] + f[u][j]);
    }
