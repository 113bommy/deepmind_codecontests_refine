...
void pushup(int num) {
  lv[num] =
      (lv[num << 1] + 1ll * lv[num << 1 | 1] * pw[siz[num << 1]] % mod) % mod;
  rv[num] =
      (rv[num << 1] + 1ll * rv[num << 1 | 1] * iw[siz[num << 1]] % mod) % mod;
  val[num] =
      (val[num << 1] + val[num << 1 | 1] +
       1ll * lv[num << 1] * rv[num << 1 | 1] % mod * iw[siz[num << 1]] % mod) %
      mod;
...
void update(int num, int l, int r, int x, int v) {
  ...
  int mid = l + r >> 1;
  if (x <= mid)
    update(num << 1, l, mid, x, v);
  else
    update(num << 1 | 1, mid + 1, r, x, v);
  pushup(num);
}
int main() {
  pw[0] = iw[0] = 1;
