...
void update(int rt, int l, int r, int pos, int id) {
  ...
  if (pos <= ((l + r) >> 1))
    update(rt << 1, l, ((l + r) >> 1), pos, id);
  else
    update(rt << 1 | 1, ((l + r) >> 1) + 1, r, pos, id);
  up(rt);
}
int query(int rt, int l, int r, int pos, int val) {
  ...
  int ans = -1;
  if (pos <= ((l + r) >> 1)) ans = query(rt << 1, l, ((l + r) >> 1), pos, val);
  if (ans != -1) return ans;
  return query(rt << 1 | 1, ((l + r) >> 1) + 1, r, pos, val);
}
int ans[maxn];
void sol() {
