...
long long t[MAXN];
vector<bool> solve(int _N, int L, long long v[]) {
  int N = _N + 1;
  vector<long long> res(N), cyc(N), order(N);
  for (int i = 0; i < _N; i++) {
    if ((v[i] + t[i]) & 1) fail();
    v[i] = (v[i] + t[i]) / 2;
    res[i] = t[i] % L;
  ...
  for (int _i = 0; _i + 1 < N; _i++) {
    ...
    long long dk = cyc[j] - cyc[i], dx = v[j] - v[i], cap = res[j] - res[i];
    if (dk == 0) {
      if (dx < 0 || dx > cap) fail();
    } else if (dk > 0) {
  ...
  for (int _i = 0; _i < N; _i++) {
    ...
    while (ans.size() < res[i]) ans.push_back(0);
  }
  return ans;
}
int N, L;
