...
bool operator==(const Modular<T>& lhs, const U& rhs) {
  return lhs == Modular<T>(rhs);
}
template <typename T, typename U>
bool operator==(const U& lhs, const Modular<T>& rhs) {
...
template <typename T, typename U>
Modular<T> operator+(const Modular<T>& lhs, const U& rhs) {
  return Modular<T>(lhs) += rhs;
}
template <typename T, typename U>
Modular<T> operator+(const U& lhs, const Modular<T>& rhs) {
  return Modular<T>(lhs) += rhs;
}
template <typename T>
Modular<T> operator-(const Modular<T>& lhs, const Modular<T>& rhs) {
...
void multiply(T* a, T* b, int n, int P) {
  ...
  for (int i = 0; i < n; i++) {
    int j = i ? n - i : 0;
    g[i] = (!(f[j] * f[j]) + f[i] * f[i]) * Complex(-0.25, 0.25) +
           !f[j] * f[i] * Complex(0.5, 0.5);
  }
  FFT(g, g + n, -1);
  for (int i = 0; i < n; i++) {
    b[i] = (b[i] + (long long)(g[i].x + 0.5) +
            (((long long)(g[i].y + 0.5) % P) << 30)) %
           P;
  }
}
template <typename T>
void multiply(T* a, T* b, T* c, int n, int P) {
  ...
  for (int i = 0; i < n; i++) {
    int j = i ? n - i : 0;
    h[i] = (!(f[j] * g[j]) + f[i] * g[i]) * Complex(-0.25, 0.25) +
           (!f[j] * g[i] + f[i] * !g[j]) * Complex(0.25, 0.25);
  }
  FFT(h, h + n, -1);
  for (int i = 0; i < n; i++) {
    c[i] = (long long)(c[i] + (long long)(h[i].x + 0.5) +
                       (((long long)(h[i].y + 0.5) % P) << 30)) %
           P;
  }
}
};  // namespace FFT
...
Polynomial product(const vector<Polynomial>& a) {
  function<bool(Polynomial, Polynomial)> compare =
      [&](Polynomial lhs, Polynomial rhs) { return lhs.size() > rhs.size(); };
  priority_queue<Polynomial, vector<Polynomial>, decltype(compare)> Q(
