...
template <typename T>
inline bool upmx(T &a, const T &b) {
  return a < b ? a = b, 1 : 0;
}
template <typename T>
inline bool upmn(T &a, const T &b) {
  return a > b ? a = b, 1 : 0;
}
template <typename T>
...
int main() {
  in, s;
  n = strlen(s);
  for (int i = (0), i_end = (n); i < i_end; ++i) ++tot[s[i] -= 'a'];
  for (int i = (0), i_end = (n); i < i_end; ++i) {
    f[s[i]].push_back(i);
  }
  for (int i = (0), i_end = (26); i < i_end; ++i) {
    if (tot[i] < 2) {
      ans += tot[i] > 0;
      continue;
    }
    ...
    for (int j = (1), j_end = (n); j < j_end; ++j) {
      static int o[26];
      memset(o, 0, sizeof o);
      for (__typeof(f[i].begin()) x = f[i].begin(); x != f[i].end(); ++x) {
        int p = (*x + j) % n;
        o[s[p]]++;
      }
      int fl = 0;
