...
int calcRes() {
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++) row[i][j] = row[i][j - 1] + (B[i][j] == '1');
  for (int j = 1; j <= m; j++)
    for (int i = 1; i <= n; i++) col[j][i] = col[j][i - 1] + (B[i][j] == '1');
  for (int j = 1; j <= m; j++)
    for (int i = 1; i <= n; i++) {
...
void Rotate() {
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++) Bn[j][n - i + 1] = B[i][j];
  swap(n, m);
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++) B[i][j] = Bn[i][j];
}
int main() {
  scanf("%d %d", &n, &m);
