...
struct InterHit {
  ...
  long long getAns() {
    std::sort(qs, qs + qs_top,
              [](QueryType a, QueryType b) { return a.r < b.r; });
    int pos = 0;
    while (pos < qs_top && qs[pos].r < 0) {
      qs[pos].ans = 0;
      pos++;
    }
...
void preInit() {}
void init() { scanf("%d%s", &n, s); }
void solve() {
  for (int i = 0; i < n; i++)
    height[i + 1] = height[i] + (s[i] == '(' ? 1 : -1);
  logArray(height, n + 1);
  ql.init(height, n + 1);
  hit.init(height, n + 1);
