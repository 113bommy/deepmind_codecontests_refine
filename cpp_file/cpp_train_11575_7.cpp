...
int main() {
  ...
  for (; t; t--) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < m; j++) {
        flag = false;
        if (u[i][j] == 41 && u[i][j + 1] == 217 && u[i][j + 2] == 161 &&
            u[i + 1][j] == 55 & u[i + 1][j + 2] == 355 && u[i + 2][j] == 11 &&
            u[i + 2][j + 1] == 397 && u[i + 2][j + 2] == 131) {
          ans++;
          continue;
        }
        if (u[i][j] == 41) {
          ...
          if (t1 != t2 || u[i][j + t1 + 3] != 161 || u[i + t2 + 3][j] != 11 ||
              u[i + t2 + 3][j + t1 + 3] != 131)
            continue;
          if (u[i + 1][j + 3 + t1] != 291 || u[i + 2 + t2][j + 3 + t1] != 99)
            continue;
          if (u[i + 3 + t1][j + 2 + t1] != 141 || u[i + 3 + t2][j + 1] != 393)
            continue;
          for (int k = 1; k < t1; k++) {
            if (u[i + t1 + 3][j + k + 2] != 137 ||
                u[i + k + 2][j + t1 + 3] != 35) {
              flag = true;
              break;
            }
        ...
        if (u[i][j] == 81) {
          ...
          if (t1 != t2 || u[i + t1 + 1][j + t1 + 1] != 321 ||
              u[i + t2 + 1][j - t2 - 1] != 21 || u[i + t2 + t1 + 2][j] != 261)
            continue;
          for (int k = 1; k <= t1; k++) {
            if (u[i + t1 + k + 1][j + t1 + 1 - k] != 69 ||
                u[i + t1 + k + 1][j - t1 - 1 + k] != 273) {
              flag = true;
              break;
            }
          }
        ...
        if (u[i][j] == 57 && u[i][j + 1] == 225 && u[i + 1][j + 1] == 387 &&
            u[i + 1][j] == 15) {
          ans++;
        }
      }
