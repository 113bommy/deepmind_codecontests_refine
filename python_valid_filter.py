...
python_json_path = './python_data/python_valid.json'
...
def make_python_file(pid, index, code):
    with open(f'./python_file/python_valid_{pid}_{index}.py', 'w') as f:
        f.write(code)
...
def run_test_cases(file, input_data_list, expected_output_list):
    ...
    for index, data in enumerate(input_data_list):
        ...
        try:
            process = subprocess.run(
                ['python', f"./python_file/{file}"], input=python_input, text=True, errors="replace", capture_output=True, timeout=2
            )
            try:
    ...
    if len(output_list) != len(input_data_list):
        with open(f'./python_result/exclude_valid_gold.txt', "w", encoding='utf-8') as err_log_file:
...
def compile_and_test(pid_index_codes):
    ...
    script_file_path = f"./python_file/python_valid_{pid}_{index}.py"
...
def save_results(pid, results):
    try:
        with open(f'./python_valid_result/python_valid_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./python_valid_result/python_valid_check_{pid}.json', 'w') as f:
...
with open('./python_data/python_valid_refine.json', 'r') as f:
