...
def make_cpp_file(pid, index, code):
    with open(f'./cpp_file/cpp_valid_{pid}_{index}.cpp', 'w') as f:
        f.write(code)
...
def run_test_cases(file, input_data_list, expected_output_list):
    ...
    if len(output_list) != len(input_data_list):
        with open(f'./cpp_result/exclude_valid_gold.txt', "w", encoding='utf-8') as err_log_file:
            err_log_file.write(f"{file}\n")
        return (None, 1.0)

    return (output_list, (1 - correct_test / len(input_data_list)))

def compile_and_test(pid_index_codes):
    pid, index, codes = pid_index_codes
    make_cpp_file(pid, index, codes)

    compiled_file_path = f"./cpp_compile/cpp_valid_{pid}_{index}"
    compile_result = subprocess.run(
        ["g++", "-std=c++11", "-static", '-O2', "-o", compiled_file_path, f'./cpp_file/cpp_valid_{pid}_{index}.cpp'],
        capture_output=True
    )

    if compile_result.returncode != 0:
        return (pid, index, False, "Compilation Failed", compiled_file_path)

    input_list = cpp_valid[pid]['test_case']['input']
    output_list = cpp_valid[pid]['test_case']['output']
    success_check, error_rate = run_test_cases(f"cpp_valid_{pid}_{index}", input_list, output_list)
    return (pid, index, success_check, error_rate, compiled_file_path)

def save_results(pid, results):
    try:
        with open(f'./cpp_valid_result/cpp_valid_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./cpp_valid_result/cpp_valid_check_{pid}.json', 'w') as f:
