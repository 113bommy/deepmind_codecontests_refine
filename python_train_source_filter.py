...
def make_python_file(pid, index, code):
    with open(f'./python_source_filter_file/python_train_{pid}_{index}.py', 'w') as f:
        f.write(code)
...
def compile_and_test(pid_index_codes):
    ...
    script_file_path = f"./python_source_filter_file/python_train_{pid}_{index}.py"
    ...
    try:
        ...
        with open(f'./python_result/exclude_train_source.txt', "w", encoding='utf-8') as err_log_file:
...
def save_results(pid, results):
    try:
        with open(f'./python_train_source_result/python_train_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./python_train_source_result/python_train_check_{pid}.json', 'w') as f:
...
if __name__ == "__main__":
    delete_all_files_in_folder('./python_train_source_result')
    with open('./python_data/python_train_gold_filtered.json', 'r') as f:
        python_train = json.load(f)
    # python_train = concat_train_case(python_train)
