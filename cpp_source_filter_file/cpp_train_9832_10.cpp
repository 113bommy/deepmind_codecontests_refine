...
inline void kruskal() {
  ...
  for (int i = 0; i < m; i++)
    if (root(e[i].x) != root(e[i].y)) {
      ...
      adj[e[i].x].push_back(make_pair(e[i].y, e[i].w));
      adj[e[i].y].push_back(make_pair(e[i].x, e[i].w));
      sum += e[i].w;
    }
}
void DFS(int u) {
...
int main() {
  ...
  for (int i = 1; i < L; i++)
    for (int j = 1; j <= n; j++) {
      ...
      mx[j][i] = max(mx[j][i - 1], mx[par[j][i - 1]][j - 1]);
    }
  cin >> q;
  while (q--) {
    int x, y;
