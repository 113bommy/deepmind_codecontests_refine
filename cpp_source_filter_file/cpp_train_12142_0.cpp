...
int main() {
  ...
  for (int i = 0; i < ALPHA; ++i) {
    ...
    for (int j = 0; j < n / 2; ++j) {
      mcf::add_edge(2 + i, 2 + ALPHA * (i + 1) + j, 1, 0);
      mcf::add_edge(2 + ALPHA * (i + 1) + j, 2 + ALPHA * (n / 2 + 1) + j, 1,
                    s[j] == i ? -b[j] : 0);
      mcf::add_edge(2 + ALPHA * (i + 1) + j,
                    2 + ALPHA * (n / 2 + 1) + (n - 1 - j), 1,
                    s[n - 1 - j] == i ? -b[n - 1 - j] : 0);
    }
  }
  for (int i = 0; i < n; ++i) {
