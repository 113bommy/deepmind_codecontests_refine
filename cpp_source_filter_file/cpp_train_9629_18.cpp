...
void dfs(int v = 0, int par = -1) {
  ...
  for (int u : nei[v]) h[u] = h[v] + 1, dfs(u, v);
  et[v] = TIME;
}
string solve(int v, int h) {
  int l = lower_bound(S[h].begin(), S[h].end(), st[v]) - S[h].begin(),
      r = upper_bound(S[h].begin(), S[h].end(), et[v]) - S[h].begin();
  return (r <= l || ((r - l) & 1) == __builtin_popcount(ps[h][r - 1] ^
                                                        (l ? ps[h][l - 1] : 0))
              ? "Yes"
              : "No");
}
int main() {
  ...
  cin >> n >> m;
  for (int i = 1; i < n; i++) cin >> par, nei[--par].push_back(i);
  cin >> s;
  dfs();
  while (m--) {
    cin >> x >> y;
