...
inline int gethash1r(int l, int r) { return sub(h1r[r] - h1r[l - 1], MOD1); }
inline int gethash2r(int l, int r) { return sub(h2r[r] - h2r[l - 1], MOD2); }
inline bool ispal(int l, int r) {
  if ((1ll * gethash1l(l, r) * pw1[l - 1]) % MOD1 == gethash1r(l, r) &&
      (1ll * gethash2l(l, r) * pw2[l - 1]) % MOD2 == gethash2r(l, r))
    return true;
  else
    return false;
}
int main() {
  ...
  for (i = 1; i <= len; i++) {
    ...
    if (i >= d && ispal(i - d + 1, i)) {
      fi = i;
      break;
    }
    if (i >= d + 1 && ispal(i - d, i)) {
      fi = i;
      break;
    }
  }
