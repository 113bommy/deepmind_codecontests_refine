...
void dfs(int x) {
  ...
  if (op[x] != 1) {
    ...
    for (int i = 0; i < (1 << 16); i++) tmp2[i] = dp[ch[x][0]][i];
    FWTand(tmp1, 1 << 16, 1);
    FWTand(tmp2, 1 << 16, 1);
...
int main() {
  ...
  for (int i = 0; i < (1 << 16); i++) {
    bool flg = 1;
    for (int j = 0; j < 16; j++)
      if (must[j] != -1 && ((i >> j & 1) != must[j])) flg = 0;
