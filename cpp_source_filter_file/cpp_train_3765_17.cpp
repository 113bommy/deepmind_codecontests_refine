...
int main() {
  ...
  for (int i = 1; i <= n; i++) pr[i] = st[a[i]], st[a[i]] = i;
  for (int i = 1; i <= n; st[++tp] = i++)
    while (tp && a[st[tp]] <= a[i]) nx[st[tp]] = i, tp--;
  while (tp) nx[st[tp]] = n + 1, tp--;
  for (int i = n; i >= 1; st[++tp] = i--)
    while (tp && a[st[tp]] <= a[i]) ls[st[tp]] = i, tp--;
  while (tp) ls[st[tp]] = 0, tp--;
  for (int i = 1; i <= n; i++) pr[i] = max(pr[i - 1], pr[i]);
  for (int i = 1; i <= n; i++)
    if (i - ls[i] < nx[i] - i) {
      for (int j = ls[i] + 1; j <= i; j++)
        if (j + a[i] - 1 >= i && j + a[i] - 1 < nx[i] && pr[j + a[i] - 1] < j)
          rs++;
    } else {
      for (int j = i; j < nx[i]; j++)
        if (j - a[i] + 1 > ls[i] && j - a[i] + 1 <= i && pr[j] < j - a[i] + 1)
          rs++;
    }
  return printf("%d\n", rs), 0;
}
