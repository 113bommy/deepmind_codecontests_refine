...
int32_t main() {
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = 1; j <= m; j++) {
      upd({i}, {j}, abs(sa - sb - 2 * (a[i] - b[j])));
    }
    for (int j = i + 1; j <= n; j++) {
      int x = lower_bound(two.begin(), two.end(),
                          array<long long, 3>({a[i] + a[j] - p / 2, -1, -1})) -
              two.begin();
      for (int k = max(0, x - 5); k < min((int)two.size(), x + 5); k++) {
        upd({i, j}, {(int)two[k][1], (int)two[k][2]},
            abs(sa - sb - 2 * (a[i] + a[j] - two[k][0])));
      }
    }
  }
  cout << ans << '\n';
