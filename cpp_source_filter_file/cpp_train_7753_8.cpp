#include <bits/stdc++.h>
using namespace std;
pair<int, int> dp1[21][12][12][12][12][12];
pair<int, int> dp2[21][12][12][12][12][12];
int A[10];
int B[25];
int N, M;
int chk[8];
...
void getDP1(int x, int lvl) {
  ...
  if (lvl == N + 1) {
    ...
    int state = du[1] * pow12[1] + du[2] * pow12[2] + du[3] * pow12[3] +
                du[4] * pow12[4] + du[5] * pow12[5];
    for (i = 0; i < dp_val[B[x]].size(); i++) {
      ...
      dp1[x][t_du[1]][t_du[2]][t_du[3]][t_du[4]][t_du[5]] =
          make_pair(state, dp_val[B[x]][i]);
    }
    return;
  }
  for (i = 0; i < 12; i++) {
...
void getDP2(int x, int lvl) {
  ...
  if (lvl == N + 1) {
    ...
    for (i = 0; i < dp_val[B[x]].size(); i++) {
      ...
      for (j = N + 1; j <= 5; j++) t_du[j] = 0;
      dp2[x][t_du[1]][t_du[2]][t_du[3]][t_du[4]][t_du[5]] =
          make_pair(state, dp_val[B[x]][i]);
    }
    return;
  }
...
bool getANS(int x, int lvl) {
  ...
  if (lvl == N + 1) {
    for (i = 1; i <= N; i++) {
      if (chk[i] >= 12 || chk2[i] >= 12 || chk[i] < 0 || chk2[i] < 0)
        return false;
    }
    if (dp1[x][chk[1]][chk[2]][chk[3]][chk[4]][chk[5]].second != -1 &&
        dp2[x + 1][chk2[1]][chk2[2]][chk2[3]][chk2[4]][chk2[5]].second != -1)
      return true;
    return false;
  }
  for (i = 0; i <= M / 2; i++) {
...
int main() {
  ...
  for (i = 0; i < dp_val[B[1]].size(); i++) {
    ...
    dp1[1][du[1]][du[2]][du[3]][du[4]][du[5]] =
        pair<int, int>(-1, dp_val[B[1]][i]);
  }
  ...
  for (i = 0; i < dp_val[B[M]].size(); i++) {
    ...
    for (j = 1; j <= N; j++) du[j] = t_pos[j] * 7;
    for (j = N + 1; j <= 5; j++) du[j] = 0;
    dp2[M][du[1]][du[2]][du[3]][du[4]][du[5]] =
        pair<int, int>(-1, dp_val[B[M]][i]);
  }
  ...
  rs[1] = chk2[1], rs[2] = chk2[2], rs[3] = chk2[3], rs[4] = chk2[4],
  rs[5] = chk2[5];
  for (i = dp1s + 1; i <= M; i++) {
