...
void NTT(vector<int> &a, int n, int t) {
  static int pw[N + 9];
  static unsigned long long ntt[N + 9];
  ...
  for (int i = 1; i < n; i <<= 1) {
    ...
    for (int j = 0; j < n; j += i << 1)
      for (int k = 0; k < i; ++k) {
        unsigned long long x = ntt[j + k], y = pw[k] * ntt[i + j + k] % mod;
        ntt[j + k] = x + y;
        ntt[i + j + k] = x + mod - y;
      }
    if (i == 1 << 15)
...
void Divide_h(int id, int l, int r) {
  ...
  Get_len(r - l + 1);
  for (int u = 0; u < 2; ++u) {
    vector<int> a = vector<int>(h[u].begin() + l, h[u].begin() + mid + 1);
    a.resize(len);
    NTT(a, len, 0);
    for (int v = 0; v < 2; ++v) {
      vector<int> tmp =
          vector<int>(f[u][v].begin(), f[u][v].begin() + r - l + 1);
      tmp.resize(len);
      NTT(tmp, len, 0);
...
void Get_ans() {
  g.resize(n + 1);
  g[0] = g[2] = 1;
  for (int i = 4; i <= n; i += 2) g[i] = g[i - 2] + g[i - 4];
  for (int u = 0; u < 2; ++u)
  ...
  for (int i = 1; i <= n; ++i)
    for (int u = 0; u < 2; ++u)
      for (int v = 0; v < 2; ++v)
        f[u][v][i] =
            i - u - v - 1 >= 0 ? mul(g[i - u - v - 1], mul(i - 1, i - 1)) : 0;
  for (int id = 0; id < 2; ++id) {
    h[0] = vector<int>(n + 1, 0);
    h[1] = vector<int>(n + 1, 0);
    Divide_h(id, 0, n);
    for (int i = 0; i < n; ++i)
      for (int u = 0; u < 2; ++u)
        sadd(ans, mul(mul(h[u][i], f[u][id][n - i]), n - i));
  }
}
void work() {
  Get_wn();
