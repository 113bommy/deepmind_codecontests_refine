    ...
    for (int j = i + 1; j <= n; j++) {
      ...
      if (d == 0) {
        if (dx == 0) {
          ...
          if (flag1 && flag2)
            res = min(res, dis / ((s[i] + s[j]) * (s[i] + s[j])));
          else if (flag1)
            res = min(res, dis / s2[i]);
          else if (flag2)
            res = min(res, dis / s2[j]);
        }
        continue;
      }
      x = dx / d;
      ...
      res =
          min(res,
              max(((x - X[i]) * (x - X[i]) + (y - Y[i]) * (y - Y[i])) / s2[i],
                  ((x - X[j]) * (x - X[j]) + (y - Y[j]) * (y - Y[j])) / s2[j]));
    }
  if (res == inf)
    cout << "No show :(";
  else
    cout << std::setprecision(15) << sqrt(res);
}
int main() {
