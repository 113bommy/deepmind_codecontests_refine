...
int main() {
  ...
  for (int i = 1; i <= n; ++i)
    flow.add_edge(i, i + n, 1, -1), flow.add_edge(i, i + n, inf, 0);
  for (int i = n; i >= 1; --i) {
    if (nxt[a[i] + 1]) flow.add_edge(i + n, nxt[a[i] + 1], 1, 0);
    if (nxt[a[i] - 1]) flow.add_edge(i + n, nxt[a[i] - 1], 1, 0);
    if (z[a[i] % 7]) flow.add_edge(i + n, z[a[i] % 7], 1, 0);
    z[a[i] % 7] = i;
    nxt[a[i]] = i;
  }
  for (int i = 1; i <= n; ++i) flow.add_edge(0, i, 1, 0);
  for (int i = 1; i <= n; ++i) flow.add_edge(i + n, n + n + 1, 4, 0);
