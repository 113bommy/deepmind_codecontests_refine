...
int tmp[N * 4], min[2][N * 4], f[N * 4 + 2][3];
...
int main() {
  ...
  for (register int i = 0; i < n; i++) {
    p[0][i].first =
        std::lower_bound(&tmp[0], &tmp[lim] + 1, p[0][i].first) - tmp;
    p[0][i].second =
        std::lower_bound(&tmp[0], &tmp[lim] + 1, p[0][i].second) - tmp;
    min[0][p[0][i].first] = std::min(min[0][p[0][i].first], p[0][i].second);
  }
  for (register int i = 0; i < m; i++) {
    p[1][i].first =
        std::lower_bound(&tmp[0], &tmp[lim] + 1, p[1][i].first) - tmp;
    p[1][i].second =
        std::lower_bound(&tmp[0], &tmp[lim] + 1, p[1][i].second) - tmp;
  ...
  for (register int i = lim - 1; i >= 0; i--) {
    ...
    g[2] = (int64)f[i + 1][2] *
           ((power(2, tmp[i + 1] - tmp[i]) - 2 + mod) % mod) % mod;
