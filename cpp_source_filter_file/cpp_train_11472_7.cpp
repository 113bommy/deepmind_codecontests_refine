...
struct BigInteger {
  ...
  BigInteger(const BigInteger& o) { copy(o.a, o.a + LEN, a); }
...
bool gao(int len) {
  ...
  for (int i = 0; i < p2; i++) cnt += BigInteger(lucky[len - 1] - R[i]);
  for (int i = 0; i + len - 1 < lucky.size(); ++i) {
    long long p3 = p2;
    while (p1 < n && L[p1] <= lucky[i]) cnt -= BigInteger(L[p1] - last), ++p1;
    while (p2 < n && R[p2] <= lucky[i + len - 1])
      cnt += BigInteger(lucky[i + len - 1] - R[p2]), ++p2;
...
const long long BigInteger::MOD = 100000000000000LL;
int main() {
  cin >> n >> k;
  for (int i = 0; i < n; i++) cin >> L[i] >> R[i];
  mini = R[0] - L[0];
  for (int i = 0; i < n; i++) mini = min(mini, R[i] - L[i]);
  sort(L, L + n);
  sort(R, R + n);
  gao(0, 0);
