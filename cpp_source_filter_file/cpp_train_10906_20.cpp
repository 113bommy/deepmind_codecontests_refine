...
void transpose(std::vector<std::vector<T>>& M) {
  auto tmp = M;
  M.resize(tmp[0].size());
  for (auto& row : M) row.resize(tmp.size());
...
class FFT {
 public:
  ...
  static int get_lg(int x) { return x ? 32 - __builtin_clz(x - 1) : 0; }
  static void precompute(int lg) {
    ...
    if (ws[lg - 1].empty()) {
      ...
      for (int j = lg - 2; j >= 0 && ws[j].empty(); j--) {
        ...
        for (int i = 0; i < sz_level; i++) ws[j][i] = ws[j + 1][i << 1];
      }
    }
  }
  ...
  template <typename T>
  static std::vector<long long> multiply_brute_force(std::vector<T> const& a,
  ...
  template <typename T>
  static std::vector<long long> multiply(std::vector<T> const& a,
                                         std::vector<T> const& b) {
 ...
 public:
  ...
  BigInteger& operator+=(BigInteger const& o) {
    ...
    for (auto i = 0u; i < o.data.size() || carry; i++) {
      if (i < o.data.size()) carry += o.data[i];
...
int main() {
  ...
  int p = max(0, (int)(a.digits() * log(10) / log(3) - 2));
  int sum = p * 3;
  BigInteger x = 1;
  BigInteger base = 3;
  while (p) {
