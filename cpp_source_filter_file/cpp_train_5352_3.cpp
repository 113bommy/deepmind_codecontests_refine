...
struct SegmentTreeMax {
  ...
  void Set(int i, int f) {
    i += N;
    mx[i] = f;
    for (i >>= 1; i; i >>= 1) mx[i] = max(mx[i << 1], mx[i << 1 | 1]);
  }
  int Get(int l, int r) {
    int ret = -inf;
    for (l += N, r += N; l <= r; l >>= 1, r >>= 1) {
...
void Set(int &c, int ss, int se, int qi, int f) {
  ...
  int mid = ss + se >> 1;
  if (qi <= mid)
    Set(ls[c], ss, mid, qi, f);
  else
    Set(rs[c], mid + 1, se, qi, f);
...
int main() {
  ...
  scanf("%i", &q);
  for (i = 1; i <= q; i++) scanf("%i %i", &l[i], &r[i]), Qs[r[i]].push_back(i);
  for (i = 1; i <= n; i++) {
  ...
  for (i = 1; i <= n; i++) {
    ...
    while (top >= bot) {
      mid = top + bot >> 1;
      int mx = MX(root[a[i]], 1, n, mid, i);
      int mn = MN(root[a[i]], 1, n, mid, i);
      if (mn == inf || mn == mx)
        rng[i] = mid, top = mid - 1;
      else
        bot = mid + 1;
