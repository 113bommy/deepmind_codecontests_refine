...
class segtree {
  ...
  void build(int x, int l, int r, vector<int>& a) {
    if (l == r) {
      ...
      for (int i = 0, j = 0; i < (int)tree[x + 1].size(); ++i) {
        long long xleft = tree[x + 1][i],
                  xright = i + 1 < (int)tree[x + 1].size()
                               ? tree[x + 1][i + 1] - 1
                               : INF;
        long long yleft = xleft + sum[x + 1] - i * p,
                  yright = xright + sum[x + 1] - i * p;
        ...
        while (j < (int)tree[z].size() && tree[z][j] <= yright) {
          tree[x][i + j] =
              min(tree[x][i + j], max(xleft, tree[z][j] - sum[x + 1] + i * p));
          j++;
        }
 ...
 public:
  ...
  int query(int x, int l, int r, int ll, int rr, long long v) {
    if (ll <= l && r <= rr) {
      return v + sum[x] -
             p * (upper_bound(tree[x].begin(), tree[x].end(), v) -
                  tree[x].begin() - 1);
    } else {
      int y = (l + r) >> 1, z = x + ((y - l + 1) << 1);
      if (rr <= y) {
