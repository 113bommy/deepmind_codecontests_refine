...
inline int new_id() {
  return ++tot;
  while (!tr.empty() && tr.front() == 0) tr.pop();
  if (tr.empty()) return ++tot;
  int x = tr.front();
  tr.pop();
  if (ch[x][0]) tr.push(ch[x][0]);
  if (ch[x][1]) tr.push(ch[x][1]);
  return x;
}
...
inline void push_down(int x) {
  if (rev[x]) set_rev(ch[x][0]), set_rev(ch[x][1]), rev[x] = 0;
  if (atag[x])
...
inline void modi(int t, int k2) {
  ...
  l = len[w];
  tr.push(w);
  if (size[z] < t - 1) lp = new_node(t - 1 - size[z], k);
  if (size[z] + l > t) rp = new_node(size[z] + l - t, k);
...
inline void modi_size(int n) {
  int x, y, z, w, k;
  if (size[rt] == n) return;
  if (size[rt] < n) return rt = merge(rt, new_node(n - size[rt], inf)), void();
  split2(rt, x, y, n);
  tr.push(y);
  if (size[x] == n) return rt = x, void();
  split(x, z, w, size[x] - 1);
  k = key[w];
  tr.push(w);
  rt = merge(z, new_node(n - size[x], k));
  return;
}
inline void reverse() {
  set_rev(rt);
...
inline void solve(int ttt) {
  ...
  cur = 0;
  tl = n;
  for (i = 1; i <= n; i++) scanf("%d", &a[i]), tl = tl * 10 + a[i];
  for (i = 1; i <= n; i++) {
    treap::modi_size(a[i] - 1);
    if (a[i] % 2 == 0) {
      bk = 1;
      if (treap::query(a[i] / 2) == 0) bk = 0;
    }
    treap::reverse();
    treap::add(1);
    treap::mini(2);
    if (a[i] % 2 == 0) treap::modi(a[i] / 2, bk);
