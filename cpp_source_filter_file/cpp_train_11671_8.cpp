...
int main(void) {
  ...
  for (int i = 1; i <= m; ++i) {
    ...
    while (pnt < vs && v[pnt].first.first <= i) {
      s.insert(make_pair(make_pair(v[pnt].first.second, v[pnt].first.first),
                         v[pnt].second));
      ++pnt;
    }
    while (!s.empty() && s.begin()->first.first < i) s.erase(s.begin());
    if (!s.empty()) bel[i] = s.begin()->second, s.erase(s.begin());
  }
  for (int i = 1; i <= m; ++i) {
    if (bel[i]) printf("%d %d\n", bel[i], i);
  }
