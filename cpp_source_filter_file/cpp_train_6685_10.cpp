...
signed main(signed Recall, char *_902_[]) {
  ...
  while (T--) {
    ...
    for (int i = 1; i <= n; i++)
      b[++cnt] = l[i] = read(), b[++cnt] = r[i] = read();
    sort(b + 1, b + 1 + cnt);
    cnt = unique(b + 1, b + 1 + cnt) - b - 1;
    for (int i = 1; i <= n; i++)
      l[i] = lower_bound(b + 1, b + 1 + cnt, l[i]) - b,
      r[i] = lower_bound(b + 1, b + 1 + cnt, r[i]) - b;
    for (int i = 1; i <= n; i++) l[i] = l[i] * 2 - 1, r[i] = r[i] * 2 - 1;
    for (int i = 1; i <= n; i++) minl = min(minl, l[i]), maxr = max(maxr, r[i]);
    cnt = cnt * 2 + 1;
    for (int i = 1; i <= n; i++) c[l[i]]++, c[r[i] + 1]--;
    for (int i = 1; i <= cnt; i++) c[i] += c[i - 1];
    for (int i = 1; i <= cnt; i++)
      if (c[i] == 1 && c[i - 1] > 1) a[i] = 1;
    for (int i = 1; i <= cnt; i++)
      if (c[i] == 1) one[i] = 1;
    for (int i = 1; i <= cnt; i++) one[i] += one[i - 1], a[i] += a[i - 1];
