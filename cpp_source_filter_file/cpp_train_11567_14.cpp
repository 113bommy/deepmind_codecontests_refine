...
inline int64_t ask(basic_string<int64_t> b) {
  cout << b.size();
  for (int64_t i : b) {
    cout << " " << i;
  }
  ...
  if (ask({204761410474}) == 0) {
    if (ask({2046}) == 0) {
      if (ask({6}) == 0) {
        ...
        int64_t q = ask({14, 30, 62, 126, 254, 1022});
        int64_t w = (8 << q) - 1;
        ask(arith(w, w, 1));
      }
    } else {
      ...
      if (w1 == 0) {
        ...
        int64_t w2 =
            ask(basic_string<int64_t>{4094, 8190} + arith(2045, base2, step2));
        if (w2 <= 2) {
          int64_t abase = (2048 << w2) - 1;
          ask(arith(abase, abase, 1));
        } else {
          ask(arith(10000, base2 + (w1 - 1) * step2 + 1, 1));
        }
      } else {
