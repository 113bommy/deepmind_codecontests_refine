...
inline void pushdown(int now) {
  ...
  t[now].sum += lazy;
  if (t[now].l != t[now].r)
    t[now * 2].lazy += lazy, t[now * 2 + 1].lazy += lazy;
  t[now].lazy = 0;
}
inline void change(int now, int l, int r, int d) {
  ...
  if (t[now * 2].r >= r)
    ...
    change(now * 2 + 1, l, r, d);
  else
    change(now * 2, l, t[now * 2].r, d),
        change(now * 2 + 1, t[now * 2 + 1].l, r, d);
  if (t[now].l != t[now].r)
    pushdown(now * 2), pushdown(now * 2 + 1),
        t[now].sum = min(t[now * 2].sum, t[now * 2 + 1].sum);
}
inline int query(int now, int l, int r) {
  ...
  if (t[now * 2].r >= r)
    ...
    return min(query(now * 2, l, t[now * 2].r),
               query(now * 2 + 1, t[now * 2 + 1].l, r));
}
multiset<int> s;
set<pair<int, int> > s2;
int main() {
