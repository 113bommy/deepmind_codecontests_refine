...
struct car {
  long long id, des;
  long long arr;
  car(long long id, long long des, long long arr)
      : id(id), des(des), arr(arr) {}
...
struct wat {
  long long id, des;
  long long arr;
  wat(long long id, long long des, long long arr)
      : id(id), des(des), arr(arr) {}
...
int main() {
  ...
  while (m--) {
    ...
    if (!E.empty()) {
      ...
      if (pik1.arr > pik2.arr || pik1.arr == pik2.arr && pik1.id < pik2.id)
        pik2 = pik1;
