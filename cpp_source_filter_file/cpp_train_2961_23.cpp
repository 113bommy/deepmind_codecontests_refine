#include <bits/stdc++.h>
using namespace std;
const int nmax = 1e5 + 42, MX_val = 2e5 + 42, BLOCK_SIZE = 512;
int n, arr[nmax];
int q, type[nmax], val[nmax], position[nmax];
int help[nmax * 2], id_help = 0;
int seen[nmax / BLOCK_SIZE + 5][MX_val];
int seen_seen[nmax / BLOCK_SIZE + 5][nmax];
...
int main() {
  ...
  for (int i = 1; i <= q; i++) {
    if (type[i] == 1)
      val[i] = lower_bound(help + 1, help + id_help + 1, val[i]) - help;
  }
  for (int group = 0; group * BLOCK_SIZE <= n; group++) {
    if (group) {
      for (int j = 0; j < MX_val; j++) seen[group][j] = seen[group - 1][j];
    }
    for (int i = group * BLOCK_SIZE;
         i <= group * BLOCK_SIZE + BLOCK_SIZE - 1 && i <= n; i++)
      seen[group][arr[i]]++;
  }
  for (int group = 0; group * BLOCK_SIZE <= n; group++) {
    if (group) {
