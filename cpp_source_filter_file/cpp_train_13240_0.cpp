...
pair<int, int> query(int beg, int en, int l, int r, int node) {
  ...
  int mid = (l + r) >> 1;
  return min(query(beg, en, l, mid, node << 1),
...
int main() {
  ...
  for (int i = 1; i < n; i++) {
    if (query(i + 1, n, 1, n, 1).first < a[i]) {
      ...
      while (l <= r) {
        mid = (l + r) >> 1;
        if (query(i + 1, mid, 1, n, 1).first > a[i])
          l = mid + 1;
        else
          r = mid - 1;
      }
      ...
      while (l <= r) {
        mid = (l + r) >> 1;
        if (query(1, mid, 1, n, 1).first > a[i])
          l = mid + 1;
        else
          r = mid - 1;
      }
    ...
    if (query(1, i - 1, 1, n, 1).first < a[i]) {
      ...
      while (l <= r) {
        mid = (l + r) >> 1;
        if (query(mid, i - 1, 1, n, 1).first > a[i])
          r = mid - 1;
        else
          l = mid + 1;
      }
      ...
      while (l <= r) {
        mid = (l + r) >> 1;
        if (query(mid, n, 1, n, 1).first > a[i])
          r = mid - 1;
        else
          l = mid + 1;
      }
