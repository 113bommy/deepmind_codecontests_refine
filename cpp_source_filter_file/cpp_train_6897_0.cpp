...
struct Treap {
  ...
  pair<long long, long long> key2;
  Treap() {}
  Treap(int _key, int _id)
      : lc(NULL),
        rc(NULL),
        sum(_key),
        sz(1),
        ...
        pri(rand()),
        id(_id),
        key2(make_pair(_key, _id)) {}
};
int Sz(Treap* t) { return t ? t->sz : 0; }
int Sum(Treap* t) { return t ? t->sum : 0; }
void pull(Treap* t) {
  if (!t) return;
...
void split_by_key(Treap* t, pair<long long, long long> k, Treap*& a,
                  Treap*& b) {
  if (!t)
    a = b = NULL;
  else if (t->key2 <= k) {
...
int main() {
  ...
  for (int i = 0; i <= max(m, k); ++i) {
    if (i < v[3].size() && k - i < (int)min(v[1].size(), v[2].size())) {
      ...
      if (left >= 0) {
        long long val = pre[3][i] + pre[1][other] + pre[2][other] + Q(left);
        ans = min(ans, val);
      }
    }
    if (i + 1 < v[3].size()) {
