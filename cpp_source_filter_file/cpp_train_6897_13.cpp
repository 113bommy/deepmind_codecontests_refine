...
int main() {
  ...
  vector<vector<pair<int, int>>> books(
      4, vector<pair<int, int>>(1, make_pair(0, 0)));
  for (int i = 0; i < n; ++i) {
    int a, b;
    cin >> t[i] >> a >> b;
  ...
  iota(perm, perm + n, 0);
  sort(perm, perm + n, [&](int a, int b) { return t[a] < t[b]; });
  for (int i = 0; i < n; ++i) {
    ndx[perm[i]] = i;
    upd(i, 1);
  }
  ...
  for (int i = 0; i < int(books[3].size()); ++i) {
    ...
    if (rem >= 0 && k - i >= 0 &&
      ...
      int res = ft_sum.qry(ndx) + books[1][k - i].first +
                books[2][k - i].first + books[3][i].first;
      if (res < ans) {
        ans = res;
        best = i;
      }
