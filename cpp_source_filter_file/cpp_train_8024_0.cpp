...
struct IT {
  ...
  TNode merge(TNode l, TNode r) {
    if (l.l.empty()) return r;
    if (r.r.empty()) return l;
    TNode tmp;
    ...
    for (int il = l.r.size() - 1, ir = 0; il >= 0; l.size = --l.r[il--].first) {
      while (ir < r.l.size() && (l.r[il].second | r.l[ir].second) < X) ++ir;
      if (ir == r.l.size()) break;
      tmp.ans +=
          1LL * (l.size - l.r[il].first + 1) * (r.size - r.l[ir].first + 1);
    }
    return tmp;
  }
  IT(int n, int x) : N(n), X(x) {
    ...
    for (int i = 2 * N; --i;)
      node[i] =
          i < N ? merge(node[2 * i], node[2 * i + 1]) : TNode(a[i - N], X);
  }
  void update(int x, int y) {
    node[x + N] = TNode(a[x] = y, X);
    for (x += N; x /= 2;) node[x] = merge(node[2 * x], node[2 * x + 1]);
  }
  int query(int x, int y) {
    TNode l, r;
    for (x += N, y += N; x < y; x /= 2, y /= 2) {
      if (x & 1) l = merge(l, node[x++]);
      if (y & 1) r = merge(node[--y], r);
    }
