...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    if (i < n - 2) f[0] += A * i * C2(n - i - 1);
    if (i > 0 && i < n - 1) f[0] += B * i * i * (n - i - 1);
    if (i > 1) f[0] += C * i * C2(i);
  }
  ...
  idx.resize(n);
  sort(P.begin(), P.end(),
       [&](const int a, const int b) { return G[a].size() < G[b].size(); });
  for (int i = 0; i < n; i++) idx[P[i]] = i;
  for (int i = 0; i < n; i++)
    sort(G[i].begin(), G[i].end(),
         [&](const int a, const int b) { return idx[a] > idx[b]; });
  for (auto x : P) {
    for (auto y : G[x]) {
      if (idx[x] < idx[y]) continue;
      for (auto z : G[y]) {
        if (idx[z] < idx[y]) break;
        if (vis[z]) {
          work(x, y, z);
        }
      }
