...
long long sum[MAX_N];
int cmp(int i, int j) { return suffix_pairs[i] < suffix_pairs[j]; }
void BuildSuffixArray() {
  ...
  for (int step = 0; 1 << (step) <= n; ++step) {
    for (int i = 0; i < n; ++i) {
      suffix_pairs[i] =
          make_pair(suffix[i], suffix[min(i + (1 << step), n - 1)]);
      indexes[i] = i;
    }
    ...
    while (j < n) {
      for (int i = j;
           j < n && suffix_pairs[indexes[i]] == suffix_pairs[indexes[j]]; ++j) {
        suffix[indexes[j]] = i;
      }
    }
  }
