...
int f[N][2][3];
inline int merge(int a, int b, bool c) { return (a < b) ^ c ? b : a; }
int tdfs(int x) {
  ...
  if (s == 1) {
    if (siz[x] & 1) {
      ...
      if (siz[lc[x]] & 1) {
        res = merge(res,
                    merge(dp(lc[x], p, 0),
                          dp(rc[x], !p, siz[lc[x]] == 1 && s == 1 ? 2 : 1), !p),
                    p);
        res = merge(res,
                    merge(dp(rc[x], p, 0),
                          dp(lc[x], !p, siz[rc[x]] == 1 && s == 1 ? 2 : 1), !p),
                    p);
      } else {
        if (siz[rc[x]])
          res =
              merge(res,
                    merge(dp(lc[x], !p, 0),
                          dp(rc[x], p, siz[lc[x]] == 0 && s == 1 ? 2 : 1), !p),
                    p);
        if (siz[lc[x]])
          res =
              merge(res,
                    merge(dp(rc[x], !p, 0),
                          dp(lc[x], p, siz[rc[x]] == 0 && s == 1 ? 2 : 1), !p),
                    p);
      }
    } else
      res = merge(dp(rc[x], p, 0),
                  dp(lc[x], p, siz[rc[x]] == 0 && s == 1 ? 2 : 1), p);
  } else {
    if (siz[x] & 1)
      if (siz[lc[x]] & 1)
