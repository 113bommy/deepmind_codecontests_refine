...
double signum(double x) { return x < -EPS ? -1 : x > EPS; }
struct Segment;
struct Point {
  double x, y;
...
istream &operator>>(istream &in, Segment &s) { return in >> s.a >> s.b; }
double Segment::norm() const { return (a - b).norm(); }
double Point::to(const Segment &s) const {
  if (s.a.cute(s.b, *this) && s.b.cute(s.a, *this)) {
    return det(s.a - *this, s.b - *this) / s.norm();
  }
...
int main() {
  ...
  if (signum((source - target).norm() - chargeTime) <= 0) {
    ...
    for (int k = 0; k < n; ++k) {
      ...
      for (int j = 0; j < n; ++j) {
        if (!visit[j] &&
            signum(getDistance(trench[i], trench[j]) - chargeTime) <= 0) {
