      ...
      for (int vx = 1; vx <= q[x]; ++vx) {
        for (int qh = 1; qh <= q[h]; ++qh) {
          for (int vh = 1; vh <= q[h]; ++vh) {
            ndp[qx + qh][vx] = (ndp[qx + qh][vx] +
                                dp[x][qx][vx] * 1LL * dp[h][qh][vh] % M * vh) %
                               M;
            ndp[qx + qh - 1][vx + vh] = (ndp[qx + qh - 1][vx + vh] +
                                         dp[x][qx][vx] * 1LL * dp[h][qh][vh]) %
                                        M;
          }
        }
      }
