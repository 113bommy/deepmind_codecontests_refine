  ...
  for (int i = 0; i < n; i++) {
    ...
    if (i > 0) {
      for (int d = 0; d < m; d++) {
        ...
        if (dx[d] != 0) {
          kx[d] = std::max((__int128)kx[d], x - dx[d] * get(_x, dx[d]));
          ky[d] = std::max((__int128)ky[d], y - dy[d] * get(_x, dx[d]));
        }
        if (dy[d] != 0) {
          kx[d] = std::max((__int128)kx[d], x - dx[d] * get(_y, dy[d]));
          ky[d] = std::max((__int128)ky[d], y - dy[d] * get(_y, dy[d]));
        }
      }
