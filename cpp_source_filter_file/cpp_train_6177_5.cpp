...
void dfs(int x, int pre, int d) {
  ...
  for (int i = g.fst[x]; i; i = g.nxt[i])
    if (g.y[i] != pre) {
      ...
      dfs(g.y[i], x, d + 1);
      size[x] += size[g.y[i]];
      if (son[x] == -1 || size[g.y[i]] > size[son[x]]) son[x] = g.y[i];
    }
}
int cnp = 0;
void Get_Top(int x, int Top) {
  ...
  for (int i = g.fst[x]; i; i = g.nxt[i]) {
    int y = g.y[i];
    if (y != fa[x][0] && y != son[x]) Get_Top(y, y);
  }
}
int LCA(int x, int y) {
...
void Tupdate(int x, int lca, int i) {
  while (top[x] != top[lca]) {
    ...
    c[p[x]].push_back(-i);
    x = fa[f][0];
    if (e[Min[f][0]].z == e[i].z) ans[Min[f][0]] = 1;
  }
  c[p[lca]].push_back(i);
  c[p[x]].push_back(-i);
}
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = 0; j < c[i].size(); j++)
      if (c[i][j] < 0)
        ...
        tax[e[c[i][j]].z]++;
    if (tax[e[Min[ap[i + 1]][0]].z] > 0) ans[Min[ap[i + 1]][0]] = 1;
  }
  for (int i = 1; i <= m; i++) rans[e[i].id] = ans[i];
  for (int i = 1; i <= m; i++) {
    if (rans[i] == 0) puts("none");
    if (rans[i] == 1) puts("at least one");
    if (rans[i] == 2) puts("any");
  }
  return 0;
}
