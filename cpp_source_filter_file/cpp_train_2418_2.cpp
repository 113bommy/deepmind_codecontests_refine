...
struct seg {
  ...
  void Update(int l, int r, int cur, const node &val) {
    ...
    long long fl1 = 1ll * l * f[cur].val.sum + f[cur].val.ans;
    long long fr1 = 1ll * r * f[cur].val.sum + f[cur].val.ans;
    long long fl2 = 1ll * l * val.sum + val.ans;
    long long fr2 = 1ll * r * val.sum + val.ans;
    if (fl1 >= fl2 && fr1 >= fr2) {
      return;
    }
    if (fl1 <= fl2 && fr1 <= fr2) {
    ...
    int mid = (l + r) >> 1;
    double pos =
        (double)(f[cur].val.ans - val.ans) / (val.sum - f[cur].val.sum);
    if (pos <= mid) {
      if (fl1 >= fl2) {
  ...
  long long Query(int l, int r, int cur, int x) {
    if (l == r) {
      return 1ll * x * f[cur].val.sum + f[cur].val.ans;
    }
    int mid = (l + r) >> 1;
    if (x <= mid)
      return max(Query(l, mid, f[cur].ls, x),
                 1ll * x * f[cur].val.sum + f[cur].val.ans);
    else
      return max(Query(mid + 1, r, f[cur].rs, x),
                 1ll * x * f[cur].val.sum + f[cur].val.ans);
  }
  __inline__ __attribute__((always_inline)) void reset() {
    while (t_pos) {
