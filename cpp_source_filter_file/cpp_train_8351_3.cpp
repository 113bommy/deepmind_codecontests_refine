...
int main() {
  ...
  for (int i = (int)(0); i <= (int)((1 << n) - 1); i++) {
    ...
    for (int j = (int)(1); j <= (int)(n); j++)
      for (int k = (int)(0); k <= (int)(n - 1); k++)
        if (i & (1 << k))
          for (int l = (int)(0); l <= (int)(n - 1); l++)
            if (i & (1 << l))
              if (tr[k][l] == 0) f[j][l] += f[j - 1][k];
    memset(val, 0, sizeof(val));
    for (int j = (int)(0); j <= (int)(n); j++)
      for (int k = (int)(0); k <= (int)(n - 1); k++) val[j] += f[j][k];
    for (int j = (int)(1); j <= (int)(top); j++) {
      long long v = 1;
      for (int k = (int)(1); k <= (int)(n); k++)
        for (int l = (int)(1); l <= (int)(sta[j].a[k]); l++) v *= val[k - 1];
      if ((n - bit[i]) & 1) v = -v;
      g[j] += v;
    }
  }
  ...
  for (int i = (int)(0); i <= (int)((1 << n) - 1); i++) printf("%d ", h[i]);
}
