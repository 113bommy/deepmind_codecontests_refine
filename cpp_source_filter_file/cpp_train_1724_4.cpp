...
struct Complex {
  long long Re, Im;
  Complex(long long _re = 0, long long _im = 0) { Re = _re, Im = _im; }
  ...
  Complex operator*(const Complex &A) const {
    return (Complex){(Re * A.Re + 5 * Im * A.Im) % p,
                     (Re * A.Im + Im * A.Re) % p};
  }
  Complex operator/(const Complex &A) const {
    Complex result = *this * (Complex){A.Re, (p - A.Im) % p};
    return result *
           (Complex){get_inv((A.Re * A.Re % p + p - 5 * A.Im * A.Im % p) % p),
                     0};
  }
...
Complex work(Complex x) {
  if (x == (Complex){1, 0}) return (Complex){r - l + 1, 0};
  return (fast_pow(x, r + 1) + p - 1) / (x + p - 1) -
         (fast_pow(x, l) + p - 1) / (x + p - 1);
}
int main() {
  ...
  for (int i = 0; i <= k; i++)
    for (int j = 0; j <= i; j++) {
      ...
      (ans +=
       (fast_pow(a, j) * fast_pow(b, i - j) * S[k][i] * C[i][j] * work(tmp))
           .Re) %= p;
    }
