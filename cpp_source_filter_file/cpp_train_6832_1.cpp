...
signed main() {
  ...
  for (int i = (1); i <= (n); i++) {
    if (vis[i]) continue;
    if ((fabs(z[i].x - sx) < 1e-6) && (fabs(z[i].y - sy) < 1e-6)) vis[i] = 1;
    for (int j = (i + 1); j <= (n); j++) {
      if (vis[j]) continue;
      if ((fabs(z[i].x + z[j].x - sx * 2) < 1e-6) &&
          (fabs(z[i].y + z[j].y - sy * 2) < 1e-6)) {
        vis[i] = vis[j] = 1;
        break;
      }
    }
  ...
  for (int ci = (1); ci <= (5); ci++) {
    ...
    for (int i = (1); i <= (n); i++) {
      ...
      int pd = 1;
      for (int j = (1); j <= (n); j++) {
        if (fabs(c[j] + c[n - j + 1] - t) > 1e-6) pd = 0;
      }
