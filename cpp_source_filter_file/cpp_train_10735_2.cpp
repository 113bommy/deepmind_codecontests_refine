...
struct SegmentTree_SAM_RQ {
  ...
  int N;
  Data T[2 * MAXN];
  const Data def = make_pair(0, 0);
  const Data qdef = make_pair(0, 0);
  Data merge(Data l, Data r) {
    return make_pair(l.first + r.first, _max(l.second + r.first, r.second));
  }
  Data applyVal(Data l, Data r) {
    return make_pair(l.first + r.first, _max(0LL, l.first + r.first));
  }
  template <class It>
...
const int MAXX = 1e6 + 5, MAXQ = 1e5 + 5;
int Q;
pll temp[MAXX];
ll A[MAXQ], B[MAXQ], V[MAXX];
SegmentTree_SAM_RQ<MAXX + 5, 0> ST;
int main() {
  read(Q);
  fill(temp, temp + MAXX, make_pair(-1, 0));
  ST.init(temp, temp + MAXX);
  char c;
  for (auto i = (0); i < (Q); i++) {
    read(c);
    if (c == '+') {
      read(A[i], B[i]);
      V[A[i]] = B[i];
      ST.update(A[i], make_pair(B[i], B[i]));
    } else if (c == '-') {
      read(A[i]);
      V[A[--A[i]]] = 0;
      ST.update(A[A[i]], make_pair(-B[A[i]], -B[A[i]]));
    } else {
      assert(c == '?');
      read(A[i]);
      writeln(ST.query(0, A[i] - 1).second + V[A[i]]);
    }
  }
  return 0;
}
