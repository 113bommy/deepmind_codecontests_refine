...
class Tree {
 public:
  int n;
  int node[202020], next[202020], to[202020], e;
  ...
  long long int F, f[202020], b1[202020], b2[202020], maxf;
  int q[202020], par[202020], dep[202020];
  void bfs() {
    int i, j, k, l, r;
    q[l = r = 1] = 1;
    while (l <= r) {
  ...
  long long int dsum[202020], siz[202020];
  void getF() {
    int i, j, k;
    long long int D = 0;
    F = 0;
    for (i = n; i; i--) {
      ...
      for (j = node[k]; j; j = next[j])
        if (to[j] != par[k]) {
          F += dsum[to[j]] * siz[k] + dsum[k] * siz[to[j]] -
               dep[k] * 2 * siz[k] * siz[to[j]];
          dsum[k] += dsum[to[j]];
          siz[k] += siz[to[j]];
        }
    }
  ...
  long long int ANS;
  void getbm(long long int n1, long long int n3) {
    int i, j, k;
    long long int tmp;
    for (i = n; i; i--) {
      k = q[i];
      b1[k] = n1 * (f[i] + n3 * dep[i]);
      b2[k] = n3 * (f[i] + n1 * dep[i]);
      for (j = node[k]; j; j = next[j])
        if (to[j] != par[k]) {
...
long long int n1, n2, n3, ANS = 0, tmp;
int main() {
  int x, y;
  gi(n1);
  T1.n = n1;
