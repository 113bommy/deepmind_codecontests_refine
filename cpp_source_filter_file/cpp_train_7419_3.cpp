...
class SegmentTree {
 ...
 public:
  ...
  void computenode(int node, int from, int to) {
    if (from < to) aint[node] = std::min(aint[node * 2], aint[node * 2 + 1]);
  }
  void update(int node, int from, int to, int x, int y, int val) {
    if (from == x && to == y) {
      ...
      if (x <= mid)
        update(node * 2, from, mid, x, (((mid) < (y)) ? (mid) : (y)), val);
      if (mid + 1 <= y)
...
int eval(int groups, int n) {
  ...
  for (int i = 1; i <= groups; i++) aint.update(1, 1, groups, i, i, -i);
  std::vector<std::pair<int, int>> events;
  for (int i = 1; i <= n; i++)
    if (0 < pref[i] || suff[i] <= groups) events.push_back({pref[i], suff[i]});
  if (events.size() < groups) return 0;
  std::vector<std::vector<int>> events2(1 + groups);
  for (int i = 0; i < events.size(); i++)
    events2[events[i].first].push_back(events[i].second);
  int bonus = events2.size();
  for (int i = 1; i <= groups; i++) {
    ...
    if (aint.query(1, 1, groups, 1, groups) + bonus < 0) return 0;
...
int main() {
  ...
  int t;
  std::cin >> t;
  for (int testcase = 1; testcase <= t; testcase++) solve();
  return 0;
}
