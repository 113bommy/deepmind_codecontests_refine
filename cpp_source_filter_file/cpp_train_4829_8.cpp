...
void push_up(int t) { num[t] = num[(t << 1)] + num[((t << 1) | 1)]; }
void push_down(int t, int l, int r) {
  if (lazy[t]) {
    lazy[t] ^= 1;
    lazy[(t << 1)] ^= 1, lazy[((t << 1) | 1)] ^= 1;
    int mid = (l + r) / 2;
    num[(t << 1)] = mid - l + 1 - num[(t << 1)],
              num[((t << 1) | 1)] = r - mid - num[((t << 1) | 1)];
  }
}
...
int query(int L, int R, int l, int r, int t) {
  ...
  push_down(t, l, r);
  int mid = (l + r) / 2, ans = 0;
  if (L <= mid) ans += query(L, R, l, mid, (t << 1));
  if (R > mid) ans += query(L, R, mid + 1, r, ((t << 1) | 1));
  return ans;
}
int n, k, s[maxn];
