...
int n, m, k, rowse[1005][1005] = {}, colss[1005][1005] = {},
             rowsw[1005][1005] = {}, colsn[1005][1005] = {};
...
bool dfs(int x, int y, int cur) {
  ...
  int dx = v[cur].second;
  if (v[cur].first == 'N') {
    if (!ongrid(x - dx, y, 1, x, y)) return 0;
    return dfs(x - dx, y, cur + 1);
  } else if (v[cur].first == 'S') {
    if (!ongrid(x + dx, y, 2, x, y)) return 0;
    return dfs(x + dx, y, cur + 1);
  } else if (v[cur].first == 'W') {
    if (!ongrid(x, y - dx, 3, x, y)) return 0;
    return dfs(x, y - dx, cur + 1);
  } else {
    if (!ongrid(x, y + dx, 4, x, y)) return 0;
    return dfs(x, y + dx, cur + 1);
  }
}
int main() {
  ...
  string s;
  for (int i = 0; i <= 26; ++i) {
    if (ch[i].first == ch[i].second && ch[i].first == -1) continue;
    if (dfs(ch[i].first, ch[i].second, 0)) s.push_back((char)(i + 'A'));
  }
  if (s.empty()) return cout << "no solution\n", 0;
  cout << s << "\n";
  return 0;
}
