...
vector<int> L, R;
int cntL[N], cntR[N];
double cal(int a, int b, int x) { return (double)(b - a) / x; }
bool the_same(double x, double y) { return fabs(x - y) < eps; }
...
int main() {
  ...
  sort(d.begin(), d.end(),
       [](pair<double, pair<int, int> > a, pair<double, pair<int, int> > b) {
