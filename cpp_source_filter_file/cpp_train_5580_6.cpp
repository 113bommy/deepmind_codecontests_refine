...
bool bfs() {
  ...
  while (!q.empty()) {
    ...
    for (int i : g[u]) {
      int v = edgs[i].v;
      if (!h[v] and edgs[i].f < edgs[i].c) q.push(v), h[v] = h[u] + 1;
    }
  }
  return h[snk];
}
...
long long test(long long lvl) {
  ...
  for (int i = 1; i <= n; i++) {
    if (c[i] == 1) {
      ...
      if (c[i] % 2) {
        if (l[i] <= lvl) odd.push_back(i), sum += p[i];
      } else {
        if (l[i] <= lvl) even.push_back(i), sum += p[i];
      }
    }
  }
  if (one) odd.push_back(one);
