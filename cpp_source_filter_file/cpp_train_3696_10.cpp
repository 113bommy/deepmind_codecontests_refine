...
void apply(int idx, int id) {
  ...
  ragetree[idx][id] = tot;
  if (idx < RAGETREE_SZ) {
    ragetreelazy[2 * idx] = idx;
    ragetreelazy[2 * idx + 1] = idx;
  }
}
...
void pullup(int idx) {
  for (int i = 0; i < 4; i++)
    ragetree[idx][i] = ragetree[2 * idx][i] + ragetree[2 * idx + 1][i];
}
pair<int, int> qry(int idx, int pos, int sz, int lhs, int rhs) {
  ...
  pushdown(idx);
  pair<int, int> ans1 = qry(2 * idx, pos, sz / 2, lhs, rhs);
  pair<int, int> ans2 = qry(2 * idx + 1, pos + sz / 2, sz / 2, lhs, rhs);
  pullup(idx);
...
void upd(int lhs, int rhs, int id) { upd(1, 0, RAGETREE_SZ, lhs, rhs, id); }
...
void init(vector<int>& v) { init(1, 0, RAGETREE_SZ, v); }
int qryi[200000];
int gasx[200001];
int gast[200001];
void solve() {
  ...
  {
    vector<int> dists;
    for (int i = 1; i < (int)(interesting).size(); i++)
      dists.push_back(interesting[i] - interesting[i - 1]);
    init(dists);
  }
  for (int x = 1; x <= 3; x++) {
    for (int i = 0; i < n; i++) {
      if (gast[i] != x) continue;
      int lhs = lower_bound(interesting.begin(), interesting.end(), gasx[i]) -
                interesting.begin();
      int rhs =
          lower_bound(interesting.begin(), interesting.end(), gasx[i] + s) -
          interesting.begin() - 1;
      upd(lhs, rhs, x);
    }
  }
  for (int i = 0; i < m; i++) {
    int lhs = lower_bound(interesting.begin(), interesting.end(), qryi[i] + s) -
              interesting.begin();
    int rhs = lower_bound(interesting.begin(), interesting.end(), e) -
              interesting.begin() - 1;
    pair<int, int> ret = qry(lhs, rhs);
