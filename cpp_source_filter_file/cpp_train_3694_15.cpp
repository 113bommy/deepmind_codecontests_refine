...
inline void mul(int f[M], int g[M][M]) {
  ...
  memset(a, 0, sizeof a);
  for (int j = 1; j <= k; j++)
    for (int i = 1; i <= k; i++) a[i] = add(a[i], mul(f[j], g[j][i]));
...
int H(int t, int l, int r) {
  if (t < 0) return 0;
  if (l == 1 && r == m) return (t == 0);
  if (v[t][l][r]) return h[t][l][r];
  v[t][l][r] = 1;
  if (l != 1 && s[l - 1] != s[r])
...
int main() {
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 1; j <= m; j++) {
      c = add(c, H(i, j, j));
      if (j != m && s[j] == s[j + 1]) c = add(c, H(i, j, j + 1));
    }
    if (i) {
  ...
  for (int i = 0; i < m; i++) {
    int c = 0;
    for (int j = 1; j < m; j++)
      if (s[j] == s[j + 1]) c = add(c, H(i, j, j + 1));
    if (i)
      g[i][k - ceil(m - i)] = c;
    else
      f[m] = c, g[k][k] = 0;
