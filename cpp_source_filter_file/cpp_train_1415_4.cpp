...
inline void pushup(int p) {
  ...
  ab[p] =
      ((ab[ls[p]] + ab[rs[p]]) % mod + 1ll * a[ls[p]] * b[rs[p]] % mod) % mod;
  bc[p] =
      ((bc[ls[p]] + bc[rs[p]]) % mod + 1ll * b[ls[p]] * c[rs[p]] % mod) % mod;
  abc[p] =
      (((abc[ls[p]] + abc[rs[p]]) % mod + 1ll * ab[ls[p]] * c[rs[p]] % mod) +
       1ll * a[ls[p]] * bc[rs[p]]) %
      mod;
}
...
int main() {
  rd(n);
  for (int i = 1, I = (n) + 1; i < I; ++i) rd(A[i]), B[i] = A[i];
  sort(B + 1, B + n + 1);
  ...
  for (int i = n, I = (1) - 1; i > I; --i) {
    suf[i] = bit.query(A[i]);
    bit.add(A[i], 1);
    modify(rt[A[i]], 1, n, i, 1);
  }
  int ans = 0;
  for (int i = 1, I = (cnt) + 1; i < I; ++i) ans = (ans + abc[rt[i]]) % mod;
  for (rd(q); q; --q) {
    rd(t), rd(p);
