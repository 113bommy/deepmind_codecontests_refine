...
inline void Init(int N) {
  khs[0] = pw2[0] = 1;
  for (int i = 1; i <= N; ++i) khs[i] = (long long)khs[i - 1] * HB % HM;
...
inline void upd(int i, int s) {
  hs[i] = ((long long)khs[s] * hs[rs[i]] + hs[ls[i]]) % HM;
}
int Calc(int i, int l = 0, int r = BB) {
  ...
  if (!~L || !~R) return -1;
  return (L + (long long)pw2[mid - l + 1] * R) % Mod;
}
...
bool Add(int &i, int p, int l = 0, int r = BB) {
  if (p == r && hs[i] == mhs[r - l + 1]) return i = 0, 1;
