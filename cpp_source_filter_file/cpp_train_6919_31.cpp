...
int main() {
  ...
  sort(p + 1, p + o + 1, [](P a, P b) { return a.x < b.x; }),
      sort(w + 1, w + o + 1), o = unique(w + 1, w + o + 1) - w - 1;
  for (i = 1; i <= m * 2; ++i) {
    ans ^= (p[i].x ^ p[i - 1].x) & t[1];
    a = lower_bound(w + 1, w + o + 1, p[i].l) - w,
    b = lower_bound(w + 1, w + o + 1, p[i].r) - w;
