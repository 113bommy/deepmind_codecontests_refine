...
int main() {
  ...
  for (int i = 0; i < (int)(div.size()); i++) {
    long long first = div[i];
    if (first > sq3) break;
    long long m = n / first;
    long long sq2 = sqrt(m + 1);
    for (int j = (i); j <= (int)((int)div.size() - 1); j++) {
      long long second = div[j];
      if (second > sq2) break;
      if (m % second == 0) {
        long long z = m / second;
        if ((first + second + z) & 1ll) continue;
        if (z > first + second) continue;
        if (first == z)
          ans += 1;
        else {
          if (second == z || first == second)
            ans += 3;
          else
            ans += 6;
        }
