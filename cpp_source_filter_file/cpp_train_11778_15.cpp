...
void Input() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int x = 0; x < (1LL << k); x++)
      ST.Update(x, 1, 0, n - 1, i, value(Point, x));
  }
}
void Solve() {
  cin >> q;
  while (q--) {
    ...
    if (t == 1) {
      ...
      for (int x = 0; x < (1LL << k); x++)
        ST.Update(x, 1, 0, n - 1, id, value(Point, x));
    } else if (t == 2) {
      int l, r;
      cin >> l >> r;
      l--;
      r--;
      int answer = 0;
      for (int x = 0; x < (1LL << (k - 1)); x++) {
        int Min = ST.GetMax((1LL << k) - 1 - x, 1, 0, n - 1, l, r);
        int Max = ST.GetMax(x, 1, 0, n - 1, l, r);
        answer = max(answer, (int)abs(Max - Min));
      }
      cout << answer << "\n";
    }
  }
