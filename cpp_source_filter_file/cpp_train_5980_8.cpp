...
int segs[max_n];
pair<int, int> get_normal_coords(int x) { return make_pair(x / n, x % n); }
...
int smallest_path(pair<int, int> v1, pair<int, int> v2) {
  ...
  int res[2][2];
  pair<int, int> temp =
      bf_smallest_path_to_pair(v1.first, v1.second, sqr_pos[s1 + 1]);
  res[0][0] = temp.first;
  res[0][1] = temp.second;
  ...
  for (int id = 0; id + 1 < (int)sqr_pos.size(); ++id)
    for (int i1 = 0; i1 < 2; ++i1)
      for (int i2 = 0; i2 < 2; ++i2) {
        dist_to_next[id][i1][i2] =
            bf_smallest_path(i1, sqr_pos[id], i2, sqr_pos[id + 1]);
      }
  for (int s = 0; s + 1 < (int)sqr_pos.size(); ++s) {
    for (int i = 0; i < 2; ++i)
      for (int j = 0; j < 2; ++j)
        dp[s][i][s][j] = bf_smallest_path(i, sqr_pos[s], j, sqr_pos[s]);
    for (int t = s + 1; t < (int)sqr_pos.size(); ++t) {
      for (int i = 0; i < 2; ++i)
        for (int j = 0; j < 2; ++j) dp[s][i][t][j] = -1;
      for (int i = 0; i < 2; ++i)
        for (int j = 0; j < 2; ++j)
          for (int k = 0; k < 2; ++k) {
