...
int n;
pair<long long, long long> si[maxn], ti[maxn];
map<int, int> S, T;
vector<data> Q;
void gans(int i, int j, int d) { Q.push_back(data(i, j, d)); }
int main() {
  scanf("%d", &n);
  ...
  for (int i = n; i >= 1; --i) {
    if (si[i].first >= ti[i].first) {
      ...
      v.push_back(
          pair<long long, long long>(si[i].first - ti[i].first, si[i].second));
    } else {
      ...
      sum += si[i].first - ti[i].first;
      long long rest = ti[i].first - si[i].first;
      while (v.size() && rest) {
        auto& a = v.back();
        if (a.first > rest) {
          a.first -= rest;
