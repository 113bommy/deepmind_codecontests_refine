...
bool srt(pair<int, int> a, pair<int, int> b) { return a > b; }
int main() {
  getline(cin, a);
  getline(cin, b);
  if (a.size() != b.size()) {
  ...
  for (int i = 0; i < b.size(); i++) {
    niz[i] = i + 1 + z1[i + 1];
    vect.push_back({niz[i], i});
  }
  vect.push_back({z1[0], -1});
  sort(vect.begin(), vect.end(), srt);
  minn[0] = vect[0].second;
  for (int i = 1; i < vect.size(); i++) {
    minn[i] = min(minn[i - 1], vect[i].second);
  }
