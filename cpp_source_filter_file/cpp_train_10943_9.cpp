...
const int maxn = 2e3 + 10, maxw = 1e6 + 100, inf = 2e9 + 100, sq = 600, X = 43,
          mod = 1e9 + 7, LG = 17;
int n;
int vsdp[maxn];
int fix[maxn];
...
int main() {
  ...
  vsdp[2] = 1;
  for (int i = 3; i <= n; i++)
    vsdp[i] = mul(i - 1, dif(vsdp[i - 1], vsdp[i - 2]));
  fix[0] = 1;
  f[0] = 1;
  for (int i = 1; i <= n; i++)
    fix[i] = mul(fix[i - 1], vsdp[n]), f[i] = mul(f[i - 1], i);
  d[0][0] = 1;
  for (int s = 1; s <= n; s++)
    for (int x = 0; x <= s; x++) {
      int y = s - x;
      if (y == 0)
        ...
        d[s][x] =
            ad(mul(x, d[s - 1][x]), (y > 1 ? mul(y - 1, d[s - 1][x + 1]) : 0));
    }
  int ans = 0;
  for (int its = 0; its < n; its++) {
    ...
    a.init();
    b.init();
    for (int i = 0; i < n; i++) em[i] = 1, vis[i] = 1;
    int cnt = n;
    int W = 0;
    for (int i = 0; i < n; i++) {
      int now;
      cin >> now;
      now--;
      if (em[gr[i]]) a.inc(gr[i], -1), em[gr[i]] = 0, cnt--;
      b.inc(now, -1);
      vis[now] = 0;
      int all = b.get(now - 1), to = a.get(now - 1);
      if (vis[gr[i]] && gr[i] < now) all--;
      W = ad(W, ad(mul(to, d[n - 1 - i][n - 1 - i - (cnt - 1)]),
                   mul(all - to, d[n - 1 - i][n - 1 - i - cnt])));
      if (em[now]) a.inc(now, -1), em[now] = 0, cnt--;
      gr[i] = now;
    }
    ans = ad(ans, mul(W, fix[n - 1 - its]));
  }
