...
struct node {
  ...
  int sz;
  long double sum;
  node(long double a, int priority)
      : sz(1), a(a), priority(priority), sum(a), left(0), right(0){};
};
void upd(node*& v) {
  v->sz = 1;
  v->sum = v->a;
...
void split(node* v, node*& l, node*& r, long double a) {
  if (!v) {
    l = r = NULL;
    return;
  }
...
void add(node*& root, long double val) {
  node* L;
  node* R;
  split(root, L, R, val);
...
void erase(node*& root, long double val) {
  node* L;
  node* R;
  split(root, L, R, val);
  node* L1;
...
signed main() {
  int n, a, b;
  cin >> n >> a >> b;
  vector<long double> p(n), u(n);
  vector<pair<long double, int> > mass;
  for (int i = 0; i < n; i++) {
    cin >> p[i];
  }
  for (int i = 0; i < n; i++) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    tree1 = NULL;
    tree = NULL;
    vector<pair<long double, int> > mass1;
    for (int j = 0; j <= i; j++) {
      add(tree1, u[mass[j].second] - p[mass[j].second]);
      mass1.push_back(
          {(1 - p[mass[j].second]) * u[mass[j].second], mass[j].second});
    }
    for (int j = i + 1; j < n; j++) {
