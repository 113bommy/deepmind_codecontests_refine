...
struct problem {
  int a0, a1, t0, t1;
  double p;
  void init() { scanf("%d %d %d %d %lf", &a0, &a1, &t0, &t1, &p); }
  cp join0(cp o) { return (cp){o.s + a0, o.t + t0}; }
  cp join1(cp o, int T) {
    return (cp){o.s + a0 + a1 * (1.0 - p),
                p * (t0 + o.t) + (1.0 - p) * (T + t0 + t1)};
  }
} pro[N];
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    for (int x = T; x >= 0; --x) {
      if (x + t0 <= T) update(dp[x + t0], pro[i].join0(dp[x]));
      if (x + t1 <= T) update(dp[x + t1], pro[i].join1(dp[x], x));
    }
  }
  for (int x = 1; x <= T; ++x) {
    update(dp[x], dp[x - 1]);
