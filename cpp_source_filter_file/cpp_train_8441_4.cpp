...
long long vec(point a, point b) { return 1LL * a.x * b.y - 1LL * a.y * b.x; }
int n;
point p[MAXN];
int semiplane(const point &a) { return (a.y > 0 || (a.y == 0 && a.x < 0)); }
...
int get(int l, int r) {
  ...
  return sum[(int)(st).size() - 1] - (l ? sum[l - 1] : 0) + sum[r];
}
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < n; j++) {
      if (i != j) {
        st.push_back({p[j] - p[i], j});
        st.push_back({p[i] - p[j], n + j});
      }
    }
    ...
    for (int j = 0; j < (int)(st).size(); j++) {
      if (st[j].second < n) {
        int l = (j + 1) % (int)(st).size();
        int r = pos[st[j].second];
        int f = get(l, r);
        int f1 = get((r + 1) % (int)(st).size(),
                     (j - 1 + (int)(st).size()) % (int)(st).size());
