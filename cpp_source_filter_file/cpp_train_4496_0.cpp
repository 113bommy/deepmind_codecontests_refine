...
std::pair<int, int> t[N << 2], f[N];
bool check(int u, int k, int delta) {
  int r = DS::rank(DS::rt[u], k), b = d[u] - (u != 1) - r;
  return b + delta - r < k;
}
...
inline std::pair<int, int> operator+(const std::pair<int, int> &x,
                                     const std::pair<int, int> &y) {
  return std::pair<int, int>(std::min(x.first, std::max(y.first, x.second)),
                             std::min(x.first, std::max(y.second, x.second)));
}
void maintain(int u) {
  update(u, f[u].first, 1);
  update(u, f[u].second, -1);
}
void modify(int p, int l, int r, int pos, std::pair<int, int> val) {
  if (l == r) {
    t[p] = val;
    return;
  }
...
std::pair<int, int> query(int p, int l, int r, int ll, int rr) {
  if (l >= ll && r <= rr) return t[p];
  int mid = l + r >> 1;
  if (mid >= ll && mid < rr)
    return query(p << 1, l, mid, ll, rr) +
           query(p << 1 | 1, mid + 1, r, ll, rr);
...
void build(int p, int l, int r) {
  if (l == r) {
    ...
    if (~c[u])
      ...
      for (auto v : g[u])
        if (v != fa[u] && v != son[u]) DS::insert(DS::rt[u], query(v));
      maintain(u), t[p] = f[u];
    }
    return;
  }
...
void modify(int u, int x) {
  if (c[u] == x) return;
  c[u] = x,
  f[u] = !x ? std::pair<int, int>(-inf, -inf) : std::pair<int, int>(inf, inf);
  for (; u; u = fa[top[u]]) {
...
int main() {
  ...
  for (int i = 1, u, v; i < n; ++i)
    scanf("%d%d", &u, &v), ++d[u], ++d[v], g[u].emplace_back(v),
        g[v].emplace_back(u);
  dfs1(dep[1] = 1), dfs2(top[1] = 1);
  for (int i = 1; i <= n; ++i)
    if (scanf("%d", &c[i]), ~c[i])
      f[i] = !c[i] ? std::pair<int, int>(-inf, -inf)
                   : std::pair<int, int>(inf, inf);
  build(1, 1, n);
  int q;
  scanf("%d", &q);
