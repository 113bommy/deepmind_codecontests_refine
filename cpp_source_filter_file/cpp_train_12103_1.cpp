...
inline mat operator*(const mat& A, const mat& B) {
  static mat C;
  int i, j, k;
  memset(C.a, 120, sizeof C.a);
  for (i = 0; i < 2; ++i)
    for (j = 0; j < 2; ++j)
      for (k = 0; k < 2; ++k) upi(C.a[i][k], A.a[i][j] + B.a[j][k]);
  return C;
}
struct segtree {
  struct node {
  ...
  inline void rev(int i, int o) {
    for (j = 0; j < 2; ++j)
      if (o == 0)
        swap(t[i].p[0][j], t[i].p[1][j]);
      else
        swap(t[i].p[j][0], t[i].p[j][1]);
    swap(t[i].s[o][0], t[i].s[o][1]);
  }
  inline void upd(int i) {
    for (j = 0; j < 2; ++j)
      for (k = 0; k < 2; ++k)
        t[i].s[j][k] = t[i << 1].s[j][k] + t[i << 1 | 1].s[j][k],
  ...
  void build(int i, int l, int r, int z) {
    ...
    if (l == r) {
      for (j = 0; j < 2; ++j)
        for (k = 0; k < 2; ++k) t[i].s[j][k] = (c[O + j][l] == 'B') == k;
      for (j = 0; j < 2; ++j)
        for (k = 0; k < 2; ++k) {
    ...
    build(i << 1, l, t[i].m, z);
    build(i << 1 | 1, t[i].m + 1, r, z);
    upd(i);
  }
  void mdy(int i, int l, int r, int o) {
    if (l <= t[i].l && t[i].r <= r)
      return (o == 0 ? t[i].r0 : t[i].r1) ^= 1, rev(i, o);
    if (l <= t[i].m) mdy(i << 1, l, r, o);
    if (t[i].m < r) mdy(i << 1 | 1, l, r, o);
    upd(i);
  }
  void mdy2(int i, int k, int dt) {
    if (k == t[i].l && k == t[i].r) {
      for (j = 0; j < 2; ++j)
        for (k = 0; k < 2; ++k)
          for (x = 0; x < 2; ++x)
            for (y = 0; y < 2; ++y) t[i].p[j][k].a[x][y] += y == 0 ? dt : -dt;
      return;
    }
    mdy2(i << 1 | (k > t[i].m), k, dt);
    upd(i);
  }
} t[2];
inline void out() {
  ...
  for (i = 0; i < 2; ++i)
    for (j = 0; j < 2; ++j)
      for (k = 0; k < 2; ++k) upi(ans, t[i].t[1].p[0][0].a[j][k]);
  printf("%d\n", ans);
}
int main() {
  scanf("%d%d%d", &n, &m, &q);
  ...
  for (; q--;) {
    ...
    if (*op == 'L') {
      ...
      t[0].mdy(1, x, y, 0);
      t[1].mdy2(1, 1, t[0].t[1].s[0][0] - ov);
    }
    if (*op == 'R') {
      int ov = t[0].t[1].s[1][0];
      t[0].mdy(1, x, y, 1);
      t[1].mdy2(1, n, t[0].t[1].s[1][0] - ov);
    }
    if (*op == 'U') {
      int ov = t[1].t[1].s[0][0];
      t[1].mdy(1, x, y, 0);
      t[0].mdy2(1, 1, t[1].t[1].s[0][0] - ov);
    }
    if (*op == 'D') {
      int ov = t[1].t[1].s[1][0];
      t[1].mdy(1, x, y, 1);
      t[0].mdy2(1, n, t[1].t[1].s[1][0] - ov);
    }
    out();
  }
}
