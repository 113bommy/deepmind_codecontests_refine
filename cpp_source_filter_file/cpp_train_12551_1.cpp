          ...
          for (int ql = 0; ql <= k + 2; ql++)
            for (int qr = 0; qr <= k + 2; qr++) {
              ...
              if (l_ok && r_ok)
                ...
                nq = min(k + 1, max(ql, qr + 1));
              np = min(pl, pr + 1);
              w[np][nq] =
                  (w[np][nq] + (long long)f[x][pl][ql] * f[to[i]][pr][qr]) %
                  MOD;
            }
      for (int p = 0; p <= k + 1; p++)
        for (int q = 0; q <= k + 2; q++) f[x][p][q] = w[p][q], w[p][q] = 0;
    }
}
int main() {
  scanf("%d%d", &n, &k);
  ...
  DFS(1, -1);
  for (int i = 0; i <= k; i++) ans = (ans + f[1][i][k + 2]) % MOD;
  printf("%d\n", ans);
  return 0;
}
