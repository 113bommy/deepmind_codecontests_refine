...
struct Segment_Tree {
  ...
  inline void build(vector<int> &val) {
    n = (int)val.size();
    build(1, 0, n - 1, val);
  }
  void update(int u, int l, int r, int ql, int qr, int k) {
    ...
    if (mid < qr) update(((u) << 1 | 1), mid + 1, r, ql, qr, k);
    push_up(u);
  }
...
void solve(void) {
  ...
  vector<int> rs(m);
  for (int i = 0; i < m; ++i) rs[i] = *(posz.end() - m + i);
  ...
  vector<int> val(m);
  for (int i = 0; i < m; ++i) val[i] = cnt_clr[rs[i]] - (i + 1);
  ...
  for (int i = beg + 1, ii = 1; i < m; ++i) {
    for (int j = posz[ii - 1] + 1; j < posz[ii]; ++j)
      if (a[j]) {
