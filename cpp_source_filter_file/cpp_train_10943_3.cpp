...
long long dp[mxN + 1][mxN + 1], ans, p1 = 1;
bool v1[mxN], v2[mxN];
void upd(int i, int j, int x) {
  for (++j; j <= n; j += j & -j) ft[i][j] += x;
}
int qry(int i, int j) {
  int r = 0;
  for (; j; j -= j & -j) r += ft[i][j];
  return r;
}
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    for (int j = 1; j <= n; ++j)
      dp[i][j] = (dp[i - 1][j - 1] * i + dp[i - 1][j] * (n - i)) % M;
  }
  for (int i = n - 1; i; --i, p1 = p1 * dp[n][0] % M) {
    memset(v1, 0, n);
    memset(v2, 0, n);
    for (int j = 0; j < n; ++j) upd(0, j, 1);
    for (int j = 0, um = 1; j < n; ++j, ++um) {
      ...
      if (!v2[a[i - 1][j]])
        ...
        --um;
      ans += (qry(0, a[i][j]) * dp[n - j - 1][um] +
              qry(1, a[i][j]) * dp[n - j - 1][um - 1]) %
             M * p1 % M;
      if (!v2[a[i - 1][j]]) upd(1, a[i - 1][j], 1);
      um -= v1[a[i][j]];
      v2[a[i][j]] = 1;
