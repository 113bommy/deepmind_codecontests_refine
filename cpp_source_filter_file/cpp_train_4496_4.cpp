...
void Add(int x, int y) {
  if (y) C[x].Add(path[y].first);
}
void Rem(int x, int y) {
  if (y) C[x].Rem(path[y].first);
}
void splay(int x) {
  while (pd(x) != -1) {
    if (pd(fa[x]) != -1) rot(pd(fa[x]) == pd(x) ? fa[x] : x);
    rot(x);
  }
}
void access(int x) {
...
int main() {
  ...
  for (int i = 1, u, v; i < n; i++)
    scanf("%i %i", &u, &v), E[u].push_back(v), E[v].push_back(u);
  for (int i = 1; i <= n; i++) scanf("%i", &st[i]);
  DFS(1, 0);
  for (int i = 2; i <= n; i++)
    if (lf[i]) C[par[i]].Add(st[i] == 1 ? inf : -inf);
