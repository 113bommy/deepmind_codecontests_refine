...
long long C(int x, int y) {
  if (x < y) return 1;
  return p[x] * rev[y] % 1000000009 * rev[x - y] % 1000000009;
}
...
int main() {
  ...
  p[0] = 1;
  ++n;
  for (int i = 1; i <= n; ++i) p[i] = p[i - 1] * i % 1000000009;
  rev[0] = 1;
  for (int i = 1; i <= n; ++i) rev[i] = pwr(p[i], 1000000009 - 2);
  if (a[1].first == 1)
    Fate(2, a[1].second);
  else
