#include <bits/stdc++.h>
using namespace std;
int n, m, p, l, r, cnt[600000], lg[600000], sum[600000], pre[600000][30],
    f[600000], g[600000];
bool cov[600000 << 5];
char s[600000];
struct Hash {
  const long long B1 = 233, mod1 = 998244353, B2 = 377, mod2 = 1000000007;
  long long h1[600000], pw1[600000], h2[600000], pw2[600000];
  Hash() {
  ...
  long long qry(int l, int r) {
    long long t1 = (h1[r] - h1[l - 1] * pw1[r - l + 1] % mod1 + mod1) % mod1;
    long long t2 = (h2[r] - h2[l - 1] * pw2[r - l + 1] % mod2 + mod2) % mod2;
    return t1 << 31 | t2;
  }
} H;
struct SA {
  int sa[600000], sa2[600000], x[600000], y[600000], rk[600000],
      lcp[22][600000];
  void resort(int m) {
...
namespace p1 {
bool check(int l, int r, int i) { return H.qry(l, r - i) == H.qry(l + i, r); }
bool ask(int l, int r) {
  int len = r - l + 1;
...
int qmin(int x, int l, int r, int L, int R) {
  ...
  if (L <= mid) ret = min(ret, qmin((x << 1), l, mid, L, R));
  if (R > mid) ret = min(ret, qmin((x << 1 | 1), mid + 1, r, L, R));
  return ret;
}
bool ask(int l, int r) {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    for (int j = i; j + i <= n; j += i)
      if (j > r) {
        ...
        for (int k = i * 2; l + k * 2 - l <= r + i; k += i)
          cov[sum[k - 1] + (l + k - 1) / k] = 1;
...
int main() {
  ...
  reverse(s + 1, s + n + 1);
  for (int i = 1; i <= n; ++i)
    for (int j = 0; j < 26; ++j) pre[i][j] = pre[i - 1][j] + (s[i] - 'a' == j);
  getsqr();
  while (m--) {
    scanf("%d%d", &l, &r);
    if (p0::ask(l, r))
