...
struct point_3d {
  ...
  point_3d operator/(double f) { return point_3d(x / f, y / f, z / f); }
...
point_3d project(double A, double B, double C, double x1, double y1,
                 double z1) {
  double lambda = (A * x1 + B * y1 + C * z1) / (A * A + B * B + C * C);
  return point_3d(x1 - lambda * A, y1 - lambda * B, z1 - lambda * C);
}
void calc2(double a, double b, double c, double d, double e, double f,
           double &x, double &y) {
...
point_3d cir_cen(point_3d &a, point_3d &b, point_3d &c, double A3, double B3,
                 double C3) {
