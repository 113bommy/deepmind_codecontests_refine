...
void compress(int u, int to) {
  ...
  at[u] = to;
  for (auto i : g[u])
    if (!vis[i.first] && !is[i.second]) compress(i.first, to);
}
void bfs() {
  queue<int> q;
  vector<int> dis;
...
void solve(int u, int rev) {
  ...
  for (int turn = 0; turn < 2; turn++) {
    ...
    for (int i = 0; i < 2 * sz; i++) {
      ...
      while (q.size() && q.front().second + i > sz / 2) q.pop_front();
      if (q.size()) Z[id] = max(Z[id], q.front().first + i);
      while (q.size() && val > q.back().first) q.pop_front();
      q.push_back({val, -i});
    }
