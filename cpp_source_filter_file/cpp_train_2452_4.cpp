...
int n, m, S, T, lnk[maxn], val[maxm], tot, path[maxn], idx, ans, out[3];
int que[maxn], pre[maxn], L, R;
...
int tarjan(int u, int Fa) {
  ...
  for (int it = lnk[u]; it != -1; it = e[it].nxt) {
    if (Fa == (it ^ 1) || (it >> 1) == idx) continue;
    int &v = e[it].v;
    if (pre[v] == -1) {
      int lowv = tarjan(v, it);
      if (lowu > lowv) lowu = lowv;
      if (pre[u] < lowv) bri[it >> 1] = 1;
    } else if (lowu > pre[v])
      lowu = pre[v];
  }
  return lowu;
...
void dfs(int u) {
  bno[u] = bcnt;
  for (int it = lnk[u]; it != -1; it = e[it].nxt)
    if (!bri[it >> 1] && bno[e[it].v] == -1 && (it >> 1) != idx) dfs(e[it].v);
}
int main() {
  ...
  for (int o = R - 1; o != -1; o = pre[o]) path[tot++] = que[o] >> 1;
  ans = ~0u >> 1;
  for (int o = 0; o < tot; ++o) {
    idx = path[o];
    tot = 0;
  ...
  printf("%d\n%d\n", ans, out[0]);
  for (int i = 1; i <= out[0]; ++i) printf("%d%c", out[i], " \n"[i == out[0]]);
  return 0;
}
