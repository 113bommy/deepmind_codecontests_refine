...
int n, m, i, j, k, u, v, head[N], Next[N * 2], adj[N * 2], d[N], df[N], fa[N],
    a[N], b[N];
    ...
    if (adj[j] != fa[i])
      if (adj[j] != mxx)
        ...
        for (int k = head[i]; k != 0; k = Next[k])
          if (adj[k] != mxx && adj[k] != fa[i]) m = max(m, d[adj[k]]);
        df[mxx] = m + 1;
      }
  for (int j = head[i]; j != 0; j = Next[j])
    if (adj[j] != fa[i]) fdfs(adj[j]);
}
...
int main() {
  ...
  for (i = 1; i <= n; ++i) {
    ...
    for (j = head[i]; j != 0; j = Next[j])
      if (adj[j] != fa[i]) a[++k] = d[adj[j]];
    if (df[i]) a[++k] = df[i];
    for (j = 1; j <= k + 1; ++j) tmp[j] = 0;
    sort(a + 1, a + 1 + k, cmp);
    for (j = head[i]; j != 0; j = Next[adj[j]])
      if (adj[j] != fa[i]) {
        int t = 0;
        for (int m = head[adj[j]]; m != 0; m = Next[m])
          if (adj[m] != fa[adj[j]]) b[++t] = d[adj[m]];
        sort(b + 1, b + 1 + t, cmp);
        for (int m = 1; m <= t; ++m) {
  ...
  ++ans[1];
  for (i = 1; i <= n; ++i) printf("%d ", max(ans[i], 1));
}
