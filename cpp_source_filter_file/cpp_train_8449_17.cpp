...
int main() {
  ...
  for (int i = 1; i <= m; i++)
    scanf("%d%d%d%d", &rec[i].x1, &rec[i].y1, &rec[i].x2, &rec[i].y2),
        rec[i].x2++, rec[i].y2++, u.push_back(rec[i].x1),
        u.push_back(rec[i].x2), v.push_back(rec[i].y1), v.push_back(rec[i].y2);
