...
struct BS {
  ...
  inline void ini(int n, int* aa) {
    ...
    memcpy(a + 1, aa + 1, n << 2);
    bl = sqrt(n);
    for (i = 1; i <= n; ++i) bel[i] = (i - 1) / bl + 1;
    bs = bel[n];
    for (i = 1; i <= n; ++i) en[bel[i]] = i;
    for (i = n; i; --i) be[bel[i]] = i;
    for (i = 1; i <= bs; ++i)
      for (j = be[i] + 1; j <= en[i]; ++j) a[j] ^= a[j - 1];
    for (i = 1; i <= n; ++i) ++buc[bel[i]][a[i]];
    for (i = 1; i <= bs; ++i) ps[i] = ps[i - 1] ^ a[en[i]];
    tot = 0;
    for (i = 1; i <= bs; ++i) tot += buc[i][ps[i - 1]];
  }
  inline void mdy(int x, int v) {
    int i, z = bel[x];
    for (i = x; i <= en[z]; ++i) --buc[z][a[x]], ++buc[z][a[x] ^= v];
...
int main() {
  ...
  for (i = 1; i <= n + 1; ++i) oa[i] ^= ob[i];
  for (i = n + 1; i; --i) oa[i] ^= oa[i - 1];
  for (i = 0; i < k; ++i) {
    x = 0;
    for (j = i == 0 ? k : i; j <= n + 1; j += k) aa[++x] = oa[j];
    B.ini(x, aa);
    auto mdy = [&](int l, int r) {
      if (B.ps[B.bs] != 0)
        ++ans2[l], --ans2[r];
      else
        ans1[l] += B.n - B.tot, ans1[r] -= B.n - B.tot;
    };
    int lst = 0;
    for (j = 0; j < ve[i].size(); ++j) {
      mdy(lst, ve[i][j].i);
      B.mdy(ve[i][j].j, ve[i][j].v);
      lst = ve[i][j].i;
    }
    mdy(lst, Q + 1);
    B.clear();
