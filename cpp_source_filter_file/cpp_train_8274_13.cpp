...
inline void link(int (*mat)[M], int u, int v) {
  ++mat[u][u], --mat[u][v], --mat[v][u], ++mat[v][v];
}
int gauss(int n, int (*mat)[M]) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (k = i + 1; k < n; ++k)
      for (coe = mod - m[k][i], j = i; j < n; ++j)
        m[k][j] = (m[k][j] + coe * m[i][j]) % mod;
  }
  return det;
}
int main() {
  ...
  std::iota(p, p + n, 0);
  for (i = 0; i < R; ++i)
    for (scanf("%s", buf_), j = 0; j < C; ++j) switch (buf_[j]) {
        case 42:
          fe[E++] = std::pair<int, int>(ID(i, j + 1), ID(i + 1, j)),
          fe[E++] = std::pair<int, int>(ID(i, j), ID(i + 1, j + 1));
          break;
        case 47:
          Union(ID(i, j + 1), ID(i + 1, j));
          break;
        case 92:
          Union(ID(i, j), ID(i + 1, j + 1));
          break;
      }
  for (i = 0; i <= R; ++i)
    for (j = 0; j <= C; ++j) type[ID(i, j)] = (i ^ j) & 1;
  for (i = 0; i < n; ++i)
    if (p[i] == i) bel[i] = V[type[i]]++;
  for (i = 0; i < n; ++i) bel[i] = bel[ancestor(i)];
  for (i = 0; i < E; ++i)
    std::tie(u, v) = fe[i], o = type[u], link(G[o], bel[u], bel[v]);
