...
struct dat {
  ...
  bool operator==(const dat &rhs) const {
    return f == rhs.f && a == rhs.a && b == rhs.b && c == rhs.c && d == rhs.d &&
           e == rhs.e;
  }
};
...
int query(dat x) {
  unsigned int hsh =
      (((((x.a * 11 + x.b) * 11 + x.c) * 11 + x.d) * 11 + x.e) * 19260817 +
       x.f) %
      199999;
  hsh %= 199999;
      ...
      for (int b = a; b < 10; b++)
        for (int c = b; c < 10; c++)
          for (int e = 1; e <= 9; e++) {
            ...
            change((dat){b, c, 10, 10, c, i}, cur + abs(a - e) + 1);
          }
    for (int a = 1; a < 10; a++)
      for (int b = a; b < 10; b++)
        for (int e = 1; e <= 9; e++) {
          ...
          change((dat){b, 10, 10, 10, b, i}, cur + abs(a - e) + 1);
        }
    for (int a = 1; a < 10; a++)
      for (int e = 1; e <= 9; e++) {
      ...
      for (int b = a; b < 10; b++)
        for (int c = b; c < 10; c++)
          for (int e = 1; e <= 9; e++) {
            ...
            so[3] = r[i];
            sort(so, so + 4);
            change((dat){so[0], so[1], so[2], so[3], l[i], i + 1},
                   cur + abs(l[i] - e) + 1);
          }
    for (int a = 1; a < 10; a++)
      for (int b = a; b < 10; b++)
