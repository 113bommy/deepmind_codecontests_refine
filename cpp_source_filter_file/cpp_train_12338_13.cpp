...
int closest(int a, int b) { return closest(a - b); }
struct Group {
  std::vector<int> a;
  int n = 0;
  ...
  int dist(int i, int j) { return closest(a[j] - a[i]); }
  template <class F>
  void calc(F f) {
    init();
    for (int d = 1; d <= 2 * n; d++) {
      for (int l = 0; l + d <= 2 * n; l++) {
        ...
        if (d == 1) {
          ...
          dp[l][r] = std::min(dp[l + 1][r] + dist(l, l + 1),
                              dp[r][l + 1] + dist(l, r - 1));
          ...
          dp[r][l] = std::min(dp[l][r - 1] + dist(r - 1, l),
                              dp[r - 1][l] + dist(r - 1, r - 2));
...
int main() {
  ...
  for (int i = n - 1; i >= 0; i--) {
    if (i + 1 == n) {
      a[i].calc([](int x) {
        return std::pair<int, std::pair<int, int>>(0,
                                                   std::pair<int, int>(x, x));
      });
    } else {
      a[i].calc([&](int x) {
        std::pair<int, std::pair<int, int>> ans(1e9,
