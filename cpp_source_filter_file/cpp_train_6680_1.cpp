...
void recalc(int v) { m[v] = max(m[2 * v + 1], m[2 * v + 2]) + d[v]; }
...
int okv(long double x) { return x + 1 - eps; }
int okn(long double x) { return x; }
...
int main() {
  ...
  for (i = 0; i < v.size(); ++i) {
    int p = j;
    while (j < v.size() && v[j].first - v[p].first < eps) {
      add(0, 1, vv + 1, okv(v[j].second.first - r), v[j].second.first + 1,
          v[j].second.second);
      ++j;
    }
    ans = max(ans, m[0]);
