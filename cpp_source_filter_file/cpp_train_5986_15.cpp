...
int main() {
  ...
  for (int i = 0; i < n; i++)
    p[i] = Cross(P[i], P[(i + 1) % n]), S += Cross(P[i], P[(i + 1) % n]);
  for (int i = n; i < n + n; i++) p[i] = p[i - n];
  S = abs(S) + 2;
  for (int i = 0; i < n; i++)
    c[i] = gcd(abs(P[(i + 1) % n].x - P[i].x), abs(P[(i + 1) % n].y - P[i].y)),
    S -= gcd(abs(P[(i + 1) % n].x - P[i].x), abs(P[(i + 1) % n].y - P[i].y));
  for (int i = n; i < n + n; i++) c[i] = c[i - n];
  for (int i = 1; i < n + n; i++) p[i] += p[i - 1], c[i] += c[i - 1];
  S >>= 1;
  ans = S;
  for (int Len = 2; Len <= min(30, n - 1); Len++)
    for (int i = 0; i < n; i++) {
      int j = (i + Len) % n;
      long long m = 0;
      ...
      ans -= m * ((Pow[Len + 1] - Pow[n]) / (1.0 - Pow[n] - 1.0 * n * Pow[n] -
                                             1.0 * n * (n - 1) * Pow[n + 1]));
    }
  printf("%.010lf\n", ans);
  return 0;
