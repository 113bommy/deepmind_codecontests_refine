...
int main() {
  ...
  if (LR.size() && RL.size()) {
    if (*LR.rbegin() > *RL.rbegin())
      RL.push_back(*LR.rbegin()), LR.pop_back();
    else
      LR.push_back(*RL.rbegin()), LR.pop_back();
  }
  ...
  if (!LR.size() && !RL.size()) {
    if (LL.size()) {
      int now = *LL.rbegin();
      for (vector<int>::iterator it = Ve[now].begin(); it != Ve[now].end();
           it++)
        De.push_back(*it);
      LL.pop_back();
    } else if (RR.size()) {
  ...
  while (LL.size() || RR.size()) {
    if (str[De.back()] == 'L' && RR.size()) {
      ...
      int now = *RR.rbegin();
      for (vector<int>::reverse_iterator it = Ve[now].rbegin();
           it != Ve[now].rend(); it++)
        De.push_front(*it);
      RR.pop_back();
  ...
  printf("%d\n", ct - 1);
  while (De.size()) printf("%d ", De.front() + 1), De.pop_front();
  return 0;
}
