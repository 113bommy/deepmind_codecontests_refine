...
int main() {
  ...
  for (i = 0; i < n; i++)
    if (v[i])
      for (j = 0; j < g[i].size(); j++) d[g[i][j]]++;
  for (i = 0; i < n; i++)
    if (!v[i])
      pq.push(std::make_pair(std::make_pair(1. * d[i] / g[i].size(), l), i));
  for (i = l; i < n; i++) {
    do {
      p = pq.top();
      pq.pop();
    ...
    for (j = 0; j < g[p.second].size(); j++) {
      ...
      pq.push(std::make_pair(
          std::make_pair(1. * d[g[p.second][j]] / g[g[p.second][j]].size(),
                         i + 1),
          g[p.second][j]));
    }
  }
