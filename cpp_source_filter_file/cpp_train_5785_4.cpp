...
bool operator<(unit A, unit B) {
  return A.x < B.x || (A.x == B.x && A.y < B.y) ||
         (A.x == B.x && A.y == B.y && A.flag < B.flag);
}
int fpow(int i, int j) {
  int ret = 1;
  for (; j; i = (long long)i * i % mod, j /= 2)
...
int main() {
  ...
  fac[0] = 1;
  for (i = 1; i <= n + m; i++) fac[i] = (long long)fac[i - 1] * i % mod;
    ...
    for (k = 0; k <= 21; k++) {
      for (j = 1; j < i; j++)
        if (a[j].x <= a[i].x && a[j].y <= a[i].y) {
          if (k >= a[i].flag)
            g[i][k] = (g[i][k] + (long long)g[j][k - a[i].flag] *
                                     comb(a[i].x + a[i].y - a[j].x - a[j].y,
                                          a[i].x - a[j].x) %
                                     mod) %
                      mod;
        }
  ...
  for (i = 0; i <= nt; i++) {
    ans = (ans + (long long)g[nt][i] * s % mod) % mod;
    s = (s + 1) / 2;
  }
  ans = (long long)ans * fpow(comb(n + m - 2, n - 1), mod - 2) % mod;
  printf("%d\n", ans);
  return 0;
}
