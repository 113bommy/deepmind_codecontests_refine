...
void solve() {
  int n, k;
  read(n, k);
  vector<int> numberLine(100002, 0);
  vector<vector<int>> p(n);
  lazy_seg ob1(numberLine, 100002);
  for (int(i) = 0; (i) < (n); ++(i)) {
    read(p[i], 2);
    p[i].push_back(i + 1);
    ob1.range_update_tree(1, 0, 100001, p[i][0], p[i][1], 0);
  }
  ...
  for (int(i) = 1; (i) <= (100000); ++(i)) {
    ...
    while (ob1.range_sum_tree(0, 100001, i, i, 0) > k) {
      vector<int> pa = ob2.range_query(0, 0, n - 1, l, r - 1);
      vector<int> ma = p1[pa[1] - 1];
      ob1.range_update_tree(-1, 0, 100001, ma[0], ma[1], 0);
      ans.push_back(ma[2]);
