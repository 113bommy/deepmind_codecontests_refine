...
bool comp2(int x, int y) { return b[x] < b[y]; }
int order[maxN];
int v[maxN], nt[maxN];
long long d[maxN];
...
void update(int i) {
  ...
  if (i >= 2) {
    if (p[i - 1] != order[i - 1] && p[i - 2] != order[i - 2]) {
      add[i][2] = max(add[i][2],
                      (long long)(a[i - 1]) * (long long)(b[order[i - 1]]) +
                          (long long)(a[i - 2]) * (long long)(b[order[i - 2]]));
    }
    if (p[i - 1] != order[i - 2] && p[i - 2] != order[i - 1]) {
      add[i][2] = max(add[i][2],
                      (long long)(a[i - 1]) * (long long)(b[order[i - 2]]) +
                          (long long)(a[i - 2]) * (long long)(b[order[i - 1]]));
    }
  }
  if (i >= 3) {
    ...
    do {
      if (p[i - 1] != z[0] && p[i - 2] != z[1] && p[i - 3] != z[2]) {
        add[i][3] =
            max(add[i][3], (long long)(a[i - 1]) * (long long)(b[z[0]]) +
                               (long long)(a[i - 2]) * (long long)(b[z[1]]) +
                               (long long)(a[i - 3]) * (long long)(b[z[2]]));
      }
