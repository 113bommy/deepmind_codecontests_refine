#include <bits/stdc++.h>
using namespace std;
long long t, x, y, n, m, q, a[1005][1005], rows_b[1005][1005],
    rows_f[1005][1005], i, j, rez;
long long cols_b[1005][1005], cols_f[1005][1005];
int main() {
  cin >> n >> m >> q;
  for (i = 0; i < n; i++)
  ...
  for (int k = 0; k < q; k++) {
    ...
    if (t == 1) {
      ...
      long long l = x, r = x, tmp = y - rows_b[x][y] + 1;
      rez = y - rows_b[x][y] + 1;
      while (a[r + 1][y] == 1 || a[l - 1][y] == 1) {
        if (a[r + 1][y] == 0 || r == n - 1) {
          l--;
          tmp = min(tmp, y - rows_b[l][y] + 1);
        } else if (a[l - 1][y] == 0 || l == 0) {
          r++;
      ...
      while (a[x][r + 1] == 1 || a[x][l - 1] == 1) {
        if (a[x][r + 1] == 0 || r == m - 1) {
          ...
          if (cols_b[x][l - 1] < x - cols_b[x][r + 1]) {
            l--;
            tmp = min(tmp, x - cols_b[x][l] + 1);
          } else {
            r++;
