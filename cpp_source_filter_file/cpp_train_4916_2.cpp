...
void dfs(int node) {
  ...
  while (it != g[node].end()) {
    ...
    sumdist[node] += sumdist[it->second.first] +
                     it->second.second * weight[it->second.first];
    it++;
  }
}
ll solve(int node, ll sum, ll acc) {
  ...
  while (it != g[node].end()) {
    ...
    int acc2 = acc + (weight[node] - weight[to]);
    ll sum2 = sum + acc2 * it->second.second +
              (sumdist[node] - sumdist[to] -
               it->second.second * weight[it->second.first]);
