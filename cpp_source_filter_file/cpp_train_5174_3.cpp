...
int bp[M], ret[N], C[N * 8][17], P[18][N], H[N], A[N], n, q;
vector<int> adj[N], prs;
vector<array<int, 4>> vec[N];
void preDFS(int v, int p = -1) {
  if (!~p) P[0][v] = 1;
  for (int i = 1; i < 18; i++) P[i][v] = P[i - 1][P[i - 1][v]];
  for (int &u : adj[v]) {
    if (u != p) H[u] = H[v] + 1, P[0][u] = v, preDFS(u, v);
  }
}
int get(int x) { return lower_bound(begin(prs), end(prs), x) - begin(prs); }
int LCA(int u, int v) {
  if (H[u] < H[v]) swap(u, v);
  for (int i = H[u] - H[v]; i; i -= i & -i) u = P[__builtin_ctz(i)][u];
  for (int i = 17; ~i; i--) {
    if (P[i][v] != P[i][u]) u = P[i][u], v = P[i][v];
  }
  return u ^ v ? P[0][v] : v;
}
...
void DFS(int v, int p = -1) {
  ...
  for (auto i : vec[v]) {
    ...
    for (int j = 1; j < 16; j++) x += min(tw, j) * C[get(pr)][j];
    if (~s)
      ret[id] = 1ll * ret[id] * Pow(pr, x, MOD) % MOD;
    else
      ret[id] = 1ll * ret[id] * Pow(Pow(pr, x, MOD), MOD - 2, MOD) % MOD;
  }
  for (int &u : adj[v])
    if (u != p) DFS(u, v);
  for (int x = A[v]; x > 1; x /= bp[x]) {
