  ...
  for (int i = 1; i <= n + 1; i++) {
    if (a[i].val != a[lst].val) {
      ...
      for (int j = 0; j < p; j++) {
        ...
        pre[0][lst + j][lst + j][0] = pre[1][lst + j][lst + j][0] =
            fa[a[lst + j].pos];
      }
      for (int len = 2; len <= i - lst; len++) {
        for (int j = 0; j < p; j++) {
          int k = (j + len - 1) % p;
          if (dp[1][lst + (j + 1) % p][lst + k] +
                  (a[lst + k].pos + n - a[lst + j].pos) % n <
              dp[0][lst + (j + 1) % p][lst + k] +
                  (a[lst + (j + 1) % p].pos + n - a[lst + j].pos) % n) {
            dp[0][lst + j][lst + k] = dp[1][lst + (j + 1) % p][lst + k] +
                                      (a[lst + k].pos + n - a[lst + j].pos) % n;
            pre[0][lst + j][lst + k][0] = 1;
            pre[0][lst + j][lst + k][1] = lst + (j + 1) % p;
            pre[0][lst + j][lst + k][2] = lst + k;
          } else {
            dp[0][lst + j][lst + k] =
                dp[0][lst + (j + 1) % p][lst + k] +
                (a[lst + (j + 1) % p].pos + n - a[lst + j].pos) % n;
            pre[0][lst + j][lst + k][0] = 0;
            pre[0][lst + j][lst + k][1] = lst + (j + 1) % p;
            pre[0][lst + j][lst + k][2] = lst + k;
          }
          if (dp[1][lst + j][lst + (k + p - 1) % p] +
                  (a[lst + k].pos + n - a[lst + (k + p - 1) % p].pos) % n <
              dp[0][lst + j][lst + (k + p - 1) % p] +
                  (a[lst + k].pos + n - a[lst + j].pos) % n) {
            dp[1][lst + j][lst + k] =
                dp[1][lst + j][lst + (k + p - 1) % p] +
                (a[lst + k].pos + n - a[lst + (k + p - 1) % p].pos) % n;
            pre[1][lst + j][lst + k][0] = 1;
            pre[1][lst + j][lst + k][1] = lst + j;
            pre[1][lst + j][lst + k][2] = lst + (k + p - 1) % p;
          } else {
            dp[1][lst + j][lst + k] = dp[0][lst + j][lst + (k + p - 1) % p] +
                                      (a[lst + k].pos + n - a[lst + j].pos) % n;
            pre[1][lst + j][lst + k][0] = 0;
            pre[1][lst + j][lst + k][1] = lst + j;
            pre[1][lst + j][lst + k][2] = lst + (k + p - 1) % p;
          }
        }
      }
      for (int j = 1; j <= n; j++) bgn[j] = INF;
      for (int j = 0; j < p; j++) {
        if (dp[0][lst + j][lst + (j + p - 1) % p] <
            dp[1][lst + (j + 1) % p][lst + j]) {
        ...
        for (int k = 1; k <= n; k++)
          if (a[lst + j].ans +
                  min(n - abs(a[lst + j].pos - k), abs(a[lst + j].pos - k)) <
              bgn[k]) {
            bgn[k] = a[lst + j].ans +
                     min(n - abs(a[lst + j].pos - k), abs(a[lst + j].pos - k));
            fa[k] = lst + j;
      ...
      if (i == n + 1) {
        int minx = lst;
        for (int j = 0; j < p; j++) {
          if (a[lst + j].ans < a[minx].ans) minx = lst + j;
        }
        printf("%d\n", a[minx].ans);
        Print1(minx);
      }
