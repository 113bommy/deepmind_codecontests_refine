...
void prepare_factorials(int maximum) {
  ...
  for (int i = 2; i <= maximum; i++) inv[i] = inv[MOD % i] * (MOD - MOD / i);
...
vector<mod_int> mod_multiply(vector<mod_int> left, vector<mod_int> right) {
  ...
  for (int i = 0; i < N; i++) left[i] *= right[i] * inv_N;
...
void add(vector<mod_int> &answers, const vector<mod_int> &product,
         size_t offset) {
  answers.resize(max(answers.size(), product.size() + offset), 0);
  for (size_t i = 0; i < product.size(); i++) answers[offset + i] += product[i];
}
...
int main() {
  ...
  int n, x, y;
  cin >> n >> x >> y;
  for (int f = 200000 + y; f > y; f--) {
    for (int k = f; k <= 400000; k += f) {
      factors[k].push_back(f);
    }
  }
  vector<mod_int> single_poly, double_poly;
  for (int j = 0; j <= 200000; j++) {
    single_poly.push_back(0);
    double_poly.push_back(0);
  }
  for (int j = 0; j <= n; j++) {
    int a;
    cin >> a;
    single_poly[a]++;
    double_poly[200000 - a]++;
  }
  vector<mod_int> product1 = {1};
  product1 =
      NTT::mod_multiply(product1, NTT::mod_multiply(single_poly, double_poly));
  int q;
  cin >> q;
  for (; q; q--) {
    int l;
    ...
    if (l % 2 == 1) {
      ...
      int res = -1;
      for (int f : factors[l / 2]) {
        if (f > y && f - y <= 200000 && product1[200000 + f - y] != 0) {
          res = 2 * f;
          break;
        }
      }
