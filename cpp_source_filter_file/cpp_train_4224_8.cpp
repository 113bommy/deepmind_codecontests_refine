...
const unsigned long long inf = 0x3f3f3f3f3f3f3f3fllu;
unsigned long long f[2][maxn], p[maxn], s[maxn], c, ans = inf;
unsigned n, cur;
__inline void trans(const unsigned &i) {
  memset(f[cur], 0x3f, sizeof(f[cur])), f[cur][0] = f[cur ^ 1][0] + p[i];
  for (unsigned j = 1; j <= i; j++)
    f[cur][j] =
        std::min(f[cur ^ 1][j] + j * c + p[i], f[cur ^ 1][j - 1] + s[i]);
}
int main() {
  scanf("%u%I64u", &n, &c);
  ...
  for (unsigned i = 1; i <= n; i++) ans = std::min(ans, f[cur][i]);
  printf("%I64u\n", ans);
  return 0;
}
