...
vector<int> suffix_array(string s) {
  ...
  for (int len = 0, num_cls = ALPHA; len < n; len ? len <<= 1 : ++len) {
    ...
    new_cls[suf[0]] = 0;
    for (int i = 1; i < n; ++i) {
      if (cls[suf[i]] != cls[suf[i - 1]] ||
          cls[suf[i] + len] != cls[suf[i - 1] + len])
        ++num_cls;
...
int32_t main() {
  ...
  for (int i = 0; i < m + n + 1; ++i) ord[suf[i]] = i;
  int sz = int((lcp).size());
  vector<int> tree(sz);
  tree.insert(tree.end(), (lcp).begin(), (lcp).end());
  assert((int)tree.size() == sz + sz);
  for (int i = sz - 1; i > 0; --i) tree[i] = min(tree[i + i], tree[i + i + 1]);
