...
int cnt, tot = 2, inq[N], hed[N], to[M], val[M], cst[M], nxt[M], pre[N], dis[N],
         flow, cost, max_flow, max_cost;
namespace mcmf {
...
void main() {
  while (spfa()) {
    ...
    for (int i = pre[e]; i; i = pre[to[i ^ 1]])
      val[i] -= flow, val[i ^ 1] += flow, cost += flow * cst[i];
    ...
    for (int j = i + 1; j <= n; j++)
      if (~G[i][j]) {
        for (int k = hed[G[i][j]]; k; k = nxt[k]) {
          if (to[k] != s && !val[k]) res[to[k]][i + j - to[k]] = true;
        }
      }
  for (int i = 1; i <= n; i++) {
