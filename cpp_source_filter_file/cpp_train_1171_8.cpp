#include <bits/stdc++.h>
using namespace std;
int const nax = 6e5 + 10;
int n, h, m, k;
struct fenwick {
  int bit[nax];
  fenwick() { memset(bit, 0, sizeof(bit)); }
  void update(int pos, int val) {
    for (; pos < nax; pos += pos & -pos) bit[pos] += val;
  }
  int get(int pos) {
    int res = 0;
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    if (interval[i].first >= m / 2)
      interval[i].first -= m / 2, interval[i].second -= m / 2;
  }
  int top = n;
  for (int i = 1; i <= n; ++i) {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    int r =
        lower_bound(com + 1, com + topCom + 1, interval[i].second + 1) - com;
    f.update(l, 1);
    f.update(r, -1);
  }
