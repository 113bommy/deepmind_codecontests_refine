...
int pd(int m) {
  ...
  if (x <= y && !(x == y && x == 0)) {
    if (x == 0) x++;
    for (int i = 1, _b = 4; i <= _b; i++) s[i] = a[i];
    gt(1, 2, x);
    gt(3, 4, x);
    int x1 = l[1], x2 = l[1] + x;
    int y1 = r[3], y2 = r[3] + x;
    ...
    s[2].x[1] = s[3].x[1] = y1;
    s[1].x[1] = s[4].x[1] = y2;
    for (int i = 1, _b = 4; i <= _b; i++) ans2[p[i]] = s[i];
  }
  return x <= y;
}
void dfs(int x) {
...
void dg(int x) {
  if (x > 4) {
    for (int i = 1, _b = 4; i <= _b; i++) b[i] = a[i];
    for (int i = 1, _b = 4; i <= _b; i++) a[i] = b[p[i]];
    dfs(1);
    for (int i = 1, _b = 4; i <= _b; i++) a[i] = b[i];
    return;
  }
