...
vector<int> square(vector<int> &a, int m) { return multiply(a, a, m, 1); }
};  // namespace ntt
using namespace ntt;
vector<int> c[N];
vector<int> yo(int l, int r) {
  ...
  if (l == r) return c[l];
  int mid = (l + r) / 2;
  vector<int> a = yo(l, mid);
  vector<int> b = yo(mid + 1, r);
  return multiply(a, b);
}
int a[N], b[N];
int32_t main() {
  ...
  while (q--) {
    ...
    for (int i = 1; i <= n; i++)
      c[i] = vector<int>({(hudai - b[i] + mod) % mod, 1});
    vector<int> ans = yo(1, n);
    printf("%d\n", ans[n - k]);
  }
  return 0;
}
