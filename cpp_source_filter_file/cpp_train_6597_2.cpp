...
pair<long long int, int> upp;
pair<long long int, int> upp2;
long long int best_other(int col) {
  if (upp.second == col)
    return upp2.first;
  else
    return upp.first;
...
long long int compute_dp(int a, int b) {
  upp = make_pair(0, 0);
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    if (dp[i] > best[c[i]]) {
      best[c[i]] = dp[i];
      if (dp[i] > upp.first) {
        if (c[i] != upp.second) upp2 = upp;
        upp = make_pair(dp[i], i);
      } else if (dp[i] > upp2.first)
        upp2 = make_pair(dp[i], i);
    }
  }
  return ans;
}
