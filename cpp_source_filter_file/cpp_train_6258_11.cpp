...
struct P {
  long double x, y;
  long double getv(long double v) { return y - x * v; }
} c[N], st[N];
P operator-(P a, P b) { return (P){a.x - b.x, a.y - b.y}; }
long double cross(P a, P b) { return a.x * b.y - b.x * a.y; }
void mv(long double x) {
  for (; pp < tp && st[pp].getv(x) < st[pp + 1].getv(x) + eps; ++pp)
    ;
}
...
struct mat {
  ...
  void trs() {
    ...
    for (int i = 0; i < 3; ++i)
      for (int j = 0; j < 3; ++j) v2[j] += v1[i] * a[i][j];
  }
} mi[35];
mat operator*(mat a, mat b) {
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
      for (int k = 0; k < 3; ++k) ret.a[i][k] += a.a[i][j] * b.a[j][k];
  return ret;
}
int main() {
  scanf("%d%lld", &n, &T);
  ...
  for (i = T;;) {
    ...
    for (j = 34; j >= 0; --j) {
      mi[j].trs();
      if (v2[1] > 0 &&
          (pp == tp || st[pp].getv(v2[0] + v2[1] * w) >
                           st[pp + 1].getv(v2[0] + v2[1] * w) + eps))
