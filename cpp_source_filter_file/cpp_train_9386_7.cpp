...
struct Tree {
  int Mn[524288 + 524288], Mx[524288 + 524288], K[524288 + 524288];
...
void Add(int a, int x) {
  ITX.Add(1, 0, 524288 - 1, R[a].bx, R[a].ex, x);
  ITY.Add(1, 0, 524288 - 1, R[a].by, R[a].ey, x);
}
bool OK(set<pair<int, int> > &SX, set<pair<int, int> > &SY) {
  if (SX.size() == 1) {
    Add(SX.begin()->second, -1);
    SX.clear(), SY.clear();
    return true;
  }
  int lx = ITX.Least(1, 0, 524288 - 1), rx = ITX.Last(1, 0, 524288 - 1);
  int ly = ITY.Least(1, 0, 524288 - 1), ry = ITY.Last(1, 0, 524288 - 1);
  int tx = ITX.Get0(1, 0, 524288 - 1, lx, rx);
  int ty = ITY.Get0(1, 0, 524288 - 1, ly, ry);
