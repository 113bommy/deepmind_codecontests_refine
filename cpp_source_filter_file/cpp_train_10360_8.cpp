...
bool operator==(point a, point b) { return fabs(dis(a - b)) < 1e-10; }
...
double dis(line l, point a) { return cross(l.dir, a - l.a) / dis(l.dir); }
...
long long check(double d) {
  cnt = 0, tot = 0;
  for (int i = 1; i <= n; i++)
    if (dis(l[i], point(0, 0)) <= d) {
      point p =
          intersect(l[i], line(point(0, 0), point(-l[i].dir.y, l[i].dir.x)));
      if (dis(p) > d) break;
