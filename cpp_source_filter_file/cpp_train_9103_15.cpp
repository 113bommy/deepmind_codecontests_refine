...
int turnToBase(int lin, int x[], vector<pair<int, int> > &ans) {
  for (int i = 1; i <= N; i++) {
    ...
    isBase[i] = 1;
    for (int j = i + 1; j <= N; j++)
      if (x[j] & (1 << k)) x[j] ^= x[i], ans.push_back({j, i});
  }
  int j = 0;
  for (int i = 1; i <= N; i++)
    if (isBase[i]) doSwap(lin, ++j, i, ans);
  return j;
}
int writeVal(int val) {
  int aux[50];
  for (int i = 1; i <= L; i++) aux[i] = x[i];
  x[0] = val;
  for (int i = 0; i <= L; i++) how[i] = 1 << i;
  for (int i = L; i >= 0; i--) {
    int k = -1;
    for (int j = 0; j < 30; j++)
      if (x[i] & (1 << j)) {
    ...
    if (k == -1) continue;
    for (int j = i - 1; j >= 0; j--)
      if (x[j] & (1 << k)) x[j] ^= x[i], how[j] ^= how[i];
  }
  for (int i = 1; i <= L; i++) x[i] = aux[i];
  if (x[0] != 0) {
    printf("-1\n");
    exit(0);
  }
...
void doMidStuff() {
  for (int i = 1; i <= R; i++) {
    ...
    for (int j = 1; j <= L; j++)
      if (msk & (1 << j)) x[i] ^= x[j], mid.push_back({i, j});
  }
  for (int i = R + 1; i <= L; i++) mid.push_back({i, i});
}
void check() {
  ifstream f("input");
  f >> N;
...
int main() {
  ...
  R = turnToBase(0, y, rgt);
  doMidStuff();
  for (auto it : lft) ans.push_back(it);
  for (auto it : mid) ans.push_back(it);
  reverse(rgt.begin(), rgt.end());
  for (auto it : rgt) ans.push_back(it);
  print(ans);
  return 0;
}
