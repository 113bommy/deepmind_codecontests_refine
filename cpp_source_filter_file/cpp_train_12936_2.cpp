...
int n, tree[800010][2], lazy[800010][2], num[200010], tot;
vector<int> Q[200010][2];
struct sb {
  int l, r, c;
} a[200010];
...
inline void xg(int l, int r, int L, int R, int v, int bj, int ver) {
  ...
  int mid = (l + r) / 2;
  down(bj, ver);
  if (L <= mid) xg(l, mid, L, R, v, bj * 2, ver);
  if (mid < R) xg(mid + 1, r, L, R, v, bj * 2 + 1, ver);
  up(bj, ver);
}
inline int cx(int l, int r, int L, int R, int bj, int ver) {
  if (L <= l && r <= R) return tree[bj][ver];
  int mid = (l + r) / 2, bck = 0;
  down(bj, ver);
  if (L <= mid) bck = cx(l, mid, L, R, bj * 2, ver);
  if (mid < R) bck = max(bck, cx(mid + 1, r, L, R, bj * 2 + 1, ver));
  return bck;
}
int main() {
  n = read();
