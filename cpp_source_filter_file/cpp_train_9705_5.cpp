...
using namespace std;
const int N = 520;
int n, m, q, s[4][N][N], id[N], s2[N / 2][N][N], Mx;
char c[N][N];
...
bool ck(int x, int y, int len) {
  ...
  return CK(s[0], x0, y0, x, y, 0) && CK(s[1], x0, y + 1, x, y1, 0) &&
         CK(s[2], x + 1, y0, x1, y, 0) && CK(s[3], x + 1, y + 1, x1, y1, 0);
}
int read() {
  int x = 0, c;
...
int main() {
  ...
  for (int i = 1, x1, y1, x2, y2; i <= q; i++) {
    ...
    x2 = read();
    y2 = read();
    int l = 1, r = min((x2 - x1 + 1) / 2, (y2 - x1 + 1) / 2), ans = 0;
    while (l <= r) {
      int mid = (l + r) >> 1;
      if (CK(s2[mid], x1 + mid - 1, y1 + mid - 1, x2 - mid, y2 - mid, 1))
        ans = mid, l = mid + 1;
      else
        r = mid - 1;
    }
