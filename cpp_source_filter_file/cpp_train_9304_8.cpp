...
struct Point {
  ...
  void show(ostream& cout) { cout << x << " " << y << endl; }
...
int main() {
  ...
  if (a == 90)
    ...
    vector<Point> pol;
    for (int i = 0; i < polMY.size(); i++) pol.push_back(polMY[i]);
    for (int i = 0; i < polPX.size(); i++) pol.push_back(polPX[i]);
    for (int i = polPY.size() - 1; i >= 0; i--) pol.push_back(polPY[i]);
    for (int i = polMX.size() - 1; i >= 0; i--) pol.push_back(polMX[i]);
    double area = 0.0;
