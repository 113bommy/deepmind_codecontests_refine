...
struct Basis {
  ...
  int toInt() { return A[0] | A[1] << 1 | A[2] << 3 | A[3] << 6 | A[4] << 10; }
  void getSt(int x) {
    A[0] = x & 1;
    A[1] = x >> 1 & 3;
    A[2] = x >> 3 & 7;
...
void DFS(int u, int c, int pa) {
  ...
  for (auto e : G[u]) {
    ...
    if (!vis[v])
      xorv[v] = xorv[u] ^ w, DFS(v, c, u);
    else if (v != 1 && v != pa && dfn[v] < dfn[u])
      rel[c] |= !B[c].Insert(xorv[v] ^ xorv[u] ^ w);
  }
}
int dp[374], pd[374];
const int Mod = 1000000007;
inline void Add(int &x, long long y) { (x += y) %= Mod; }
int main() {
  Init();
  scanf("%d%d", &N, &M);
  for (int i = 1; i <= M; ++i) {
  ...
  for (auto e : G[1]) {
    int v = e.first, w = e.second;
    if (!vis[v])
      uid[++C] = v, cyw[C] = w, DFS(v, C, 1);
    else {
      int vv = uid[vis[v]];
      for (auto e2 : G[v])
        if (e2.first == vv) {
          cyw[vis[v]] = -1 - (cyw[vis[v]] ^ w ^ e2.second);
          break;
        }
    }
  }
