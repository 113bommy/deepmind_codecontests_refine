    ...
    for (unsigned int k = 0; k <= p; k++)
      if (a.s[i][k])
        for (unsigned int j = 0; j <= p; j++)
          c.s[i][j] = (c.s[i][j] + 1ll * a.s[i][k] * b.s[k][j]) & 4294967295ll;
  return c;
}
...
unsigned int C(unsigned int i, unsigned int j) {
  ...
  for (unsigned int k = 1; k <= j; k++) {
    ...
    while (~l & 1) l >>= 1, st2++;
    while (~r & 1) r >>= 1, st2--;
    st1 = (1ll * st1 * l) & 4294967295ll;
    st1 = (1ll * st1 * getinv(r)) & 4294967295ll;
  }
  st1 <<= st2;
