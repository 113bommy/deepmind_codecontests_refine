...
int main() {
  ...
  for (int i = (0), _end_ = (N); i < _end_; ++i) {
    ...
    if (!flag) tmp = 1;
    if (m - tmp < 2) break;
    if (tmp && Min >= lst - sa[i] && lst - sa[nxt] <= (lst - sa[i]) << 1)
      continue;
    for (int j = (sa[i]), _end_ = (lst); j < _end_; ++j) putchar(s[j]);
    flag = sa[i] == lst - 1;
    lst = sa[i];
    s[lst] = 0;
    m -= tmp;
  ...
  if (m == 2) {
    ...
    int Max = -1;
    for (int i = (1), _end_ = (lst); i < _end_; ++i) {
      if (Max == -1 || Max + nxt[Max] <= i)
        nxt[i] = 0;
      else
        nxt[i] = min(Max + nxt[Max] - i, nxt[i - Max]);
    ...
    int last_match = -1;
    int place = lst;
    for (int i = (0), _end_ = (lst); i < _end_; ++i) {
      if (i >= place) break;
      int tmp = 0;
      if (Max == -1 || max_reach <= i)
        tmp = 0;
      else
        tmp = min(max_reach - i, nxt[i - Max]);
    ...
    for (int i = (0), _end_ = (N); i < _end_; ++i) {
      ...
      int tmp = lst - sa[i];
      int flag = -1;
      for (int j = (now), _end_ = (tmp); j < _end_; ++j) {
      ...
      if (flag == 1) break;
      for (int j = (now), _end_ = (tmp); j < _end_; ++j) s2[j] = s[sa[i] + j];
      now = lst - sa[i];
      int Min = height[i];
