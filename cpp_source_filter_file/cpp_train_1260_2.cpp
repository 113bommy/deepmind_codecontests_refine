...
vector<pair<pair<int, int>, int> > V[110];
int pL[101000], n, Dep[101000], par[101000][20], Num[101000], cnt, C[101000],
    PPP[101000], ReNum[101000];
long long BS = 1879, Mod[2] = {1000000007, 1000000009}, po[101000][2];
...
int Get(int b, int e, int z1, int z2) {
  return lower_bound(V[m].begin(), V[m].end(),
                     pair<pair<int, int>, int>(pair<int, int>(z1, z2), e + 1)) -
         lower_bound(V[m].begin(), V[m].end(),
                     pair<pair<int, int>, int>(pair<int, int>(z1, z2), b));
}
...
int main() {
  ...
  po[0][0] = po[0][1] = 1;
  for (i = 1; i <= 100000; i++)
    for (j = 0; j < 2; j++) po[i][j] = po[i - 1][j] * BS % Mod[j];
  scanf("%d", &n);
  for (i = 1; i < n; i++) {
    scanf("%d%d", &a, &b);
    scanf("%s", pp);
