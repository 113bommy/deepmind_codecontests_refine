...
int sz[maxn], L, R, edge[maxn][maxb], h[maxb][maxn], s[maxn], seg[maxn * 4];
bool picked[maxn];
...
void plant(int v, int p = -1) {
  ...
  for (auto e : adj[v]) {
    int u = from[e] ^ to[e] ^ v;
    if (!picked[u] && u != p) plant(u, v), sz[v] += sz[u];
  }
}
int find_centroid(int v, int n, int p = -1) {
  for (auto e : adj[v]) {
    int u = from[e] ^ to[e] ^ v;
    if (!picked[u] && u != p && sz[u] * 2 > n) return find_centroid(u, n, v);
  }
  return v;
}
...
int ver;
bool check(int x) {
  for (int i = 0; i < n - 1; i++) w[i] = ((c[i] >= x) ? 1 : -1);
  for (int lvl = 0; !ord[lvl].empty(); lvl++) {
    ...
    for (int i = 0; i < (int)ord[lvl].size(); i++) {
      int last = i + 1;
      while (!empt[lvl][ord[lvl][last - 1]]) last++;
      int m = (last - i + 1);
      fill(seg, seg + 4 * n, -maxn);
      int pt = i;
...
void handle(int v, int len = 0, int sum = 0, int p = -1) {
  ...
  for (auto e : adj[v]) {
    int u = from[e] ^ to[e] ^ v;
    if (u != p) handle(u, len + 1, sum + w[e], v);
  }
}
int main() {
