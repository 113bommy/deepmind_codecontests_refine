...
int main() {
  ...
  for (int i = n; i; i--) {
    ...
    while (s3.size() && a[x = *s3.begin()] <= a[i]) s3.erase(x);
    while (s4.size() && a[x = *s4.begin()] >= a[i]) s4.erase(x);
    if (s3.size() && s4.size()) {
    ...
    for (int j : p[i]) {
      if (b.size() && b.back() <= r[i])
        s[j] = b;
      else {
        if (s3.size()) {
          auto it = s6.lower_bound(*s3.begin());
          if (it != s6.end() && *it <= r[j]) s[j] = {i, *it - 1, *it};
        }
        if (s4.size()) {
          auto it = s7.lower_bound(*s4.begin());
          if (it != s7.end() && *it <= r[j]) s[j] = {i, *it - 1, *it};
        }
      }
    }
    c[i] = 2;
