...
vector<int> g[110000];
unsigned long long key[110000];
int st[110000];
int ra[110000];
unsigned long long ran[] = {114514, 1919810, 364364, 4545334};
int n;
...
unsigned long long calc(int a, int b) {
  ...
  unsigned long long ret = ch.size() + 1;
  for (int i = 0; i < ch.size(); i++) {
    ret = (ret * mod + ch[i] * ran[i]);
  }
  key[a] = ret;
  return ret;
}
