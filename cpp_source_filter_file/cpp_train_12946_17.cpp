...
void initlca() {
  ...
  for (int k = 1; k < MAXLG; k++)
    for (int i = 0; i < n; i++) jump[i][k] = jump[jump[i][k - 1]][k - 1];
}
int lca(int a, int b) {
  if (dep[a] < dep[b]) swap(a, b);
  for (int k = MAXLG - 1; k >= 0; k--)
    if (dep[jump[a][k]] >= dep[b]) a = jump[a][k];
  if (a == b) return a;
  for (int k = MAXLG - 1; k >= 0; k--)
...
int qcount(node *cur1, node *cur2, int lt, int rt, int ql, int qr) {
  ...
  if (lt >= ql && rt <= qr)
    return (rt - lt) - (cur2->sum - cur1->sum);
  else {
    int mid = (lt + rt) / 2;
    return qcount(cur1->l, cur2->l, lt, mid, ql, qr) +
           qcount(cur1->r, cur2->r, mid, rt, ql, qr);
  }
}
int query(node *cur1, node *cur2, int lt, int rt, int ql, int qr, int k) {
  if (lt + 1 == rt) {
    ...
    if (k <= res)
      return query(cur1->l, cur2->l, lt, mid, ql, qr, k);
    else
      return query(cur1->r, cur2->r, mid, rt, ql, qr, k - res);
  }
}
int tcount(int cur, int nd, int y) {
  ...
  while (cur >= 0) {
    if (cmp[cur] != cmp[nd]) {
      if (ind[cur] + 1 == chain[cmp[cur]].size())
        ret +=
            chain[cmp[cur]].size() -
            (attack[cmp[cur]].end() -
             lower_bound(attack[cmp[cur]].begin(), attack[cmp[cur]].end(), y));
      else {
        int a =
            lower_bound(attack[cmp[cur]].begin(), attack[cmp[cur]].end(), y) -
            attack[cmp[cur]].begin() - 1;
        ret += qcount(tree[cmp[cur]][a], tree[cmp[cur]].back(), 0,
                      chain[cmp[cur]].size(), 0, ind[cur] + 1);
      }
      cur = par[head[cur]];
    } else {
      int a = lower_bound(attack[cmp[cur]].begin(), attack[cmp[cur]].end(), y) -
              attack[cmp[cur]].begin() - 1;
      ret += qcount(tree[cmp[cur]][a], tree[cmp[cur]].back(), 0,
                    chain[cmp[cur]].size(), ind[nd], ind[cur] + 1);
      break;
    }
  }
  return ret;
...
int getk(int cur, int nd, int y, int k) {
  while (cur >= 0) {
    if (cmp[cur] != cmp[nd]) {
      int res;
      if (ind[cur] + 1 == chain[cmp[cur]].size())
        res = chain[cmp[cur]].size() -
              (attack[cmp[cur]].end() - lower_bound(attack[cmp[cur]].begin(),
                                                    attack[cmp[cur]].end(), y));
      else {
        int a =
            lower_bound(attack[cmp[cur]].begin(), attack[cmp[cur]].end(), y) -
            attack[cmp[cur]].begin() - 1;
        res = qcount(tree[cmp[cur]][a], tree[cmp[cur]].back(), 0,
                     chain[cmp[cur]].size(), 0, ind[cur] + 1);
      }
      if (res >= k) {
        int a =
            lower_bound(attack[cmp[cur]].begin(), attack[cmp[cur]].end(), y) -
            attack[cmp[cur]].begin() - 1;
        int ans = query(tree[cmp[cur]][a], tree[cmp[cur]].back(), 0,
                        chain[cmp[cur]].size(), 0, ind[cur] + 1, res - k + 1);
      ...
      int a = lower_bound(attack[cmp[cur]].begin(), attack[cmp[cur]].end(), y) -
              attack[cmp[cur]].begin() - 1;
      int res = qcount(tree[cmp[cur]][a], tree[cmp[cur]].back(), 0,
                       chain[cmp[cur]].size(), ind[nd], ind[cur] + 1);
      int ans =
          query(tree[cmp[cur]][a], tree[cmp[cur]].back(), 0,
                chain[cmp[cur]].size(), ind[nd], ind[cur] + 1, res - k + 1);
      return chain[cmp[cur]][ans];
    }
  }
}
