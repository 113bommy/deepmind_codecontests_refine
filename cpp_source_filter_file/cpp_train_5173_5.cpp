...
int n, q, u, v, x, pr = 0, cnt = 0, cur = 1, a[N], h[N], rt[N], par[N], col[N],
                   sub[N], in[N], d[MX], pos[MX];
long long ans[Q];
class CBIT {
 private:
  int bit[N];
 ...
 public:
  void build() { fill(bit, bit + N, 0); }
  void upd(int u, int v) {
    for (; u <= n; u += u & -u) bit[u] += v;
  }
  int que(int u) {
    int ret = 0;
    for (; u > 0; u -= u & -u) ret += bit[u];
    return ret;
  }
...
void HLD(int u, int p = 0) {
  in[u] = ++cnt;
  col[u] = cur;
  if (rt[cur] == 0) rt[cur] = u;
  int mc = 0;
  for (int &v : adj[u])
    if (v != p && sub[v] > sub[mc]) mc = v;
  if (mc != 0) HLD(mc, u);
  for (int &v : adj[u])
    if (v != p && v != mc) {
      ++cur;
...
void get_ans(const SQuery &a, int pr) {
  ...
  while (col[u] != col[v]) {
    if (h[rt[col[u]]] < h[rt[col[v]]]) swap(u, v);
...
int main() {
  ...
  HLD(1);
  for (int i = 1; i <= n; i++) {
    for (scanf("%d", &x); x != 1; x /= d[x]) pro.push_back(d[x]);
  ...
  for (int i = 1; i <= q; i++) {
    ans[i] = 1;
    for (scanf("%d%d%d", &u, &v, &x); x != 1; x /= d[x]) pro.push_back(d[x]);
    sort(pro.begin(), pro.end());
    pro.push_back(-1);
    int cnt = 1;
    for (unsigned int j = 1; j < pro.size(); j++) {
  ...
  for (int i = 1; i <= pr; i++) {
    if (que[i].empty() || val[i].empty()) continue;
    sort(que[i].begin(), que[i].end(),
         [](const SQuery &a, const SQuery &b) { return a.x > b.x; });
    sort(val[i].begin(), val[i].end(), greater<pair<int, int>>());
    for (pair<int, int> &u : val[i]) sum.upd(in[u.second], u.first);
    unsigned int pt = 0;
    for (SQuery &u : que[i]) {
      for (; pt < val[i].size() && val[i][pt].first >= u.x; pt++) {
        sum.upd(in[val[i][pt].second], -val[i][pt].first);
        chk.upd(in[val[i][pt].second], 1);
      }
      get_ans(u, sav[i - 1]);
    }
    for (unsigned int j = 0; j < pt; j++) chk.upd(in[val[i][j].second], -1);
    for (unsigned int j = pt; j < val[i].size(); j++)
      sum.upd(in[val[i][j].second], -val[i][pt].first);
  }
