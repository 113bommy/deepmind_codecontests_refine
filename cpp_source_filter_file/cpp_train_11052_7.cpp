...
struct d_tree {
  ...
  void put(int i) {
    if (i) z[i] = !z[i], a[i] = !a[i], swap(x[i], y[i]);
  }
  void lazy(int i) {
    if (!i) return;
    if (z[i]) {
...
int C() {
  int x;
  TREE.access(g[1]), TREE.splay(x = g[0], 0);
  return TREE.x[TREE.r[x]] + mdb;
}
int main() {
  int n, m, a, b;
  mdb = 0;
  ...
  ans = n;
  while (m--) {
    RD(a, b), TREE.cover(a), TREE.cover(b);
    if (abs(o[FA[a]] - o[FA[b]]) > (T - abs(o[FA[a]] - o[FA[b]])) ||
        abs(o[FA[a]] - o[FA[b]]) == (T - abs(o[FA[a]] - o[FA[b]])) &&
            (o[FA[a]] < o[FA[b]] ? g[o[FA[a]] + 1] : g[o[FA[a]] - 1]) >
                (o[FA[a]] < o[FA[b]] ? g[o[FA[a]] - 1] : g[o[FA[a]] + 1])) {
      mdb ^= 1, ans -= mdb ? 1 : -1;
      TREE.cover(g[1]);
    }
