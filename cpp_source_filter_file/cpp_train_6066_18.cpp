...
bool common(seg A, seg B) {
  return (A.st == B.st || A.st == B.en || A.en == B.st || A.en == B.st) ? 1 : 0;
}
pair<int, int> mid[2];
...
bool ok(seg A, pair<int, int> mid) {
  long long x = dist(A.st, mid), y = dist(mid, A.en);
  if (x > y) swap(x, y);
  if (x * 16 >= y) return 1;
  return 0;
}
bool deg(seg A, seg B) {
  ...
  long long x = dist(A.st, A.en), y = dist(B.st, B.en), z = dist(A.en, B.en);
  if (x + y < z) return 0;
  return 1;
}
int main() {
  ...
  while (t--) {
    ...
    if (A.st == A.en || B.st == B.en || C.st == C.en) {
      puts("NO");
      continue;
    }
    if (common(A, C))
