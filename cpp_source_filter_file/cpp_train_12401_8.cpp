...
struct point {
  ...
  void set(int n = 0) { lmx = rmx = tot = tmx = n; }
} se[8000001];
struct num {
  int val, orig;
} NI[500010];
...
point merge(point a, point b) {
  return (point){max(a.lmx, a.tot + b.lmx), max(b.rmx, b.tot + a.rmx),
                 a.tot + b.tot, max(max(a.tmx, b.tmx), a.rmx + b.lmx)};
}
...
int main() {
  ...
  for (t = 1; t <= top; t++) {
    h = t;
    while (NI[t].val == NI[h].val && t <= top)
      update(1, 1, n, NI[t].orig, 1), t++;
