...
int cmp(const query &a, const query &b) { return a.r < b.r; }
...
int main() {
  ...
  for (map<int, int>::iterator it = nu.begin(); it != nu.end(); it++)
    it->second = ++tot;
  ...
  for (int r = 1; r <= n; r++) {
    add(r);
    while (cur <= n && qu[cur].r == r) ans[qu[cur].i] = ask(qu[cur].l), cur++;
  }
