...
long long sub_inversions(vector<T>& A, vector<T>& store, int lower, int upper) {
  ...
  long long out = sub_inversions(store, A, lower, mid) +
                  sub_inversions(store, A, mid + 1, upper);
  int j = mid + 1, k = lower;
  for (int i = lower; i <= mid; i++) {
    while (j <= upper && A[j] < A[i]) store[k++] = A[j++];
    store[k++] = A[i];
    out += j - mid - 1;
  }
  while (j <= upper) store[k++] = A[j++];
  return out;
}
template <typename T>
...
int main() {
  ...
  vector<int> a(n);
  for (auto& x : a) cin >> x;
  cout << (inversions(a) & 1 == n & 1 ? "Um_nik" : "Petr") << "\n";
}
