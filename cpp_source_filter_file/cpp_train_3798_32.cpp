...
int main() {
  ...
  for (int i = 0; i < events.size(); i++) {
    if (events[i].second == -1) {
      ...
      if (querymax(qm[tren][1], qm[tren][3], 1, off - 1, 1) < inf &&
          querymin(qm[tren][1], qm[tren][3], 1, off - 1, 1) >= qm[tren][0])
        sol[tren] = true;
    }
  }
  events.clear();
  ...
  for (int i = 0; i < events.size(); i++) {
    if (events[i].second == -1) {
      ...
      int tren = events[i].second;
      if (querymax(qm[tren][0], qm[tren][2], 1, off - 1, 1) < inf &&
          querymin(qm[tren][0], qm[tren][2], 1, off - 1, 1) >= qm[tren][1])
        sol[tren] = true;
    }
  }
  for (int i = 0; i < q; i++) {
