...
struct SBT {
  ...
  inline void fixup(int x) {
    for (; x; x = t[x].fa) {
      ...
      if (t[z].sz < t[t[y].s[k]].sz)
        rotate(y), x = y;
      else if (t[z].sz < t[w].sz)
        rotate(w), rotate(w), x = w;
    }
  }
  inline void insert(int key) {
    if (rt == 0) return void(rt = new_node(0, key));
    int x = rt;
    while (t[x].s[key > t[x].key]) x = t[x].s[key > t[x].key];
    int p = new_node(x, key);
    fixup(t[x].s[key > t[x].key] = p);
  }
  inline void erase(int key) {
    int x = rt;
    while (key != t[x].key) x = t[x].s[key > t[x].key];
    if (t[x].s[0] && t[x].s[1]) {
      int y = t[x].s[1];
...
int main() {
  ...
  for (int i = 0; i <= m; ++i) {
    ...
    long long cnt1 = cnt + sbt.size();
    for (; poi <= q && que[poi].first <= cnt1; ++poi)
      ans[que[poi].second] = sbt.k_th(que[poi].first - cnt);
    cnt = cnt1;
  }
  for (; poi <= q; ++poi)
    ans[que[poi].second] = sbt.k_th((que[poi].first - cnt - 1) % n + 1);
