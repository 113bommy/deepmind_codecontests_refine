...
void dfs(int x) {
  ...
  for (int i = 0; i < v[x].size(); i++) {
    ...
    mi[x] += mi[t] + max(0ll, e[v[x][i]].w - (e[v[x][i]].p - mi[t]));
...
void dfs2(int x, long long W) {
  ...
  for (int i = 0; i < v[x].size(); i++) {
    t = e[v[x][i]].y;
    tmp -= mi[t] + max(0ll, e[v[x][i]].w - (e[v[x][i]].p - mi[t]));
  }
  long long tmp3;
  for (int i = 0; i < v[x].size(); i++) {
    t = e[v[x][i]].y;
    tmp3 = e[v[x][i]].w + min((long long)e[v[x][i]].p, mx[t]) -
           (mi[t] + max(0ll, e[v[x][i]].w - (e[v[x][i]].p - mi[t])));
    if (tmp3 >= tmp) {
      tmp3 = tmp;
      tmp = 0;
    } else {
    ...
    long long tmp4 =
        mi[t] + max(0ll, e[v[x][i]].w - (e[v[x][i]].p - mi[t])) + tmp3;
