...
int main() {
  ...
  for (int s = 2; s <= N; ++s) {
    for (int i = 1; i < s; ++i)
      ans[s] =
          (ans[s] + 1ll * strong[i] * cp[s][i] % MOD *
                        (1ll * i * (s - i) % MOD + 1ll * i * (i - 1) / 2 % MOD +
                         ans[i] + ans[s - i])) %
          MOD;
    ans[s] = (ans[s] + 1ll * s * (s - 1) / 2 * strong[s]) % MOD *
             inv((1 - strong[s] + MOD) % MOD) % MOD;
  }
