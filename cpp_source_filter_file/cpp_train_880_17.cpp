...
int main() {
  ...
  for (i = 1; i < q + 1; i++) {
    ...
    qx[y1].push_back(make_pair(make_pair(i, y), make_pair(x, x1)));
  }
  for (i = 1; i < n + 1; i++) {
    for (j = 0; j < rx[i].size(); j++) update(treey, 1, 1, m, ry[i][j], i);
  ...
  for (i = 1; i < m + 1; i++) {
    for (j = 0; j < ry[i].size(); j++) update(treex, 1, 1, n, rx[i][j], i);
    for (j = 0; j < qy[i].size(); j++) {
      if (query(treey, 1, 1, n, qy[i][j].second.first,
