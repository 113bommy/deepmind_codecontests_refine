...
struct seg2d {
  ...
  void SI(int s, int e, int x, int y, int p) {
    v->SI(1, 0, v->n.size() - 1,
          lower_bound(v->n.begin(), v->n.end(), y) - v->n.begin(), p);
    if (s == e) return;
    int m = s + e >> 1;
    if (x > m)
      r->SI(m + 1, e, x, y, p);
  ...
  int SM(int s, int e, int sx, int ex, int sy, int ey) {
    if (s > ex || sx > e) return -INF;
    if (s >= sx && e <= ex) {
      sy = lower_bound(v->n.begin(), v->n.end(), sy) - v->n.begin();
      ey = upper_bound(v->n.begin(), v->n.end(), ey) - v->n.begin();
      if (sy > ey) return -INF;
      return v->SM(1, 0, v->n.size() - 1, sy, ey - 1);
    }
    int m = s + e >> 1;
...
int main() {
  ...
  for (i = 0; i < M; i++) {
    cin >> nx[i] >> ny[i] >> nz[i];
    if (nz[i] <= mZ) MST.init(1, MX, nx[i], ny[i]);
    if (nz[i] >= mz) mst.init(1, MX, nx[i], ny[i]);
  }
  MST.init(1, MX, true);
  mst.init(1, MX, true);
