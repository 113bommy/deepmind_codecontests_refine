...
void Merge(int id, int l, int r) {
  ...
  for (int i = 0; i < pre[r].size(); i++)
    if (pre[id].size() < 13) pre[id] += pre[r][i];
  for (int i = (int)suf[l].size() - 1; i >= 0; i--)
    if (suf[id].size() < 13) suf[id] = suf[l][i] + suf[id];
}
int main() {
  int n, q, i;
  scanf("%i", &n);
  for (i = 1; i <= n; i++) {
    ...
    if (s.size() < 13)
      ...
      pre[i] = s.substr(0, 13), suf[i] = s.substr(s.size() - 13, 13);
  }
  scanf("%i", &q);
  while (q--) {
    n++;
