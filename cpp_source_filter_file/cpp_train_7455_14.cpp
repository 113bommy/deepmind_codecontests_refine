...
vector<edge> ed[MN];
vector<int> adj[MN];
long long st[3 * MN], lz[3 * MN], dp[MN], g[MN];
...
long long qu(int i, int s, int e, int ind) {
  upd_lz(i, s, e);
  if (s == e)
    return st[i];
  else if ((s + e) / 2 < ind)
    return qu(2 * i + 1, (s + e) / 2 + 1, e, ind);
  else
    return qu(2 * i, s, (s + e) / 2, ind);
}
void solve(int n) {
  for (auto v : adj[n]) {
    solve(v);
  ...
  dp[n] = g[n];
  for (auto e : ed[n])
    dp[n] = max(dp[n], e.w + qu(1, 1, N, vis[e.x][0]) +
                           qu(1, 1, N, vis[e.y][0]) + g[n]);
...
int main() {
  ...
  dfs(1, 0);
  for (j = 1; j < LG; j++) {
    for (i = 1; i <= N; i++) sp[j][i] = sp[j - 1][sp[j - 1][i]];
  }
  for (i = 1; i <= M; i++) {
    scanf("%d%d%d", &x, &y, &w);
    ed[lca(x, y)].push_back({x, y, w});
  }
  solve(1);
  printf("%lld\n", dp[1]);
  return 0;
