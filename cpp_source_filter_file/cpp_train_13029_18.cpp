...
int main() {
  ...
  for (int t = 0; t <= m; ++t) {
    add(val0,
        1ll * iW0 % 998244353 * f[m][t] % 998244353 * (m - t) % 998244353);
    add(val1, 1ll * iW1 % 998244353 * f[m][t] % 998244353 * t % 998244353);
  }
  for (int i = 1; i <= n; ++i)
    if (!A[i])
      ans[i] = (w[i] - 1ll * w[i] * val0 % 998244353 + 998244353) % 998244353;
    else
      ans[i] = (ans[i] + 1ll * w[i] * val1 % 998244353) % 998244353;
