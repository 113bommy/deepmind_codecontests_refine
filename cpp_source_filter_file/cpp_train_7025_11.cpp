#include <bits/stdc++.h>
using namespace std;
const int N = 200005, S = N * 2, K = 21, T = N * 17 * 2, C = 26;
int cnt, last, tot, q, m, pos[N], len[S], fa[S], f[S], root[S], s[T], lson[T],
    rson[T], ch[S][C];
char a[N];
vector<int> e[S];
int extend(int c) {
...
void insert(int l, int r, int &t, int x) {
  ...
  int mid = (l + r) >> 1;
  if (x <= mid)
    insert(l, mid, lson[t], x);
  else
    insert(mid + 1, r, rson[t], x);
...
void dfs(int u) {
  for (int v : e[u])
    f[v] = len[v] == len[u] ? f[u] : v, dfs(v), merge(root[u], root[v]);
}
int main() {
  scanf("%d%d", &m, &q);
  cnt = 1;
  for (int i = 1; i <= m; ++i) {
    scanf("%s", a + 1), last = 1;
    for (int j = 1; a[j]; ++j)
      insert(1, m, root[pos[i] = extend(a[j] - 'a')], i);
  }
