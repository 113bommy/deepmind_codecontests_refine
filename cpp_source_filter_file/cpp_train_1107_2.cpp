...
class MapManager {
 public:
  ...
  Edge<T> &operator[](int p) { return es[p]; }
};
class Dcmp {
 public:
  const static double eps;
  bool operator()(double x) { return fabs(x) >= eps; }
} dcmp;
const double Dcmp ::eps = 1e-8;
