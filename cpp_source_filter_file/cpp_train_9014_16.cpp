...
void Solve() {
  ...
  for (int i = 1; i <= n; i++)
    if (G[i].size() == 3 && dfs2(i, 0)) {
      for (int j = 1; j <= n; j++)
        if (!ans[j] && comp[j] == comp[i]) ans[j] = 1;
      SIK();
      return;
    }
  for (int i = 1; i <= n; i++)
    if (G[i].size() == 3) {
      ...
      for (int j = 0; j < 3; j++) {
        vec[j] = {i, G[i][j]};
        while (G[vec[j].back()].size() == 2)
          vec[j].push_back(G[vec[j].back()][0] ^ G[vec[j].back()][1] ^
                           vec[j][vec[j].size() - 2]);
      }
