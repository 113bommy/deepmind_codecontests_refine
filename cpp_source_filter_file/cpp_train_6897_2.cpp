...
struct Book {
  int t, a, b, idx;
  bool operator<(const Book &other) const { return t < other.t; }
} book[200005];
int cnt[4 * 200005], st[4 * 200005];
...
int main() {
  ...
  int ret = INT_MAX, retIdx = -1, retA = -1, retB = -1, retT = -1, aCnt = 0,
      bCnt = 0, ptrA = 0, ptrB = 0, cur = 0, tot = 0;
  ...
  if (retIdx == -1)
    ...
    cout << ret << "\n";
    set<int> st;
    for (int i = 0; i < retIdx; i++) st.insert(book[both[i].second].idx);
    for (int i = 0; i < retA; i++) st.insert(book[alice[i].second].idx);
    for (int i = 0; i < retB; i++) st.insert(book[bob[i].second].idx);
