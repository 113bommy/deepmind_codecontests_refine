...
long long n, cnt[18][M], f[18][M], inv2 = (MOD + 1) / 2;
long long ans, a[M], b[M], c[M], fb[M], bit[M];
...
signed main() {
  ...
  for (long long i = 0; i < 18; i++) fwt_xor(cnt[i], M, 1);
  for (long long i = 0; i < 18; i++)
    for (long long j = 0; j <= i; j++)
      for (long long k = 0; k < M; k++)
        f[i][k] = (f[i][k] + cnt[j][k] * cnt[i - j][k]) % MOD;
  for (long long i = 0; i < 18; i++) fwt_xor(f[i], M, -1);
  ...
  fwt_and(a, M, -1);
  for (long long i = 0; i < 18; i++) ans = (ans + a[1 << i]) % MOD;
  printf("%lld\n", ans);
}
