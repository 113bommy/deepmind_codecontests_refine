...
struct two_sat {
  ...
  void tarjan(int u) {
    ...
    if (low[u] == dfn[u]) {
      ++id;
      do instk[stk[top]] = 0, bl[stk[top]] = id;
      while (stk[top--] != u);
    }
  }
  bool Solve(int n) {
...
int main() {
  ...
  for (int i = 1; i < cnt; ++i)
    wish.add(i + 1, i), wish.add(tot + i, tot + i + 1),
        wish.add(i + cnt + 1, i + cnt),
        wish.add(tot + i + cnt, tot + i + cnt + 1);
  for (int i = 1; i <= cnt; ++i) {
    int x = lower_bound(a + 1, a + cnt + 1, t1 - a[i - 1]) - a;
    if (x <= cnt) {
      wish.add(tot + i, x + cnt), wish.add(tot + x + cnt, i);
      wish.add(tot + i + cnt, x), wish.add(tot + x, tot + i + cnt);
    }
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    if (x <= cnt)
      wish.add(x, tot + i + cnt * 2), wish.add(i + cnt * 2, tot + x),
          wish.add(cnt + x, i + cnt * 2),
          wish.add(tot + i + cnt * 2, tot + cnt + x);
  }
  if (wish.Solve(tot)) {
    puts("POSSIBLE");
    for (int i = cnt; i; --i)
