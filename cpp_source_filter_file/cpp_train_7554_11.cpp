...
map<tuple<int, int, int, int>, int> solve(vector<vector<int>> S,
                                          vector<vector<int>> T,
                                          bool full = false) {
  map<tuple<int, int, int, int>, int> MEM;
  int N = S.size() - 1, M = T.size() - 1;
  function<int(int, int, int, int)> solve = [&](int sa, int sp, int ta,
                                                int tp) {
    ...
    for (auto sx : S[sa])
      if (sx != sp) {
        ...
        vector<int> child;
        for (auto tx : T[ta])
          if (tx != tp) child.push_back(solve(sx, sa, tx, ta));
