...
template <class T>
vector<int> compress(const vector<T> &v, vector<T> *unq = NULL) {
  int n = v.size();
  ...
  if (unq) {
    ...
    cur = 0;
    for (int i = 0; i < n; i++)
      if (i == 0 || t[i - 1].first < t[i].first) (*unq)[cur++] = t[i].first;
  }
  return r;
}
template <class T>
struct RMQ {
  ...
  int min_i(int x, int y) {
    ...
    for (; x < y; x >>= 1, y >>= 1) {
      if (y & 1) {
        --y;
        if (p < 0 || A[I[y]] < A[p]) p = I[y];
      }
      if (x & 1) {
        if (p < 0 || A[I[x]] < A[p]) p = I[x];
        x++;
      }
    }
    return p;
...
int main() {
  ...
  N += 1;
  vector<int> unqA;
  vector<int> cA = compress(vector<int>(A, A + N), &unqA);
  for (int i = 1; i < N; i++) G[cA[i]].push_back(make_pair(B[i], i));
  for (int i = 0, i_len = (N); i < i_len; ++i) sort(G[i].rbegin(), G[i].rend());
  RMQ<int> rmq(vector<int>(N, INF));
  for (int i = 0, i_len = (N); i < i_len; ++i)
    if (G[i].size()) rmq.modify(i, G[i].back().first);
  ...
  while (!qu.empty()) {
    ...
    while (rmq.min_v(0, x) <= D[v]) {
      int i = rmq.min_i(0, x);
      int t = G[i].back().second;
      G[i].pop_back();
      if (G[i].empty())
        rmq.modify(i, INF);
      else
        rmq.modify(i, B[G[i].back().first]);
      from[t] = v;
      qu.push(t);
    }
  }
  if (from[N - 1] == -1)
    ...
    printf("%lu\n", v.size());
    for (int i = 0, i_len = (v.size()); i < i_len; ++i)
