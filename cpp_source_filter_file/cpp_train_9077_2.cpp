...
void update(int root) {
  ...
  if (tree[root << 1].mini == tree[root].mini)
    tree[root].cnt += tree[root << 1].cnt;
  if (tree[root << 1 | 1].mini == tree[root].mini)
    tree[root].cnt += tree[root << 1 | 1].cnt;
}
...
void modify1(int root, int pos, int w) {
  if (tree[root].lt + 1 == tree[root].rt)
    ...
    if (pos >= mid)
      modify1(root << 1 | 1, pos, w);
    else
      modify1(root << 1, pos, w);
    update(root);
  }
}
void modify2(int root, int lt, int rt, int w) {
  if (lt == tree[root].lt && rt == tree[root].rt)
    ...
    int mid = (tree[root].lt + tree[root].rt) >> 1;
    if (lt >= mid)
      modify2(root << 1 | 1, lt, rt, w);
    else if (rt <= mid)
      modify2(root << 1, lt, rt, w);
    else
      modify2(root << 1, lt, mid, w), modify2(root << 1 | 1, mid, rt, w);
    update(root);
  }
}
int query(int root, int lt, int rt) {
  if (lt == tree[root].lt && rt == tree[root].rt)
    ...
    if (lt >= mid)
      ...
      return query(root << 1, lt, mid) + query(root << 1 | 1, mid, rt);
  }
}
...
int main() {
  ...
  for (int i = 0; i <= n; i++) change(a[i], a[i + 1], 1), modify1(1, a[i], 1);
  while (q--) {
    int x, y;
    scanf("%d%d", &x, &y);
    y++;
    change(a[x - 1], a[x], -1), change(a[x], a[x + 1], -1),
        modify1(1, a[x], -1);
    a[x] = y;
