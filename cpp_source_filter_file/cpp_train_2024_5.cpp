...
using namespace std;
const int N = 200010;
const double arefa = 0.8;
int n, q, a[N];
int len, root, size[N], son[N][2], s[N], tot;
short num[N];
long long sum[N], dt[N];
bool b;
void read(int &a) {
  char ch;
...
void ins(int &i, int x, int dep) {
  ...
  if (x <= dt[i])
    ...
    ins(son[i][1], x, dep + 1);
  if (!b) return;
  if (size[son[i][0]] <= 0.8 * size[i] && size[son[i][1]] <= 0.8 * size[i])
    return;
  tot = 0;
  dfs(i);
  i = rebuild(1, tot);
...
double getans(long long v) {
  int i = root;
  long long cursum = 0, cursize = 0, tmpsum, tmpsize;
  while (i) {
    tmpsum = sum[son[i][0]] + dt[i] * num[i] + cursum;
    tmpsize = cursize + size[son[i][0]] + num[i];
    if (v >= (long long)dt[i] * (long long)tmpsize - tmpsum) {
      cursum = tmpsum;
      cursize = tmpsize;
      i = son[i][1];
    } else
