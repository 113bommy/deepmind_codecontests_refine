...
inline bool turnLD(int i, int j) {
  int d = abs(i - j) + 1;
  return d * 2 <= m && (L->lcp[j][m - d] >= d && l[1].lcp[j][d] >= d);
}
inline bool turnLU(int i, int j) {
  int d = abs(i - j) + 1;
  return d * 2 <= m && (L[1].lcp[j][m - d] >= d && l->lcp[j][d] >= d);
}
inline bool turnRD(int i, int j) {
  int d = abs(i - j) + 1;
  return d * 2 <= m && (l->lcp[i][m - 2 * d] >= d && L[1].lcp[i][0] >= d);
}
inline bool turnRU(int i, int j) {
  int d = abs(i - j) + 1;
  return d * 2 <= m && (l[1].lcp[i][m - 2 * d] >= d && L->lcp[i][0] >= d);
}
void DP() {
  int i, j, D;
  for (i = 0; i <= n; ++i)
    memset(f[0][i], 0, (m + 1) << 2), memset(f[1][i], 0, (m + 1) << 2);
  for (i = 0; i < n; ++i) {
    f[0][i][0] = f[1][i][0] = 1;
  ...
  if (!(m & 1) && m <= 2 * n)
    add(add(ans, turnLD(n - 1, n - m / 2)), turnLU(n - 1, n - m / 2));
}
int main() {
  int i;
  scanf("%s%s%s", *map, map[1], s), n = strlen(*map), m = strlen(s);
  if (m == 1)
    ans = std::count(*map, *map + n, *s) + std::count(map[1], map[1] + n, *s);
  else if (m == 2)
    for (i = 0; i < n; ++i)
    ...
    if (!(m & 1)) {
      for (i = m / 2 - 1; i < n; ++i)
        sub(sub(ans, turnLD(i, i - m / 2 + 1)), turnLU(i, i - m / 2 + 1));
      for (i = 0; i <= n - m / 2; ++i)
        sub(sub(ans, turnRD(i, i + m / 2 - 1)), turnRU(i, i + m / 2 - 1));
    }
  }
  printf("%d\n", ans);
  return 0;
