...
int main() {
  ...
  vector<vector<long long> > sq(n + 1, vector<long long>(1));
  vector<vector<long long> > dn(n + 1, vector<long long>(1));
  ...
  for (int i = 1; i <= n; i++) {
    ...
    ans = 0;
    if (L[k] != 0)
      ans += 1ll * max(0, min(R[k], r) - max(L[k], l) + 1) *
             max(0, r - max(l, T[k]) + 1);
    int L = 0, R = s[k].size();
    while (L + 1 < R) {
      int M = (L + R) / 2;
      if (s[k][M].x2 >= l && s[k][M].y2 >= l)
        R = M;
      else
        L = M;
    }
    if (L < s[k].size())
      ans += 1ll * max(0, min(r, s[k][L].y2) - max(l, s[k][L].y1) + 1) *
             max(0, min(r, s[k][L].x2) - max(l, s[k][L].x1) + 1);
    L++;
    if (L < s[k].size()) {
      ...
      if (L2 < s[k].size())
        ans += 1ll * max(0, min(r, s[k][L2].y2) - max(l, s[k][L2].y1) + 1) *
               max(0, min(r, s[k][L2].x2) - max(l, s[k][L2].x1) + 1);
      if (L < L2) {
        int L3 = L - 1, R3 = L2;
        while (L3 + 1 < R3) {
          int M3 = (L3 + R3) / 2;
