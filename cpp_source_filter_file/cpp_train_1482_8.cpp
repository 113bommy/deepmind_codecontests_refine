...
int _a[20005], tree[100005], tag[100005];
inline int gets(int i, int l, int r) { return sum[i][r] - sum[i][l - 1]; }
...
int main() {
  ...
  for (int i = 2; i <= n + 1; i++) {
    ...
    for (int j = 1; j <= m; j++) _a[j] = f[i - 1][j];
    build(1, 1, m);
    for (int j = 1; j <= m; j++) {
      update(1, 1, m, j, min(j + k - 1, m), -a[i - 1][j]);
      f[i][j] = gets(i - 1, max(j - k + 1, 1), j) +
                gets(i, max(j - k + 1, 1), j) + query();
      ans = max(ans, f[i][j]);
