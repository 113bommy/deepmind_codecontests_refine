...
string POSSIBLE[2] = {"IMPOSSIBLE\n", "POSSIBLE\n"};
bool dfs(int v, int &T, vector<vector<int> > &g, vector<int> &vis,
         vector<int> &mt) {
...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      for (int k = 0; k < 4; k++) {
        int nx = i + cx[k], ny = j + cy[k];
        if (0 <= nx && nx < n && 0 <= ny && ny < m && A[nx][ny] == A[i][j])
          g[i * m + j].push_back(nx * m + j);
      }
    }
  }
  int ok = 1, T = 1;
  vector<int> vis(n * m), mt(n * m);
  auto pairing = [&](vector<int> L, vector<int> R) {
    vector<int> use(n * m);
    for (auto v : R) use[v] = 1, mt[v] = -1;
    for (auto v : L) {
      vector<int> act;
      for (auto u : g[v])
        if (use[u]) act.push_back(u);
      g[v] = act;
    }
    sort((L).begin(), (L).end(), [&](int v, int u) { return bad[v] > bad[u]; });
    vector<int> res;
    for (auto v : L) {
      if (dfs(v, T, g, vis, mt))
        res.push_back(v);
  ...
  vector<int> x = pairing(L, R);
  vector<int> y = pairing(R, x);
  if (!ok)
    cout << "NO\n";
  else {
    cout << "YES\n";
