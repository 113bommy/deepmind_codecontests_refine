...
int main() {
  ...
  int res = 0;
  for (int i = 1; i <= n; ++i) {
    int t = 1LL * C(n, i) * power(3, M - 1 - (1LL * n * i) % (M - 1)) % M *
            (power(M + 1 - power(3, M - 1 + i - n), n) - 1) % M;
    if (i & 1)
      now += t;
    else
      now -= t;
  }
  res = 1LL * now * power(3, (1LL * n * n + 1) % (M - 1)) % M;
  (res -= 2LL * power(3, 1LL * n * n % (M - 1)) % M *
          (power(M + 1 - power(3, M - n), n) - 1) % M) %= M;
  printf("%d\n", (res + M) % M);
  return 0;
}
