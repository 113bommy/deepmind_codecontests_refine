...
int main(){
  ...
  rep(q,0,Q - 1){
    ...
    if(x == 3){
      ...
      while(y < z){
        if(y % Interval == 0 && y + Interval <= z){
          if(lazy[y / Interval] == 0) ans = min(ans , Ab[y / Interval]);
          else ans = min(ans , Bb[y / Inteval]);
          y += Interval;
        }
        else{
          LAZY(y / Interval);
    ...
    if(x == 4){
      ...
      while(y < z){
        if(y % Interval == 0 && y + Interval <= z){
         if(lazy[y / Interval] == 0) ans = min(ans , Bb[y / Interval]);
          else ans = min(ans , Ab[y / Inteval]);
          y += Interval;
        }
        else{
          LAZY(y / Interval);
