...
const long long maxn = 300010, MOD = 1e10 + 7;
...
long long M(long long x) { return x >= MOD ? x - MOD : x; }
...
signed main() {
  ...
  for (long long i = 1; i <= cnt; i++) {
    x = 1 - x;
    dp[x][0] =
        (1ll * ans[i][0] * dp[1 - x][0] + 1ll * ans[i][1] * dp[1 - x][1]) % MOD;
    dp[x][1] =
        (1ll * ans[i][1] * dp[1 - x][0] + 1ll * ans[i][0] * dp[1 - x][1]) % MOD;
  }
