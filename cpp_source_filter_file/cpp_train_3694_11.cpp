...
void mul(int a[], int b[], int c[], int f[], int len) {
  ...
  memset(X, 0, sizeof(X));
  for (int i = 0; i < len; ++i)
    for (int j = 0; j < len; ++j) X[i + j] = (X[i + j] + a[i] * b[j]) % mod;
  for (int i = 2 * len - 2; i >= len; --i)
    if (X[i]) {
      for (int j = 1; j < len; ++j)
        X[i - j] = (X[i - j] + X[i] * f[len - 1 - j]) % mod;
      X[i] = 0;
    }
  memcpy(c, X, len << 2);
}
int BM(int a[], int lena, int n) {
  ...
  for (int i = 0; i <= lena; ++i) {
    ...
    for (int j = 0; j < lenf1; ++j)
      nf[j + i - p] = (nf[j + i - p] + res * f1[j]) % mod;
    ...
    for (int j = 1; j <= n; ++j)
      for (int k = j; k <= n; ++k)
        if (dp[t ^ 1][j][k]) {
          if (s[j] == s[k]) {
            if (j + 1 >= k)
              add(dp[t][0][0], dp[t ^ 1][j][k]);
            else
              add(dp[t][j + 1][k - 1], dp[t ^ 1][j][k]);
