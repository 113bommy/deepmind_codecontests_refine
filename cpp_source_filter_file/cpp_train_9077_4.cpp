...
void update(int l, int r, int v) {
  if (l == r) return;
  if (l > r) swap(l, r);
  update(1, 0, 1000000, l, r - 1, v);
}
int main() {
  n = read(), m = read(), a[0] = 1000000;
  for (int i = 1; i <= n; i++) a[i] = read();
  for (int i = 0; i <= n; i++)
    update(a[i], a[i + 1], 1), update(1, 0, 1000000, a[i], 1);
  while (m--) {
    int x, k;
    x = read(), k = read();
    update(a[x - 1], a[x], -1), update(a[x], a[x + 1], -1),
        update(1, 0, 1000000, a[x], -1), a[x] = k;
    update(a[x - 1], a[x], 1), update(a[x], a[x + 1], 1),
        update(1, 0, 1000000, a[x], 1),
        printf("%d\n", query(1, 0, 1000000, 1, 1000000 - 1));
  }
  return 0;
}
