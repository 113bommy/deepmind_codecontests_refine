...
pair<int, int> rec(int u) {
  ...
  for (int i = 0; i < (int)tree[u].size(); ++i) {
    ...
    if (dp[v].first == -1) {
      p = rec(v);
      if (p.first < save[u].first)
        save[u].first = p.first, save[u].second = p.second;
      else if (p.first == save[u].first)
        save[u].second = min(save[u].second, p.second);
    } else if (dp[v].first < save[u].first)
      save[u].first = dp[v].first, save[u].second = dp[v].second;
    else if (dp[v].first == save[u].first)
      save[u].second = min(save[u].second, dp[v].second);
  }
  return ret = save[u];
}
void topsort(void) {
