    ...
    for (int i = 1; i <= n; i++)
      for (int j = i + 1; j <= n; j++) {
        ...
        for (int sum = j + 1; sum <= i + n; sum++) {
          dp[cs][i][j] += (1.0 - dp[cs ^ 1][sum - j][sum - i]) *
                          (min(i, sum - j) - max(1, sum - n) + 1);
        }
        for (int sum = i + 1; sum <= i + j - 1; sum++)
          dp[cs][i][j] += dp[cs ^ 1][sum - i][j] *
                          (min(i, sum - i) - max(1, sum - j + 1) + 1);
        for (int sum = j + i + 1; sum <= n + j; sum++)
          dp[cs][i][j] += dp[cs ^ 1][i][sum - j] *
                          (min(j, sum - j) - max(i + 1, sum - n) + 1);
        dp[cs][i][j] /= c2(n + 1);
      }
  }
  double ans = 0;
  for (int i = 1; i <= n; i++)
    for (int j = i + 1; j <= n; j++) ans += dp[cs ^ 1][i][j];
  printf("%.12f\n", ans);
}
