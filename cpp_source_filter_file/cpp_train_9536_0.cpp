...
int main() {
  ...
  sort(vs.begin(), vs.end(),
       [](const abc &l, const abc &r) { return l.c < r.c; });
  vi amax(n, 0LL), bmax(n, 0LL);
  amax.back() = vs.back().a;
  bmax.back() = vs.back().b;
  for (int i = n - 2; i >= 0; --i)
    amax[i] = max(amax[i + 1], vs[i].a), bmax[i] = max(bmax[i + 1], vs[i].b);
  ll cur = (p - amax[0]) * (q - amax[0]), ans = 0LL;
  map<ll, ll> pareto;
  ...
  for (ll c = 1LL; c <= r; ++c) {
    ...
    for (; i < n && vs[i].c == c; ++i) {
      ...
      cur += insert_in_front(pareto, ai, bi, amax[i], bmax[i]);
      ...
      while (it->first > at) {
        ...
        --it2;
        ll h = it->second;
        cur += (min(af, it->first) - max(at, it2->first)) * (q - max(h, bf));
        it = it2;
      }
      ll l = 0LL, r = p;
      while (l < r) {
      ...
      while (it->second > bt) {
        ...
        ++it2;
        ll h = it->first;
        cur += (min(bf, it->second) - max(bt, it2->second)) * (p - max(h, at));
        it = it2;
      }
    }
  }
