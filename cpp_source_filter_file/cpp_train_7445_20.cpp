...
int b[400005];
vector<int> u[400005];
int ei(int i, int j) { return lower_bound(e, e + ne, make_pair(i, j)) - e; }
int cdp(int i, int j) {
  int ed = ei(i, j);
  ...
  if (nv[j] <= 2) {
    ...
    if (nv[j] == 2) {
      tt[j] = 0;
      for (int z = (0); z < (3); z++) bex[j][z] = make_pair(0, -z - 1);
      for (int k : u[j]) {
        int oe = cdp(j, k);
        if (mxv[oe] == ss[oe]) {
          tt[j] += mxv[oe];
        } else {
          bex[j][2] = make_pair(mxv[oe], 2);
          sort(bex[j], bex[j] + 3);
          reverse(bex[j], bex[j] + 3);
        }
      }
    }
  } else {
