...
int en[300007];
int cost[300007];
vector<int> add[300007];
vector<int> rem[300007];
int qpos[300007];
long long dp[300007];
class Tree {
 public:
  long long tr[3 * 300007];
  long long aux[3 * 300007];
  ...
  void range_update(int where, int IL, int IR, int CURL, int CURR,
                    long long val) {
    if (IR < CURL || CURR < IL) {
      return;
    }
  ...
  long long query(int where, int IL, int IR, int CURL, int CURR) {
    ...
    if (CURL <= IL && IR <= CURR) {
      return min(tr[where], inf);
    }
    int mid = (IL + IR) / 2;
    double ret = inf;
    double cur = min(query(2 * where, IL, mid, CURL, CURR),
...
void calc(int vertex, int prv) {
  ...
  for (i = 0; i < sz; i++) {
    ...
    w.range_update(1, 1, m, st[v[vertex][i]], en[v[vertex][i]],
                   sm - dp[v[vertex][i]]);
  }
