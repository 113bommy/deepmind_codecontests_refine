...
void update(long long l, long long r, long long now, long long ll, long long rr,
            long long v) {
  if (lazy[now]) {
    tree[now] = lazy[now];
...
long long query(long long l, long long r, long long now, long long ll,
                long long rr) {
  if (lazy[now]) {
    tree[now] = lazy[now];
    if (l != r) lazy[now * 2] = lazy[now * 2 + 1] = lazy[now];
...
int main() {
  long long n, m, u, v;
  scanf("%lld %lld", &n, &m);
  for (long long i = 1; i <= n; i++) scanf("%lld", &c[i]), c[i]--;
