...
double dp[55][1 << 16], g[55][1 << 16], p[1 << 16];
...
} A[1 << 17], B[1 << 17], omega[1 << 17], inv[1 << 17];
void init(int lim) {
...
void cdq(int l, int r) {
  ...
  cdq(mid + 1, r);
  int lim = 1;
  while (lim <= 2 * r - mid - l - 1) lim <<= 1;
  init(lim);
  for (int i = 1; i <= m; i++) {
    int v = Edge[i].v;
    for (int j = 0; j < lim; j++) A[j].x = A[j].y = B[j].x = B[j].y = 0;
    for (int j = mid + 1; j <= r; j++) A[j - mid - 1].x = dp[v][j];
    for (int j = l; j <= r; j++) B[r - j].x = Edge[i].p[j - l];
    fft(A, omega, lim);
    fft(B, omega, lim);
