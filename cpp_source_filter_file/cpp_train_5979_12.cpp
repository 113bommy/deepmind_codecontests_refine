...
info active[nmax * 4];
int id_other;
pair<pair<int, int>, int> other[nmax * 4];
int tree[16 * nmax];
void update(int node, int l, int r, int pos, int val) {
  ...
  int av = (l + r) / 2;
  if (pos <= av)
    update(node * 2, l, av, pos, val);
  else
    update(node * 2 + 1, av + 1, r, pos, val);
...
int query(int node, int l, int r, int lq, int rq) {
  ...
  if (av < rq)
    ret = max(ret, query(node * 2 + 1, av + 1, r, max(av + 1, lq), rq));
  return ret;
}
bool cmp(info l, info r) { return l.y1 < r.y1; }
bool cmp2(pair<pair<int, int>, int> u, pair<pair<int, int>, int> v) {
