...
int main() {
  int Q, j = 0, t = 0;
  scanf("%d", &Q);
  for (int i = (int)(1); i <= (int)(4 * Q); i++) tg[i] = -1;
  for (int i = (int)(1); i <= (int)(Q); i++) {
    ...
    for (; j && s[j + 1] != s[i]; j = nxt[j])
      change(1, 1, Q, i - j, -1), e[i].push_back(j);
    if (i != 1 && s[j + 1] == s[i]) ++j;
    for (auto d : e[j]) e[i].push_back(d), change(1, 1, Q, i - d, -1);
    nxt[i] = j;
    for (; t && v[q[t]] >= v[i]; --t)
      ;
    change(1, 1, Q, q[t] + 1, i, v[i]);
    q[++t] = i;
    ans1 = (ans1 + sum[1]) % 26;
    ...
    for (int i = (int)(1); i <= (int)(8); i++)
      res[i + 1] += res[i] / 10000, res[i] %= 10000;
    for (; !res[tp]; --tp)
      ;
    printf("%lld", res[tp]);
    for (int j = (int)(tp - 1); j >= (int)(1); j--) printf("%04lld", res[j]);
    puts("");
  }
}
