...
inline long long check(long long l, long long r) {
  ...
  long long zs = a[r];
  for (int i = r; r >= l; i--)
    if (a[i] == 1) return check(l, r - 1);
  for (long long i = r - 1; i >= l; i--)
    if (a[i] > 1) {
      if (log(p) / log(a[i]) < zs - 1)
        return p + 1;
      else
        zs = ksm(a[i], zs, 1e18);
    } else
...
long long solve(long long l, long long r, long long mod) {
  if (a[l] % mod == 0)
    ...
    return ksm(a[l], a[r], mod);
  else if (l == r)
    return a[l] % mod;
  long long t = check(l + 1, min(l + 5, r));
  if (t > phi[mod])
    return ksm(a[l], solve(l + 1, r, phi[mod]) % phi[mod] + phi[mod], mod);
  else
    return ksm(a[l], t, mod);
}
...
int main() {
  n = read();
  long long mod = read();
  for (long long i = mod; i > 1; i = phi[i]) phi[i] = calc(i);
  phi[1] = 1;
