 ...
 public:
  ...
  void set(const int &position, int value) {
    for (int i = 0; i < kPeriod; ++i) {
      ...
      value = (((value * value) % kModulo) * value) % kModulo;
    }
    for (int i = (position + size_) / 2; i > 0; i /= 2) update(i);
  }
  int query(int left, int right) const {
    return query(1, 0, size_, left, right, 0);
  }
  void pow(int left, int right) { pow(1, 0, size_, left, right); }

 private:
  void update(int node) {
    for (int i = 0; i < kPeriod; ++i)
      data_[node][i] =
          (data_[node * 2][(i + lazy_[node * 2]) % kPeriod] +
           data_[node * 2 + 1][(i + lazy_[node * 2 + 1]) % kPeriod]) %
          kModulo;
  }
  ...
  void pow(int node, int begin, int end, int left, int right) {
    ...
    int mid = (begin + end) / 2;
    if (left < mid) pow(node * 2, begin, mid, left, right);
    if (mid < right) pow(node * 2 + 1, mid, end, left, right);
    update(node);
  }
  int size_;
