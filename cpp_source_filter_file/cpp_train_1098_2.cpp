...
struct segment_tree {
  ...
  void push_down(int pos) {
    if (tree[pos].mark != -1) {
      ...
      tree[pos << 1].mark %= 100000007;
      tree[pos << 1 | 1].mark = tree[pos].mark;
      tree[pos << 1 | 1].mark %= 100000007;
      tree[pos << 1].sum = tree[pos].mark * tree[pos << 1].len();
      tree[pos << 1].sum %= 100000007;
      tree[pos << 1 | 1].sum = tree[pos].mark * tree[pos << 1 | 1].len();
      tree[pos << 1 | 1].sum %= 100000007;
      tree[pos].mark = -1;
    }
  }
  void update(int L, int R, long long val, int pos) {
    if (L <= tree[pos].l && R >= tree[pos].r) {
      tree[pos].mark = val;
      tree[pos].mark %= 100000007;
      tree[pos].sum = val * tree[pos].len();
      tree[pos].sum %= 100000007;
      return;
    }
    push_down(pos);
...
int main() {
  ...
  printf("%I64d\n", T.query(last.first + 1, m, 1) % 100000007);
}
