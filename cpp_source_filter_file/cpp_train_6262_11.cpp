...
long long pow(long long x, long long k, long long p) {
  long long ans = 1;
  for (long long i = k; i; i >>= 1, x = (x * x) % p)
    if (i & 1) ans = ans * x % p;
  return ans;
}
inline void pmod(long long &x) { x = (x >= mod ? x - mod : x); }
int n;
int a[MAXN], p[MAXN], cnt[MAXN];
long long dp[MAXN], sum[MAXN], inv[MAXN];
void init() {
  scanf("%d", &n);
  for (int i = 1; i <= n; i++)
    scanf("%d", &a[i]), cnt[a[i]]++, inv[i] = pow(i, mod - 2, mod - 2);
  sort(a + 1, a + n + 1);
  p[n] = n + 1;
  cnt[0] = 1;
