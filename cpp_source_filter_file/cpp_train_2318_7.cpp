...
Per operator+(Per a, Per p) {
  Per ret;
  for (int i = 1; i <= K; i++) ret[i] = a[p[i]];
  return ret;
}
int Transform(Per num) {
  int ret = 0;
  for (int i = 1; i <= K; i++) ret = ret << 1 | num[i];
  return ret;
}
int main() {
  ...
  for (int i = 1, a, b; i <= N; i++)
    read(a), read(b), suf[i] = Permutation(), swapp(suf[i][a], suf[i][b]);
  suf[N + 1] = Permutation();
  for (int i = N; i; i--) suf[i] = suf[i] + suf[i + 1];
  int upper = 1 << K;
  for (int S = 0; S < upper; S++) f[0][S] = INF, f[1][S] = -INF;
  for (int i = N + 1; i; i--) {
  ...
  int a = __builtin_popcount(Transform(s)),
      b = __builtin_popcount(Transform(t)), c, ans = 0, L, R, val;
  for (int S = upper - 1; S; S--) {
    ...
    if (f[1][S] - f[0][S] >= M) {
      ...
      val = 2 * c - a - b + K;
      if (val > ans) L = f[0][S], R = f[1][S], ans = val;
    }
  }
  write(ans), putchar('\n');
