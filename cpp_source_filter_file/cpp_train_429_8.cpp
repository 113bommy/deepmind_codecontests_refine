...
void doit() {
  ...
  for (int i = 0; i <= n; i++) inver[i] = qpow(pw[i], mod - 2);
  for (int i = 1; i <= n; i++) {
  ...
  for (int i = n / c + 1; i <= n; i++) printf("%d ", 0);
  puts("");
}
}  // namespace Sol1
namespace Sol2 {
    ...
    for (int j = 0; j <= i / c; j++) {
      for (int s = 0; s < (1 << c); s++)
        if (dp[cur ^ 1][j][s]) {
          ...
          if ((s | (1 << (a[i + 1] - 1))) == lim)
            dp[cur][j + 1][0] = add(dp[cur][j + 1][0], v);
          else
            dp[cur][j][s | (1 << (a[i + 1] - 1))] =
                add(dp[cur][j][s | (1 << (a[i + 1] - 1))], v);
        }
    }
  }
  for (int i = 0; i <= n; i++) {
