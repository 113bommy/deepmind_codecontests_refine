...
struct poi {
  ...
  void r() { cin >> x >> y >> z; }
  void w() { cout << x << ' ' << y << ' ' << z << '\n'; }
  poi operator-(poi p) { return {x - p.x, y - p.y, z - p.z}; }
  poi operator+(poi p) { return {x + p.x, y + p.y, z + p.z}; }
  poi operator*(db d) { return {x * d, y * d, z * d}; }
  db dot(poi p) { return x * p.x + y * p.y + z * p.z; }
  poi cross(poi p) {
...
db solve1(poi p, poi v, poi o, db r) {
  if (sgn((o - p).len(), r) <= 0) return 0;
  poi proj = o.proj(p, p + v);
  if (sgn((o - proj).len(), r) > 0) return 1e10;
  if (sgn((proj - p).dot(v)) < 0) return 1e10;
  db h = (o - proj).len();
  db s = isqrt(r * r - h * h);
  return ((proj - p).len() - s) / v.len();
}
db solve2(poi p, poi v, poi a, poi b, db r) {
  if (sgn(v.cross(b - a).len()) == 0) return 1e10;
  poi hor = (b - a) * (v.dot(b - a) / (b - a).len2());
  poi ver = v - hor;
  db ret = solve1(p, ver, p.proj(a, b), r);
  if (ret == 1e10) return ret;
  poi q = p + v * ret;
  db t = (q - a).dot(a) / (b - a).len2();
  if (sgn(t) >= 0 && sgn(t, 1) <= 0) return ret;
  return 1e10;
}
int main() {
