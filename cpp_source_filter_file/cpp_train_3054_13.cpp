...
int32_t main() {
  ...
  for (int i = 1; i <= n; ++i)
    if (stid[i] && edid[i]) mat[stid[i]][edid[i]] = 1;
  auto calc = [&](int a, int b, int c, int d) -> int {
    int x = (1ll * a * d - 1ll * b * c) % p;
    return x < 0 ? x + p : x;
  };
  for (; z > 2; --z) {
    ...
    if (pi != 1) {
      offset *= -1;
      for (int j = 1; j <= z; ++j) swap(mat[1][j], mat[pi][j]);
    }
    if (pj != 1) {
      offset *= -1;
      for (int i = 1; i <= z; ++i) swap(mat[i][1], mat[i][pj]);
    }
    offset *= pw(inv(mat[1][1]), z - 2, p);
    if (offset < 0) offset += p;
    for (int i = 2; i <= z; ++i)
      for (int j = 2; j <= z; ++j)
        mat[i][j] = calc(mat[1][1], mat[1][j], mat[i][1], mat[i][j]);
    for (int i = 1; i < z; ++i)
      for (int j = 1; j < z; ++j) mat[i][j] = mat[i + 1][j + 1];
