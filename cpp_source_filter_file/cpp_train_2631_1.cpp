...
struct DSU {
  ...
  vector<int> sz, ez;
  DSU(int n)
      : n(n), link(n, -1), dp(n, vector<long long>(1, 1)), sz(n, 1), ez(n, 0) {}
  ...
  void Union(int a, int b) {
    ...
    ez[b] += 1;
    if (ez[b] == sz[b] * (sz[b] - 1) / 2) dp[b][0] = 1;
  }
};
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < i; ++j) all[m[i][j] - 1] = {i, j};
  }
  DSU dsu(n);
  for (auto [i, j] : all) dsu.Union(i, j);
  for (auto x : dsu.dp[dsu.Find(0)]) cout << x << " ";
  cout << '\n';
  return 0;
}
