...
struct pt {
  ...
  bool operator==(pt p) { return abs(x - p.x) < 1e-12 && abs(y - p.y) < 1e-12; }
  ...
  bool operator<(pt p) const {
    return x < p.x || (abs(x - p.x) < 1e-12 && y < p.y);
  }
  bool left(pt p, pt q) { return (q - p) % (*this - p) >= 1e-12; }
...
int main() {
  ...
  set<int> ans;
  for (auto p : concha) {
    if (p.x - 1e-12 < downmost.x && p.y - 1e-12 < leftmost.y &&
        (p - leftmost) % (downmost - leftmost) >= -1e-12) {
      int xx = (1. / p.x + 1e-3);
      int yy = (1. / p.y + 1e-3);
