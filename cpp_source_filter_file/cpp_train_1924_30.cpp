...
int main() {
  ...
  for (int steps = 0; steps < min(850, k); ++steps, swap(u, v)) {
    calcSums();
    ...
    for (int i = 1; i <= n; ++i) {
      for (int j = 1; j <= n; ++j) {
        ...
        }
        {
          if (i <= j - i) {
            d[v][oldi][oldj] +=
                (sa[s][j][j - i] - sa[s][j][i - 1]) * (double)(i);
          } else {
            d[v][oldi][oldj] +=
                (sa[s][j][i - 1] - sa[s][j][j - i - 1]) * (double)(j - i);
          }
          {
            int l = max(j - i + 1, i) - 1;
            d[v][oldi][oldj] += (sa[s][j][j - 1] - sa[s][j][l]) * (double)(j) -
                                (sb[s][j][j - 1] - sb[s][j][l]);
            int r = min(i - 1, j - i - 1);
            d[v][oldi][oldj] += sb[s][j][r];
          }
          if (n + i + 1 - j <= j) {
            d[v][oldi][oldj] += (sa[1 - s][i][j] - sa[1 - s][i][n + i - j]) *
                                (double)(n - j + 1);
          } else {
            d[v][oldi][oldj] +=
                (sa[1 - s][i][n + i + 1 - j] - sa[1 - s][i][j]) *
                (double)(j - i);
          }
          {
            int r = min(j, n + i + 1 - j - 1);
            d[v][oldi][oldj] +=
                (sa[1 - s][i][r] - sa[1 - s][i][i - 1]) * (double)(-i) +
                (sb[1 - s][i][r] - sb[1 - s][i][i - 1]);
            int l = max(j + 1, n + i + 1 - j + 1);
            d[v][oldi][oldj] +=
                (sa[1 - s][i][n] - sa[1 - s][i][l - 1]) * (double)(n + 1) -
                (sb[1 - s][i][n] - sb[1 - s][i][l - 1]);
          }
        }
        if (s) {
          swap(i, j);
        ...
        {
          int ways = c[min(i, j) - 1] + c[max(i, j) - min(i, j) - 1] +
                     c[n - max(i, j)];
