...
void FWT(int len, T *f) {
  ...
  for (i = 0; i < len; ++i)
    for (j = 0; j < 1 << len; ++j)
      if (!(j >> i & 1)) r = f[j | 1 << i], f[j | 1 << i] = f[j] - r, f[j] += r;
}
int main() {
  int i, j, k, fy[8], a, b, c;
  long long r, iv;
  scanf("%d%d%lld%lld%lld", &n, &K, &X, &Y, &Z);
  Fy[0] = (-X - Y - Z) % mod, Fy[1] = (X - Y - Z) % mod,
  Fy[2] = (-X + Y - Z) % mod, Fy[3] = (X + Y - Z) % mod,
  Fy[4] = (-X - Y + Z) % mod, Fy[5] = (X - Y + Z) % mod,
  Fy[6] = (-X + Y + Z) % mod, Fy[7] = (X + Y + Z) % mod;
  for (i = 0; i < n; ++i) {
    scanf("%d%d%d", &a, &b, &c);
    ++A[a], ++B[b], ++C[c], ++AB[a ^ b], ++BC[b ^ c], ++CA[c ^ a],
        ++ABC[a ^ b ^ c];
  }
  FWT(K, A), FWT(K, B), FWT(K, C), FWT(K, AB), FWT(K, BC), FWT(K, CA),
      FWT(K, ABC);
  for (i = 0; i < 1 << K; ++i) {
    *fy = n;
    fy[1] = A[i];
