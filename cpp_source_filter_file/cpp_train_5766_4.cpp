...
struct Point {
  ...
  Point transform() const { return Point(x + y, x - y); }
  Point inverse() const { return Point(x + y >> 1, x - y >> 1); }
};
...
long long get_hash(const Point &p, const Point &d) {
  long long ret = p.x;
  ret *= m + 2;
  ret += p.y;
  ret <<= 1;
...
bool has_block(int x, int y) {
  return std::binary_search((blocks).begin(), (blocks).end(), Point(x, y));
}
std::set<long long> state_hashes;
const int LARGE = 1000000;
struct Event {
  int x_1, x_2, y, d;
  Event(int x_1, int x_2, int y, int d)
      : x_1(x_1), x_2(x_2), y(y + LARGE >> 1), d(d) {}
};
bool operator<(const Event &a, const Event &b) {
  return a.y < b.y || (a.y == b.y && a.d < b.y);
}
...
int main() {
  ...
  for (__typeof((blocks).begin()) iter = (blocks).begin();
  ...
  for (int t = 0; t < 2; ++t) {
    for (__typeof((indices[t]).begin()) iter = (indices[t]).begin();
         iter != (indices[t]).end(); ++iter) {
      std::sort((iter->second).begin(), (iter->second).end());
    }
  }
  Point now(s_x, s_y);
  Point now_d(names[0] == 'S' ? 1 : -1, names[1] == 'E' ? 1 : -1);
  std::vector<Event> events[2];
  while (true) {
    long long hash = get_hash(now, now_d);
    ...
    Point r(*(q.x > 0 ? upper_bound((v).begin(), (v).end(), p.x)
                      : lower_bound((v).begin(), (v).end(), p.x) - 1) -
                q.x,
            p.y);
    if (t) {
      std::swap(p.x, p.y);
