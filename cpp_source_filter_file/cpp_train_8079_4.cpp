...
int discr(std::vector<std::pair<vec, int> > v, int* tar) {
  const int L = v.size();
...
int nl;
void add(int i, int v) { add(1, 1, nl, ys[i], tr(v, xs[i])); }
long long R;
bool incir(vec x, vec y) { return (x * y) < 0 || (x ^ y) <= 0; }
int trans[MAXN];
int main() {
  ...
  std::cin >> R >> n;
  vec pol(-R, 0), por(R, 0);
  std::vector<std::pair<vec, int> > ll;
  for (int i = 1, bak = 0; i <= n; ++i) {
    ...
    if (opt[i] == 1) {
      ...
      xs[i] = trans[xs[i]], ys[i] = trans[ys[i]];
  }
  static int ls[MAXN << 1], app[MAXN];
  nl = discr(ll, ls);
  for (int i = 1; i <= n; ++i)
    if (opt[i] == 1) xs[i] = ls[i], ys[i] = ls[i + n];
  for (int T = 1; T <= n; ++T) {
    if (opt[T] == 1)
      add(T, 1), app[T] = true;
    else if (opt[T] == 2)
      add(xs[T], -1), app[xs[T]] = false;
    else {
      bool can = true;
      int A = xs[T], B = ys[T];
