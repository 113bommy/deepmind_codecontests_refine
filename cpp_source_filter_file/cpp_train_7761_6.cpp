...
int n, m, k;
int kek;
vector<pair<int, int> > z[1000];
pair<int, int> z1[1000];
int l = 0;
...
void seak(int i, int j) {
  a[i][j] = '*';
  if (a[i - 1][j] == '.' && i - 1 >= 0) sea(i - 1, j);
  if (a[i][j - 1] == '.' && j - 1 >= 0) sea(i, j - 1);
  if (a[i][j + 1] == '.' && j + 1 < m) sea(i, j + 1);
  if (a[i + 1][j] == '.' && i + 1 < n) sea(i + 1, j);
  return;
}
int main() {
  cin >> n >> m >> k;
