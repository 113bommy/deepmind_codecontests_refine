...
int64 ModInverse(const int64 x) {
  int64 u = x;
  int64 v = kModulo;
  int64 p1 = 1, q1 = 0;
  int64 p2 = 0, q2 = 1;
  while (v != 0) {
    const int64 q = u / v;
    const int64 p3 = p1 - q * p2;
    const int64 q3 = q1 - q * q2;
    const int64 r = u - q * v;
    u = v;
    v = r;
    p1 = p2;
...
vector<int64> ComputeCoefs(const vector<int>& phi) {
  ...
  vector<int64> c(n + 1);
  for (int g = 1; g <= n; ++g) {
    const int64 cur = (g * ModInverse(phi[g])) % kModulo;
    for (int d = g; d <= n; d += g) {
      c[d] += mu[d / g] * cur;
      if (c[d] < 0) {
        c[d] += kModulo;
  ...
  int Build() {
    ...
    for (int layer = 0; layer <= max_layer; ++layer) {
      ...
      for (int i = 1; i <= n; ++i) {
        ...
        phi_times_depth[i - 1] = (2 * phi_[i] * root_depth_[u]) % kModulo;
      }
      for (int g = 1; g <= n; ++g) {
        for (int i = g; i <= n; i += g) {
    ...
    answer_ = (answer_ * ModInverse(n)) % kModulo;
    answer_ = (answer_ * ModInverse(n - 1)) % kModulo;
    if (answer_ < 0) {
