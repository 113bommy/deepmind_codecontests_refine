...
void getHull() {
  ...
  for (int i = 0; i < M + 2; i++) {
    ...
    while (hull.size() >= 2 && getCross(minA[hull[hull.size() - 2]],
                                        minA[hull[hull.size() - 1]], minA[i]))
      hull.pop_back();
    hull.push_back(i);
  }
}
int main() {
  int x, y;
  cin >> N >> M >> x >> y;
  for (int i = 0; i < N; i++) cin >> maxA[i].x >> maxA[i].y;
  for (int i = 0; i < M; i++) cin >> minA[i].x >> minA[i].y;
  getHull();
  int j = 0;
  for (int i = 0; i < N; i++) {
    while (j < hull.size() && maxA[i].x > minA[hull[j]].x) j++;
    if (j == hull.size()) {
      cout << "Max\n";
      return 0;
    }
    ...
    } else if ((maxA[i].y - minA[hull[j - 1]].y) *
                   (minA[hull[j]].x - minA[hull[j - 1]].x) >=
               (maxA[i].x - minA[hull[j - 1]].x) *
                   (minA[hull[j]].y - minA[hull[j - 1]].y)) {
      cout << "Max\n";
      return 0;
    }
  }
