...
long long n, m, cnt[1 << 20], lg, lim = 1, h[1 << 20], rev[1 << 20],
                                  sum[1 << 20];
struct cp {
  double x, y;
...
void FFT(cp *a, long long tp) {
  for (long long i = 0; i < lim; i++)
    if (i < rev[i]) swap(a[i], a[rev[i]]);
  for (long long md = 1; md < lim; md <<= 1) {
    cp rt = cp(cos(pi / md), tp * sin(pi / md));
    for (long long stp = md << 1, pos = 0; pos < lim; pos += stp) {
      cp w = cp(1, 0);
      for (long long i = 0; i < md; i++, w = w * rt) {
...
signed main() {
  ...
  for (long long i = 1, x; i <= n; i++)
    scanf("%lld", &x), sum[i] = sum[i - 1] + (x < m), cnt[sum[i]]++;
  for (long long i = 0; i <= n; i++)
    f[i] = cp(cnt[i], 0), g[i] = cp(cnt[n - i], 0);
  FFT(f, 1), FFT(g, 1);
  for (long long i = 0; i < lim; i++) f[i] = f[i] * g[i];
  FFT(f, -1);
  for (long long i = 0; i <= n; i++) h[n - i] = (long long)(f[i].x / lim + 0.5);
  printf("%lld ", zero());
