...
int main() {
  ...
  for (i = 0; i < N; i++)
    for (j = i + 1; j < N; j++) {
      t1 = 0;
      if (fabs(vx[i] - vx[j]) > 1e-8) t1 = (x[i] - x[j]) / (vx[j] - vx[i]);
      if (fabs(vy[i] - vy[j]) > 1e-8) t1 = (y[i] - y[j]) / (vy[j] - vy[i]);
      if (fabs(x[i] + t1 * vx[i] - (x[j] + t1 * vx[j])) < 1e-8 &&
          fabs(y[i] + t1 * vy[i] - (y[j] + t1 * vy[j])) < 1e-8) {
        con[i][j] = con[j][i] = 1;
  ...
  for (i = 0; i < N; i++) {
    ...
    sort(t, t + n);
    for (j = 0; j < n; j = k) {
      for (k = j; k < n && fabs(t[k] - t[j]) < 1e-8; k++)
        ;
      chkmax(ans, k - j + 1);
    }
    for (j = n = 0; j < i; j++)
    ...
    sort(dv, dv + n);
    for (j = 0; j < n; j += m) {
      for (k = j, m = 0; k < n && fabs(dv[k].first - dv[j].first) < 1e-8; k++)
        t[m++] = dv[k].second;
      sort(t, t + m);
      for (k = cnt = 0; k < m; k++)
        if (k == 0 || t[k] - t[k - 1] > 1e-8) cnt++;
      chkmax(ans, cnt + 1);
    }
  }
  printf("%d\n", ans);
