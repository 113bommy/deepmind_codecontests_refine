...
int query(int i, int u, int d) {
  ...
  if (u <= tree[i * 2].r)
    return max(ans, query(i * 2, u, d));
  else
    return max(ans, query(i * 2 + 1, u, d));
}
int main() {
  scanf("%d%d%d", &h, &w, &n);
  for (int i = 1; i <= n; i++) {
  ...
  n++, a[n].h = 0, a[n].l = 1, a[n].r = w, a[n].m = 1e9;
  sort(a + 1, a + n + 1, cmp);
  a[1].ans = 1;
  build(1, 1, w);
  for (int i = 2; i <= n; i++) {
    if (a[i].l == 1)
      a[i].ans = 2 * a[query(1, a[i].r + 1, a[i].h)].ans % MOD;
    else if (a[i].r == w)
      a[i].ans = 2 * a[query(1, a[i].l - 1, a[i].h)].ans % MOD;
    else
      a[i].ans = (a[query(1, a[i].l - 1, a[i].h)].ans +
                  a[query(1, a[i].r + 1, a[i].h)].ans) %
                 MOD;
    update(1, a[i].l, a[i].r, i);
  }
  long long res = 0;
  for (int i = 1; i <= w; i++) res = (res + a[query(1, i, h + 1)].ans) % MOD;
  printf("%lld", res);
  return 0;
}
