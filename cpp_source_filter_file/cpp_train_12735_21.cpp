...
void update(int poz, int x, int val) {
  for (int i = poz; i < maxi; i += i & (-i)) bit[x][i] += val;
}
int get(int poz, int x) {
  int ans = 0;
  for (int i = poz; i > 0; i -= i & (-i)) ans += bit[x][i];
  return ans;
}
int main() {
  cin >> n;
  ...
  for (int i : v[root]) {
    ...
    level = 0;
    dfs(root, i, cnt);
    for (int j = 1; j <= level; j++) update(j, cnt, 1);
  }
  cin >> q;
  while (q--) {
    int tip;
    ...
    if (tip == 3) {
      ...
      if (snode == root && enode == root)
        ...
        if (get(len, kk) == len)
          ...
          printf("-1\n");
      } else if (krak[snode].first == krak[enode].first) {
        int len = max(krak[snode].second, krak[enode].second) -
                  min(krak[snode].second, krak[enode].second);
        int kk = krak[snode].first;
        if (abs(get(krak[snode].second, kk) - get(krak[enode].second, kk) ==
                len))
          printf("%d\n", len);
        else
          printf("-1\n");
        ...
        if (get(krak[snode].second, krak[snode].first) +
                get(krak[enode].second, krak[enode].first) ==
            len)
          printf("%d\n", len);
        else
          printf("-1\n");
