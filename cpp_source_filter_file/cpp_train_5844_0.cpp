#include <bits/stdc++.h>
char str[200001];
int n, son[400010][26], len[400010], fa[400010], pos[200010], cnt = 1, last = 1;
void add(int x) {
  int p = last;
...
void getans(int x) {
  for (int i = x; i; i = Son[i])
    for (int j = head[i]; j; j = nxt[j])
      if (b[j] != Fa[i] && b[j] != Son[i]) getans(b[j]);
  Top = 0;
  for (int i = x; i; i = Son[i]) {
    for (int j = head[i]; j; j = nxt[j])
  ...
  for (int i = 1; i <= Top; i++) {
    ...
    for (int j = 0; j < que[u].size(); j++)
      ans[que[u][j]] +=
          bit.Query(L[que[u][j]], R[que[u][j]] - tem[i][1], 0, 1) * tem[i][1];
  }
  ...
  for (int i = 1; i <= Top; i++) {
    ...
    for (int j = 0; j < que[u].size(); j++) {
      int L = std::max(::L[que[u][j]], ::R[que[u][j]] - tem[i][1] + 1),
          R = ::R[que[u][j]];
  ...
  for (int i = 0; i < vec.size(); i++)
    if (!vec[i].type)
      bit.Add(vec[i].val, 1, vec[i].val), sum += vec[i].t;
    else {
      int coe = vec[i].type == 1 ? -1 : 1;
      ans[vec[i].ord] +=
          (bit.Query(1, vec[i].val - 1, 1, 0) +
           bit.Query(vec[i].val, n + 1, 0, 1) * (vec[i].val) - sum) *
          coe;
    }
  for (int i = 0; i < vec.size(); i++)
    if (!vec[i].type) bit.Add(vec[i].val, -1, -vec[i].val);
  vec.clear();
}
int main() {
  scanf("%s", str + 1);
