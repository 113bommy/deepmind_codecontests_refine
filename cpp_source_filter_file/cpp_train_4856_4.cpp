...
using std::max;
using std::min;
int n, len, inv, cnt[140000], size[140000], fib[140000], A[17][140000],
    B[17][140000], C[140000], D[140000], E[140000], ans;
int cal(int x) {
...
int main() {
  ...
  for (int i = 0; i < 17; i++) fwt.dft(A[i], 1, 1);
  for (int i = 0; i < 17; i++)
    for (int j = 0; i + j < 17; j++)
      for (int k = 0; k < len; k++)
        B[i + j][k] =
            cal(B[i + j][k] + (long long)A[i][k] * A[j][k] % 1000000007);
  for (int i = 0; i < 17; i++) fwt.dft(B[i], -1, 1);
  for (int i = 0; i < len; i++)
  ...
  fwt.dft(C, -1, 2);
  for (int i = 0; i < 17; i++) ans = cal(ans + C[1 << i]);
  printf("%d\n", ans);
  return 0;
}
