...
void dfs1(int u, int fa = 0) {
  ...
  for (int i = head[u]; i; i = nxt[i])
    if (to[i] != fa && !vis[to[i]]) {
      ...
      rt[to[i]] = rt[u];
      dfs1(to[i], u);
      sum[u] += sum[to[i]] + (long long)sz[to[i]] * val[i];
      sz[u] += sz[to[i]];
    }
}
long long totsum[MAXN], rtv[MAXN];
void dfs2(int u, long long tsum, int tsz, int fa = 0) {
  totsum[u] = tsum;
  for (int i = head[u]; i; i = nxt[i])
    if (to[i] != fa && !vis[to[i]])
      dfs2(to[i], tsum - (tsz - sz[to[i]] * 2ll) * val[i], tsz, u);
}
long long szmul[MAXN], totvl, szpre[MAXN];
int main() {
  ...
  for (int i = 1; i <= bak * 2; ++i) {
    ...
    szmul[i] = szmul[i - 1] + sz[nodes[i]] * (long long)pre[i];
  }
  for (int i = 1; i <= bak; ++i) {
    while (pre[cur] - pre[i] <= totcir / 2) ++cur;
    long long v = totvl - sum[nodes[i]];
    v += szmul[cur - 1] - szmul[i];
    v -= szmul[i + bak - 1] - szmul[cur - 1];
    v -= (szpre[cur - 1] - szpre[i]) * (long long)pre[i];
    v += (szpre[i + bak - 1] - szpre[cur - 1]) * (long long)(totcir + pre[i]);
    rtv[nodes[i]] = v;
  }
  for (int i = 1; i <= n; ++i) {
    long long val =
        totsum[i] + rtv[rt[i]] + (long long)dep[i] * (n - sz[rt[i]]);
    std::cout << val << " ";
  }
  std::cout << std::endl;
  return 0;
