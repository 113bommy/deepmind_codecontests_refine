...
long long int prob[200010], sfx[200010], invSfx[200010], invSfxSum[200010];
...
long long int rangeExpt(int qLeftPt, int qRightPt) {
  return (invSfxSum[qLeftPt] - invSfxSum[qRightPt + 1]) * sfx[qRightPt + 1] %
         mod;
}
int main() {
  ...
  cin >> len >> qNum;
  for (int i = 0; i < len; i++) cin >> prob[i], prob[i] = prob[i] * inv % mod;
  sfx[len] = 1;
  for (int i = len - 1; i >= 0; i--) sfx[i] = sfx[i + 1] * prob[i] % mod;
  invSfx[len] = 1, invSfxSum[len] = 1;
  invSfx[0] = fastPow(sfx[0], mod - 2, mod);
  for (int i = 1; i <= len; i++) invSfx[i] = invSfx[i - 1] * prob[i - 1] % mod;
  for (int i = len - 1; i >= 0; i--)
    invSfxSum[i] = invSfxSum[i + 1] + invSfx[i];
  set<int> st;
  st.insert({0, len});
