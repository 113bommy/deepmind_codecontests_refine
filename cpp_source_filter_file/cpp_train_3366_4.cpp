...
int s, t, label[kMaxV], start[kMaxV], q[kMaxV], qh, qt, stack[kMaxV],
    edge[kMaxV], stop;
int maxflow;
...
void DoAug() {
  ...
  while (stop) {
    ...
    if (u != t) {
      ...
      int flow = kInf;
      for (int i = stop - 1; i >= 1; --i) flow = min(flow, cap[edge[i]]);
      for (int i = stop - 1; i >= 1; --i) {
        cap[edge[i]] -= flow, cap[((edge[i]) ^ 1)] += flow;
  ...
  for (int i = req - 1; i >= 0; --i) {
    ...
    for (int j = 0; j < n; ++j)
      if (!del[j] && j != u && j != v) {
        ...
        if (x1 * y2 - x2 * y1 >= 0)
          bel[j] = 0, rec[j] = flow::cnt, flow::AddPair(n, j, 1), s1[d1++] = j;
        else
          bel[j] = 1, rec[j] = flow::cnt, flow::AddPair(j, n + 1, 1),
          s2[d2++] = j;
      } else
        bel[j] = -1;
    for (int j = 0; j < i; ++j) {
    ...
    if (succ) {
      ...
      for (int j = 0; j < d1; ++j) {
        mat[s1[j]] = -1;
        if (flow::cap[rec[j]]) continue;
        for (int now = flow::begin[j]; now != -1; now = flow::next[now])
          if (flow::cap[now] == 0) {
