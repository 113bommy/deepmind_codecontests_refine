...
int getmaxw(int u, int v) {
  ...
  for (int i = 0, k = dep[u] - dep[v]; i < Log; ++i)
    if (k >> i & 1) chkmax(ret, max[u][i]), u = Fa[u][i];
  if (u == v) return ret;
  for (int i = Log - 1; ~i; --i) {
    if (Fa[u][i] != Fa[v][i]) {
      chkmax(ret, max[u][i]);
...
void exec() {
  ...
  if (pos == 0) {
    ...
    while (u != v) {
      if (dep[u] < dep[v]) std::swap(u, v);
      if (chkmax(W, max[u][0])) U = u, V = Fa[u][0];
      u = Fa[u][0];
    }
    if (U < V) std::swap(U, V);
    for (int i = 1; i <= m; ++i) {
      if (vis[to[i]] && (std::max(E[to[i]].u, E[to[i]].v) != U ||
                         std::min(E[to[i]].u, E[to[i]].v) != V))
        write(i, ' '), write(E[to[i]].w);
      else if (to[i] == pos)
