...
int main()
{
	...
	for (i=1;i<=n;i++) {
		...
		d[a[i]-1]--;
		if (b[i]<a[i])
			next[i]=head[a[i]-1],head[a[i]-1]=i;
		side[i]=true;
	}
	for (s=-1,i=n+1;i;i--,s++) {//一开始多了一个,每一轮可以抵消一个
		s+=d[i];
		for (j=head[i];j;j=next[j]) q.push(make_pair(-b[j],j));
	...
	for (i=1;i<=n;i++) if (side[i]) next[i]=head[b[i]],head[b[i]]=i;
	for (i=1;i<=n+1;i++) {
		for (j=head[i];j;j=next[j]) q.push(make_pair(a[j]-1,j));
		f[i]=f[i-1];
		if (d[i]-i==-1&&cur<i) {
