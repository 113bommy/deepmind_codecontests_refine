...
void read_img() {
  fin >> h >> w >> k;
  for (int i = 1; i <= h; ++i)
    for (int j = 1; j <= w; ++j) fin >> img[i][j];
}
int s_part[NMAX][NMAX];
void compute_s_part() {
  for (int i = 1; i <= h; ++i)
    for (int j = 1; j <= w; ++j)
      s_part[i][j] = img[i][j] + s_part[i][j - 1] + s_part[i - 1][j] -
                     s_part[i - 1][j - 1];
}
int average(int l1, int c1, int l2, int c2) {
  int sum = s_part[l2][c2] - s_part[l2][c1 - 1] - s_part[l1 - 1][c2] +
            s_part[l1 - 1][c1 - 1];
  ...
  for (int i = 1; i <= k; ++i) {
    ...
    for (int lin = u; lin <= d; ++lin)
      for (int col = 1; col <= w; ++col)
        new_img[lin][col] = average(max(u, lin - lat), max(1, col - lat),
                                    min(d, lin + lat), min(w, col + lat));
  }
}
int cost[KMAX][KMAX];
int compute_cost(int u1, int d1, int u2, int d2) {
  int ans = 0;
  for (int col = 1; col <= w; ++col)
    ans += (new_img[d1][col] - new_img[u2][col]) *
           (new_img[d1][col] - new_img[u2][col]);
  return ans;
}
...
void solve() {
  ...
  reverse(p + 1, p + k + 1);
  for (int i = 1; i <= k; ++i) p_inv[p[i]] = i;
  for (int i = 1; i <= k; ++i) cout << p_inv[i] << " \n"[i == k];
}
int main() {
  int q = 0;
  fin >> q;
