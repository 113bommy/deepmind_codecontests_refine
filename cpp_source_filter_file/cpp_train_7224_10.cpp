...
int n, m, x, y, cnt, start[1111];
bool a[1111], b[1111], fix[1111];
int main() {
  sIO();
  scanf("%d%d", &n, &m), memset(a, 0, sizeof(a)), memset(b, 0, sizeof(b)),
      memset(fix, 0, sizeof(fix)), memset(start, 0, sizeof(start));
  for (int i = 0; i < m; ++i) scanf("%d%d", &x, &y), a[x] = b[y] = true;
  cnt = 0;
  for (int i = 2; i < n; ++i)
    if (!a[i]) ++cnt, start[i] = 1;
  for (int i = 2; i < n; ++i)
    if (!b[i] && (i != (x = n - i + 1) || !a[x])) {
      if (start[i] != start[x] || !start[i]) ++cnt, fix[x] = fix[i] = true;
      if (!fix[i]) ++cnt, start[i] = n, fix[x] = fix[i] = true;
    }
  printf("%d", cnt);
  return 0;
}
