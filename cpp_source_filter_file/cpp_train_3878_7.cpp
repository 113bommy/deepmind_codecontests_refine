...
void dfs(int s, int cur = 0) {
  val[s] += cur;
  cur = val[s];
  for (auto v : adj[s]) dfs(v, cur);
}
pair<int, long long> walk(int st, string &s) {
  long long res = 0;
  for (auto c : s) {
    st = to[st][c - 'a'];
    res += val[st];
...
int main() {
  ...
  build_aho();
  dfs(0);
  vector<vector<long long>> dp(sz + 1, vector<long long>(1 << 14, -INF));
  dp[0][0] = 0;
  ...
  for (auto t : pieces) {
    ps++;
    vector<vector<long long>> ndp(sz + 1, vector<long long>(1 << 14, -INF));
    for (int i = 0; i <= sz; i++) {
      auto dst = walk(i, t);
      for (int ms = 0; ms < (1 << 14); ms++) {
      ...
      for (int ch = 0; ch < 14; ch++) {
        for (auto ms : masks[ch][ps - 1]) {
          for (int st = 0; st <= sz; st++) {
            dp[to[st][ch]][ms ^ (1 << ch)] = max(dp[to[st][ch]][ms ^ (1 << ch)],
                                                 dp[st][ms] + val[to[st][ch]]);
          }
        }
      }
    }
  ...
  ps--;
  for (int ms = 0; ms < (1 << 14); ms++) {
    if (__builtin_popcount(ms) != ps) continue;
    for (int i = 0; i <= sz; i++) ans = max(ans, dp[i][ms]);
  }
  cout << ans << '\n';
}
