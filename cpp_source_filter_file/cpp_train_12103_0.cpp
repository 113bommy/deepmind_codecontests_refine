...
struct dat {
  ...
  void tog(int z) {
    if (z & 2)
      for (int i = 0; i < 2; ++i)
        std::swap(v[i][0], v[i][3]), std::swap(v[i][1], v[i][2]);
    if (z & 1 ^ z >> 1)
      for (int i = 0; i < 4; ++i) std::swap(v[0][i], v[1][i]);
    this->z ^= z;
  }
};
struct ST {
  ...
  dat merge(const dat& l, const dat& r) {
    ...
    for (int i = 0; i < 2; ++i)
      for (int j = 0; j < 4; ++j)
        for (int k = 0; k < 4; ++k)
          ckmin(n.v[i][j & 1 | k & 2],
                l.v[i][j] + r.v[i][k] + (j >> 1 ^ k & 1) * C);
    return n;
  }
  void down(int n) {
    if (v[n].z) v[n << 1].tog(v[n].z), v[n << 1 | 1].tog(v[n].z), v[n].z = 0;
  }
...
void ans() {
  int f = INF;
  {
    int c[2] = {ll.v[1], rr.v[1]};
    for (int i = 0; i < 4; ++i)
      ckmin(f, H.v[1].v[0][i] + (i & 1 ? c[0] : N - c[0]) +
                   (i & 2 ? c[1] : N - c[1]));
  }
  {
    int c[2] = {uu.v[1], dd.v[1]};
    for (int i = 0; i < 4; ++i)
      ckmin(f, V.v[1].v[0][i] + (i & 1 ? c[0] : M - c[0]) +
                   (i & 2 ? c[1] : M - c[1]));
  }
  printf("%d\n", f);
}
