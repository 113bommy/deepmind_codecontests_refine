#include <bits/stdc++.h>
using namespace std;
int N, Q, S, id = 0, tos[10000010], prevs[10000010], lasts[10000010],
             weights[10000010];
...
void build(int p, int L, int R, int offset) {
  for (int i = L; i <= R; i++) {
    if (offset < 0)
      addEdge(p - offset, i, 0);
    else
      addEdge(i, p + offset, 0);
  }
  if (L == R) return;
  int li = left(p), ri = right(p);
  build(li, L, (L + R) / 2, offset);
  build(ri, (L + R) / 2 + 1, R, offset);
}
void modify(int p, int L, int R, int l, int r, int s, int offset, int w) {
  if (l <= L && r >= R) {
    if (offset < 0)
      addEdge(s, p - offset, w);
    else
      addEdge(p + offset, s, w);
    return;
  }
  int li = left(p), ri = right(p);
...
void dijkstra() {
  priority_queue<pair<long long, int>, vector<pair<long long, int>>,
      ...
      pq;
  fill(dists, dists + 1000010, 1000000000000000000);
...
int main() {
  ...
  for (int i = 0; i < Q; i++) {
    ...
    if (tp == 1) {
      ...
      l--;
      r--;
      modify(1, 0, N - 1, l, r, v, N + 400010, w);
    }
  }
  dijkstra();
  for (int i = 0; i < N; i++) {
    if (dists[i] == 1000000000000000000)
      cout << "-1 ";
    else
      cout << dists[i] << ' ';
  }
