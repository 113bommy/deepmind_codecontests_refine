...
void solve(vector<pair<int, pair<int, int> > > group, int sol[][2],
  ...
  while (start < startPos && fullBack >= startPos) {
    sol[group[start].first][0] = group[pos].second.first;
    sol[group[start].first][1] = height;
    sol[group[fullBack].first][0] = width;
    sol[group[fullBack].first][1] = group[posBack].second.second;
    start++;
    pos++;
    posBack--;
    fullBack--;
