...
void dft(poly *a, int type, int limit) {
  ...
  for (mid = 1; mid < limit; mid <<= 1) {
    wn = (type == 1) ? mypow(g, (mod - 1) / 2 / mid)
                     : mypow(g, mod - 1 - (mod - 1) / 2 / mid);
...
poly poly_inv(poly a, int n) {
  ...
  for (i = cnt; i > 0; i--) {
    h = Minus(scalar_mul(h, 2LL),
              ext(mul(ext(mul(f[i], h), seq[i] - 1), h), seq[i] - 1));
  }
  return h;
}
...
poly poly_sqrt(poly a, int n) {
  ...
  h.x[0] = 1;
  for (i = cnt; i > 0; i--) {
    h = add(ext(mul(f[i], poly_inv(scalar_mul(h, 2), seq[i])), seq[i]),
            scalar_mul(h, inv2));
  }
  return h;
}
poly poly_pow(poly a, int m, int n) {
  int i, np, tp, inv, tmp;
  poly f, g;
  np = a.deg;
...
poly solve(int l, int r) {
  ...
  int mid = (l + r) >> 1;
  return mul(solve(1, mid), solve(mid + 1, r));
}
int main() {
  int n;
  cin >> n;
