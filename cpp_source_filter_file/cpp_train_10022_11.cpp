...
bool bfs() {
  ...
  while (h < t) {
    ...
    for (int i = head[x]; i; i = e[i].nxt)
      if (e[i].f && !dep[y = e[i].to]) dep[y] = dep[x] + 1, q[++t] = y;
  }
  return dep[T] != 0;
}
...
int main() {
  ...
  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= m; ++j) id[i][j] = tot++, tot++;
  int cnt = 0, cntb = 0;
  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= m; ++j)
      if (mp[i][j] == '#') {
        ...
        if (mp[i + 1][j] == '#')
          ADD(S, id[i][j], 1), ADD(id[i][j], id[i + 1][j] + 1, 1);
        if (mp[i][j + 1] == '#')
          ADD(id[i][j] + 1, T, 1), ADD(id[i][j + 1], id[i][j] + 1, 1);
        cnt += mp[i + 1][j] == '#';
        cnt += mp[i][j + 1] == '#';
        ++cntb;
      }
