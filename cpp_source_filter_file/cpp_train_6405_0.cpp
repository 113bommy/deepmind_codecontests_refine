...
struct tree {
  ...
  int stk[maxn], top;
  vector<pair<int, int> > extract(vector<int>& vs) {
    ...
    for (auto x : vs) {
      ...
      if (top) ret.push_back(make_pair(stk[top], x));
      stk[++top] = x;
    }
    return ret;
  }
...
int main() {
  ...
  for (int qq = 1; qq <= q; qq++) {
    ...
    for (int i = 1; i <= N; i++) {
      scanf("%d", va + i);
      va[i] = (va[i] + R) % n + 1;
      vs.push_back(G1.bcc[va[i]]);
    }
    for (int i = 1; i <= M; i++) {
      scanf("%d%d", eu + i, ev + i);
      eu[i] = (eu[i] + R) % n + 1;
      ev[i] = (ev[i] + R) % n + 1;
      vs.push_back(G1.bcc[eu[i]]);
      vs.push_back(G1.bcc[ev[i]]);
    }
    sort(vs.begin(), vs.end());
    vs.erase(unique(vs.begin(), vs.end()), vs.end());
    vector<pair<int, int> > es = T.extract(vs);
    int sz = (int)vs.size();
    for (int i = 0; i < sz; i++) id[vs[i]] = i + 1;
    G2.clear(sz);
    for (auto e : es) {
      G2.addedge(id[e.first], id[e.second]);
    }
    for (int i = 1; i <= M; i++)
      G2.addedge(id[G1.bcc[eu[i]]], id[G1.bcc[ev[i]]]);
    G2.shrink(sz);
    bool ok = 1;
    int piv = G2.bcc[id[G1.bcc[va[1]]]];
