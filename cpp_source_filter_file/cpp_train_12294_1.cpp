#include <bits/stdc++.h>
using namespace std;
const int MAXN = 2e5 + 5, MAXS = 1e7, mod = 998244353, G = 3,
          invG = (mod + 1) / 3;
int n;
long long m;
...
void ntt(int *a, int n, int tp) {
  ...
  for (int mid = 1; mid < n; mid <<= 1) {
    long long w = 1, w1 = Fstpw(tp == 1 ? G : invG, (mod - 1) / mid / 2);
...
int *NTT(int *a, int n, int *b, int m, int *res) {
  ...
  ntt(f, siz, 1);
  ntt(g, siz, 1);
  for (int i = 0; i < siz; i++) f[i] = 1ll * f[i] * g[i] % mod;
  ntt(f, siz, -1);
  int k = n + m;
...
void Dfs3(int k, int l, int r) {
  if (l == r) {
    ...
    if (g[u]) {
      ...
      ans[k][0] = 0;
      ans[k][1] = 1;
      for (int i = 2; i <= len[k]; i++) ans[k][i] = buf[seg[g[u]] + i - 2];
      pt += len[k] + 1;
      mul[k] = pt;
  ...
  mul[k] = pt;
  pt = NTT(mul[k << 1], len[k << 1], mul[k << 1 | 1], len[k << 1 | 1], mul[k]);
  ans[k] = pt;
  pt = NTT(ans[k << 1 | 1], len[k << 1 | 1], mul[k << 1], len[k << 1], ans[k]);
  for (int i = 0; i <= len[k << 1]; i++)
    ans[k][i] = (ans[k][i] + ans[k << 1][i]) % mod;
  return;
}
void Dfs2(int u) {
  seg[u] = ++*seg;
