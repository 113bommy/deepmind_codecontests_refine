...
int n, d[250005], X, vis[250005];
long long sum;
std ::vector<std ::pair<int, int> > E[250005];
std ::pair<int, int> D[250005];
...
struct Normal_Heap {
  ...
  void push(long long x) {
    a.push_back(x), std ::push_heap(a.begin(), a.end());
  }
  long long top() { return a[0]; }
  void pop() { std ::pop_heap(a.begin(), a.end()), a.pop_back(); }
  long long popn() {
    long long x;
    return std ::pop_heap(a.begin(), a.end()), x = a[a.size() - 1],
                                               a.pop_back(), x;
  }
...
inline void dfs(int u, int fa = 0) {
  ...
  for (int i = 0; i < E[u].size(); i++) {
    ...
    res += dp[v][0], H[u].push(x);
    del.push_back(x);
  }
  for (; H[u].size() && H[u].size() > num; H[u].pop())
    tmp.push_back(H[u].top());
  dp[u][0] = res + H[u].sum;
  for (; H[u].size() && H[u].size() > num - 1; H[u].pop())
    tmp.push_back(H[u].top());
  dp[u][1] = res + H[u].sum;
