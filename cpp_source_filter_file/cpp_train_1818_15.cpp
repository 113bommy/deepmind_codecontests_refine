...
int add(int x, int y) { return (long long)(x + y) % MOD; }
bool vis[N];
bool in[N];
vector<int> vec, vec1;
void dfs(int x) {
...
bool solve1(int x) {
  ...
  for (int u = 0; u < siz; u++) {
    int xx = vec[u];
    now ^= 1;
    for (int i = 0; i < (1 << siz1); i++) g[now][i] = g[now ^ 1][i];
    for (int i = 0; i < (1 << siz1); i++) {
      ...
      for (int o = last[xx]; o != -1; o = e[o].last) {
        ...
        g[now][i | (1 << id[y])] =
            add(g[now][i | (1 << id[y])], mul(g[now ^ 1][i], e[o].z - 1));
      }
    }
  }
...
void dfs3(int x) {
  ...
  for (int u = last[x]; u != -1; u = e[u].last) {
    ...
    for (int i = 0; i <= num; i++)
      for (int j = 0; j <= i; j++) {
        F[x][i][0] =
            add(F[x][i][0], mul(FF[j][0], add(F[y][i - j][0], F[y][i - j][1])));
        F[x][i][1] =
            add(F[x][i][1], mul(FF[j][1], add(F[y][i - j][0], F[y][i - j][1])));
      }
    if (ok[y] == false && ok[x] == false) {
      for (int i = 0; i <= num; i++) {
        ...
        for (int j = 0; j <= num; j++) {
          if (i + j >= num) continue;
          F[x][i + j + 1][1] = add(
              F[x][i + j + 1][1], mul(mul(FF[i][0], F[y][j][0]), (e[u].z - 1)));
        }
      }
    }
  }
...
void DP(int sum, int tot) {
  dfs3(root);
  for (int u = 0; u <= num; u++)
    f1[u + tot] = add(f1[u + tot], mul(sum, add(F[root][u][0], F[root][u][1])));
}
...
void solve2(int x) {
  ...
  for (int u = 0; u <= k; u++)
    for (int i = 0; i <= u; i++) ff[u] = add(ff[u], mul(f[u - i], f1[i]));
  for (int u = 0; u <= k; u++) f[u] = ff[u];
  vec.clear();
}
int main() {
  ...
  for (int u = 1; u <= 2 * n; u++)
    if (vis[find_fa(u)] == false) {
      if (!solve1(find_fa(u))) solve2(find_fa(u));
    }
  int ans = 0;
