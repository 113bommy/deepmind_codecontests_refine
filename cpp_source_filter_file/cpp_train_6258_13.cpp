...
int n, rk[100005], hull[100005], tl;
long long t;
double p[100005], g[100005], a[100005], b[100005], M;
struct Matrix {
  double A[4][4];
  int r, c;
...
Matrix mul(Matrix A, Matrix B) {
  ...
  for (int i = 1; i <= A.r; ++i)
    for (int j = 1; j <= A.c; ++j)
      for (int k = 1; k <= B.c; ++k) C.A[i][k] += A.A[i][j] * B.A[j][k];
  return C;
}
Matrix build(int x) {
  Matrix A = Matrix(3, 3);
...
double get(double s, int x) { return p[x] * s + g[x]; }
int cmp(int a, int b) {
...
int main() {
  ...
  for (int j = 1; j <= n; ++j) {
    ...
    if (tl < 2)
      ...
      while ((g[i] - g[hull[tl]]) * (p[hull[tl]] - p[hull[tl - 1]]) >
             (g[hull[tl]] - g[hull[tl - 1]]) * (p[i] - p[hull[tl]]) + eps)
        tl--;
      hull[++tl] = i;
    }
  }
  ...
  F.A[1][3] = 1;
  while (1) {
    while (p1 < tl && g[hull[p1 + 1]] - g[hull[p1]] + eps >
                          -cur_pt * (p[hull[p1 + 1]] - p[hull[p1]]))
      p1++;
    ...
    for (int i = 34; i >= 0; i--) {
      ...
      if (p1 == tl || get(new_pt, hull[p1]) + eps > get(new_pt, hull[p1 + 1])) {
        F = N;
        cur += (1ll << i);
      }
    }
