...
struct seg {
  set<int> tree[270000];
  int lim;
  void init(int n, pair<int, int> *b) {
    for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < n; i++) {
      int t1 = lower_bound(vx.begin(), vx.end(), b[i].first) - vx.end();
      t1 += lim;
      while (t1) {
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    while (s != e) {
      int m = (s + e) / 2;
      if (query(a[i].first - m, a[i].first + m, a[i].second - m,
                a[i].second + m)) {
        e = m;
      } else
        s = m + 1;
  ...
  sort(a, a + n, [&](const t1 &a, const t1 &b) { return a.nsub > b.nsub; });
  vector<int> cnd;
  for (int i = 0; i < n; i++) {
