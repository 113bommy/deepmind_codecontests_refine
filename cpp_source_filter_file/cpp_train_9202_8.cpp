...
int n;
long long solve() {
  long long dp[10005][8] = {0};
  dp[0][0] = 1;
  for (int i = 1; i <= n; i++) {
    for (int mask = 0; mask < 8; mask++) {
      if (((mask & (1)) && grid[0][i] == 'X') ||
          ((mask & (2)) && grid[1][i] == 'X') ||
          ((mask & (4)) && grid[2][i] == 'X'))
        continue;
      int newmask = mask;
      ...
      if (newmask == 0)
        dp[i][mask] += dp[i - 1][1] + dp[i - 1][4] + dp[i - 1][7];
      else if (newmask == 1)
        dp[i][mask] += dp[i - 1][6] + dp[i - 1][0];
      else if (newmask == 2)
        dp[i][mask] += dp[i - 1][5];
      else if (newmask == 3)
        dp[i][mask] += dp[i - 1][4];
      else if (newmask == 4)
        dp[i][mask] += dp[i - 1][3] + dp[i - 1][0];
      else if (newmask == 5)
        dp[i][mask] += dp[i - 1][2];
      else if (newmask == 6)
        dp[i][mask] += dp[i - 1][1];
...
int main() {
  ...
  long long ans = solve();
  if (pos.first == 1 && pos.second != 0 && pos.second != n - 1) {
    grid[0][pos.second] = 'X';
    grid[2][pos.second] = 'X';
