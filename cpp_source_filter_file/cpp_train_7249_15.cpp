...
inline bool cmp(aaa a, aaa b) { return a.num < b.num ? 1 : 0; }
...
int main() {
  ...
  for (int i = 0; i < Mx; ++i)
    if (one[i] == m) st[to[i] = (++t)] = i;
  for (int i = 1; i <= t; ++i)
    for (int j = 1; j <= t; ++j) Mat[0].a[i][j] = ans.a[i][j] = inf;
  ...
  for (int i = 1, l, w; i <= t; ++i) {
    ...
    if (one[l] == m)
      ...
      for (int j = k - 1; ~j; --j)
        if (!((l >> j) & 1)) Mat[0].a[i][to[l | (1 << j)]] = c[k - j];
  }
  ...
  for (int i = 1; i <= q; ++i) {
    ...
    for (int j = 1; j <= t; ++j)
      for (int l = 1; l <= t; ++l)
        if ((st[l] >> (k - 1)) & 1) ans.a[j][l] += sp[i].w;
  }
