...
bool pre(int n, const string &s, int st, long long len) {
  int id = n <= 14 ? n : 14 - (n & 1);
  return len <= f[id].length() && f[id].compare(0, len, s, st, len) == 0;
}
bool suf(int n, const string &s, int st, long long len) {
  int id = max(n, 14);
  return len <= f[id].length() &&
         f[id].compare(f[id].length() - len, len, s, st, len) == 0;
}
long long calc(int n, const string &s) {
  for (int i = 0; i <= n; ++i) {
    if (i <= 1) {
      g[i] = f[i] == s ? 1 : 0;
      continue;
