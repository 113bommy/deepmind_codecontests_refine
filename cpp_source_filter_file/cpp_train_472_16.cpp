#include <bits/stdc++.h>
using namespace std;
int n, m, cnt, pt, bin[25], lg2[200005], fst[200005], nxt[200005], sa[200005],
    sum[200005], rnk[200005 << 1], q[200005 << 1], f[18][200005], g[18][200005],
    ans[200005];
char s[200005];
struct node {
  int l, r, x, y;
...
void gethgt() {
  ...
  for (i = 1; i <= n; i++) {
    if (k) k--;
    j = sa[rnk[i] - 1];
    for (; i + k <= n && j + k <= n && s[i + k] == s[j + k]; i++, j++)
      ;
    f[0][rnk[i]] = k;
  }
}
int rmq(int f[][200005], int x, int y) {
  if (x > y) swap(x, y);
  int k = lg2[y - x + 1];
  return min(f[k][x], f[k][y - bin[k] + 1]);
}
void build(int f[][200005], int n) {
  int i, j;
  for (i = 1; i <= 17; i++)
    for (j = 0; j <= n; j++) {
      f[i][j] = f[i - 1][j];
  ...
  for (i = 1; i <= m; i++)
    if (fst[i] && i <= 80)
      ...
      for (j = fst[i]; j; j = nxt[j])
        for (k = 0; k * i <= m; k++) {
          x = max(k * i + op[j].x, op[j].l);
          y = min(k * i + op[j].y, op[j].r);
