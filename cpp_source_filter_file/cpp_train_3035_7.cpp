...
void dfs1(int x) {
  ...
  for (int i = head[x]; i; i = nxt[i])
    if (val[i] == A && vis[to[i]] == 0) dfs1(to[i]);
}
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    for (int s = 0; s < (1 << cId); ++s)
      if (dis[at(s, i)] < ans) ans = dis[at(s, i)];
    printf("%d ", ans);
  }
  return 0;
}
