...
struct segtree {
  ...
  segtree(vector<int>& v) {
    ...
    n = 1 << h;
    t = vector<pair<long long, long long> >(
        n * 2, pair<long long, long long>(INF, -1));
    for (int i = 0; i < v.size(); ++i) {
      t[i + n] = pair<long long, long long>(v[i], i);
    }
    for (int i = n - 1; i; --i) {
  ...
  pair<long long, long long> query(int l, int r) {
    pair<long long, long long> ans(INF, -1);
  ...
  void assign(int i, int v) {
    i += n;
    t[i] = pair<long long, long long>(v, i - n);
    for (i /= 2; i; i /= 2) {
      t[i] = min(t[i << 1], t[i << 1 | 1]);
    }
  }
...
long long solve() {
  cin >> n;
  f = vector<int>(n + 1, 0);
  mp = vector<pair<long long, long long> >();
  for (int i = 0; i < n; ++i) {
    int m, p;
    cin >> m >> p;
    mp.emplace_back(m, p);
  ...
  for (int i = n - 1; i; --i) {
    ...
    while (need > 0) {
      extra++;
      pair<long long, long long> gg = st.query(f[i], vals.size());
      assert(gg.second != -1);
      ans += gg.first;
