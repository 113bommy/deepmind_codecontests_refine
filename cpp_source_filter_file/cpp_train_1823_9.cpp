...
struct ver {
  ...
  pair<int, int> P;
  ver(int a, int b, int c, int d, int e)
      : x(a),
        y(b),
        z(c),
        carry(d),
...
ver par[11][11][11][11];
bool operator<(const ver &a, const ver &b) { return a.dist > b.dist; }
int sol() {
  ...
  while (Q.size()) {
    ver t = Q.top();
    Q.pop();
    if (t.dist >= dist[t.x][t.y][t.z][t.carry] || t.x > a.size() ||
        t.y > b.size() || t.z > c.size())
      continue;
    ...
    if (t.x == (int)a.size() && t.y == (int)b.size() && t.z == (int)c.size() &&
        t.carry == 0)
      return t.dist;
    if (t.x == a.size() - 1) {
      t.x++;
...
int main() {
  for (int i = 0; i < 11; i++)
    for (int j = 0; j < 11; j++)
      for (int k = 0; k < 11; k++)
        for (int m = 0; m < 11; m++) dist[i][j][k][m] = 1e9;
  char cc;
  string str;
  cin >> str;
  stringstream ss(str);
