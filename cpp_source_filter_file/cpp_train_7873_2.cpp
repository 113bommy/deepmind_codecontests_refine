#include <bits/stdc++.h>
using namespace std;
bool try_solve(int rb, int rt, vector<vector<int> > &Gb,
               vector<vector<int> > &Gt, vector<int> &match) {
  int N = Gb.size();
  ...
  part[rt] = rt;
  for (int i = 0; i < N; i++)
    if (match[i] == -1) freev.push_back(i);
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (auto it = Gb[v].begin(); it != Gb[v].end(); it++) block[*it] = true;
    ...
    for (auto it = Gb[v].begin(); it != Gb[v].end(); it++) block[*it] = false;
    q.pop();
  }
  return true;
}
