...
const long long MOD = (long long)mod * mod;
const int M = 64;
const int L = 30;
...
vector<int> possible;
int GetIndex(int x) {
  return lower_bound(possible.begin(), possible.end(), x) - possible.begin();
}
struct Matrix {
  long long a[M][M];
  void init() {
...
Vector operator*(Vector a, Matrix b) {
  ...
  for (int i = 0; i < possible.size(); i++) {
    ans.v[i] = 0;
    for (int j = 0; j < possible.size(); j++) {
      ans.v[i] = add(ans.v[i], a.v[j] * b.a[j][i]);
    }
  }
  return ans;
}
...
void input() {
  ...
  for (int i = 1; i <= m; i++) {
    int x, y, col;
    scanf("%i %i %i", &x, &y, &col);
    pos[x].push_back({y, col});
  }
  for (int i = 1; i <= 3; i++)
    for (int j = 1; j <= 3; j++) scanf("%i", &can[i][j]);
}
bool was[M];
void BFS(int st) {
  queue<int> q;
...
void Solve() {
  for (int i = 1; i <= n; i++) {
    Vector vec = Solve(i);
    for (int j = 0; j < possible.size(); j++)
      ways[i][possible[j] % 4] = add(ways[i][possible[j] % 4], vec.v[j]);
  }
  dp[0][0] = 1;
  for (int i = 1; i <= n; i++) {
    for (int j = 0; j < 4; j++) {
