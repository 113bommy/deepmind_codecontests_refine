...
void dfs(int x, int y) {
  ...
  dp[x] = dp[y] + 1;
  for (i = 0; i < 20; ++i)
    f[i + 1][x] = f[i][f[i][x]], g[i + 1][x] = g[i][f[i][x]] + g[i][x];
  for (i = he[x]; i; i = ne[i])
    if ((j = to[i]) != y) {
...
int main() {
  ...
  for (i = 1; i <= tp; ++i) {
    memset(b, 0, n + 1), memset(d[i], 9, n + 1 << 3), d[i][p[i]] = 0,
                                                      pq.push({0, p[i]});
    while (!pq.empty()) {
      j = pq.top().second, pq.pop();
      if (b[j]) continue;
      for (b[j] = 1, k = he[j]; k; k = ne[k])
        if (!b[l = to[k]] && d[i][l] > d[i][j] + len[k]) {
  ...
  while (q--) {
    ...
    for (k = 20; ~k; --k)
      if (dp[f[k][i]] >= dp[j]) o += g[k][i], i = f[k][i];
    if (i != j) {
      for (k = 20; ~k; --k)
        if (f[k][i] != f[k][j])
          o += g[k][i] + g[k][j], i = f[k][i], j = f[k][j];
      o += g[0][i] + g[0][j];
    }
    printf("%lld\n", min(w, o));
  }
