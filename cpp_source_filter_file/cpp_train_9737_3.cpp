...
int main() {
  ...
  for (int i = 0; i < wyn1.size(); i++) {
    if (wyn1[i].first > wyn1[i].second) swap(wyn1[i].first, wyn1[i].second);
    if (wyn2[i].first > wyn2[i].second) swap(wyn2[i].first, wyn2[i].second);
    raz[wyn1[i]] = i + 1;
    dwa[wyn2[i]] = i + 1;
  }
  for (int i = 0; i < wyn1.size(); i++) {
  ...
  for (int i = 0; i < wyn1.size(); i++) {
    if (!dwa[wyn1[i]]) {
      u = i;
      while (raz[wyn2[u]]) u = raz[wyn2[u]] - 1;
      printf("%d %d %d %d\n", wyn1[u].first, wyn1[u].second, wyn2[u].first,
             wyn2[u].second);
    }
  }
  return 0;
