...
void pushup(int root) {
  ...
  if (mn[root] == mn[root << 1])
    L[root] = L[root << 1];
  else
    L[root] = max(mx[root << 1], L[root << 1 | 1]);
  if (mn[root] == mn[root << 1 | 1])
    R[root] = R[root << 1 | 1];
  else
    R[root] = max(mx[root << 1], R[root << 1 | 1]);
  if (mn[root] != mn[root << 1 | 1])
    ans[root] = ans[root << 1], cnt[root] = cnt[root << 1] + len[root << 1 | 1];
  else if (mn[root] != mn[root << 1])
    ans[root] = ans[root << 1 | 1],
    cnt[root] = cnt[root << 1 | 1] + len[root << 1];
  else {
...
void update(int x, int l, int r, int root) {
  if (l == r) {
    mx[root] = max(d[x], d[x + 1]);
    return;
  }
  pushdown(root);
  int mid = (l + r) >> 1;
  if (x <= mid)
    update(x, l, mid, root << 1);
  else
    update(x, mid + 1, r, root << 1 | 1);
  pushup(root);
}
