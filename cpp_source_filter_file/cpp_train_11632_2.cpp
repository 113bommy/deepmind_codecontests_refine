...
void NTT(int *a, int mode) {
  ...
  for (int len = 2; len <= N; len <<= 1) {
    int delta;
    if (mode == 1)
      delta = power(G, (P - 1) / len);
    else
      delta = power(G, P - 1 - (P - 1) / len);
...
vector<int> times(vector<int> a, vector<int> b) {
  ...
  NTT(tmp, 1);
  NTT(tnp, 1);
  for (int i = 0; i < N; i++) tmp[i] = 1ll * tmp[i] * tnp[i] % P;
  NTT(tmp, -1);
  vector<int> ans;
  ans.resize(limit);
  for (unsigned i = 0; i < ans.size(); i++) ans[i] = tmp[i];
  return ans;
}
}  // namespace NTT
...
void init(int n) {
  ...
  for (int i = n - 1; i >= 0; i--) inv[i] = inv[i + 1] * (i + 1ll) % P;
  for (int i = 1; i <= n; i++) vin[i] = power(i, P - 2);
}
void getans(int pos, int fa) {
  ...
  for (unsigned i = 0; i < a[pos].size(); i++)
    if (a[pos][i] != fa) {
      ...
      update(sum[pos], ans[pos][b[pos][i]]);
