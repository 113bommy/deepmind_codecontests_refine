#include <bits/stdc++.h>
using namespace std;
int ch[507][10], val[507], dp[507][10][507], tp[10][507];
      ...
      for (int j = 0; j <= K; ++j)
        for (int k = 0; k + j <= K; ++k)
          for (int l = 0; l < maxLen; ++l)
            tp[j + k][l] =
                min(tp[j + k][l], dp[x][j][l] + dp[ch[x][i]][k][l + 1]);
      memcpy(dp[x], tp, sizeof(tp));
    }
  for (int j = 0; j <= K; ++j)
    for (int k = 0; k <= maxLen; ++k) dp[x][j][k] += k * val[x];
  for (int j = 0; j < K; ++j)
...
int main() {
  ...
  calc(1);
  int minN = 1e9;
  for (int i = 0; i <= K; ++i) minN = min(minN, dp[1][i][0]);
  cout << minN;
  return 0;
}
