...
int main() {
  ...
  for (int i = 1; i <= 6000000; i++) pref[i] += pref[i - 1];
  for (int i = 1; i <= 6000000; i++) sum[i] += sum[i - 1];
  for (long long i = 2; i <= 6000000; i++)
    if (p[i] == 0) {
      for (long long j = i * i; j <= 6000000; j += i) p[j] = 1;
    }
  ...
  for (int i = 1; i <= h1; i++) {
    a = st[i];
    now = 0;
    for (int j = a; j <= 6000000; j += a) {
      mid = max(j - a + 1, j - can);
      if (mid > j - a + 1) now += col(j - a + 1, mid - 1) * x;
      now += y * (col(mid, j - 1) * j - getsum(j - 1, mid));
    }
    ans = min(ans, now);
  }
  cout << ans << endl;
