...
int Left[N], Right[N];
long long int tree[4][T + T + 7];
map<int, int> M;
int prv[N], nxt[N];
void addRes(long long int v) { res = (res + v) % MX; }
void update(int t2, int p, long long int v) {
  p += T;
  v %= MX;
  while (p) {
...
long long int ask(int t, int from, int to) {
  long long int ret = 0LL;
  from += T, to += T;
  while (from < to) {
    if (from & 1) ret += tree[t][from];
    if (!(to & 1)) ret += tree[t][to];
    from = (from + 1) >> 1;
    to = (to - 1) >> 1;
  }
  if (from == to) ret += tree[t][from];
  return ret % MX;
}
void DC(int from, int to) {
  ...
  while (!Q.empty()) {
    ...
    long long int cur = ask(0, max(mid, t) + 1, Right[t]) +
                        1LL * t * Left[t] * ask(3, max(mid, t) + 1, Right[t]);
    cur -= 1LL * Left[t] * ask(1, max(mid, t) + 1, Right[t]) +
           1LL * t * ask(2, max(mid, t) + 1, Right[t]);
    cur += 1LL * (Right[t] - t) *
           (ask(2, Right[t] + 1, n) - 1LL * Left[t] * ask(3, Right[t] + 1, n));
    addRes(cur);
  }
  for (int i = mid + 1; i <= to; ++i)
    if (Left[i] < mid && i < Right[i]) {
  ...
  for (int i = from; i <= mid - 1; ++i)
    if (Left[i] < i && mid < Right[i]) Q.push({Left[i], i});
  while (!Q.empty()) {
    int t = Q.top().second;
    Q.pop();
    if (t > mid) {
    ...
    addRes(1LL * (Left[t] - t) * ask(0, mid + 1, Right[t]));
  }
  int last = -1;
  for (int i = mid + 1; i <= to; ++i)
    if (Left[i] < mid && i < Right[i]) {
