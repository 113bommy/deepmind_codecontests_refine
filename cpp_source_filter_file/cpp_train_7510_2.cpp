...
int main() {
  ...
  for (int a = 0; a < n; a++) {
    ...
    for (int b = 0; b < str.size(); b++) {
      if (str[b] == '=') p1 = b;
      if (str[b] == '$' or str[b] == '^' or str[b] == '#' or str[b] == '&')
        p2 = b;
    }
  ...
  if (val["res"] != "res") {
    ...
    for (int a = 0; a < var[x].size(); a++) {
      if (var[x][a] == '$' or var[x][a] == '^' or var[x][a] == '#' or
          var[x][a] == '&')
        p = a;
    }
    if (p == -1)
      ...
      dfs(x);
      ans.back() = "res" + ans.back().substr(p);
    }
  }
  cout << ans.size() << "\n";
