...
void rev(int x) {
  if (!x) return;
  r[x] ^= 1, swap(c[x][0], c[x][1]);
}
void push_down(int x) {
  if (!x) return;
  if (r[x]) rev(c[x][0]), rev(c[x][1]), r[x] = 0;
}
void push_up(int x) {
  if (!x) return;
  mx[x] = v[mx[c[x][0]]] > v[mx[c[x][1]]] ? mx[c[x][0]] : mx[c[x][1]];
  if (v[x] > v[mx[x]]) mx[x] = x;
  siz[x] = vsiz[x] + siz[c[x][0]] + siz[c[x][1]] + (x <= n);
}
void push_all(int x) {
  if (!x) return;
...
int addedge(int x) {
  ...
  if (fa == fb) {
    int t = T::getmax(a, b);
    if (T::v[t] < l) return cnt == 0 ? len[S.begin()->e] : -1;
    ins(t - n, 0);
  } else {
    if (T::siz[fa] & 1) cnt--;
    if (T::siz[fb] & 1) cnt--;
