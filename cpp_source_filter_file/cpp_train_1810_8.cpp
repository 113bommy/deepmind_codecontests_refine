...
struct tup {
  int l, r, v;
  friend tup operator+(const tup &L, const tup &R) {
    return {L.l, R.r, L.v + R.v};
  }
...
struct tag {
  ...
  bool rev;
  tag(int l = 0, int x = 0) : rev() {
    lmax = rmax = gmax = lmin = rmin = gmin = sum = {l, l, x};
  }
  friend tag operator+(const tag &L, const tag &R) {
    ...
    ret.rmin = std::min(R.rmin, L.rmax + R.sum);
...
struct node {
  ...
  void rev() {
    std::swap(t.lmax, t.lmin), std::swap(t.rmax, t.rmin),
        std::swap(t.gmax, t.gmin), t.rev ^= 1;
    t.lmax.v *= -1, t.lmin.v *= -1, t.rmax.v *= -1, t.rmin.v *= -1,
        t.gmax.v *= -1, t.gmin.v *= -1, t.sum.v *= -1;
  }
  void pushdown() {
...
int main() {
  ...
  for (scanf("%d", &m); m--;) {
    ...
    if (op) {
      ...
      for (tag i; !q.empty(); q.pop())
        i = q.front(), root->update2(i.gmax.l, i.gmax.r);
      printf("%d\n", lans);
    } else {
      root->update(x, y);
    }
