...
void solve(P *S, vector<pair<P, P> > &res) {
  ...
  for (int i = 1; i <= m; i++) move(S[i], (P){i, i, S[i].id}, res);
}
...
int main() {
  scanf("%d%d", &n, &m);
  for (int i = 1; i <= m; i++) scanf("%d%d", &S[i].x, &S[i].y), S[i].id = i;
  ...
  for (int i = 0; i < SS.size(); i++)
    printf("%d %d %d %d\n", SS[i].first.x, SS[i].first.y, SS[i].second.x,
           SS[i].second.y);
  for (int i = 0; i < MM.size(); i++)
    printf("%d %d %d %d\n", MM[i].first.x, MM[i].first.y, MM[i].second.x,
           MM[i].second.y);
  for (int i = TT.size() - 1; i >= 0; i--)
    printf("%d %d %d %d\n", TT[i].second.x, TT[i].second.y, TT[i].first.x,
           TT[i].first.y);
}
