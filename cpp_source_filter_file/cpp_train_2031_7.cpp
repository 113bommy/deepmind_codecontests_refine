...
long long DFS(const int u, const int S) {
  long long& ret = f[u][S];
  if (~ret) return ret;
  ret = 0;
  ...
  for (int T = (S - 1) & S; T; T = (T - 1) & S)
    if (in(T, ref[pos]) && !in(T, ref[u])) {
      ...
      if (!flag) continue;
      for (int i = 1; i <= M; i++)
        if (!onE(u, i) && (in(T, ref[Ev[i]]) ^ in(T, ref[Ev[i]]))) {
          flag = false;
          break;
        }
      if (!flag) continue;
      if (!inter(T, ERis[u])) {
        for (int i = 1; i <= N; i++)
          if (in(T, ref[i])) ret += DFS(u, S ^ T) * DFS(i, T);
...
int main() {
  ...
  memset(f, -1, sizeof f);
  for (int i = 1; i <= N; i++)
    f[i][1 << i - 1] = 1, bit[ref[i] = (1 << i - 1)] = i;
  for (int i = 1; i <= M; i++)
    read(Eu[i]), read(Ev[i]), ERis[Eu[i]] |= ref[Ev[i]],
        ERis[Ev[i]] |= ref[Eu[i]];
  for (int i = 1, a, b; i <= Q; i++)
    read(a), read(b), read(LCAc[i]), LCARis[i] = ref[a] | ref[b];
