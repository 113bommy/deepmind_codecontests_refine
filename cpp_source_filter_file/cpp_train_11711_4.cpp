...
double x, f[55][60005], g[105][60005], p[105][20005], dis[105][105];
int limit, L, R[100005];
...
void solve(int l, int r) {
  ...
  for (int i = 1; i <= m; i++) {
    ...
    for (int j = mid + 1; j <= r; j++) A[j - mid - 1] = Complex(f[v][j], 0);
    for (int j = 1; j <= r - l; j++) B[r - l - j] = Complex(p[i][j], 0);
    FFT(A, 1);
    FFT(B, 1);
    for (int j = 0; j < limit; j++) A[j] = A[j] * B[j];
    FFT(A, -1);
    for (int j = l; j <= mid; j++) g[i][j] += A[j - mid + r - l - 1].x;
  }
  solve(l, mid);
}
int main() {
  ...
  for (int i = 1; i <= n; i++)
    for (int j = t + 1; j <= 2 * t; j++) f[i][j] = dis[i][n] + x;
  solve(0, 2 * t);
  printf("%.12lf\n", f[1][0]);
  return 0;
}
