...
inline bool samedir(int i, int a, int b) {
  long long xa = mx[a] - ax[i], ya = my[a] - ay[i];
  long long xb = mx[b] - ax[i], yb = my[b] - ay[i];
  return xa * yb - ya * xb == 0 && xa * xb + ya * yb > 0;
}
inline long long dist(int i, int a) {
  long long xd = mx[a] - ax[i], yd = my[a] - ay[i];
  return xd * xd + yd * yd;
}
int Ans;
...
int main() {
  ...
  for (int i = 0; i < K; ++i) {
    for (int j = 1; j <= N; ++j) {
      at2[i][j] = atan2(my[j] - ay[i], mx[j] - ax[i]);
      p[i][j] = j;
    }
    ...
    for (int j = 1; j <= N; ++j)
      lp[i][j] = j == 1                             ? 1
                 : samedir(i, p[i][j - 1], p[i][j]) ? lp[i][j - 1]
                                                    : j,
      q[i][p[i][j]] = j;
  }
  for (int i = 1; i <= N; ++i) {
