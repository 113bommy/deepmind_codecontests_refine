...
nod *empty = new nod();
nod *sg[100009];
nod *ins(nod *root, int x, int y, int st = 1, int en = n + 1) {
  if (x < st || x > en) return root;
  if (st == en) return new nod(y);
  int md = st + (en - st) / 2;
  nod *u = ins(root->l, x, y, st, md);
  nod *v = ins(root->r, x, y, md + 1, en);
  return new nod(min(u->mn, v->mn), u, v);
}
int go(nod *root, int x, int st = 1, int en = n + 1) {
  int mid = st + (en - st) / 2;
  if (st == en) return st;
  if (root->l->mn < x)
    go(root->l, x, st, mid);
  else
    go(root->r, x, mid + 1, en);
}
int cal(int x, int y) {
  if (x > y) return 0;
  return go(sg[y], x);
