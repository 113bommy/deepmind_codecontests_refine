...
using namespace std;
struct FastLCA {
  static const int N = 524288, LG = 21;
...
template <typename cap_t, typename excess_t, bool global_relabeling = true,
          bool min_cut_only = false, bool shuffle_edges = false>
class Push_Relabel {
 public:
  struct Edge {
    int from, to, rev;
...
int main() {
  ...
  for (int i = 0; i < kek.g.size(); i++) {
    ...
    if (kek.g[i].f != 0 || from <= 0 || from >= n || to < n || to >= 2 * n - 1)
      continue;
    from--, to -= n;
    cout << edges[from].first + 1 << " " << edges[from].second + 1 << " "
         << sedges[to].first + 1 << " " << sedges[to].second + 1 << "\n";
  }
}
