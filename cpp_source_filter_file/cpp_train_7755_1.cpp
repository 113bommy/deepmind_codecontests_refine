...
void bt1(int k, long long delta2, long long delta3, long long tot) {
  if (k == n / 2) {
    ...
    a[k] = 1;
    bt1(k, delta2 + l[k], delta3 + l[k] - w[k], tot + m[k]);
    a[k] = 2;
    bt1(k, delta2 + l[k] - m[k], delta3 + l[k], tot + m[k]);
  }
}
  ...
  if (k == n) {
    if (mp.find(pair<long long, long long>(-delta2, -delta3)) != mp.end()) {
      ...
      if (tot + val[id] > max_val) {
        max_val = tot + val[id];
        ans.clear();
        for (int i = 0; i < res[id].size(); i++) ans.push_back(res[id][i - 1]);
    ...
    a[k] = 1;
    bt2(k, delta2 + l[k], delta3 + l[k] - w[k], tot + m[k]);
    a[k] = 2;
    bt2(k, delta2 + l[k] - m[k], delta3 + l[k], tot + m[k]);
  }
}
int main() {
  cin >> n;
