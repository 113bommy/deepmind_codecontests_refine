...
inline bool check(int x, int y) {
  ...
  for (int i = x; i < x + 3; i++)
    for (int j = y; j < y + 3; j++) {
      if (app[board[i][j] / 4]) f1 = false;
      app[board[i][j] / 4] = true;
      if (board[i][j] % 4 != board[x][y] % 4) f2 = false;
    }
  return f1 || f2;
}
inline bool inSquare(int sr, int sc, int x, int y) {
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      for (int I = 0; I < 3; I++)
        for (int J = 0; J < 3; J++)
          if (sr + i == x + I && sc + j == y + J) return false;
  return true;
}
inline vector<pair<int, int> > check() {
  vector<pair<int, int> > ret;
  for (int i = 0; i < n - 2 && (int)(ret.size()) < 2; i++)
    for (int j = 0; j < m - 2 && (int)(ret.size()) < 2; j++)
      if (check(i, j)) {
        bool f = true;
        for (int _ = 0; _ < (int)(ret.size()); _++)
          f ^= inSquare(ret[_].first, ret[_].second, i, j);
        if (f) ret.push_back(make_pair(i, j));
      }
  return ret;
}
inline pair<int, int> Find(int index) {
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++)
      if (board[i][j] == index) return make_pair(i, j);
...
int cnt;
void init() {
  for (int i = 0; i < 13; i++) Number[rank_[i]] = i;
  for (int i = 0; i < 4; i++) Number[suit[i]] = i;
  cin >> n >> m;
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++) {
      string S;
...
int main() {
  ...
  if (!cnt) {
    ...
    if ((int)(sqr.size()) < 2) die("No solution.");
