#include <bits/stdc++.h>
int N, head[51], next[99], from[99], to[99], len[99], E, root, M, q[51], fa[51],
    count_e[51], f[51][99][51], vi[51][99];
void BFS() {
  int H = 0, T = 1, u;
  q[1] = root;
  while (H < T)
  ...
  if (!next[head[to[e]]]) {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      while (l < r) {
        ...
        for (int g = head[to[e]]; g; g = next[g])
          if (to[g] != from[e])
            count += std::upper_bound(f[n][g], f[n][g] + n + 1, m - len[g],
