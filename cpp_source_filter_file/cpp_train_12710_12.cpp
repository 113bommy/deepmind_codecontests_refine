...
int cmpd;
bool cmp(qq x, qq y) {
  return x.d[cmpd] < y.d[cmpd] ||
         (x.d[cmpd] == y.d[cmpd] &&
          x.d[(cmpd + 1) % 3] < y.d[(cmpd + 1) % 3]) ||
         (x.d[cmpd] == y.d[cmpd] &&
          x.d[(cmpd + 1) % 3] == y.d[(cmpd + 1) % 3] &&
          x.d[(cmpd + 2) % 3] == y.d[(cmpd + 2) % 3]);
}
...
int bt(int l, int r, int d) {
  ...
  if (l < mid)
    tr[mid].son[0] = bt(l, mid - 1, (d + 1) % 3), update(mid, tr[mid].son[0]);
  if (mid < r)
    tr[mid].son[1] = bt(mid + 1, r, (d + 1) % 3), update(mid, tr[mid].son[1]);
  return mid;
}
...
bool find(int now) {
  ...
  if (tr[now].d[0] >= Mn1 && tr[now].d[0] <= Mx1 && tr[now].d[1] >= Mn2 &&
      tr[now].d[1] <= Mx2 && tr[now].d[2] >= Mn3 && tr[now].d[2] <= Mx3)
    return true;
