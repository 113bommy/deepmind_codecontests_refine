...
inline bool On_left(Point A, Point B) { return dcmp(Cross(B, A)) > 0; }
int main() {
  ...
  for (int i = 1, side; i <= n; i++) {
    top = 0;
    Point v = Point(-e[i].b, e[i].a);
    if (e[i].a == 0)
      side = e[i].c < 0;
    else
      side = ((e[i].c < 0) ^ (e[i].a < 0));
    for (int j = 1; j <= n; j++) {
      if (i == j) continue;
      inter = Get_Inter(i, j);
      if (On_left(Point(-e[j].b, e[j].a), v) ^ side)
        st[++top] = Angle(Point(e[j].b, -e[j].a), Point(-inter.x, -inter.y));
      else
        st[++top] = Angle(Point(-e[j].b, e[j].a), Point(-inter.x, -inter.y));
    }
    sort(st + 1, st + top + 1);
