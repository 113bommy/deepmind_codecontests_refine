...
bool check(int mid) {
  ...
  std::sort(t + 1, t + n + n + 1,
            [&](int A, int B) -> bool { return e[A] < e[B]; });
  int id = 0;
  for (int i = 1; i <= n + n; ++i) {
    int j = i;
...
int dfs(int x, int flow) {
  ...
  for (int &i = cur[x]; ~i; i = c[i].nxt) {
    int v = c[i].v;
    if (!inq[v] && dep[v] == dep[x] + c[i].cost &&
        (rf = dfs(v, min(flow, c[i].w)))) {
      flow -= rf, f += rf;
      c[i].w -= rf, c[i ^ 1].w += rf;
      if (!flow) return f;
    }
...
int calc(int mid) {
  ...
  for (int i = 1; i <= n; ++i)
    addedge(S, i, 1, 0), addedge(i, i + n, 1, 1), addedge(i, i + 3 * n, 1, 0);
  for (int i = 1; i <= n + n; ++i)
    addedge(n + i, 3 * n + i, inf, 0), addedge(3 * n + i, 5 * n + i, inf, 1),
        addedge(3 * n + i, 7 * n + i, inf, 0),
        addedge(5 * n + i, 7 * n + i, inf, 0);
