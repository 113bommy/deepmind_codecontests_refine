...
char s[155];
int sc[155];
int ac[155][155][155], dp[155][155];
inline void relax(int &x, int val) {
  if (val > x) x = val;
}
inline int getac(int i, int j, int l) {
  return i == j + 1 ? sc[l] : ac[i][j][l];
}
inline int solve() {
  for (int i = 0; i < sl; i++) {
    for (int j = i; j < sl; j++) {
      for (int l = 0; l <= sl; l++) ac[i][j][l] = -inf;
      dp[i][j] = 0;
    }
  }
  for (int i = 0; i < sl; i++) {
  ...
  for (int len = 2; len <= sl; len++) {
    for (int i = 0; i + len <= sl; i++) {
      int j = i + len - 1;
      for (int k = i; k < j; k++) relax(dp[i][j], dp[i][k] + dp[k + 1][j]);
      for (int l = 0; l <= sl - (i - j + 1); l++) {
        for (int t = 2; t <= l; t += 2)
