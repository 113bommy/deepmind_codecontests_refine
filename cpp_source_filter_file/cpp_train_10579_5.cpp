...
const int N = 1e7 + 5;
int n, m, p[N], v[N], a[N], f[N], g[N];
long long s1, s2, s3, phi[N];
int main() {
  n = read();
  for (int i = 2; i <= n; i++) {
  ...
  for (int i = 2; i <= n; i++)
    g[i] = g[i - 1] + f[i], s1 += i - phi[i] - 1,
    s2 -= (1ll * a[i] * a[i] <= n), s3 += (a[i] * 2 <= n);
  for (int i = 2; i <= n; i++) s2 += f[i] * g[n / i];
  s2 = s2 / 2 - s1, s3 = s3 * (s3 - 1) / 2 - s1 - s2;
  write(s1 + 2 * s2 + 3 * s3, '\n');
  return ot(), 0;
}
