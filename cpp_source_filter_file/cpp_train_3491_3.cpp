...
const int P = 1e9 + 7;
const int N = 100050;
long long fac[N], inv[N], m25[N], m26[N], im26[N];
...
void prework(void) {
  ...
  m25[0] = 1, m26[0] = 1;
  long long inv = fpw(26, P - 2);
  im26[0] = 1;
  for (int i = 1; i <= lim; i++)
    m25[i] = m25[i - 1] * 25 % P, m26[i] = m26[i - 1] * 26 % P,
    im26[i] = im26[i - 1] * inv % P;
}
...
int Calc(int x) {
  ...
  for (int i = x; i <= lim; i++)
    st[cnt][i] =
        (st[cnt][i - 1] + m25[i - x] * C(i - 1, x - 1) % P * im26[i]) % P;
  return cnt;
}
int main() {
  read(q);
