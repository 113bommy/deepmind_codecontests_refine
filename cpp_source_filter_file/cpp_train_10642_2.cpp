...
class HLD {
 ...
 public:
  ...
  vector<vector<pair<int, int>>> inducedTree(vector<int>& v) const {
    ...
    for (auto j : act) {
      if (!sta.empty()) {
        int x = lca(v[j], v[sta.back()]);
        while (sta.size() > 1 && dep[v[sta.back()]] > dep[x]) sta.pop_back();
        int i = sta.back();
        int d = dep[v[i]] + dep[v[j]] - 2 * dep[x];
...
void solve(const vector<pair<int, int>>& virus, const vector<int>& city,
  ...
  while (!que.empty()) {
    ...
    for (auto pr : g[i]) {
      ...
      int rest =
          max(0, ((-t_sta) + (virus[vi].second - 1)) / (virus[vi].second));
      ll t_di = di + rest;
  ...
  for (auto i : city) cout << sic[i].first.second + 1 << ' ';
  cout << '\n';
}
int main() {
