...
using namespace std;
int n, q;
long long segt[4000005], flip[4000005];
bool lazy[4000005];
vector<int> ch[200005];
int ord[200005], eord[200005], lt[200005];
...
inline void evallazy(int nd, int nl, int nr) {
  ...
  if (lazy[nd]) {
    ...
    if (nr - nl > 1) {
      ...
      lazy[nd * 2 + 2] ^= 1;
      if (lazy[nd * 2 + 1])
        flip[nd * 2 + 1] += (long long)(mid - nl) - segt[nd * 2 + 1] * 2;
      if (lazy[nd * 2 + 2])
        flip[nd * 2 + 2] += (long long)(nr - mid) - segt[nd * 2 + 2] * 2;
    }
    flip[nd] = 0;
    lazy[nd] = false;
  }
...
int flipseg(int l, int r, int nd = 0, int nl = 0, int nr = n) {
  ...
  evallazy(nd, nl, nr);
  if (l == nl && r == nr) {
    flip[nd] += (long long)(nr - nl) - segt[nd] * 2;
    lazy[nd] = true;
