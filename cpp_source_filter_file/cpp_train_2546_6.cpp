...
int n, m;
double V;
bool cmpa(const node i, const node j) { return i.alpha < j.alpha; }
bool cmpid(const node i, const node j) { return i.id < j.id; }
bool cmpx(const point i, const point j) { return i.x < j.x; }
double get_x(double alpha) { return (2 * V * V * sin(alpha) * cos(alpha) / g); }
