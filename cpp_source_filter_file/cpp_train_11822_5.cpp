...
ostream &operator<<(ostream &out, pair<T, TT> t) {
...
void add(int &p, int q, int l, int r, int x) {
  ...
  if (x <= o)
    ...
    add(rc[p], rc[q], o + 1, r, x);
}
bool check(int p, int l, int r, int ql, int qr) {
  if (!p || ql > r || qr < l) return false;
  if (ql <= l && r <= qr) return true;
  int o = l + r >> 1;
  return check(lc[p], l, o, ql, qr) | check(rc[p], o + 1, r, ql, qr);
}
void dfs(int x) {
  for (int y : edge[x]) dfs(y), rot[x] = add(rot[x], rot[y]);
  if (sam::mark[x]) add(rot[x], rot[x], 1, n, pos[x]);
}
}  // namespace seg
int dp[MX];
...
int main() {
  ...
  for (int i = sam::cnt; i > 1; i--) edge[sam::par[i]].push_back(i);
  seg::dfs(1);
  for (int u : edge[1]) dp[u] = 1, dfs(u, u);
