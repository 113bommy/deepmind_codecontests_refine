...
using namespace std;
const int maxn = 50 + 1;
bitset<26> can[maxn][maxn], nac[maxn][maxn];
int dp[maxn][maxn], q;
char qu[maxn][3];
string s, t;
void fill(string &s, bitset<26> can[maxn][maxn]) {
  for (int i = 0; i < ((int)(s).size()); i++) can[i][i][s[i] - 'a'] = 1;
    ...
    for (int j = 1; j <= ((int)(t).size()); j++)
      for (int ii = i; ii > 0; ii--)
        for (int jj = j; jj > 0; jj--)
          if ((can[ii - 1][i - 1] & can[jj - 1][j - 1]).any())
            ((dp[i][j]) = min((dp[i][j]), (dp[ii - 1][jj - 1] + 1)));
  cout << (dp[((int)(s).size())][((int)(t).size())] < 100
               ? dp[((int)(s).size())][((int)(t).size())]
               : -1)
       << '\n';
  return 0;
}
