...
void InsertACAM(string &s, int k) {
  ...
  for (i = p = 0; i < (signed)s.size(); i++) {
    if (!f[p].s[s[i] - 97]) f[p].s[s[i] - 97] = c++;
    p = f[p].s[s[i] - 97];
  }
  f[p].g = k;
  return;
...
void InitACAM(void) {
  static int q[10000020];
  int i, s, t;
  int l, r;
  l = r = 0;
  if (f[0].s[0]) q[r++] = f[0].s[0];
  if (f[0].s[1]) q[r++] = f[0].s[1];
  for (; l < r; l++) {
    s = q[l];
    for (i = 0; i < 2; i++) {
      t = f[s].s[i];
...
int BlockDinic(int x, int t, int v) {
  ...
  for (f = 0; r[x] < (signed)e[x].size(); r[x]++) {
    k = e[x][r[x]];
    if (d[k].second && l[d[k].first] == l[x] + 1 &&
        (f = BlockDinic(d[k].first, t, min(v, d[k].second)))) {
      d[k].second -= f;
      d[k ^ 1].second += f;
      break;
    }
...
int Dinic(int s, int t) {
  ...
  for (o = 0; LevelDinic(s, t);) {
    memset(r, 0, sizeof(r));
    while ((f = BlockDinic(s, t, INT_MAX))) o += f;
  }
  return o;
}
...
int main(void) {
  ...
  for (i = 1; i <= n; i++) {
    for (j = p = 0; j < (signed)a[i].size(); j++) {
      if (ACAM::f[p].g) e[i][ACAM::f[p].g] = true;
      p = ACAM::f[p].s[a[i][j] - 97];
    }
    if (ACAM::f[ACAM::f[p].f].g) e[i][ACAM::f[ACAM::f[p].f].g] = true;
  }
  for (k = 1; k <= n; k++)
    for (i = 1; i <= n; i++)
      for (j = 1; j <= n; j++) e[i][j] |= e[i][k] & e[k][j];
  for (i = 1; i <= n; i++)
    for (j = 1; j <= n; j++)
      if (e[i][j]) Dinic::AddEdge(i, j + n + 1, 1);
  for (i = 1; i <= n; i++) {
  ...
  for (i = 1; i <= n; i++)
    if (Dinic::l[i] && !Dinic::l[i + n]) cout << i << ' ';
  cout << endl;
  return 0;
}
