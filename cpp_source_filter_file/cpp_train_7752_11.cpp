...
void out(const vector<double>& p) {
  for (int i = 0; i < p.size(); ++i) printf("%lf ", p[i]);
  printf("\n");
}
int main() {
  int n, k, x0, y0, e;
  ...
  while (r - l > 1e-12) {
    ...
    if (k <= cnt) {
      ...
      int k1 = k - cnt;
      vector<vector<long double> > prob(p.size() + 1,
                                        vector<long double>(k1 + 1, 0.));
      prob[0][0] = 1.;
      ...
      if (prob[p.size()][k1] > e / 1000.) kill = true;
    }
    if (kill)
      r = x;
    else
