...
int main() {
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    if (ml > mr)
      ...
      cu[i] = sub(
          1, mul(mr - ml + 1,
                 pow(mul(r[i] - l[i] + 1, r[i + 1] - l[i + 1] + 1), mod - 2)));
    a = add(a, cu[i]);
  }
  int csu = 0;
  for (int i = 1; i < n - 1; i++) {
    int cuu = 1;
    int ml3 = max(l[i - 1], max(l[i], l[i + 1])),
        mr3 = min(r[i - 1], max(r[i], r[i + 1]));
    int cho3 = pow(mul(r[i - 1] - l[i - 1] + 1,
                       mul(r[i + 1] - l[i + 1] + 1, r[i] - l[i] + 1)),
                   mod - 2);
    ...
    if (ml2 <= mr2) {
      cuu = sub(cuu,
                mul(cho3, mul(mr2 - ml2 + 1 - isec, r[i + 1] - l[i + 1] + 1)));
