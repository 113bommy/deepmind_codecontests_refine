...
struct pt {
  ...
  bool operator==(pt p) { return abs(x - p.x) < 1e-18 && abs(y - p.y) < 1e-18; }
  ...
  bool operator<(pt p) const {
    return x < p.x || (abs(x - p.x) < 1e-18 && y < p.y);
  }
  bool left(pt p, pt q) { return (q - p) % (*this - p) >= -1e-18; }
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    int xi, yi;
    scanf("%d %d", &xi, &yi);
    if (!ereccion[make_pair(xi, yi)].size())
  ...
  set<int> ans;
  for (auto p : concha) {
    if (p.x - 1e-18 < downmost.x && p.y - 1e-18 < leftmost.y &&
        (p - leftmost) % (downmost - leftmost) >= -1e-6) {
      int xx = (1. / p.x + 1e-3);
      int yy = (1. / p.y + 1e-3);
