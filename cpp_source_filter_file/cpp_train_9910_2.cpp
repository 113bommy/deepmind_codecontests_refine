...
int main() {
  ...
  for (int i = 0; i < cor2.size(); i++) {
    if (i == n2 - 1) {
      ...
      cor3.push_back(make_pair(make_tuple(y, z, x), cor2[i].second));
      break;
    }
    int y = get<0>(cor2[i].first);
  ...
  for (int i = 0; i < cor3.size(); i++) {
    if (i == n3 - 1) {
      int z = get<0>(cor[i].first);
      int x = get<1>(cor[i].first);
      int y = get<2>(cor[i].first);
      cor4.push_back(make_pair(make_tuple(y, z, x), cor3[i].second));
      break;
    }
