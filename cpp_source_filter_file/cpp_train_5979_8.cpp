...
bool ok(vector<pair<int, pair<int, int> > >& new_vert,
        vector<pair<int, pair<int, int> > >& new_hor) {
  ...
  for (auto& it : new_vert) {
    it = make_pair(by_x[it.first],
                   make_pair(by_y[it.second.first], by_y[it.second.second]));
    events[it.first].emplace_back(make_pair(ASK, it.second));
  }
  for (auto& it : new_hor) {
    it = make_pair(by_y[it.first],
                   make_pair(by_x[it.second.first], by_x[it.second.second]));
    events[it.second.first].push_back(
        make_pair(ADD, make_pair(it.first - 1, -1)));
...
bool check(int val) {
  ...
  for (auto t : vert) {
    ...
    new_vert.emplace_back(
        t.first, make_pair(t.second.first + val, t.second.second - val));
  }
  for (auto t : hor) {
    if (t.second.second - t.second.first < 2 * val) continue;
    new_hor.emplace_back(
        t.first, make_pair(t.second.first + val, t.second.second - val));
  }
