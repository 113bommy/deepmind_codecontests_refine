...
struct BIT {
  ...
  inline void add(int p, int x) {
    ...
    for (; p; p -= ((p) & -(p))) c[p] = max(c[p], x);
  }
  inline int ask(int p) {
    p = lower_bound(v.begin(), v.end(), p) - v.begin() + 1;
    int maxn = -2147483648;
    for (; p <= n; p += ((p) & -(p))) maxn = max(maxn, c[p]);
    return maxn;
  }
} t;
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    f[i] = stk[tp];
    while (tp && 2ll * d[stk[tp]] + a[f[stk[tp]]] <= 2ll * d[i] + a[d[i]]) --tp;
    stk[++tp] = i;
  }
  stk[tp = 0] = n + 1;
  for (int i = n; i; i--) {
  ...
  for (int i = 1; i <= n; i++)
    if (a[g[i]] - a[f[i]] <= 2 * d[i]) puts("0"), exit(0);
  int ans = a[n + 1];
  for (int i = 1; i <= n; i++) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    if (a[g[i]] - a[i] < 2 * d[i])
      ans = min(ans, a[i] - t.ask(a[g[i]] - 2 * d[i]));
