...
int main() {
  ...
  while (not q.empty()) {
    ...
    int panelColor = cur[getParent(pColor[now.first][now.second])];
    if (panelColor != 0 and panelColor != color) {
      ans += w[getParent(pColor[now.first][now.second])];
      sort(geo[panelColor].begin(), geo[panelColor].end(), cmp);
      for (pair<int, int> &next : geo[panelColor]) {
        q.push(next);
      }
      geo[panelColor].clear();
      if (getParent(pColor[now.first][now.second]) != getParent(color))
        p[getParent(pColor[now.first][now.second])] = getParent(color);
      cur[getParent(pColor[now.first][now.second])] = color;
    }
  }
  cout << ans << endl;
  return 0;
