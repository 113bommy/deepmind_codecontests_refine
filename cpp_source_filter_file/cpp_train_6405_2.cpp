...
void tarjan(int pos, int fa) {
  ...
  for (unsigned i = 0; i < c[pos].size(); i++) {
    ...
    } else if (instack[c[pos][i]])
      Low[pos] = min(Low[pos], Dfn[c[pos][i]]);
  }
  if (Low[pos] == Dfn[pos]) {
    int tmp = Stack[top--];
    belong[tmp] = ++tot;
...
void solve(int task) {
  ...
  for (int i = 1; i <= cnt; i++) {
    ...
    if (num[points[i - 1]] != num[points[i]]) {
      ...
      while (top >= 2 && depth[lca(x, Stack[top])] <= depth[top - 1]) {
        addedge(Stack[top], Stack[top - 1]);
        top--;
      }
      if (lca(x, Stack[top]) == Stack[top])
        Stack[++top] = x;
      else {
  ...
  bool ans = true;
  for (int i = 1; i <= n; i++) ans &= belong[pos[i]] == belong[pos[1]];
  if (ans)
    puts("YES"), online += task;
  else
    puts("NO");
  tot = Timer = 0;
  for (int i = 1; i <= cmt; i++) {
...
int main() {
  ...
  for (int i = 1; i <= n; i++)
    for (auto x : a[i])
      if (find(i) != find(x)) b[find(i)].push_back(find(x));
  for (int i = 1; i <= n; i++)
    if (find(i) == i && dfn[i] == 0) {
      work(i, 0, i);
    }
