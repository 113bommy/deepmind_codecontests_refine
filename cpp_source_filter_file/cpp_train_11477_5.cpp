...
int n, K;
long long Len, a[N];
std::pair<long long, long long> pts[N];
inline void up(long long &x, const long long y) { x < y ? x = y : 0; }
...
inline bool check_inf(long long x) {
  long long s = 0;
  for (int i = 0; i < n; ++i) s += a[i] / x;
  return s >= K;
}
inline bool check_sup(long long x) {
  long long s = 0;
...
void work() {
  int i, cnt = 0;
  long long inf, sup, L, R, M, u, v, ans;
  scanf("%lld%d%d", &Len, &n, &K), a[n] = Len;
  ...
  for (L = 1, R = min(u, Len / K); L < R;
       check_inf(M = (L + R + 1) / 2) ? L = M : R = M - 1)
    ;
  inf = L;
  for (L = (Len - 1) / K + 1, R = v; L < R;
       check_sup(M = (L + R) / 2) ? R = M : L = M + 1)
    ;
  sup = R;
  for (i = 0; i < n; ++i)
    if ((u = (a[i] - 1) / sup + 1) > (v = a[i] / inf))
      pts[cnt++] =
          std::pair<long long, long long>(a[i] / u, -((a[i] - 1) / v + 1));
