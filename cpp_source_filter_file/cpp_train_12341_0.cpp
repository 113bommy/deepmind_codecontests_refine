...
int T, n;
long long a[3], x[N], y[N], z[N];
bool check(long long mid) {
  long long l[4] = {-2 * inf, -2 * inf, -2 * inf, -2 * inf},
            r[4] = {2 * inf, 2 * inf, 2 * inf, 2 * inf};
  for (int i = 1; i <= n; ++i) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (L <= (r[3] - i * 3) >> 1 && R >= (l[3] - i * 3 + 1) >> 1) {
      long long x = max(0ll, ((l[3] - i * 3 + 1) >> 1) - L);
      for (int j = 0; j < 3; ++j) {
        long long y = min(x, ((r[j] - i) >> 1) - a[j]);
        a[j] += y, x -= y;
      }
      for (int j = 0; j < 3; ++j) a[j] = a[j] * 2 + i;
      a[1] = (a[0] + a[1]) / 2, a[2] = (a[0] + a[2]) / 2,
      a[0] = a[1] + a[2] - a[0];
      return 1;
    }
  skip:;
