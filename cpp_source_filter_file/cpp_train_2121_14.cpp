...
int n, rt, siz[_], ch[3][_], f[_], g[_], d[_];
vector<int> e[_];
long long F(int x);
...
long long F(int x) {
  ...
  for (int i = 0; i <= 1; ++i, swap(lc, rc)) {
    if (!ch[1][rc]) {
      if (ch[0][rc]) ans += G(lc, ch[0][rc]);
      if (!g[rc] && d[rc] <= d[x]) ans += F(lc) * (d[x] == d[rc] ? 1 : 2);
    } else {
      if (!g[ch[0][rc]] && d[ch[0][rc]] <= d[x]) ans += G(lc, ch[1][rc]);
      if (!g[ch[1][rc]] && d[ch[1][rc]] <= d[x]) ans += G(lc, ch[0][rc]);
    }
  }
  return f[x] = ans % yl;
}
...
int main() {
  ...
  if (rt == 0) cout << (n * n * 2 - n * 2 + 4) % yl << endl, exit(0);
  dfs(0, rt);
  cout << solve() << endl;
  return 0;
}
