...
struct Ver {
  int a, b, m, rm;
  bool operator<(const Ver& other) const { return a < other.a; }
};
...
void dfs1(int v, int p) {
  vab.push_back({a[v], b[v], m[v], rm[v]});
  vb.push_back(b[v]);
  for (const auto& e : g[v]) {
    if (!ban[e.to] && p != e.to) {
      dfs1(e.to, v);
...
void calc(int v, int sgn) {
  ...
  FenwickSum ts((int)((vb).size()));
  FenwickMul tm((int)((vb).size()));
  int pt = -1;
  int mul = 1;
  for (int i = (int)((vab).size()) - 1; i >= 0; i--) {
    const Ver& el = vab[i];
    while (pt < i - 1 && vab[pt + 1].a <= -el.a) {
      const Ver& to_add = vab[++pt];
      int pos = lower_bound((vb).begin(), (vb).end(), to_add.b) - vb.begin();
      ts.upd(pos, 1);
      tm.upd(pos, to_add.m);
    }
    if (pt == i) {
      const Ver& to_del = vab[pt--];
      int pos = lower_bound((vb).begin(), (vb).end(), to_del.b) - vb.begin();
      ts.upd(pos, -1);
      tm.upd(pos, to_del.rm);
    }
    int pos = upper_bound((vb).begin(), (vb).end(), -el.b) - vb.begin() - 1;
    if (pos > -1) {
...
int32_t main() {
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    --v;
    int ry = pw(y, mod - 2);
    g[u].push_back({v, x, y, ry});
    g[v].push_back({u, x, y, ry});
  }
  cent_dec(0, -1);
  cout << ans;
}
