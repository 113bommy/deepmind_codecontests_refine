...
long long inv(long long a) { return my_pow(a, mod - 2); }
int n, inp[nmax];
int sum = 0;
long long diff[nmax];
long long when[nmax];
int main() {
  ...
  for (int i = 1; i < sum; i++) {
    long long RHS =
        1LL * (sum - i) * inv(sum) % mod * 1 % mod * inv(n - 1) % mod;
    long long LHS = (1 + 1LL * i * inv(sum) % mod * diff[i - 1]) % mod;
    diff[i] = LHS * inv(RHS) % mod;
  }
  for (int i = sum - 1; i >= 0; i--) {
    when[i] = when[i + 1] + diff[i];
  }
  long long outp = 0;
  for (int i = 1; i <= n; i++) outp = (outp + when[inp[i]]) % mod;
