...
int n, m;
int tot;
int posSeg[100005], idinSeg[5005], inv[5005];
...
struct Dinitz {
  ...
  int traverse(int u) {
    for (auto id : adj[u]) {
      if (E[id].v != T and E[id].v > u and E[id ^ 1].f) {
        E[id ^ 1].f--;
...
void addSeg(int id, int L, int R) { addSeg(1, 1, m, id, L, R); }
signed main() {
  ...
  for (int i = 1, id; i <= n; i++) {
    while ((id = Flow.traverse(i)) != i) target[i].push_back(inv[id]);
    if (omg[i])
      for (auto x : target[i]) byOmg[x] = true;
  }
  for (int i = 1; i <= n; i++) {
    if (omg[i] and (int) target[i].size() == 1) {
      for (auto x : vert[i]) {
        if (!byOmg[x]) {
          byOmg[x] = true;
  ...
  for (int i = 1; i <= n; i++) {
    for (auto id : target[i]) {
      if (!omg[i] and byOmg[id]) continue;
      cout << i << ' ' << id << '\n';
    }
  }
}
