...
int k, q, lst = 0;
long long m, x, y;
set<pair<int, int>, greater<pair<int, int>>> second;
struct SPoint {
  long long x, y;
...
bool convex(SPoint a, SPoint b, SPoint c) {
  SPoint first(b.x - a.x, b.y - a.y);
  SPoint second(c.x - a.x, c.y - a.y);
  return first.x * second.y < first.y * second.x;
}
void add(SPoint u) {
  set<SPoint>::iterator it = con.insert(u).first;
  second.insert(it->to_pair());
  if (con.size() == 1) {
    return;
  }
  if (it != con.begin() && next(it) != con.end() &&
      !convex(*prev(it), *it, *next(it))) {
    second.erase(it->to_pair());
    con.erase(it);
    return;
  }
  while (it != con.begin()) {
  ...
  for (int i = 1; i <= q; i++) {
    ...
    if (k == 1) {
      ...
      if (con.empty()) {
        ...
        SPoint mx = SPoint(second.begin()->first, second.begin()->second);
        long long mul, val;
        if (it == con.end() || mx < *it) {
          mul = mx.y;
          val = x;
