...
class PersisdentSegementree {
 private:
  ...
  Integer Merge(Integer x, Integer y) { return max(x, y); }
 ...
 public:
  ...
  void Build(int node, int l, int r) {
    ...
    L[node] = ++cnt;
    R[node] = ++cnt;
    Build(L[node], l, ((l + r) >> 1));
    Build(R[node], ((l + r) >> 1) + 1, r);
  }
  int upd(int node, int l, int r, int x, Integer val) {
    ...
    L[ans] = upd(L[node], l, ((l + r) >> 1), x, val);
    R[ans] = upd(R[node], ((l + r) >> 1) + 1, r, x, val);
  ...
  Integer Query(int node, int l, int r, int x, int y) {
    ...
    return Merge(Query(L[node], l, ((l + r) >> 1), x, y),
                 Query(R[node], ((l + r) >> 1) + 1, r, x, y));
  }
  void Add(Integer X) { lst.push_back(make_pair(X, root)); }
  void update(int x, Integer val) { root = upd(root, 1, n, x, val); }
  Integer ask(Integer x, int l, int r) {
    int f = upper_bound(lst.begin(), lst.end(), make_pair(x, root + 10)) -
            lst.begin() - 1;
    if (f == -1) {
      return 0;
    }
  ...
  Integer range(int l, int r, int x, int y) {
    if (r < l || x > y) {
      return 0;
    }
...
int main() {
  ...
  long long ans = 0;
  if ((cnt[n][0]) == 0) {
    if ((cnt[n][1])) ans += g[1].back() - g[1].front();
    if ((cnt[n][2])) ans += g[2].back() - g[2].front();
    cout << ans;
    return 0;
  }
  if ((cnt[n][1])) {
