...
void dfs(int x, int id) {
  while (num[x] < deg[x]) {
    pair<int, int> v = vec[x][num[x]];
    ++num[x];
    if (used[v.second]) continue;
    used[v.second] = 1;
    dfs(v.first, v.second);
  }
  if (id != -1) Eular.push_back(id);
}
int main() {
  n = read(), m = read();
  cnt = m;
  ...
  for (int i = 1; i <= n; ++i) {
    if ((deg[i] & 1) ^ 1) {
      deg[i] += 2;
      deg[0] += 2;
      vec[0].emplace_back(i, ++cnt);
      vec[i].emplace_back(0, cnt);
      vec[0].emplace_back(i, ++cnt);
      vec[i].emplace_back(0, cnt);
      fa[find(i)] = 0;
    } else if (deg[i] & 1) {
  ...
  if (cnt == m) {
    deg[0] += 2, deg[1] += 2;
    vec[0].emplace_back(1, ++cnt);
    vec[1].emplace_back(0, cnt);
    vec[0].emplace_back(1, ++cnt);
    vec[1].emplace_back(0, cnt);
  }
  dfs(0, -1);
