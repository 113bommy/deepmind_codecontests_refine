...
struct line {
  ...
  long long get(int x) const { return 1LL * k * x + b; }
  bool operator<(const line &l) const { return k > l.k || k == l.k && b < l.b; }
};
...
bool cmp(const pair<long long, int> &a, const pair<long long, int> &b) {
  ...
  return 1LL * (a.first % a.second) * b.second <
         1LL * (b.first % b.second) * a.second;
}
...
long long get_max_line(const vector<line> &v, int x) {
  int l = -1, r = v.size() - 1;
  while (r - l > 1) {
    int mid = (l + r) / 2;
...
line *pt;
bool cmp2(int a, int b) { return pt[a] < pt[b]; }
struct sqrt_decomp {
  line ln[max_n];
  int add[max_blocks];
  ...
  long long get_max(int l, int r) {
    ...
    for (int i = n1 + 1; i < n2; ++i) {
      res = max(res, get_max_line(v[i], add[i]));
    }
    return res;
  }
};
...
int main() {
  ...
  while (q--) {
    ...
    if (tp == 1) {
      ...
      scanf("%d", &v);
      --v;
      long long mx = max(s1.get_max(l[v], r[v]), s2.get_max(l[v], r[v]));
      printf("%I64d\n", mx);
    }
  }
  return 0;
