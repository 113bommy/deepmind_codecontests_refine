...
int tar[N], aaa[N];
vector<pair<P, P> > Sp, Md, Tp;
void mv(P &w, P t, vector<pair<P, P> > &Sp) {
  P v;
  while (w.x < t.x) {
    v = w;
    w.x++;
    Sp.push_back(make_pair(v, w));
  }
  while (w.x > t.x) {
    v = w;
    w.x--;
...
void solve(P S[], vector<pair<P, P> > &Sp) {
  sort(S + 1, S + m + 1,
...
void tern(int j, vector<pair<P, P> > &Md) {
  swap(aaa[j], aaa[j - 1]);
...
int main() {
  ...
  for (int i = 1; i <= m; ++i) aaa[S[i].x] = aaa[S[i].i];
  for (int i = 1; i <= m; ++i)
    for (int j = i; j > 1; --j) {
  ...
  for (int i = 0; i < Sp.size(); ++i)
    printf("%d %d %d %d\n", Sp[i].first.x, Sp[i].first.y, Sp[i].second.x,
           Sp[i].second.y);
  for (int i = 0; i < Md.size(); ++i)
    printf("%d %d %d %d\n", Md[i].first.x, Md[i].first.y, Md[i].second.x,
           Md[i].second.y);
  for (int i = Tp.size() - 1; ~i; --i)
    printf("%d %d %d %d\n", Tp[i].second.x, Tp[i].second.y, Tp[i].first.x,
           Tp[i].first.y);
}
