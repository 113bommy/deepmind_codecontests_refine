...
int n, m, Log, a[35], x[MAXN], y[MAXN], bit[MAXN];
...
int main() {
        ...
        for (int nx = 0; nx <= lim; nx++)
          for (int ny = 0; ny <= lim; ny++) {
            ...
            for (int s = 0; s <= (1 << n) - 1; s++) {
              ...
              if (px + 1 >= incpx && py >= incpy && nx + 1 >= incnx &&
                  ny >= incny) {
                ...
                if (tpx <= lim && tpy <= lim && tnx <= lim && tny <= lim) {
                  ...
                  if (a[i] == 1)
                    update(dp[t][tpx][tpy][tnx][tny][1][a[i] == 0], voo);
                }
              }
              if (px >= incpx && py + 1 >= incpy && nx >= incnx &&
                  ny + 1 >= incny) {
                int tpx = (px - incpx) * 2;
                ...
                if (tpx <= lim && tpy <= lim && tnx <= lim && tny <= lim) {
                  ...
                  update(dp[t][tpx][tpy][tnx][tny][a[i] == 0][0], voz);
                  if (a[i] == 1)
                    update(dp[t][tpx][tpy][tnx][tny][a[i] == 0][1], voo);
                }
              }
              if (px + 1 >= incpx && py + 1 >= incpy && nx + 1 >= incnx &&
                  ny + 1 >= incny) {
                int tpx = (px + 1 - incpx) * 2;
                int tpy = (py + 1 - incpy) * 2;
                int tnx = (nx + 1 - incnx) * 2;
  ...
  for (int l1 = 0; l1 <= 1; l1++)
    for (int l2 = 0; l2 <= 1; l2++) update(ans, dp[0][0][0][0][0][l1][l2]);
  cout << ans << endl;
  return 0;
}
