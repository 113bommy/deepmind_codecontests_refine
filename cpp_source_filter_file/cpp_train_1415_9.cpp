...
void upd(int k1) {
  ...
  BCD[k1] = ad(BCD[lc[k1]], BCD[rc[k1]], mu(BC[lc[k1]], D[rc[k1]]),
               mu(B[lc[k1]], CD[rc[k1]]));
}
void mdf(int& k1, int k2, int k3, int k4, int k5, int k6, int k7) {
  ...
  int mid = (k2 + k3) >> 1;
  if (k4 <= mid)
    mdf(lc[k1], k2, mid, k4, k5, k6, k7);
  else
    mdf(rc[k1], mid + 1, k3, k4, k5, k6, k7);
  upd(k1);
}
int main() {
  scanf("%d", &n);
  ...
  t.clear();
  for (int i = (1); i <= (n); ++i) pre[i] = t.qry(a[i]), t.upd(a[i], 1);
  t.clear();
  for (int i = (n); i >= (1); --i) suf[i] = t.qry(a[i]), t.upd(a[i], 1);
  for (int i = (1); i <= (n); ++i) {
    mdf(rt[a[i]], 1, n, i, pre[i], suf[i], 1);
  }
  int ans = 0;
  for (int i = (1); i <= (m); ++i) uad(ans, BCD[rt[i]]);
  scanf("%d", &Q);
  for (int i = (1); i <= (Q); ++i) {
    int p, x;
    scanf("%d%d", &x, &p);
