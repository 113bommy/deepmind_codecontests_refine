...
inline void update(int x) {
  int c = x;
  for (int k = 1; k < K && c; ++k) c = nodes[c].nx;
  xans -= ans[x];
  if (!nodes[x].la)
    ans[x] = X[c] * (R + 1 - X[x]);
  else
...
int main() {
  ...
  for (int ups = 1; ups <= C; ++ups) {
    ...
    for (int i = 1; i <= cn; ++i) update(ord[i]);
    rans += xans;
    for (int i = C; i > ups; --i) {
      for (int j : yp[i]) {
      ...
      for (int j : yp[i]) {
        for (int k = 1, c = nodes[j].la; k < K && c; ++k, c = nodes[j].la)
          update(c);
        if (nodes[j].nx) update(nodes[j].nx);
        xans -= ans[j];
      }
      rans += xans;
    }
