...
struct SSS {
  int cnt[mxn], sa[mxn], rk[mxn], h[mxn], tmp[mxn * 2];
  char ss[mxn];
  int b[22][mxn];
  bool rv;
  ...
  void W() {
    ...
    if (rv) reverse(ss + 1, ss + n + 1);
    for (int i = 1; i <= n; ++i) rk[i] = ss[i], ++cnt[rk[i]];
    ...
    for (int k = 1, m = 200;; k *= 2) {
      ...
      for (int i = 2; i <= n; ++i)
        rk[sa[i]] =
            tmp[sa[i]] == tmp[sa[i - 1]] && tmp[sa[i] + k] == tmp[sa[i - 1] + k]
                ? p
                : ++p;
      if (p == n) break;
      m = p;
    ...
    for (int j = 1; j <= lg[n]; ++j)
      for (int i = 1; i + (1 << j) - 1 <= n; ++i)
        b[j][i] = min(b[j - 1][i], b[j - 1][i + (1 << (j - 1))]);
  }
} S1, S2;
struct asfdas {
  ...
  inline void down(int x) {
    if (tg[x]) put(x * 2, tg[x]), put(x * 2 + 1, tg[x]), tg[x] = 0;
  }
  ...
  inline void cg(int lc, int rc, int k, int x = 1, int l = 1, int r = n) {
    ...
    down(x);
    if (lc <= ((l + r) >> 1)) cg(lc, rc, k, x * 2, l, ((l + r) >> 1));
    if (rc > ((l + r) >> 1)) cg(lc, rc, k, x * 2 + 1, ((l + r) >> 1) + 1, r);
    up(x);
  }
  inline int ask(int lc, int rc, int x = 1, int l = 1, int r = n) {
    ...
    down(x);
    int res = 1e9;
    if (lc <= ((l + r) >> 1)) res = ask(lc, rc, x * 2, l, ((l + r) >> 1));
    if (rc > ((l + r) >> 1))
      res = min(res, ask(lc, rc, x * 2 + 1, ((l + r) >> 1) + 1, r));
    return res;
  }
  ...
  inline void dfs2(int a[], int x = 1, int l = 1, int r = n) {
    ...
    down(x);
    dfs2(a, x * 2, l, ((l + r) >> 1)),
        dfs2(a, x * 2 + 1, ((l + r) >> 1) + 1, r);
  }
} S3, S4;
inline int gg(int l, int r) {
  bool ok = 1;
  for (int c = 0; c < 26; ++c) ok &= sm[c][r] - sm[c][l - 1] <= 1;
  if (ok) return -1;
  int c = s[l] - 'a';
  ...
  for (int i = 2; i * i <= L; ++i)
    if (L % i == 0) {
      if (S1.lcp(l, l + i) >= L - i || S1.lcp(l, l + L / i) >= L - L / i)
        return 1;
    }
  if (ln[l] <= r || rn[r] >= l) return 2;
  int J = (r - l + 1) >> 1;
  for (int i = 1; i <= J && i <= T; ++i)
    if (S1.lcp(l, r - i + 1) >= i) return 2;
  for (int i = S1.rk[l] - 1, k = S1.h[i + 1]; i && i >= S1.rk[i] - 500 && k > T;
       k = min(k, S1.h[i]), --i)
    if (S1.sa[i] >= l && S1.sa[i] <= r && k >= r - S1.sa[i] + 1 &&
        r - S1.sa[i] + 1 <= J)
      return 2;
  for (int i = S1.rk[l] + 1, k = S1.h[i];
       i <= n && i <= S1.rk[i] + 500 && k > T; ++i, k = min(k, S1.h[i]))
    if (S1.sa[i] >= l && S1.sa[i] <= r && k >= r - S1.sa[i] + 1 &&
        r - S1.sa[i] + 1 <= J)
      return 2;
  if (sm[c][r] - sm[c][l]) return 3;
  c = s[r] - 'a';
  if (sm[c][r - 1] - sm[c][l - 1]) return 3;
  if (S3.ask(l, r) <= r) return 3;
  return 4;
}
int main() {
  for (int i = 2; i < mxn; ++i) lg[i] = lg[i >> 1] + 1;
  a14 = scanf("%d%s%d", &n, s + 1, &qn);
  for (int c = 0; c < 26; ++c)
    for (int i = 1; i <= n; ++i) sm[c][i] = sm[c][i - 1] + (s[i] == c + 'a');
  ...
  for (int i = n; i; --i)
    for (int j = 2 * i; j <= n; j += i)
      if (s[j] == s[j - i]) {
        ...
        S3.cg(j - i - l1 + 1, j - i + l2 - i, i),
            S4.cg(j - l1 + i, j + l2 - 1, i);
      }
