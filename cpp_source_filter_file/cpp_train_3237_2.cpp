...
struct pnt {
  ...
  ll operator%(const pnt& p) const { return x * p.y - y * p.x; }
...
struct seg {
  int i, j;
  pnt vec() const { return ps[j] - ps[i]; }
  bool operator<(const seg& s) const { return vec() % s.vec() > 0; }
};
void solve() {
  S *= 2;
  vector<seg> segs;
  for (int i = int(0); i < int(n); ++i)
    for (int j = int(0); j < int(n); ++j) {
      if (ps[i] < ps[j]) {
        segs.push_back(seg{i, j});
      }
    }
  sort(begin(segs), end(segs));
  assert(int((segs).size()) == n * (n - 1) / 2);
  if (false) cerr << "segs:" << endl;
  for (const auto& s : segs) {
    if (false) cerr << s.i << " " << s.j << endl;
  }
  if (false) cerr << endl;
  vi order(n);
  iota(begin(order), end(order), 0);
  const auto vec0 = segs[0].vec();
  sort(begin(order), end(order), [&](int k1, int k2) -> bool {
  ...
  for (const auto& s : segs) {
    ...
    const auto area = [&](int k) -> ll { return vec % (ps[k] - ps[s.i]); };
