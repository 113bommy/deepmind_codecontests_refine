...
struct node {
  int x, prex, t, val, id;
  node(int _x = 0, int _p = 0, int _t = 0, int _v = 0, int _i = 0) {
    x = _x, prex = _p, t = _t, val = _v, id = _i;
  }
} t[MAXN];
int top;
int pre(int x) {
  auto it = s[a[x]].find(x);
  return it == s[a[x]].begin() ? -1 : *(--it);
}
int nex(int x) {
  auto it = ++s[a[x]].find(x);
  return it == s[a[x]].end() ? -1 : *it;
}
namespace BIT {
long long tree[MAXN];
