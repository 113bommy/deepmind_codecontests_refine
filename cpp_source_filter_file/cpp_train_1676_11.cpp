#include <bits/stdc++.h>
using namespace std;
int K = 2, x[20000], y[20000];
int dp[20000], dp2[20000], DP[20000][3];
signed main() {
  ...
  auto get = [&](int a, int b) { return (x[a] + y[b]) % p; };
  dp[0] = get(0, 0);
  ...
  for (int i = 1; i <= m; i++) {
    for (int j = 0; j <= n; j++) {
      if (j == 0)
        ...
        dp2[j] = max(dp2[j - 1], dp[j]) + get(j, i);
    }
  ...
  for (int c = SZ - 1; c >= 0; c--) {
    for (int i = 0; i <= n; i++) DP[i][0] = g[c][0];
    for (int j = 1; j < m - L[c] + 1; j++) {
      for (int i = 0; i <= n; i++) {
        if (i == 0)
          DP[i][j] = DP[i][j - 1] + get(i, j + L[c]);
        else
          DP[i][j] = max(DP[i][j - 1], DP[i - 1][j]) + get(i, j + L[c]);
      }
    }
    while (m != L[c]) {
