...
struct element {
  unsigned long long coef[5];
  element(void) { memset(coef, 0, sizeof coef); }
...
unsigned long long kissme(unsigned long long x, unsigned long long y) {
...
int main(void) {
  ...
  dft(a, -1);
  unsigned long long inv = kissme(125 * 25, (mod >> 1) - 1);
  for (int i = 0; i < n; i++) {
    printf("%llu\n", ((a[i].coef[0] - a[i].coef[1]) * inv) >> 5);
  }
  return 0;
}
