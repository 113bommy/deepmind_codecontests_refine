...
int main() {
  ...
  static int optimal_pointer[N][N];
  for (int i = 0; i <= n; i++) {
  ...
  for (int d = 2 * n - 2; d >= 0; d--) {
    ...
    for (int i = max(0, d - (n - 1)); i < min(n, d + 1); i++) {
      ...
      if (order_in_diagonal[i + 1][j] < order_in_diagonal[i][j + 1]) {
        ...
        orders_of_pointer_by_character[a[i][j] - 'a'].push_back(
            {order_in_diagonal[i + 1][j], i});
      } else {
        optimal_pointer[i][j] = 0;
        orders_of_pointer_by_character[a[i][j] - 'a'].push_back(
            {order_in_diagonal[i][j + 1], i});
      }
    }
    int curr_order = 0;
    for (int c = 0; c < 26; c++) {
      if (!orders_of_pointer_by_character[c].empty()) {
        sort(orders_of_pointer_by_character[c].begin(),
             orders_of_pointer_by_character[c].end());
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      required_changes[i + 1][j + 1] =
          min(required_changes[i][j + 1], required_changes[i + 1][j]) +
          ((a[i][j] == 'a') ? 0 : 1);
    }
  }
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (min(required_changes[i][j + 1], required_changes[i + 1][j]) <= k &&
          (i + j >
               optimal_path_delimiter.first + optimal_path_delimiter.second ||
           (i + j ==
                optimal_path_delimiter.first + optimal_path_delimiter.second &&
            order_in_diagonal[i][j] <
                order_in_diagonal[optimal_path_delimiter.first]
                                 [optimal_path_delimiter.second]))) {
        optimal_path_delimiter = {i, j};
      }
    }
  }
  if (required_changes[n - 1][n - 1] <= k) {
    cout << string(2 * n - 1, 'a');
    return 0;
  }
  string ans =
      string(optimal_path_delimiter.first + optimal_path_delimiter.second, 'a');
  ans.push_back(a[optimal_path_delimiter.first][optimal_path_delimiter.second]);
  while (optimal_path_delimiter != make_pair(n - 1, n - 1)) {
    if (optimal_pointer[optimal_path_delimiter.first]
                       [optimal_path_delimiter.second]) {
    ...
    ans.push_back(
        a[optimal_path_delimiter.first][optimal_path_delimiter.second]);
  }
  cout << ans;
  return 0;
}
