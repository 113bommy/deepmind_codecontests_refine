...
bool Cmp(const int& u, const int& v) {
  return a1[u] == a1[v] ? a2[u] < a2[v] : a1[u] < a1[v];
}
int main() {
  int u, v;
  scanf("%d%d", &n, &m);
  ...
  for (int j = 1; j <= m; j++) {
    ...
    if (r == 1)
      Insert(u, v, d[1]), Insert(v, u, d[1]);
    else {
      Insert(u, ++t, d[r]);
      for (int jj = r - 1; jj >= 2; jj--) Insert(t, t + 1, d[jj]), ++t;
      Insert(t, v, d[1]);
      Insert(v, ++t, d[r]);
      for (int jj = r - 1; jj >= 2; jj--) Insert(t, t + 1, d[jj]), ++t;
      Insert(t, u, d[1]);
  ...
  for (; l <= r;) {
    ...
    for (int i = head[u]; i; i = g[i].nxt)
      if (!dep[v = g[i].v]) q[++r] = v, dep[v] = dep[u] + 1;
  }
  dis[1] = 0;
  ...
  for (int j = 1; h[j].size(); j++) {
    ...
    for (int i = 0; i < h[j].size(); i++) {
      u = h[j][i];
      if (!i || a1[u] != a1[h[j][i - 1]] || a2[u] != a2[h[j][i - 1]])
        a3[u] = ++r;
      for (int i = head[u]; i; i = g[i].nxt)
        if (dep[v = g[i].v] > dep[u] &&
            (a3[u] < a1[v] || a3[u] == a1[v] && g[i].c < a2[v]))
