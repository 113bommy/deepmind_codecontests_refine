        ...
        for (int c = b; c <= 9; c++)
          for (int d = c; d <= 9; d++)
            for (int j = 1; j <= 9; j++) {
              ...
              if (a) res = min(res, abs(j - a) + dp[1][j][0][b][c][d]);
              if (b) res = min(res, abs(j - b) + dp[1][j][0][a][c][d]);
              if (c) res = min(res, abs(j - c) + dp[1][j][0][a][b][d]);
              if (d) res = min(res, abs(j - d) + dp[1][j][0][a][b][c]);
              dp[1][j][a][b][c][d] = res;
            }
    swap(dp[0], dp[1]);
  }
