...
int find_right(int left, int val, int sti = 1, int il = 0, int ir = _N - 1) {
  push_lazy(sti, il, ir);
...
void update(int l, int r, int val, int sti = 1, int il = 0, int ir = _N - 1) {
  push_lazy(sti, il, ir);
  if (l > ir || r < il) return;
  if (il >= l && ir <= r) {
...
void _update(int left, int delta) {
