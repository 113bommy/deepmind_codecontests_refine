...
struct seg {
  ...
  void upd(int s, int t, int C) {
    ...
    for (s += M - 1, t += M + 1; s ^ t ^ 1; s >>= 1, t >>= 1) {
      if (~s & 1) chmin(mn[s ^ 1], C);
      if (t & 1) chmin(mn[t ^ 1], C);
    }
  }
...
int sl[N], sr[N], cnt[26], B, tr[N], tl[N], last[26];
rmq _;
...
bool check2(int l, int r) {
  ...
  int len = r - l + 1;
  for (int i = 1; i <= B && i + i <= len; ++i)
    if (lcp(l, r - i + 1) >= i) return true;
  for (int _ = std::max(1, a.rk[l] - B), ed = std::min(n, a.rk[l] + B); _ <= ed;
       ++_) {
    int i = a.sa[_];
...
inline bool check3(int l, int r) {
  return tr[l] <= r || tl[r] >= l || _.ask(l, r) >= l;
}
int main() {
  ...
  for (int len = n / 2; len >= 1; --len) {
    for (int x = 1, y = len + 1; y <= n; x += len, y += len)
      if (s[x] == s[y]) {
        int l1 = std::min(len, lcp(x, y)), l2 = std::min(len, lcs(x, y));
        c.upd(x - l2 + 1, x + l1 - len, len),
            d.upd(x - l2 + len, y + l1 - 1, len);
      }
  }
  c.out(sl, 1), d.out(sr, -1), _.init(sr);
  memset(last, 0, sizeof(last));
  for (int i = 1; i <= n; ++i) tl[i] = last[s[i] - 97], last[s[i] - 97] = i;
  memset(last, 63, sizeof(last));
  for (int i = n; i >= 1; --i) tr[i] = last[s[i] - 97], last[s[i] - 97] = i;
  int q, l, r;
  read(q);
  for (B = 1; B * B <= n; ++B)
    ;
