...
inline void modify(SegTree *&now, int l, int r, int le, int ri, int v) {
  if (le <= l && r <= ri)
    cmax(now->val, v), cmax(now->tag, v);
  else {
    pushdown(now);
    int mid = l + r >> 1;
    if (ri <= mid)
      modify(now->lc, l, mid, le, ri, v);
    else if (le > mid)
      modify(now->rc, mid + 1, r, le, ri, v);
    else
      modify(now->lc, l, mid, le, ri, v),
          modify(now->rc, mid + 1, r, le, ri, v);
    update(now);
...
inline int getmin(SegTree *&now, int l, int r, int le, int ri) {
  ...
  return min(getmin(now->lc, l, mid, le, ri),
             getmin(now->rc, mid + 1, r, le, ri));
}
int main(int argc, char **argv) {
  ...
  vy = -vy;
  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= n; ++j) gint(v[i][j]);
  if (vy < 0) {
    vy = -vy;
    for (int i = 1; i <= n; ++i)
      for (int j = 1; j * 2 <= n; ++j) swap(v[i][j], v[i][n - j + 1]);
  }
  if (vx < 0) {
    vx = -vx;
    for (int i = 1; i * 2 <= n; ++i)
      for (int j = 1; j <= n; ++j) swap(v[i][j], v[n - i + 1][j]);
  }
  int l, r;
  long long ans = 0;
  for (int i = n; i; --i)
