...
void convo(const int* A, const int* B, int* dst) {
  ...
  for (int i = 0; i < lsz; ++i)
    for (int j = 0; j < rsz; ++j) gma(tmp[i + j], A[i] + B[j]);
  for (int i = 0; i < sz; ++i) gma(dst[i], tmp[i]);
}
int dfs(int u) {
  int rr = 0;
  dp[u][0][0] = 0;
  ...
  if (!rr) {
    ...
    for (int j = 0; j < 3; ++j)
      for (int k = 0; k < 3; ++k)
        if ((j | k) != 3) convo(dp[fa[u]][j], dp[u][k], tmp[j]);
    for (int j = 0; j < 3; ++j)
      for (int k = 0; k <= n; ++k) gma(dp[fa[u]][j][k], tmp[j][k]);
  } else if (to[rr] == fa[u]) {
    ...
    for (int i = 0; i != 3; ++i) {
      ...
      while (v != fa[u]) {
        ...
        for (int j = 0; j < 3; ++j)
          for (int k = 0; k < 3; ++k)
            if ((j | k) != 3) convo(f[lst][j], dp[v][k], f[now][k]);
        v = fa[v];
      }
      static int tmp[MAXN];
      memset(tmp, 0xcf, sizeof tmp);
      for (int j = 0; j < 3; ++j)
        if ((i | j) != 3) convo(dp[fa[u]][i], f[now][j], tmp);
      for (int j = 0; j <= n; ++j) gma(dp[fa[u]][i][j], tmp[j]);
    }
  }
  return rr;
}
int main() {
  ...
  for (int i = 0; i <= n; ++i) {
    int t = 0;
    for (int j = 0; j < 3; ++j) gma(t, dp[1][j][i]);
    std::cout << t << (" \n"[i == n]);
  }
  return 0;
}
