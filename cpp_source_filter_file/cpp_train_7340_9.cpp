...
void dfs(int u, int DD = 0) {
  st[u] = T++;
  par[0][u] = DD;
  for (int i = 1; i < mx; i++) par[i][u] = par[i - 1][par[i - 1][u]];
  for (int i = 0; i < V[u].size(); i++) {
    int v = V[u][i];
    if (v != DD) h[v] = h[u] + 1, dfs(v, u);
  }
  en[u] = T++;
}
int LCA(int u, int v) {
...
bool check() {
  bool res = true;
  for (int i = 0; i < N; i++) mark[k[i]] = true;
  for (int i = 0; i < N; i++)
    if (mark[par[0][k[i]]]) res = false;
  return res;
}
void solve() {
  if (!check()) {
    for (int i = 0; i < N; i++) mark[k[i]] = false;
    ans = -1;
    return;
  }
  ans = 0;
  ver.clear();
  lca.clear();
  for (int i = 0; i < N; i++) ver.insert(make_pair(st[k[i]], k[i]));
