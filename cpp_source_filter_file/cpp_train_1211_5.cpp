...
void dfs(int x, int fa) {
  ...
  for (int y : g[x]) {
      ...
      for (int j = 1; j <= size[x]; j++) {
        for (int k = 1; k <= size[y]; k++) {
          for (int l = 1; l <= size[y]; l++) {
            upd(tmp[i + k][j], 1ll * f[x][i][j] * f[y][k][l] % mod * l);
    ...
    size[x] += size[y];
    for (int i = 1; i <= size[x]; i++)
      for (int j = 1; j <= size[x]; j++) f[x][i][j] = tmp[i][j], tmp[i][j] = 0;
  }
}
int main() {
  cin >> n;
  ...
  for (int i = 0; i < n; i++)
    for (int j = 1; j <= n; j++)
      upd(ans[i], 1ll * f[1][n - i][j] * j % mod * Power(n, n - i - 2) % mod);
  for (int i = n - 1; i >= 0; i--)
    for (int j = i + 1; j < n; j++)
      ans[i] = (ans[i] - 1ll * ans[j] * C[j][i] % mod + mod) % mod;
