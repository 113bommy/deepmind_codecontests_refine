...
struct Edge {
  ...
  flowUnit cap, flow;
  int index;
  Edge(int from, int to, flowUnit cap, flowUnit flow, int index)
...
struct PushReLabel {
  ...
  flowUnit max_flow(int source, int dest) {
    ...
    flowUnit max_flow = 0;
    for (auto &it : g[source]) max_flow += it.flow;
    return max_flow;
  }
};
const int LIM = 10;
ll cache[LIM + 1][LIM + 1];
vector<pair<int, int>> dirs = {{1, -2}, {2, -1}, {3, 0},
                               {2, 1},  {1, 2},  {0, 3}};
ll brute_force(int a, int b) {
  if (a <= 0 || b <= 0 || a > LIM || b > LIM) return 0;
  ll &ret = cache[a][b];
  if (ret != -1) return ret;
