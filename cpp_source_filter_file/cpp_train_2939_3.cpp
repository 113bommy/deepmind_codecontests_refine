...
int n, m, a[N], l[N], r[N], cnt[N][N], ans1, ans2 = 1;
int main() {
  scanf("%d%d", &n, &m);
  ...
  for (int i = 0; i <= n; i++) {
    ...
    for (int j = 1; j <= n; j++)
      if (j != x) {
        ...
        if (cnt[j][mx]) {
          if (!cnt[j][mn])
            now1++, now2 = 1ll * now2 * cnt[j][mx] % mod;
          else if (cnt[j][mx] == 1)
            now1++, now2 = 2ll * now2 * cnt[j][mx] % mod;
          else
            now1 += 2,
                now2 = 1ll * now2 * cnt[j][mn] % mod * (cnt[j][mx] - 1) % mod;
        }
      }
    if (now1 > ans1)
      ans1 = now1, ans2 = now2;
