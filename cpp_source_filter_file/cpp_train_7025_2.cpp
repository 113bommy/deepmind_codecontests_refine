...
struct SAM {
  ...
  SAM() {
    top = 1;
    memset(tree[1].son, 0, sizeof(int) * 26);
  }
  int insert(int p, int c) {
    if (tree[p].son[c] == 0) {
      int x = ++top;
      memset(tree[x].son, 0, sizeof(int) * 26);
      tree[x].max = tree[p].max + 1;
      for (; tree[p].son[c] == 0 && p > 0; p = tree[p].fail) tree[p].son[c] = x;
      if (p == 0)
        tree[x].fail = 1;
      else {
        int t = tree[p].son[c];
        if (tree[t].max == tree[p].max + 1)
          tree[x].fail = t;
        else {
          int q = ++top;
...
int main() {
  ...
  bit.init(sam.top);
  for (int i = 0; i <= n; ++i) {
    for (int j = 0; j < pos[i].size(); ++j) bit.add(dfn[pos[i][j]], 1);
    for (int j = 0; j < q[i].size(); ++j)
      ans[q[i][j].id] +=
          q[i][j].t * (bit.query(e[q[i][j].k]) - bit.query(dfn[q[i][j].k] - 1));
  }
