...
using namespace std;
const int N = 3e5 + 5;
int n, p[N], sl[N], sr[N], cnt[N], mi[N], ma[N];
long long ans;
void solve(int *a, int *b) {
  ...
  ma[0] = nowma = 0;
  for (int i = 1; i <= lb; i++)
    mi[i] = min(mi[i - 1], b[i]), ma[i] = max(ma[i - 1], b[i]);
  for (int i = 1, nl = 1, nr = 1, len; i <= la; i++) {
    nowmi = min(nowmi, a[i]);
    nowma = max(nowma, a[i]);
    len = nowma - nowmi + 1;
    if (len > i && len - i <= lb && mi[len - i] > nowmi && ma[len - i] < nowma)
      ans++;
    while (nr <= lb && mi[nr] > nowmi) cnt[ma[nr] - nr + 1]++, nr++;
    while (nl <= lb && ma[nl] < nowma) cnt[ma[nl] - nl + 1]--, nl++;
    ans += (long long)cnt[i + nowmi];
  }
