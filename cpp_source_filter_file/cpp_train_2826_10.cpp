...
struct BIT2 {
  struct BIT {
    ...
    void u(int y, int v) {
      for (y = i(y); y <= s; y += y & -y) d[y] += v;
    }
    int g(int y) {
      int r = 0;
      for (y = i(y); y; y -= y & -y) r += d[y];
      return r;
    }
  } d[N];
  void b(int x, int y) {
    for (; x < N; x += x & -x) d[x].b(y);
  }
  void a() {
    for (int i = 1; i < N; i++) d[i].a();
  }
  void u(int x, int y, int v) {
    for (; x < N; x += x & -x) d[x].u(y, v);
  }
  int g(int x, int y) {
    int r = 0;
    for (; x; x -= x & -x) r += d[x].g(y);
    return r;
  }
} A, B;
int n, m;
...
int main() {
  ...
  sort(y, y + c);
  sort(z, z + d);
  auto cpr = [](ll &x, ll a[], int c) {
    x = int(lower_bound(a, a + c, x) - a + 1);
  };
  for (int i = 1; i <= n; i++) {
    cpr(qx[i], x, c);
    cpr(qy[i], y, c);
  ...
  for (int i = 1; i <= m; i++) {
    ...
    cpr(wy[i], y, c);
    cpr(wz[i], z, d);
    printf("%lld ", A.g(wx[i], wz[i]) - B.g(wy[i], wz[i]));
  }
  puts("");
  return 0;
}
