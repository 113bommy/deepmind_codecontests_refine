...
class AddHistory {
 ...
 public:
  AddHistory() : pref(1, 0LL) { time_to_idx[0] = 0; }
  void event(int arg, int t) {
    int idx = pref.size();
    pref.push_back(pref.back() + (long long)arg);
    time_to_idx[t] = idx;
  }
...
class ZeroHistory {
 ...
 public:
  void event(int arg, int t) { mx = max(mx, t); }
  long long query(int t) { return mx >= t ? mx : 0; }
};
struct MaxAdder {
  long long init() { return 0; }
  long long operator()(long long a, long long b) { return max(a, b); }
};
...
class Tree {
 ...
 public:
  ...
  int size(int u) { return desc[find(u)].size(); }
  void event(int u, int arg, int t) { hist[find(u)].event(arg, t); }
  long long query(int u, int t) {
    Adder adder;
    long long ans = adder.init();
    for (auto pr : log[u]) {
