...
int main() {
  ...
  for (int cif = 0; cif <= 9; cif++) dp[0][cif][0][true] = 1;
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = 0; j < cif; j++) {
        if (j == limit[i]) {
          ...
          dp[i][cif][0][j < limit[i]] +=
              dp[i - 1][cif][0][true] + dp[i - 1][cif][0][false];
        }
      }
      ...
      for (int k = 1; k <= i; k++) {
        for (int j = 0; j < cif; j++) {
          if (j == limit[i]) {
            ...
            dp[i][cif][k][j < limit[i]] +=
                dp[i - 1][cif][k][true] + dp[i - 1][cif][k][false];
          }
        }
        for (int j = cif; j <= 9; j++) {
          if (j == limit[i]) {
            ...
            dp[i][cif][k][j < limit[i]] +=
                dp[i - 1][cif][k - 1][true] + dp[i - 1][cif][k - 1][false];
          }
        }
