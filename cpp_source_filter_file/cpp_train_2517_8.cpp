...
long long pos[MN * 3], aim[MN], Ans[MN * 2];
int QwQ[MN];
int main() {
  scanf("%d%d", &M, &N);
  ...
  for (int i = 1; i <= N; ++i)
    pos[i] = A[pA[i]], pos[N + i] = M + A[pA[i]],
    pos[2 * N + i] = 2ll * M + A[pA[i]], aim[i] = M + B[pB[i]];
  int j = N;
  for (int i = 1; i <= N; ++i) {
  ...
  j = 0;
  for (int i = 1; i <= N; ++i) {
    while (j <= N + N && aim[j] <= pos[N + i]) ++j;
    Ans[1] -= pos[i], Ans[i + 1] += pos[i];
    Ans[N + i + 1] += pos[N + N + i];
    Ans[i + 1] -= pos[N + i], Ans[i + N - j + 2] += 2 * pos[N + i],
        Ans[N + i + 1] -= pos[N + i];
  }
