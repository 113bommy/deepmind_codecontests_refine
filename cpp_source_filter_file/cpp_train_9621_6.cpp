...
bool OK(int x, int y, int z) {
  ...
  if (DP[x] >= b[z].y)
    ...
    A = dp[x] + (b[z].y - DP[x] - 1) / b[x].x + 1;
  if (DP[y] >= b[z].y)
    B = dp[y];
  else
    B = dp[y] + (b[z].y - DP[x] - 1) / b[y].x + 1;
  if (A < B) return true;
  if (A > B) return false;
  long long C = (A - dp[x]) * b[x].x + DP[x] - b[z].y,
            D = (A - dp[y]) * b[y].x + DP[y] - b[z].y;
  if (C > D)
    return true;
  else
    return false;
...
int main() {
  ...
  for (i = 2; i < o; i++) {
    if (DP[st[l].z] >= b[i].y)
      ...
      dp[i] = dp[st[l].z] + (b[i].y - DP[st[l].z] - 1) / b[st[l].z].x + 1;
    DP[i] = (dp[i] - dp[st[l].z]) * b[st[l].z].x + DP[st[l].z] - b[i].y;
    st[l].x++;
    if (i == o) break;
    while (st[l].x > st[l].y) l++;
