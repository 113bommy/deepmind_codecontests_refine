...
long long mul[N], imul[N], a[N];
int n, q;
bool vis[NUM];
int pr[NUM], len, minp[NUM], mulp[NUM], inv[NUM];
long long val[NUM], ival[NUM];
void init(int n) {
...
int main() {
  ...
  for (register int r = 1; r <= n; ++r) {
    ...
    for (register int _ = fir[r]; _; _ = nxt[_]) {
      l = L[_], id = _;
      ans[id] =
          qry(r) * qpow(qry(l - 1)) % mod * mul[r] % mod * imul[l - 1] % mod;
