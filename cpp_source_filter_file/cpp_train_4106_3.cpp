...
long long two(vector<int> s1, vector<int> s2) {
  ...
  if (high1 < low2) {
    ...
    res += (long long)chk2[low2] * (s2.size() - chk2[low2]) / 2 * chk1[high1];
  } else if (high2 < low1) {
    res += (long long)chk2[high2] * (chk2[high2] - 1) / 2 * chk1[low1];
    res += (long long)chk2[high2] * (s2.size() - chk2[high2]) / 2 * chk1[low1];
  } else {
    long long all2 = 0, pair = 0;
    for (auto p : chk2) {
      long long cnt2 = p.second;
      if (chk1.count(p.first))
        res += (cnt2 * (cnt2 - 1) / 2 + cnt2 * all2) * chk1[p.first];
...
int main() {
  ...
  if (s[1].empty())
    ...
    int r = max(max(getmaxone(s[0]), getmaxone(s[1])),
                max(getmaxtwo(s[0], s[1]), getmaxtwo(s[1], s[0])));
