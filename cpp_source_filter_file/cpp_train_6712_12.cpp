...
int kpow(int x, int y, int rex = 1) {
  for (; y; y >>= 1, x = mul(x, x))
    if (y & 1) rex = mul(rex, x);
  return rex;
}
void solve(int u, int fa, int tp) {
  top[u] = tp;
  for (auto v : g[u])
    if (v != fa && !vis[v]) solve(v, u, tp);
}
int main() {
  scanf("%d%d", &n, &m);
  ...
  for (int rt = 1; rt <= n; ++rt) {
    if ((!vis[rt] && !top[rt]) || top[rt] == rt) {
      ...
      sum[x][siz[rt]] += dp[rt][siz[rt]];
      for (auto v : g[rt]) {
        int y = dp[rt][siz[rt] - 1];
        y = mul(mul(y, kpow(dp[v][siz[v]], mod - 2)),
                kpow(c[siz[rt] - 1][siz[v]], mod - 2));
        for (int j = 0; j < siz[v]; ++j)
          add(sum[x][siz[rt] - siz[v] + j],
              mul(mul(y, dp[v][j]), c[siz[rt] - siz[v] + j - 1][j]));
      }
    }
  }
  ans[0] = 1;
