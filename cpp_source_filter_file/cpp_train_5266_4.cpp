...
bool f(int b) {
  ...
  std::merge(bu.end() - b, bu.end(), normal.begin(), normal.end(),
             nums.begin());
  int on = 0;
  for (int i = 0; i < nums.size(); i += k, on++) {
    if (nums[i] < on) return false;
  }
  return true;
}
int main() {
  ...
  for (int i = 0; i < m; i++) {
    int temp;
    scanf("%i", &temp);
    buy.push_back(std::pair<int, int>(temp, i + 1));
    bu.push_back(temp);
  }
  std::sort(buy.begin(), buy.end(), std::greater<std::pair<int, int> >());
