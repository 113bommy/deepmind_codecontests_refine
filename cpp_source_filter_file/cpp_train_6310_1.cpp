...
int64_t solve(vector<Point> a, int m) {
  ...
  sort(a.begin() + 1, a.end(),
       [o](const Point& a, const Point& b) { return cross(o, a, b) > 0; });
  ...
  for (int i = 1; i < n; ++i) {
    ...
    dp[i] = vector<vector<int64_t>>(((int)(works[i]).size()),
                                    vector<int64_t>(m + 1, -LINF));
    for (int j = 0; j < ((int)(works[i]).size()); ++j) {
      int prev = works[i][j];
      int pos = 0, step = 1;
      while (step < ((int)(works[prev]).size())) {
        step *= 2;
      }
      for (; step > 0; step /= 2) {
        if (pos + step < ((int)(works[prev]).size()) &&
            cross(a[i], a[prev], a[works[prev][pos + step]]) <= 0) {
          pos += step;
        }
      }
      for (int k = 2; k <= m; ++k) {
        dp[i][j][k] = max(j == 0 ? 0 : dp[i][j - 1][k],
