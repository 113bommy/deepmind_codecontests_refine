...
long long sumx[410][410], sumy[410][410];
long long sxum[410][410], syum[410][410];
int main() {
  int n, m;
  scanf("%d%d", &n, &m);
  ...
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++)
      sumx[i][j] = sumx[i - 1][j] + (mp[i][j] == '1'),
      sumy[i][j] = sumy[i][j - 1] + (mp[i][j] == '1');
  for (int i = 1; i <= n; i++)
    for (int j = 1; j <= m; j++)
      sxum[i][j] = sxum[i - 1][j - 1] + (mp[i][j] == '1'),
      syum[i][j] = syum[i - 1][j + 1] + (mp[i][j] == '1');
  for (int i = 1; i <= n; i++) {
    for (int j = 1; j <= m; j++) {
      ...
      for (int k = 1; i - k >= 1 && j - k >= 1; k++) {
        if (mp[i - k][j] == '1' || mp[i][j - k] == '1') break;
        if (syum[i][j - k] - syum[i - k - 1][j - k + 1] >= 1) continue;
        dp[i][j]++;
      }
