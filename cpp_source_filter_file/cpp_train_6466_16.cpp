...
const int MX = 205;
int n, m;
long long k, lim = 1e18 + 1;
string f[15];
void solve() {
  vector<string> vs;
  for (int i = 0; i < f[n].size(); i++) {
    vs.push_back(f[n].substr(i, f[n].size() - i));
  }
  sort(vs.begin(), vs.end());
  for (int i = 0; i < min(m, (int)vs[k - 1].size()); i++) cout << vs[k - 1][i];
  cout << '\n';
}
long long cn[MX];
