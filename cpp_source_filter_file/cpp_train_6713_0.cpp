...
struct segment_tree {
  ...
  monoid range_fold(int L, int R, int d, int i, int l, int r) {
    if (r <= L || R <= l) {
      ...
      int m = (l + r) / 2;
      if (d == 0) {
        return merge(range_fold(L, R, d, i * 2 + 1, l, m),
                     range_fold(L, R, d, i * 2 + 2, m, r));
      } else {
        return merge(range_fold(L, R, d, i * 2 + 2, m, r),
                     range_fold(L, R, d, i * 2 + 1, l, m));
      }
    }
  }
...
struct heavy_light_decomposition {
  ...
  void update(int v) { ST.update(in[v]); }
  monoid path_fold(int u, int v) {
    int w = lca(u, v);
    monoid L(-1);
    while (next[u] != next[w]) {
      L = merge(L, ST.range_fold(in[next[u]] + 1, in[u] + 1, 1));
      u = p[next[u]];
    }
    L = merge(L, ST.range_fold(in[w] + 1, in[u] + 1, 1));
    monoid R(-1);
    while (next[v] != next[w]) {
      R = merge(ST.range_fold(in[next[v]] + 1, in[v] + 1, 0), R);
      v = p[next[v]];
    }
