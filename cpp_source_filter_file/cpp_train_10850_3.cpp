...
int a[MAXN];
long long t, sa[MAXN], sl[MAXN], sr[MAXN];
bool check() {
  for (int i = 1; i <= m; i++) sa[i] = sa[i - 1] + a[i];
  for (int i = 0, j = m; i <= n; i++) {
    while (j >= 1 && a[j] >= n - i) j--;
    if (sl[i] + sa[j] + 1ll * (n - i) * (m - j) < sl[m]) return false;
...
bool check(int cnt, int val) {
  ...
  for (int i = 1, last = 0; i <= m; i++)
    if (x[i] != -1 || m - i + 1 <= cnt) {
      ...
      if (1ll * inc * rng <= sum) {
        sum -= 1ll * inc * rng;
        for (int j = last + 1; j <= i - 1; j++) a[j] += inc;
      } else {
  ...
  if (!flg) {
    ...
    if (sum != 0) {
      ...
      long long inc = (sum - 1) / tmp + 1;
      for (int i = 1; i <= m; i++) {
        a[i] = max(a[i - 1], x[i]);
        if (m - i + 1 <= cnt) chkmax(a[i], val);
      }
      if (a[m] + inc > n) return false;
      for (int i = 1; i <= cnt; i++) a[m - i + 1] += inc;
      sum = bak - inc * cnt;
      if (sum < 0) return false;
      for (int i = 1, last = 0; i <= m; i++)
        if (x[i] != -1 || m - i + 1 <= cnt) {
