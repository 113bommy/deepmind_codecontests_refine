...
void convex() {
  for (int T : {0, 1}) {
    for (pt p : ps) {
      while (ch.size() >= 2 && bad(p, ch[ch.size() - 2], ch.back()))
        ch.pop_back();
      ch.push_back(p);
    }
...
void upt(vector<pt> &cur, int fid, int tid, int ban) {
  if (tid > fid) {
    for (int i = fid; i <= tid; i++) {
      if (i == ban) continue;
      while (cur.size() >= 2 && bad(ps[i], cur[cur.size() - 2], cur.back()))
        cur.pop_back();
      cur.push_back(ps[i]);
    }
  } else {
...
void check(int id) {
  vector<pt> cur;
  pt prv = ch[(id - 1 + ch.size()) % ch.size()], now = ch[id],
     nxt = ch[(id + 1) % ch.size()];
...
int main() {
  ...
  convex();
  for (int i = 0; i < ch.size(); i++) check(i);
  puts("Borna");
}
