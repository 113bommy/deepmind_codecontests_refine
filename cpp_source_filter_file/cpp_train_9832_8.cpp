...
inline void Build(int Node, int st, int dr, int whichPath) {
  ...
  arb[whichPath][Node] =
      max(arb[whichPath][Node << 1], arb[whichPath][(Node << 1) | 1]);
}
inline int Query(int Node, int st, int dr, int a, int b, int whichPath) {
  if (a <= st && dr <= b) return arb[whichPath][Node];
  int mid = ((st + dr) >> 1);
  int ansmax = -oo;
  if (a <= mid)
...
inline int QueryHeavyPath(int x, int y) {
  if (pathWhere[x] == pathWhere[y]) {
    if (pathPosition[x] > pathPosition[y]) swap(x, y);
    if (pathPosition[x] == pathPosition[y]) return 0;
    return Query(1, 0, Path[pathWhere[x]].size() - 1, pathPosition[x] + 1,
                 pathPosition[y], pathWhere[x]);
  }
  if (depth[pathFather[pathWhere[x]]] < depth[pathFather[pathWhere[y]]])
    swap(x, y);
  int actMax = Query(1, 0, Path[pathWhere[x]].size() - 1, 0, pathPosition[x],
                     pathWhere[x]);
...
int main() {
  ...
  int APM = 0;
  for (vector<pair<int, pair<int, int> > >::iterator it = Edges.begin(),
                                                     fin = Edges.end();
       it != fin; ++it) {
    int z = it->first;
  ...
  for (int i = 1; i <= numPaths; ++i) {
    ...
    for (int j = 0; j < int(Path[i].size()); ++j) pathPosition[Path[i][j]] = j;
