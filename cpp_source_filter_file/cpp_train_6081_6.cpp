...
int m, n, a[1111][1111], deg[1111][1111], visited[1111][1111],
    isIntersection[16], mask[1111][1111];
int gcd(int x, int y) { return x && y ? gcd(y, x % y) : x + y; }
int valid(int x, int y) { return x && x <= m && y && y <= n && a[x][y]; }
...
int main() {
  ...
  for (int i = 1; i <= m; i++)
    for (int j = 1; j <= n; j++)
      if (a[i][j] && !visited[i][j]) visit(i, j), cnt++;
  if (cnt != 1) {
    cout << "-1\n";
    return 0;
  }
