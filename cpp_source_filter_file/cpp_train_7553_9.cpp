...
int n, m, a[100010], x, k[100010];
string s;
struct node {
  string name;
  int point;
} racer[100010], temp[100010], me;
int cmp(const int &a, const int &b) {
  return racer[a].point == racer[b].point ? racer[a].name > racer[b].name
                                          : racer[a].point < racer[b].point;
}
inline int findme() {
  for (int i = 0; i < n; i++)
...
int main() {
  ...
  for (int i = 0; i < n; i++)
    if (i != me) {
      int delta = mypoint - racer[i].point;
      if (racer[i].name > s) delta++;
