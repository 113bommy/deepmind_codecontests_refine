...
const int mn = 2e5 + 10;
struct ht {
  const static long long mod1 = 1e9 + 7, mod2 = 1e9 + 9;
  long long a, b;
  ht() { a = b = 0; }
  ht(long long x) { a = b = x; }
  ht(long long x, long long y) { a = x, b = y; }
  ht operator+(ht ot) { return {(a + ot.a) % mod1, (b + ot.b) % mod2}; }
  ht operator-(ht ot) {
    return {((a - ot.a) % mod1 + mod1) % mod1,
            ((b - ot.b) % mod2 + mod2) % mod2};
  }
  ht operator*(ht ot) { return {a * ot.a % mod1, b * ot.b % mod2}; }
  long long hail() {
    ;
...
int main() {
  ...
  while (q--) {
    ...
    if (s[ra] == '0') ra--;
    if (s[rb] == '0') rb--;
    if ((hsh[ra] - hsh[la - 1] * po[num[ra] - num[la - 1]]).hail() ==
        (hsh[rb] - hsh[lb - 1] * po[num[rb] - num[lb - 1]]).hail()) {
      printf("Yes\n");
    } else
      printf("No\n");
  }
