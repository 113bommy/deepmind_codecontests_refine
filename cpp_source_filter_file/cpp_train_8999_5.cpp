...
int main() {
  ...
  cin >> n >> k;
  if (n >= 1e8) {
    for (long long i = n + n; i <= 1e14; i += n) {
      if (check(i)) {
        k--;
        if (k == 0) {
          cout << i << endl;
  ...
  for (int i = 0; i < l.size(); i++) {
    ...
    if (x % 10 > 0 && (x < 10 || x / 10 % 10 < x % 10)) {
      ...
      int pl = lower_bound(vecr.begin(), vecr.end(), make_pair(needmod, 0)) -
               vecr.begin();
      int pr =
          lower_bound(vecr.begin(), vecr.end(), make_pair(needmod + 1, 0)) -
          vecr.begin();
      if (pr - pl < k) {
        k -= pr - pl;
      } else {
