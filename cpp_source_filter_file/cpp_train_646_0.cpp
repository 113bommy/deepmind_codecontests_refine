...
void solve(int u, int id, int root) {
  ...
  if (dep[u] - dep[old[id]] >= k) {
    ...
    if (tops[::u[id]] < tops[u]) {
      int mao = jump(::u[id], dep[::u[id]] - dep[old[id]] - 1);
      ans -= query(root, 1, n, tops[mao], tops[mao] + siz[mao] - 1);
    }
  } else if (tops[::u[id]] < tops[u]) {
    int cat = jump(
        ::u[id], (dep[::u[id]] - dep[old[id]]) - (k - (dep[u] - dep[old[id]])));
...
void dfs2(int u) {
  ...
  for (int i = 0; i < (int)adj[u].size(); i++) {
    ...
    for (int j = 0; j < (int)id[vec[v]].size(); j++)
      solve(u, id[vec[v]][j], rt[now]), id[vec[u]].push_back(id[vec[v]][j]);
    merge(rt[u], rt[v]);
  }
}
int main() {
