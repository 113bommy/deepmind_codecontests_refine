...
long double getint(int a, int b) {
  return 1.0 * (arr[a].second - arr[b].second) /
         (1.0 * arr[b].first - arr[a].first);
}
void build(int l, int r, int idx) {
  ...
  while (idx1 < seg[2 * idx].v.size() || idx2 < seg[2 * idx + 1].v.size()) {
    pair<int, int> p;
    int k;
    if (idx2 == seg[2 * idx + 1].v.size() ||
        (idx1 < seg[2 * idx].v.size() &&
         seg[2 * idx].v[idx1] < seg[2 * idx + 1].v[idx2])) {
      p = seg[2 * idx].v[idx1];
    ...
    while (seg[idx].v.size() >= 2) {
      ...
      if (arr[b].first == arr[k].first ||
          getint(seg[idx].idx.back(), b) >= getint(seg[idx].idx.back(), k)) {
        seg[idx].v.pop_back();
      } else {
...
long long query(int l, int r, long long x, int idx) {
  if (seg[idx].l == l && seg[idx].r == r) {
    ...
    while (lft <= rht) {
      ...
      if (a > b) {
        ans =
            a > arr[ans].first * x + arr[ans].second ? seg[idx].idx[mid] : ans;
        lft = mid + 2;
      } else {
        ans =
            b > arr[ans].first * x + arr[ans].second ? seg[idx].idx[temp] : ans;
        rht = mid - 1;
      }
    }
    return ans;
