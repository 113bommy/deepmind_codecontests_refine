...
double Inline(Tpoint p0, Tpoint p1, Tpoint t) {
...
bool Connect(Tpoint a, Tpoint b, Tpoint c, Tpoint d, int type) {
  if (!type) {
    if (Inline(a, b, c) && Inline(a, b, d)) return 0;
  }
  if (Inline(a, b, c) || Inline(a, b, d) || Inline(c, d, a) || Inline(c, d, b))
    return 1;
  return sgn(Cross(a, b, c)) != sgn(Cross(a, b, d)) &&
         sgn(Cross(c, d, a)) != sgn(Cross(c, d, b));
}
Tpoint InterPt(Tpoint a, Tpoint b, Tpoint c, Tpoint d) {
  double s1 = Cross(a, c, b), s2 = Cross(a, b, d);
  return Tpoint((c.x * s2 + d.x * s1) / (s1 + s2),
                (c.y * s2 + d.y * s1) / (s1 + s2));
}
Tpoint OppPt(Tpoint p0, Tpoint p1, Tpoint t) {
  Tpoint det = p1 - p0;
  double alpha = Pi / 2;
  det = Tpoint(det.x * cos(alpha) - det.y * sin(alpha),
               det.x * sin(alpha) + det.y * cos(alpha));
...
Tpoint v, p, q;
void init() {
  cin >> v.x >> v.y >> p.x >> p.y >> w0.x >> w0.y >> w1.x >> w1.y >> m0.x >>
      m0.y >> m1.x >> m1.y;
}
void print() {
