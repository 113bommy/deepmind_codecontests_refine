...
struct st {
  int nd;
  int val[2][Maxk];
  st(int nd = 0) : nd(nd) { fill((int*)val, (int*)val + 2 * Maxk, -Inf); }
};
int n;
...
void Add(int arr[], int val) {
  if (val > arr[0])
    arr[2] = arr[1], arr[1] = arr[0], arr[0] = val;
  else if (val > arr[1])
    arr[2] = arr[1], arr[1] = val;
  else if (val > arr[2])
    arr[2] = val;
}
...
int Get(int arr[], int g1, int g2) {
  if (g1 < g2) swap(g1, g2);
  if (arr[0] == g1) return arr[1] == g2 ? arr[2] : arr[1];
  return arr[0];
}
  ...
  for (int t = 0; t < 2; t++)
    for (int j = 1; j < Maxk; j++)
      for (int i = 0; i + (1 << j) <= chains[c].size(); i++)
        chains[c][i].val[t][j] = max(chains[c][i].val[t][j - 1],
                                     chains[c][i + (1 << j - 1)].val[t][j - 1]);
}
int Get(int c, int t, int l, int r) {
  ...
  for (int i = Maxk - 1; i >= 0; i--)
    if (l + (1 << i) <= r + 1) {
      res = max(res, chains[c][l].val[t][i]);
      l += 1 << i;
    }
  return res;
}
...
int calcDown(int a, int b, int lvl) {
  ...
  if (a != b) res = max(res, Get(cid[a], 0, cno[a], cno[b] - 1));
  return res + lvl;
}
...
int Climb(int a, int b) {
  ...
  if (P[a] != b) return chains[cid[b]][cno[b] - 1].nd;
  return a;
}
int main() {
  scanf("%d", &n);
  ...
  Traverse2(1, 0);
  Traverse3(1);
  for (int i = 0; i < curc; i++) Precompute(i);
  scanf("%d", &m);
  while (m--) {
    int a, b;
    scanf("%d %d", &a, &b);
    ...
    if (u != lca)
      res = max(res, Get(bot[u], bot[v][0] + 1) + L[u] - 2 * L[lca] + L[b]);
    ...
    if (b != lca) {
      ...
      res = max(res,
                Get(bot[lca], bot[na][0] + 1, bot[nb][0] + 1) + L[b] - L[lca]);
    } else {
      int na = Climb(a, lca);
