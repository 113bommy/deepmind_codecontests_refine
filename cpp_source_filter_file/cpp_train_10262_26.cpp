...
int n, m;
pair<double, int> q[1000 * 4 + 10];
double dist[1000 * 2 + 10][1000 * 2 + 10], ang[1000 * 2 + 10][1000 * 2 + 10],
    ans;
struct point {
  int x, y;
  inline point() {}
...
void read() {
  Read(n), Read(m);
  int i, j;
  for (i = 1; i <= n + m; i++) Read(a[i].x), Read(a[i].y);
  for (i = 1; i <= n + m; i++)
    for (j = 1; j <= n + m; j++) {
...
bool check1(int pos, double mid) {
  ...
  for (i = n + 1; i <= n + m; i++) {
    ...
    fix(l), fix(r);
    if (l > r) cnt++;
    q[++bk] = pair<double, int>(l, 1);
    q[++bk] = pair<double, int>(r, -1);
  }
  sort(q + 1, q + bk + 1);
  if (!cnt) return 1;
  for (i = 1; i <= bk; i++)
    if (!(cnt += q[i].second)) return 1;
  return 0;
}
bool check2(int pos, double mid) {
  ...
  for (i = 1; i <= n + m; i++) {
    ...
    fix(l), fix(r);
    if (l > r) cnt[i > n ? 1 : 0]++;
    q[++bk] = pair<double, int>(l, i > n ? 2 : 1);
    q[++bk] = pair<double, int>(r, i > n ? -2 : -1);
  }
  sort(q + 1, q + bk + 1);
...
inline bool check(int pos, double mid) {
  if (pos <= n) return check1(pos, mid);
...
void solve() {
  ...
  for (i = 1; i <= n + m; i++) {
    ...
    ans = max(ans, Divide_Conqure(0, 1e10, i));
  }
}
int main() {
  read();
