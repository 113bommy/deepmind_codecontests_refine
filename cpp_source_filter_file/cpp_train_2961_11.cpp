...
using namespace std;
const int MAXN = 100010, SQ = 300, NSQ = MAXN / SQ + 1;
int n, m, k, u, v, x, y, t, a, b, ans;
int A[MAXN], typ[MAXN], Q[MAXN][2];
int cnt[NSQ][MAXN * 2], C[SQ][MAXN];
;
vector<int> comp;
...
int main() {
  ...
  for (int i = 1; i <= m; i++) {
    ...
    if (typ[i] == 1) comp.push_back(Q[i][0]);
  }
  ...
  for (int i = 1; i <= m; i++)
    if (typ[i] == 1)
      Q[i][0] = lower_bound(comp.begin(), comp.end(), Q[i][0]) - comp.begin();
