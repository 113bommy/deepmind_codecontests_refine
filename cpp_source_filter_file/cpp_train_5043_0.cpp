...
struct SNumber {
  ...
  void read() {
    ...
    cin >> n;
    ve = vector<int>(n, 0);
    for (int i = n - 1; i >= 0; i--) cin >> ve[i];
  }
} le, ri, num;
void add(SNumber &tmp, int v) {
  int cur = 0;
  for (int &v : tmp.ve) {
    if (ch[cur][v] == 0) ch[cur][v] = ++cnt;
    cur = ch[cur][v];
  }
  sum[cur] = min(k, sum[cur] + v);
}
void BFS() {
  queue<int> q;
  for (q.push(0); !q.empty(); q.pop()) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      if (ch[u][i] > 0) {
        int v = ch[u][i];
        kmp[v] = nxt[u][i];
        if (kmp[v] == v) kmp[v] = nxt[kmp[u]][i];
        if (kmp[v] == v) kmp[v] = 0;
        sum[v] = min(k, sum[v] + sum[kmp[v]]);
        q.push(v);
      }
    }
  }
...
int find_ans(SNumber &cur) {
  ...
  for (int nod = 0; nod <= cnt; nod++)
    for (int val = 0; val <= k; val++)
      for (int le = 0; le < 2; le++)
        for (int ze = 0; ze < 2; ze++) dp[nod][val][1][le][ze] = 0;
  dp[0][0][1][1][1] = 1;
  for (int i = 0; i < ve.size(); i++) {
    ...
    for (int nod = 0; nod <= cnt; nod++)
      for (int val = 0; val <= k; val++)
        for (int le = 0; le < 2; le++)
          for (int ze = 0; ze < 2; ze++) dp[nod][val][cur][le][ze] = 0;
    for (int nod = 0; nod <= cnt; nod++)
      for (int val = 0; val <= k; val++)
        for (int le = 0; le < 2; le++)
...
int main() {
  ...
  for (int i = 0; i < le.ve.size(); i++)
    if (le.ve[i] > 0) {
      ...
      le.ve[i] = m - 1;
  if (le.ve.back() == 0) le.ve.pop_back();
  ri.read();
  while (n--) {
    num.read();
    cin >> v;
