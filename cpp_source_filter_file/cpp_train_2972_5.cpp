...
struct point {
  ...
  void print() { cout << fixed << setprecision(9) << x << " " << y << "\n"; }
};
...
bool clockwise(point a, point b, point c) { return cross(b - a, c - a) < eps; }
bool anticlock(point a, point b, point c) { return cross(b - a, c - a) >= eps; }
...
bool inside(vector<point> &polygon, const point &p) {
  int n = polygon.size();
  for (int i = 0; i < n; ++i) {
...
void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = i + 1; j != i; j = (j + 1) % n) {
      while (area(arr[i], arr[j], arr[(k + 1) % n]) >
             area(arr[i], arr[j], arr[k])) {
        k = (k + 1) % n;
      }
