...
node operator+(const node &a, const node &b) {
  ...
  for (register int i = 0; i < b.cnt; ++i) {
    ...
    for (register int j = 0; j < c.cnt; ++j)
      if (c.val[j] == b.val[i]) {
        c.hp[j] += b.hp[i], flag = 0;
        break;
      }
    if (!flag) continue;
    if (c.cnt < p) {
      c.val[c.cnt] = b.val[i], c.hp[c.cnt] = b.hp[i], ++c.cnt;
      continue;
    }
    int mn = b.hp[i];
    ...
    if (b.hp[i] > mn) c.val[k] = b.hp[i], c.hp[k] = b.hp[i] - mn, k++;
    c.cnt = k;
  }
  return c;
}
...
void cover(int u, int v, int len) { a[u].init(v, len), lz[u] = v; }
void down(int u, int l, int r, int md) {
  if (lz[u])
    cover(u << 1, lz[u], md - l + 1), cover(u << 1 | 1, lz[u], r - md),
        lz[u] = 0;
}
void modify(int u, int l, int r, int L, int R, int v) {
  if (L <= l && r <= R) return cover(u, v, l - r + 1), void(0);
  int md = (l + r) >> 1;
  down(u, l, r, md);
...
int main() {
  ...
  while (q--) {
    ...
    if (op == 1)
      ...
      print(tmp.cnt, ' ');
      for (register int i = 0; i < tmp.cnt; ++i) print(tmp.val[i], ' ');
      putchar('\n');
    }
  }
}
