...
int gao(const list<string>& a, const list<string>& b) {
  int k = 0;
  for (list<string>::const_iterator i = a.begin(), j = b.begin();
       i != a.end() && j != b.end();) {
    if (i->at(0) < j->at(0)) {
      ++i;
...
int main() {
  ...
  for (list<string>::iterator i = a.begin(); i != a.end();) {
    s = *i;
    i = a.erase(i);
    for (list<string>::iterator j = b.begin(); j != b.end(); ++j) {
      t = *j;
      j = b.erase(j);
  ...
  c.sort();
  for (list<pair<string, string> >::const_iterator i = c.begin(); i != c.end();
       ++i) {
    if (i != c.begin()) {
      cout << ", ";
    }
