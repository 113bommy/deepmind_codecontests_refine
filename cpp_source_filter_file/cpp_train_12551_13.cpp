      ...
      for (kk = 0; kk <= H[a] + 1; kk++) {
        ...
        for (j = 0; j <= K; j++) {
          for (k = 0; k <= H[x] + 1; k++) {
            ...
            if (j == 0) {
              if (k && jj + k <= K)
                TP[cc][0][hh] = (TP[cc][0][hh] + d2) % Mod;
              else
                TP[cc][jj][hh] = (TP[cc][jj][hh] + d2) % Mod;
            } else if (k == 0) {
              if (kk && j + kk <= K)
                TP[cc][jj][hh] = (TP[cc][jj][hh] + d2) % Mod;
              else
                TP[cc][max(jj, j)][hh] = (TP[cc][max(jj, j)][hh] + d2) % Mod;
            } else {
              int pp = 0;
              if (jj && (!k || jj + k > K)) pp = jj;
              if (j && (!kk || j + kk > K)) pp = max(pp, jj);
  ...
  for (i = 0; i <= K; i++) {
    for (j = 0; j <= H[a] + 1; j++) {
      ...
      if (i == 0 && (tj && tj <= K + 1)) {
        ...
        D[a][i + 1][tj] = (D[a][i + 1][tj] + TP[cc][i][j]) % Mod;
    }
  }
}
int main() {
