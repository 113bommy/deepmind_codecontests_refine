...
using namespace std;
struct segtree {
  vector<pair<int, int> > vx[200050];
  vector<int> ans;
  int pos[200050], mn[1024 * 128 * 2 * 2];
  int mum;
  void init(int n) {
    mum = 1;
    while (mum < n) mum <<= 1;
    for (int i = 0; i < mum * 2; i++) mn[i] = 1000000005;
    for (int i = 0; i < mum; i++) {
      pos[i] = 0;
      vx[i].clear();
    }
  ...
  void ins(int x, int y, int id) {
    vx[x].push_back(pair<int, int>(y, id));
  ...
  void query(int a, int b, int k, int l, int r, int t) {
    if (b <= l || r <= a || mn[k] > t) return;
    if (l + 1 == r) {
      while (pos[l] < vx[l].size() && vx[l][pos[l]].first <= t)
        ans.push_back(vx[l][pos[l]++].second);
      if (pos[l] < vx[l].size())
        mn[l] = vx[l][pos[l]].first;
      else
        mn[l] = 1000000005;
    } else {
...
segtree seg;
int A[200050], B[200050], C[200050], D[200050];
int dp[200050], back[200050];
vector<int> vx;
int main() {
  int n;
  scanf("%d", &n);
