...
const int N = 5e5 + 5, md = 1e9 + 7, inv2 = (md + 1) >> 1;
  ...
  for (auto v : a[u]) {
    ...
    if (!dfn[v]) {
      ...
      if (low[v] >= dfn[u]) {
        ...
        if (b[newn].size() > 2) {
          for (auto i : pairs)
            incyc[i] = incyc[make_pair(i.second, i.first)] = pw[b[newn].size()];
        }
      }
    } else
...
int main() {
  ...
  for (int i = n + 1; i <= newn; i++)
    if (b[i].size() > 2) C = add(C, pw[b[i].size()]);
  D = add(sub(A, B), C);
  AA = add(A, mul(1ll * n * (n - 1) % md, pw[2]));
  for (int i = 1; i <= n; i++)
    AB = add(AB, add(mul(a[i].size(), pw[2]), mul(m - a[i].size(), pw[3])));
  for (int i = n + 1; i <= newn; i++)
    if (b[i].size() > 2) cyc = add(cyc, pw[b[i].size()]);
  for (int i = 1; i <= n; i++) {
    for (auto v : b[i])
      if (b[v].size() > 2) sum[i] = add(sum[i], pw[b[i].size()]);
    for (auto v : b[i])
      if (b[v].size() > 2) sum[v] = add(sum[v], sub(sum[i], pw[b[v].size()]));
