...
class SuffixAutomaton {
 public:
  SuffixAutomaton(const string &S) {
    ...
    for (Node *k = lastNode; k != 0; k = k->link) k->ends = 1;
    getCount(root);
    getAnswer(root);
  }
 ...
 private:
  class Node {
   public:
    Node(const int &length_ = 0, Node *const link_ = 0,
         map<char, Node *> next_ = map<char, Node *>())
        : length(length_),
          ends(0),
          count(0),
          solved(false),
  ...
  int getCount(Node *current) {
    if (current->solved == true) return current->ends;
    for (auto &it : current->next) current->ends += getCount(it.second);
    current->solved = true;
    return current->ends;
  }
  int getAnswer(Node *current) {
    if (current->solved == false) return current->count;
    current->count = current->ends;
    for (auto &it : current->next) {
      current->count += getAnswer(it.second);
      if (current->count > kUpperBound) current->count = kUpperBound;
    }
  ...
  void extend(char c) {
    ...
    for (; k != 0 && k->next[c] == p; k = k->link) k->next[c] = clone;
  }
  Node *root, *lastNode;
};
int main() {
  ...
  int K;
  cin >> K;
  if (1ULL * S.size() * (S.size() - 1) < 1ULL * K) {
    cout << "No such line.\n";
    return 0;
  }
  SuffixAutomaton A(S);
