...
void updMax(int *a, int b) { *a = max(*a, b); }
int abs(int a) { return a < 0 ? -a : a; }
void updMin(int *a, int b) { *a = min(*a, b); }
int dx[2] = {-1, 1};
int main() {
  int n;
  scanf("%s %d", cmd, &n);
  ...
  for (i = n & 1; i <= n; i += 2) {
    for (k = 0; k < 2; k++) {
      if (left[len][i][k] != 100500) updMax(&ans, abs(left[len][i][k]));
      if (right[len][i][k] != -100500) updMax(&ans, abs(right[len][i][k]));
    }
  }
  printf("%d\n", ans);
  return 0;
