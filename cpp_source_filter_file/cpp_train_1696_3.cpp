...
pair<long double, long double> L[MAXN];
void load() {
  scanf("%d%lld", &N, &K);
  for (int i = 0; i < N; i++) {
    int x, y;
...
long long calc(long double r) {
  ...
  vector<Interval> v;
  auto intersect = [&](long double a, long double b, int who) {
    long double A = a * a + 1;
    long double B = 2 * a * b;
    ...
    v.push_back({alpha1, alpha2});
    all.push_back(alpha1);
    all.push_back(alpha2);
  };
  for (int i = 0; i < N; i++) intersect(L[i].first, L[i].second, i);
...
long double solve() {
  ...
  for (int i = 0; i < N; i++)
    L[i] = {-pts[i].first / pts[i].second, 1 / pts[i].second};
