...
struct Matrix {
  ...
  Matrix() {
    for (int i = 0; i < 256; i++)
      for (int j = 0; j < 256; j++) a[i][j] = inf;
  }
  void id() {
    for (int i = 0; i < 256; i++) a[i][i] = 0;
  }
    ...
    if (count(i) == x) {
      if (i & 1) {
        ...
        for (j = 0; j < k; j++) {
          if (!((mask >> j) & 1)) {
            tmp[mask | (1 << j)] =
                min(tmp[mask | (1 << j)], dp[i] + c[j + 1] + w[fir + j + 1]);
          }
        }
      } else {
...
void MoveMatrix(int k, int x, int len) {
  for (int l = 0; l < 30; l++) {
    ...
    for (i = 0; i < 1 << k; i++) {
      for (j = 0; j < 1 << k; j++) {
        tmp[i] = min(tmp[i], dp[j] + mul[l].a[i][j]);
      }
    }
    for (i = 0; i < 1 << k; i++) dp[i] = tmp[i];
  }
...
bool Check(int pos) {
  int j = upper_bound(Qs.begin(), Qs.end(), make_pair(pos, H)) - Qs.begin();
  if (j) {
    j--;
...
int main() {
  ...
  int p, val;
  while (q--)
    scanf("%i %i", &p, &val), w[p] = val, Qs.push_back(make_pair(p - k, p));
