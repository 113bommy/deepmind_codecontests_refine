...
void upd(int v, int tl, int tr, int l, int r, pair<long long, long long> x) {
  push(v, tl, tr);
  if (tl > r || l > tr) return;
  if (l <= tl && tr <= r) {
    add(v, x);
...
void solve() {
  ...
  for (int i = 0; i < q; i++) {
    ...
    if (type == 1) {
      int v, x, k;
      cin >> v >> x >> k;
      upd(1, 0, n - 1, tin[v], tout[v],
          make_pair((x + d[v] * 1ll * k) % mod, k));
    } else {
      int v;
      cin >> v;
      pair<long long, long long> cur = get(1, 0, n - 1, tin[v], tout[v]);
