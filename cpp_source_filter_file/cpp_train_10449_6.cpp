...
struct Seg {
  ...
  void init(int u, int l, int r, int L, int R) {
    ...
    int mid = (l + r) >> 1, pp;
    for (int i = L; i <= R; i++) {
      tree[u].co.push_back(p[i].second);
      if (p[i].first <= e[mid]) pp = i;
    }
    sort(tree[u].co.begin(), tree[u].co.end());
    if (l == r) return;
    init(u << 1, l, mid, L, pp);
  ...
  int query(int u, int x, int y) {
    if (e[tree[u].r] <= x)
      return upper_bound(tree[u].co.begin(), tree[u].co.end(), y) -
             tree[u].co.begin();
    int ret = query(u << 1, x, y);
    if (e[tree[u << 1 | 1].l] <= x) ret += query(u << 1 | 1, x, y);
    return ret;
  }
} seg;
bool solve() {
  ...
  if (q[c].first == x || q[d].first == yy) return 0;
  if (seg.query(1, x, y) != C[0]) return 0;
  if (seg.query(1, xx, y) != C[0] + C[1]) return 0;
  if (seg.query(1, x, yy) != C[0] + C[4]) return 0;
  if (seg.query(1, xx, yy) != C[0] + C[1] + C[4] + C[5]) return 0;
