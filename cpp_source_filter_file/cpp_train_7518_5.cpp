#include <bits/stdc++.h>
const int N = 500054, M = N * 2, LN = 19;
int n, m, E = 0;
...
int jump_until(int x, int d) {
  int S = dep[x] - d;
  for (assert(S >= 0); S; S &= S - 1) x = P[__builtin_ctz(S)][x];
  return x;
}
namespace dsu {
int p[M];
...
inline void equiv(int x, int y) {
  test(x, y, true), test(((((x)-1) ^ 1) + 1), ((((y)-1) ^ 1) + 1), true);
}
...
bool main(int R) {
  int i, x, y, cl, cr, cc, B, at, bt, top, _;
  bool Lr, Rr;
  memset(stamp, 0, (n + 1) << 2);
  for (_ = n; _; --_) {
    x = o[_], top = 0, cl = 1, cr = R, cc = (R + 1) / 2;
    for (i = first[x]; i; i = next[i]) {
      y = to[i], B = bel[(2 * (y)-1)], at = (B + 1) / 2;
      if (stamp[at] != _) stamp[at] = _, l[at] = 1, r[at] = R, li[top++] = at;
      B & 1 ? up(l[at], f[y] + 1) : down(r[at], R - f[y]);
    }
    B = bel[(2 * (x)-1)], bt = (B + 1) / 2;
    for (i = 0; i < top; ++i) {
      if (y = li[i], l[y] > r[y]) return false;
      if (y == bt)
        B & 1 ? (up(cl, l[y]), down(cr, r[y]))
              : (up(cl, R + 1 - r[y]), down(cr, R + 1 - l[y]));
      else
        Lr = l[y] * 2 <= R + 2, Rr = r[y] * 2 >= R, assert(Lr || Rr),
        down(cc, min(r[y], R + 1 - l[y])), up(cl, min(l[y], R + 1 - r[y])),
        down(cr, max(r[y], R + 1 - l[y]));
    }
    if (cc < cl) up(cl, R + 1 - cc);
    if (cl > cr) return false;
    for (f[x] = cl, i = 0; i < top; ++i)
      y = li[i], contain[y] = (l[y] <= cl && cl <= r[y]);
    for (i = first[x]; i; i = next[i])
      y = to[i], B = bel[(2 * (y)-1)], at = (B + 1) / 2,
      rev[y] = (B ^ (at == bt ? bel[(2 * (x)-1)] : contain[at])) & 1;
  }
  return true;
}
void dump(int R) {
...
int main() {
  ...
  for (i = n; i; --i)
    if (v = o[i], cong[p[v]] += cong[v], cong[v])
      dsu::equiv((2 * (v)-1), (2 * (p[v]) - 1));
  for (i = 2; i <= n; ++i)
    if (dsu::test((2 * (i)-1), (2 * (i)))) return puts("-1"), 0;
  for (u = 1; u <= n; ++u) {
    if (to[i = first[u]] == p[u])
      first[u] = next[i];
    else
      for (i = first[u]; next[i]; i = next[i])
        if (to[next[i]] == p[u]) next[i] = next[next[i]];
    if (!bel[dsu::ancestor((2 * (u)-1))])
      bel[dsu::ancestor((2 * (u)-1))] = (2 * (u)-1);
    if (!bel[dsu::ancestor((2 * (u)))])
      bel[dsu::ancestor((2 * (u)))] = (2 * (u));
  }
