...
template <class T>
inline T &checkMin(T &a, const T b) {
  if (b < a) a = b;
  return a;
}
template <class T>
inline T &checkMax(T &a, const T b) {
  if (a < b) a = b;
  return a;
}
template <class T>
inline T &checkMin(T &a, T &b, const T x) {
  checkMin(a, x), checkMin(b, x);
  return a;
}
template <class T>
inline T &checkMax(T &a, T &b, const T x) {
  checkMax(a, x), checkMax(b, x);
  return a;
}
template <class T, class C>
inline T &checkMin(T &a, const T b, C c) {
  if (c(b, a)) a = b;
  return a;
}
template <class T, class C>
inline T &checkMax(T &a, const T b, C c) {
  if (c(a, b)) a = b;
  return a;
}
template <class T>
...
int A[N][N], lu[N][N], lb[N][N], ru[N][N], rb[N][N];
int n, m;
int main() {
  RD(n, m);
  for (int i = 1; i <= n; ++i)
  ...
  for (int i = 1; i <= n; ++i) {
    for (int j = 1; j <= m; ++j)
      lu[i][j] = A[i][j] + max(lu[i - 1][j], lu[i][j - 1]);
    for (int j = m; j >= 1; --j)
      ru[i][j] = A[i][j] + max(ru[i - 1][j], ru[i][j + 1]);
  }
  for (int i = n; i >= 1; --i) {
    for (int j = 1; j <= m; ++j)
      lb[i][j] = A[i][j] + max(lb[i + 1][j], lb[i][j - 1]);
    for (int j = m; j >= 1; --j)
      rb[i][j] = A[i][j] + max(rb[i + 1][j], rb[i][j + 1]);
  }
  int ans = 0;
  for (int i = 2; i <= n - 1; ++i)
    for (int j = 2; j <= m - 1; ++j) {
      checkMax(ans,
               max(lu[i - 1][j] + rb[i + 1][j] + lb[i][j - 1] + ru[i][j + 1],
                   lu[i][j - 1] + rb[i][j + 1] + lb[i + 1][j] + ru[i - 11][j]));
    }
  OT(ans);
}
