...
int spfa() {
  memset(fl, 0, sizeof(fl)), memset(l, 0, sizeof(l)), memset(le, 0, sizeof(le)),
      memset(in, 0, sizeof(in)), memset(sw, 0x3f, sizeof(sw));
  queue<int> q;
...
int I(int x, int y) { return x + y * 2 * n; }
int main() {
  scanf("%d", &n);
  for (int i = 1; i <= 2 * n; i++) {
    scanf("%d%d", &a[i], &b[i]);
  ...
  sort(p + 1, p + 2 * n + 1, cmp0);
  for (int i = 1; i <= 2 * n; i++) {
    G.add(I(p[i], 0), I(p[i], 2), INF, 0);
    G.add(I(p[i], 1), I(p[i], 2), INF, 1);
    if (i > 1) {
      G.add(I(p[i], 0), I(p[i - 1], 1), INF, 0);
      G.add(I(p[i], 1), I(p[i - 1], 1), INF, 0);
    }
  }
  sort(p + 1, p + 2 * n + 1, cmpt);
  for (int i = 1; i <= 2 * n; i++) {
    G.add(I(p[i], 2), I(p[i], 4), INF, 0);
    G.add(I(p[i], 3), I(p[i], 4), INF, 1);
    if (i > 1) {
      G.add(I(p[i], 2), I(p[i - 1], 3), INF, 0);
      G.add(I(p[i], 3), I(p[i - 1], 3), INF, 0);
    }
  }
  while (int F = spfa()) {
    minw += F * sw[T];
