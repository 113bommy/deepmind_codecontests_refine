...
int solve(long double C) {
  for (int i = 1; i <= n; i++) {
    for (int j = 0; j <= a; j++) {
      ...
      long double both = p[i] + u[i] - p[i] * u[i];
      if (j && both + dp[i - 1][j - 1] - C > ret) {
        ret = both + dp[i - 1][j - 1] - C;
        tot = opt[i - 1][j - 1] + 1;
      }
    }
  }
