...
struct FFT {
  ...
  FFT() : COS((1 << 20) + 1), SIN((1 << 20) + 1) {
    for (lli i = 0; i < (lli)((1 << 20) + 1); ++i) {
      COS[i] = cos(2 * 3.141592653589793 * i / (double)(1 << 20));
      SIN[i] = sin(2 * 3.141592653589793 * i / (double)(1 << 20));
    }
  }
...
int main() {
  ...
  int T_;
  scanf("%d", &T_);
  for (lli t_ = 0; t_ < (lli)(T_); ++t_) {
    int n = -1;
  l2:;
    scanf("%d", &n);
    if (n == -1) goto l2;
    int size = N;
    while (size > 16 && n <= size / 2) {
      size /= 2;
    }
    for (lli i = 0; i < (lli)(size); ++i) V0[i] = 0;
    for (lli i = 0; i < (lli)(size); ++i) K0[i] = 0;
    ...
    for (lli i = 0; i < (lli)(size); ++i) VK2[i] = V2[i] * K2[i];
    fft.fft(size, VK2, VK, 1, size, -1);
    for (lli i = 0; i < (lli)(2 * n - 1); ++i)
      if (VK[i].real() > size / 2) OK[abs(n - 1 - i)] = 0;
    for (lli i = (1); i <= (lli)(n - 1); ++i) {
      for (lli j = i; j < n; j += i) {
        OK[i] &= OK[j];
      }
    }
    int n0 = 1;
    for (lli d = (1); d <= (lli)(n - 1); ++d)
      if (OK[d]) n0++;
    printf("%d\n", n0);
    for (lli d = (1); d <= (lli)(n - 1); ++d)
      if (OK[d]) {
