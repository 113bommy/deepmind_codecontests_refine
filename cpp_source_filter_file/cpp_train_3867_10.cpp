...
void pushdown(int k) {
  if (s[k].lz != 1) {
    s[k << 1].mul = 1ll * s[k << 1].mul *
                    qpow(s[k].lz, s[k << 1].r - s[k << 1].l + 1) % MOD;
    s[k << 1].lz = 1ll * s[k << 1].lz * s[k].lz % MOD;
    s[k << 1].msk = s[k << 1].msk | s[k].tag_msk;
    s[k << 1].tag_msk = s[k << 1].tag_msk | s[k].tag_msk;
    s[k << 1 | 1].mul = 1ll * s[k << 1 | 1].mul *
                        qpow(s[k].lz, s[k << 1 | 1].r - s[k << 1 | 1].l + 1) %
                        MOD;
...
void modify(int k, int l, int r, int x, long long y) {
  if (l <= s[k].l && s[k].r <= r) {
    s[k].mul = 1ll * s[k].mul * qpow(x, s[k].r - s[k].l + 1) % MOD;
    s[k].msk |= y;
...
int query_mul(int k, int l, int r) {
  ...
  int mid = (s[k].l + s[k].r) >> 1;
  if (r <= mid)
    return query_mul(k << 1, l, r);
  else if (l > mid)
    return query_mul(k << 1 | 1, l, r);
  else
    return 1ll * query_mul(k << 1, l, mid) * query_mul(k << 1 | 1, mid + 1, r) %
           MOD;
}
long long query_msk(int k, int l, int r) {
  if (l <= s[k].l && s[k].r <= r) return s[k].msk;
  pushdown(k);
  int mid = (s[k].l + s[k].r) >> 1;
  if (r <= mid)
    return query_msk(k << 1, l, r);
  else if (l > mid)
    return query_msk(k << 1 | 1, l, r);
  else
    return query_msk(k << 1, l, mid) | query_msk(k << 1 | 1, mid + 1, r);
}
int main() {
  scanf("%d%d", &n, &qu);
  sieve(MAXV);
  ...
  while (qu--) {
    ...
    if (opt[1] == 'M') {
      ...
      long long msk = 0;
      for (int i = 1; i <= prcnt; i++)
        if (x % pr[i] == 0) msk |= 1ll << (i - 1);
      modify(1, l, r, x, msk);
    } else {
      int l, r;
      scanf("%d%d", &l, &r);
      int mul1 = 1, mul2 = 1, mul = query_mul(1, l, r);
      long long msk = query_msk(1, l, r);
      for (int i = 1; i <= prcnt; i++)
        if (msk >> (i - 1) & 1) {
          mul1 = 1ll * mul1 * (pr[i] - 1) % MOD;
          mul2 = 1ll * mul2 * pr[i] % MOD;
        }
