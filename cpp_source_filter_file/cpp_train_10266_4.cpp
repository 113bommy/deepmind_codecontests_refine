...
edge edg[524312];
segt tree1[524312], tree2[524312];
int dfsord[524312 >> 1];
int subsz[524312 >> 1];
int height[524312 >> 1];
long long backward[524312 >> 1];
vector<pair<int, long long> > g[524312 >> 1];
long long depth[524312 >> 1];
int k = 1;
int parent[524312 >> 1][19];
int remap[524312 >> 1];
void dfs(int v, int pre, long long w, int ht) {
  parent[v][0] = pre;
  int i;
  dfsord[v] = k++;
  remap[k - 1] = v;
...
void update(segt tree[], int start, int end, int l, int r, int index,
            long long val) {
...
long long query(segt tree[], int start, int end, int l, int r, int index) {
...
void build(int l, int r, int index) {
  ...
  tree1[index].value = max(tree1[index * 2].value, tree1[index * 2 + 1].value);
  tree2[index].value = max(tree2[index * 2].value, tree2[index * 2 + 1].value);
}
int n;
int main() {
  int m;
  ...
  while (m > 0) {
    ...
    if (tp == 1) {
      ...
      scanf("%d%lld", &id, &wei);
      if (id <= n - 1) {
        update(tree1, dfsord[edg[id].to],
               dfsord[edg[id].to] + subsz[edg[id].to] - 1, 1, n, 1,
               wei - edg[id].w);
        update(tree2, dfsord[edg[id].to],
               dfsord[edg[id].to] + subsz[edg[id].to] - 1, 1, n, 1,
               wei - edg[id].w);
        edg[id].w = wei;
      } else {
        update(tree2, dfsord[edg[id].from], dfsord[edg[id].from], 1, n, 1,
               wei - edg[id].w);
        edg[id].w = wei;
      }
    } else {
      ...
      int anc = LCA(u, v);
      if (anc == u) {
        printf("%lld\n", query(tree1, dfsord[v], dfsord[v], 1, n, 1) -
                             query(tree1, dfsord[u], dfsord[u], 1, n, 1));
      } else {
        printf("%lld\n",
               query(tree2, dfsord[u], dfsord[u] + subsz[u] - 1, 1, n, 1) -
                   query(tree1, dfsord[u], dfsord[u], 1, n, 1) +
                   query(tree1, dfsord[v], dfsord[v], 1, n, 1));
      }
    }
