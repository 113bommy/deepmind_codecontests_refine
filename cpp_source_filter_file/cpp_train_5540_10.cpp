...
inline bool f(long long x) {
  unsigned long long now = hs[x] * p[100004],
                     end = (hs[n] - hs[n - x]) * p[100004 - (n - x)];
  if (now != end) return false;
  long long cnt = 0;
  for (long long i = x; i <= n; i++)
    if ((hs[i] - hs[i - x]) * p[100004 - (i - x)] == now) cnt++;
  return cnt >= 3;
}
...
int main(void) {
  ...
  for (long long i = 1; i <= n; i++) {
    unsigned long long now = hs[i] * p[100004],
                       end = (hs[n] - hs[n - i]) * p[100004 - (n - i)];
    if (now == end) a[sz++] = i;
  }
