...
const long long mod = 1ll * md * (md - 1) / 2;
...
atom calc(int l, int r, int n1, int n2, int len) {
  ...
  if (x == 1)
    ...
    ans1 = mul(sub(l, 1), mul(sub(fpow(x, n2), 1), fpow(x - 1, md - 2)));
  int invx = fpow(x, md - 2);
  if (x == 1)
    ans1 = add(ans1, mul(mul(n1, add(n1, 1)), inv2));
  else {
    int res = 0;
    res = add(res, mul(mul(n1, fpow(invx, n2 + 1)), fpow(invx - 1, md - 2)));
    res = sub(res, mul(sub(fpow(invx, n2 + 1), invx),
                       fpow(mul(invx - 1, invx - 1), md - 2)));
...
int main() {
  for (int i = 1, l = 0, r = 9; i <= 100000; i++) {
    int n1 = 9ll * fpow(10, i - 1) % md,
        n2 = 9ll * fpow(10, i - 1, md - 1) % (md - 1);
  ...
  for (int l = 1, r; l <= n; l = r + 1) {
    ...
    if (st[top] == -3) {
      ...
      if (len1 == len2) {
        ...
        st[top] = ++tot;
        tmp[tot] = calc(l1, r1, (r1 - l1 + 1 + md) % md,
                        (r2 - l2 + md) % (md - 1), len1);
      } else {
        int l1 = 0, l2 = 0, r1 = fpow(10, len1) - 1,
            r2 = (fpow(10, len1, md - 1) + md - 2) % (md - 1);
        for (int i = 0; i < (int)str[id].size(); i++) {
          l1 = (10ll * l1 + str[id][i] - '0') % md;
          l2 = (10ll * l2 + str[id][i] - '0') % (md - 1);
        }
        atom ans = calc(l1, r1, (r1 - l1 + 1 + md) % md,
                        (r2 - l2 + md) % (md - 1), len1);
        for (int i = len1 + 1; i <= len2 - 1; i++) ans = ans + val[i];
        l1 = fpow(10, len2 - 1);
        l2 = fpow(10, len2 - 1, md - 1);
        r1 = 0;
        r2 = 0;
        for (int i = l; i <= r; i++) {
          r1 = (10ll * r1 + c[i] - '0') % md;
        ...
        ans = ans + calc(l1, r1, (r1 - l1 + 1 + md) % md,
                         (r2 - l2 + md) % (md - 1), len2);
        st[top] = ++tot;
        tmp[tot] = ans;
      }
    ...
    if (r < n && c[r + 1] == '(') {
      long long ans = 0;
      for (int i = l; i <= r; i++) ans = (10 * ans + c[i] - '0') % mod;
      st[++top] = ans;
      continue;
    }
    atom ans = atom(0, 1);
