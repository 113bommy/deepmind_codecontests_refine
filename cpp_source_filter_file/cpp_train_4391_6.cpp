...
T dist(T d, T r1, T r2) {
  ...
  for (int a : {-1, 1})
    for (int b : {-1, 1}) ans = min(ans, d + a * r1 + b * r2);
  return ans;
}
template <class T>
T sqr(T x) {
...
double len2(ipoint A) { return sqr((ll)A.first) + sqr((ll)A.second); }
point circumcircle(ipoint A, ipoint B, ipoint C) {
  auto D = A.first * (B.second - (double)C.second) +
           B.first * (C.second - (double)A.second) +
           C.first * (A.second - (double)B.second);
  D *= 2;
  double x =
      (len2(A) * (B.second - C.second) + (len2(B)) * (C.second - A.second) +
       len2(C) * (A.second - B.second)) /
      D;
  double y = (len2(A) * (C.first - B.first) + len2(B) * (A.first - C.first) +
              len2(C) * (B.first - A.first)) /
             D;
  ipoint U(x, y);
    ...
    for (int i = 0; i < 1e3 && !bad.empty(); i++) {
      ...
      if (bad.size() < 3) {
        ...
        if (cent.first != numeric_limits<int>::max()) {
          ...
          r = max(r, 250000);
          for (auto it : pts)
            if (abs(sqrt(len2(cent - it)) - r) <= 0.1 * r + miss) cnt++;
          if (cnt >= n) {
            if (centers.size() < k) {
              ...
              vector<int> d(k);
              for (int j = 0; j < k; j++)
                d[j] = dist((int)sqrt(len2(centers[j].first - cent)),
                            centers[j].second, r);
              int wh = min_element(d.begin(), d.end()) - d.begin();
              centers[wh].first.first =
                  (centers[wh].first.first + cent.first) / 2;
              centers[wh].first.second =
                  (centers[wh].first.second + cent.second) / 2;
      ...
      for (auto it : pts) {
        ...
        for (auto cent : centers) {
          if (abs(sqrt(len2(cent.first - it)) - cent.second) <=
              0.1 * cent.second + miss) {
            good = true;
            break;
          }
        }
        if (!good) bad.push_back(it);
      }
    }
