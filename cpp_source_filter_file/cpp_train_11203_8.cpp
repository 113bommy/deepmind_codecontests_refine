...
int n, a[50011];
point dir;
double area(point x, point y, point z) { return fabs((y - x).det(z - x)); }
...
void add(int x, int y, int id) {
  ...
  pp[id] = make_pair(fabs(area(E, F, A) - area(E, F, B)), id);
}
int ans[50011];
int main() {
  n = getnum();
  ...
  sort(pp + 1, pp + n + 1);
  for (int i = 1; i <= n; i++) ans[pp[i].second] = n - i + 1;
  for (int i = 1; i <= n; i++) putnum(ans[i] - 1), putchar(' ');
  return 0;
}
