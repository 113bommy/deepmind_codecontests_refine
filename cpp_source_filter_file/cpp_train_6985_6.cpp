...
bool vis[MAXN];
long long x[MAXN];
bool cmp(info a, info b) { return a.delta < b.delta; }
int F(int x) {
  if (f[x] == x)
    return x;
  else
...
int main() {
  ...
  bit[0] = 1;
  for (int i = 1; i <= n; i++) bit[i] = bit[i - 1] * 2 % P;
  for (int i = 1; i <= m; i++) {
  ...
  for (int i = 1, nxt; i <= m; i = nxt + 1) {
    for (nxt = i; nxt < n && a[nxt].delta == a[i].delta; nxt++)
      ;
    int comp = n;
    cnt++;
    for (int j = i; j <= nxt; j++) {
    ...
    for (int j = i; j <= nxt; j++) f[F(a[j].l)] = F(a[j].r);
    for (int j = i; j <= nxt; j++) {
