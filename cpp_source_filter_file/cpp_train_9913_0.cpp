...
int q5(int u) {
  ...
  fflush(stdout);
  if (read(x), x) return mi;
  printf("? 5 %d %d %d %d %d\n", pos[1], pos[2], pos[3], pos[n - 1], u);
  fflush(stdout);
  if (read(x), x) return (mi + 1) % 5;
  printf("? 5 %d %d %d %d %d\n", pos[1], pos[2], pos[3], pos[n - 2], u);
  fflush(stdout);
  if (read(x), x) return (mi + 2) % 5;
  printf("? 5 %d %d %d %d %d\n", pos[1], pos[2], pos[3], pos[n - 3], u);
  fflush(stdout);
  if (read(x), x) return (mi + 3) % 5;
  return (mi + 4) % 5;
}
int orz(int x, int y, int u) {
  ...
  int tmp;
  for (int i = 1; i <= 4; i++)
    if (i != x) printf("%d ", pos[i]);
  for (int i = n - 3; i <= n; i++)
    if (i != y) printf("%d ", pos[i]);
  printf("%d\n", u);
  fflush(stdout);
  return read(tmp), tmp;
}
...
int main() {
  ...
  for (int i = 2; i <= 4 && i <= n / 2; i++) {
    jjd(i);
    if (odd[pos[i]] ^ (i & 1))
      Swap(pos[i], pos[n - i + 1]), Swap(ans[pos[i]], ans[pos[n - i + 1]]);
  }
