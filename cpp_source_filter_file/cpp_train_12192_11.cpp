...
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    sort(begin(hor[i]), end(hor[i]));
    hor[i].resize(unique(begin(hor[i]), end(hor[i])) - hor[i].begin());
    sort(begin(ver[i]), end(ver[i]));
    ver[i].resize(unique(begin(ver[i]), end(ver[i])) - ver[i].begin());
  }
  for (int i = 1; i <= n; i++) {
    for (int j : hor[i]) {
  ...
  for (int v = 0; v < sz; v++) {
    ...
    if (i != j) ver[i].push_back(j);
  }
  vector<pair<pt, pt> > hh, vv;
  vector<int> in(n + 1);
  for (int i = 1; i <= n; i++)
    for (int j : hor[i]) in[j]++;
  for (int i = 1; i <= n; i++) {
    if (in[i]) continue;
    int j = i;
