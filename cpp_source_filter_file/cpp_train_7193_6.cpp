...
void pre(int n) {
  for (int i = 1; i <= n; i++)
    minn[0][i] = maxn[0][i] = cmin[i] = cmax[i] = num[i];
  for (int i = 1; i < 17; i++)
...
int jump(int x, int d, int n) {
  for (int j = 16; j >= 0; j--)
    if (r[x] + (1 << j) <= n &&
        max(cmax[x], maxn[j][r[x] + 1]) - min(cmin[x], minn[j][r[x] + 1]) <=
            d) {
...
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    for (int j = 1; j <= SIZE && i + j <= n; j++) {
      minn = min(minn, num[i + j]);
      maxn = max(maxn, num[i + j]);
      a[++cnt] = pair<int, int>(maxn - minn, i);
    }
  }
  sort(a + 1, a + cnt + 1);
  for (int i = 1; i <= k; i++) {
    int x;
    scanf("%d", &x);
    q[i] = pair<int, int>(m - x, i);
  }
  sort(q + 1, q + k + 1);
  for (int i = 1; i <= k; i++) {
    update(q[i].first, n);
    ans[q[i].second] = query(q[i].first, n);
  }
