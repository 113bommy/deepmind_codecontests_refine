...
struct MinCostFlow {
  ...
  vector<TCOST> h;
  bool _calc_distance_bellman_ford(int s) {
  ...
  void add_edge(int from, int to, TFLOW cap, TCOST cost, int eid = -1) {
    ...
    G[to].emplace_back(
        edge{from, (int)G[from].size() - 1, (TFLOW)0, -cost, eid});
  }
...
int main() {
  ...
  lint cost = 0;
  for (int i = (0), i_end_ = (N1); i < i_end_; i++) {
  ...
  for (int j = (0), j_end_ = (N2); j < j_end_; j++) {
