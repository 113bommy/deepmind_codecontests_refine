...
void solve(int t) {
  sort(c + 1, c + m + 1, [](int p, int q) { return x[p] < x[q]; });
...
int main() {
  ...
  for (int i = 0; i < mv[0][0].size(); i++) {
    cout << mv[0][0][i] << " " << mv[0][1][i] << " " << mv[0][2][i] << " "
         << mv[0][3][i] << endl;
  }
  for (int i = (int)mv[1][0].size() - 1; i >= 0; i--) {
    cout << mv[1][2][i] << " " << mv[1][3][i] << " " << mv[1][0][i] << " "
         << mv[1][1][i] << endl;
  }
  return 0;
}
