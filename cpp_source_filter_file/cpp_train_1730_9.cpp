...
long long int cnt[MAX], sum[MAX], x, y, n;
bool vis[MAX];
int main() {
  ...
  long long int ans = infLL;
  for (i = 2; i < MAX; ++i) {
    if (vis[i] == true) continue;
    long long int now = 0LL, till = min(x / y + 1, i);
    for (j = i; j < MAX; j += i) {
      if (j != i) vis[j] = true;
      now += (((j * (cnt[j] - cnt[j - till])) - (sum[j] - sum[j - till])) * y);
      now += ((cnt[j - till] - cnt[j - i]) * x);
    }
    ans = min(ans, now);
  }
  cout << ans << '\n';
