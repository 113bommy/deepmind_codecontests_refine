...
int f[2][N][N];
int n;
bool inF(int x, int y) { return x >= 0 && y >= 0 && x < n && y < n; }
...
void setVal(int i, int j, int fi, int fj, int x) {
  for (int t = 0; t < 2; t++) f[t][i][j] = f[t][fi][fj] ^ x;
}
bool dp[N][N];
int has(int t, int x1, int y1, int x2, int y2) {
  for (int i = 0; i <= x2 - x1; i++)
    for (int j = 0; j <= y2 - y1; j++) dp[i][j] = false;
  dp[0][0] = true;
  for (int i = 0; x1 + i <= x2 - i; i++)
    for (int j = 0; y1 + j <= y2 - j; j++) {
      if (dp[i][j] == false) continue;
      if (f[t][x1 + i][y1 + j] != f[t][x2 - i][y2 - j]) continue;
      if ((x2 - i - y2 - j) - (x1 + i + y1 + j) <= 1) return true;
      dp[i + 1][j] = true;
      dp[i][j + 1] = true;
    }
  return false;
...
bool tryAsk(int x1, int y1, int x2, int y2) {
  ...
  printf("!\n");
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) printf("%d", f[t][i][j]);
    printf("\n");
  }
  fflush(stdout);
  return true;
    ...
    for (int li = 1; li < len; li++) {
      ...
      for (int i = 0; i + li <= n; i++)
        for (int j = 0; j + lj <= n; j++) {
          if (tryAsk(i, j, i + li - 1, j + lj - 1)) return 0;
        }
    }
  }
  return 0;
