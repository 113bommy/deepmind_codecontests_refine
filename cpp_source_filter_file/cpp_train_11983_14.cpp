...
void reverse(int x, int y) { reverse(x, (x + y) / 2, y); }
...
void bf(int l, int r) {
  ...
  for (int st = 0; st < (1 << opcnt); st++) {
    ...
    if (judge(l, r)) {
      flag = 1;
      for (int i = 1; i <= opcnt; i++)
        if (st & (1 << (i - 1))) reverse(opL[i], opR[i]);
      return;
    }
  }
}
void solve(int l, int r) {
  ...
  if (checkl(l, 1, 0, 1)) return reverse(l, l + 2, l + 4), solve(l + 5, r);
  if (checkl(l, 1, 0, 0)) return reverse(l, l + 3, l + 6), solve(l + 7, r);
  if (checkr(r, 1, 1, 1)) return reverse(r - 2, r - 1, r), solve(l, r - 3);
  if (checkr(r, 1, 0, 1)) return reverse(r - 4, r - 2, r), solve(l, r - 5);
  if (checkr(r, 1, 0, 0)) return reverse(r - 6, r - 3, r), solve(l, r - 7);
  if ((r - l) & 1) {
    reverse(l, r - 1);
    reverse(l + 1, r);
