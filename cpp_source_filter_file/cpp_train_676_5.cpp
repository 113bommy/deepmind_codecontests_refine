...
int32_t main() {
  ...
  for (x = 1; x <= n; x++)
    for (y = 1; y <= m; y++)
      for (z = 1; z <= K; z++) cin >> c, a[x][y][z] = c - '0';
  auto check = [&]() {
    int i, j, k;
    ...
    if (a[x][y - 1][z] and a[x + 1][y][z] and !a[x][y - 1][z]) return true;
    if (a[x - 1][y][z] and a[x][y][z + 1] and !a[x - 1][y][z + 1]) return true;
    if (a[x][y][z - 1] and a[x + 1][y][z] and !a[x][y][z - 1]) return true;
