...
struct DSU {
  ...
  DSU(const int &n) {
    ...
    for (int i = 1; i <= n; i++) par.push_back(i), size.push_back(1);
  }
  int findRoot(int v) {
  ...
  bool Union(int v, int u) {
    ...
    if (size[pv] > size[pu]) swap(pv, pu);
    par[pv] = par[pu], size[pu] += size[pv], comp--;
    return false;
  }
  int sz(int v) { return size[findRoot(v)]; }
  bool check(int v, int u) { return findRoot(v) == findRoot(u); }
};
int main() {
  ...
  while (m--) {
    int v, u;
    cin >> v >> u;
    if (G.Union(v, u)) ans = (ans * 2 + 1) % mod;
    cout << ans << '\n';
  }
  return 0;
}
