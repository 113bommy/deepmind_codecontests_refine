...
int m;
vector<int> ans;
long long getCost(int L, int R, int f1, int f2) {
  int len = R - L;
  if (len == 1) {
    if (a[L] == 0) return 0;
  ...
  long long cost = abs(sum[R] - sum[L]);
  if (len == 3) {
...
void restoreAns(int L, int R, int f1, int f2) {
  ...
  if (f1 == 0 && a[L] != 0 && a[L + 1] != 0) {
    long long x = min(a[L], a[L + 1]);
    ans.push_back(L);
    a[L] -= x;
    a[L + 1] -= x;
  }
  if (f2 == 0 && a[R - 1] != 0 && a[R - 2] != 0) {
    long long x = min(a[R - 1], a[R - 2]);
    ans.push_back(R - 2);
    a[R - 2] -= x;
    a[R - 1] -= x;
  }
...
int main() {
  ...
  for (int i = 0; i <= n; i++) {
    ...
    if (a[i - 1] <= a[i - 2] || a[i] <= a[i + 1]) intPoint[m++] = i;
  }
      ...
      for (int j = i + 1; j < min(m, i + K); j++) {
        for (int f2 = 0; f2 < 2 - f1; f2++)
          for (int f3 = 0; f3 < 2; f3++) {
            long long cost = getCost(intPoint[i], intPoint[j], f2, f3);
            if (cost == -1) continue;
