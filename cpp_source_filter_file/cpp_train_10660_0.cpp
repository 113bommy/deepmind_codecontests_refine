...
int get_index(int x) { return (upper_bound(b, b + len, x) - b) - 1; }
void solve() {
  for (int i = 0; i < len; ++i) b[i] = h - b[i];
  sort(b, b + len);
  segment_tree.build(0, 0, len - 1, len);
  for (int i = 0; i < len; ++i) {
    int index = get_index(a[i]);
    if (index != -1) segment_tree.update(0, 0, len - 1, 0, index, 1);
  }
  int answer = 0;
  if (segment_tree.tr[0] >= 0) answer++;
  for (int i = len; i < n; ++i) {
    int index = get_index(a[i]);
    if (index != -1) segment_tree.update(0, 0, len - 1, 0, index, 1);
