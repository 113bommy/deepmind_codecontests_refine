...
struct Line {
  ...
  long double f(Point A) { return a * A.x + b * A.y + c; }
};
long double vp, v, r;
...
vector<Point> cut(Line l, long double r) {
  ...
  if (ab(delta) < 1e-9) {
    ...
    if (l.f(Point(x, y)) < 1e-3) {
      if (order(l.A, Point(x, y), l.B)) res.push_back(Point(x, y));
    } else {
      if (order(l.A, Point(-x, y), l.B)) res.push_back(Point(-x, y));
    }
  } else if (delta > 0) {
...
long double dist(Point Q, Point M) {
  ...
  res +=
      min(min(cirlen(alpha1 + acos(r / Q.len()), alpha2 + acos(r / M.len())),
...
int main() {
  ...
  for (int turn = 0, _a = (1000); turn < _a; turn++) {
