...
void rd() {}
template <typename... T>
void rd(int &x, T &...args) {
  scanf("%i", &x);
  rd(args...);
}
template <typename... T>
void rd(long long &x, T &...args) {
  scanf("%lld", &x);
  rd(args...);
}
template <typename... T>
void rd(double &x, T &...args) {
  scanf("%lf", &x);
  rd(args...);
}
template <typename... T>
void rd(pair<int, int> &x, T &...args) {
...
void Z(char a[], char s[], int lcp[]) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    while (i + lcp[i] <= n && lcp[i] + 1 <= m && s[lcp[i] + 1] == a[i + lcp[i]])
      lcp[i]++;
...
pair<long long, long long> sum[N];
void Add(int i, pair<long long, long long> f) {
  for (i++; i < N; i += i & -i) sum[i] += f;
}
...
int main() {
  ...
  for (int i = 1, ptr = 1; i <= n; i++) {
    ...
    pair<long long, long long> now = Get(N - 1) - Get(m - A[i] - 1);
