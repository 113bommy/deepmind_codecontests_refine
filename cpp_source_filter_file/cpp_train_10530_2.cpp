...
int n, m, k, inv[222], cnt[22], dp[22][111][22], nmsk, ddp[22][22][111],
    shold[22][111], ans;
    ...
    for (int j = 0; j < maxn; j++) {
      for (int msk = 0; msk < (1 << m); msk++) {
        ...
        for (int d = 1; d <= m; d++) {
          ...
          nmsk = (msk << (d - 1)) & ((1 << m) - 1);
          if (j + d <= maxn)
            add(dp[i + 1][j + d][((nmsk << 1) & ((1 << m) - 1)) | 1],
                1ll * dp[i][j][msk] * (cnt[nmsk] + 1) % mod);
        }
      }
    }
  }
      ...
      for (int h = 0; h < maxn; h++) {
        ...
        for (int a = 1; a <= k; a++) {
          for (int b = 1; b <= maxn; b++) {
            ...
            if (!i) {
              add(ddp[i + 1][j + a][h + b],
                  1ll * ddp[i][j][h] * shold[a][b] % mod);
            } else {
              if (h + b + m <= maxn)
                add(ddp[i + 1][j + a][h + b + m],
                    1ll * ddp[i][j][h] * shold[a][b] % mod);
            }
          }
        }
