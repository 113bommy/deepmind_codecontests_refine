...
void reverse(int id) {
  lazy[id] = !lazy[id];
  for (int i = 0; i <= m; i++) swap(seg[id][i][0], seg[id][i][1]);
}
void shift(int id) {
  if (!lazy[id]) return;
  lazy[id] = false;
...
void build(int id = 1, int st = 0, int en = n) {
  if (en - st == 1) {
    seg[id][m][a[st] & 1] = m;
    for (int i = 0; i < m; i++) seg[id][i][0] = seg[id][i][1] = i + 1;
    return;
  }
  int mid = st + en >> 1;
  build(id << 1, st, mid);
...
void update(int l, int r, int id = 1, int st = 0, int en = n) {
  if (r <= st || en <= l) return;
  if (l <= st && en <= r) return reverse(id);
  shift(id);
  int mid = st + en >> 1;
...
int get(int l, int r, int x = 0, int id = 1, int st = 0, int en = n) {
  if (r <= st || en <= l) return x;
  if (l <= st && en <= r) return seg[id][x][0];
  shift(id);
  int mid = st + en >> 1;
