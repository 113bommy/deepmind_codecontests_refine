...
bool bfs() {
  ...
  for (h = 0; h < t; h++) {
    ...
    for (i = first[x]; i; i = next[i])
      if (dep[y = e[i].v] == -1 && e[i].f) que[t++] = y, dep[y] = dep[x] + 1;
  }
  return false;
}
...
int Dinic() {
  for (F = 0; bfs(); F += dfs(si, INT_MAX)) memcpy(cur, first, sizeof cur);
  return F;
}
}  // namespace F
namespace ST {
...
int dfsA(int x, int px = 0) {
  ...
  int i, y;
  for (i = first[x]; i; i = next[i])
    if ((y = e[i].v) != px && e[(((i - 1) ^ 1) + 1)].f) {
      if (2 + n < y && y <= 2 + n + R)
        return ++e[i].f, --e[(((i - 1) ^ 1) + 1)].f, y - n - 2;
      if (x < y) return ++e[i].f, --e[(((i - 1) ^ 1) + 1)].f, dfsA(y, x);
    }
  return 0;
}
int main() {
  ...
  for (i = 1; i <= n; ++i) {
    ...
    switch (type[i]) {
      case 0:
        for (scanf("%d", &c); c; --c)
          scanf("%d", &v), F::addedge(2 + i, 2 + n + v, 1);
        break;
      case 1:
        scanf("%d%d", &u, &v), ST::locate(root, 1, R, u, v, 2 + i);
        break;
      case 2:
        scanf("%d%d%d", &u, &v, &w), F::addedge(2 + i, 2 + n + u, 1),
            F::addedge(2 + i, 2 + n + v, 1), F::addedge(2 + i, 2 + n + w, 1);
        break;
    }
  }
  printf("%d\n", F::Dinic());
  for (v = 1; v <= n; ++v)
    if (type[v] == 1) {
      ...
      for (i = F::first[2 + v]; i; i = F::next[i])
        if (!F::e[i].f)
          if (u = F::e[i].v - n - 2, 0 < u && u <= R) link(v, u);
  for (v = 1; v <= R; ++v)
    if (type[v] == 2 && count[v] == 1) {
      for (i = F::first[2 + v]; i; i = F::next[i])
        if (F::e[i].f) {
          u = F::e[i].v - n - 2;
          if (0 < u && u <= R && from[u]) {
