...
const int base = 113, mod = 100003;
int has(const vector<int>& p) {
  long long res = 0, d = 1;
  for (int x : p) (res += x * d) %= mod, (d *= base) %= mod;
  return (int)res;
}
bool vis[100010];
...
void cal(int state) {
  ...
  for (int i = 0; i < n; i++) {
    if ((1 << i) & state)
      v.push_back({a[i].first, a[i].second + 1});
    else
      l.push_back({a[i].first, a[i].second + 1}), sum += a[i].first;
  }
