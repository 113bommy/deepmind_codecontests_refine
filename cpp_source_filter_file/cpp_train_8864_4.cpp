...
const int N = 200005;
const int inf = 1e9;
int n, m, mxr[N * 4], bz[N][25], bin[25];
struct val {
  int l, r;
} a[N];
bool cmp(val a, val b) { return a.l < b.l || a.l == b.l && a.r < b.r; }
int get_max(int x, int y) { return a[x].r > a[y].r ? x : y; }
void ins(int d, int l, int r, int x, int y) {
  ...
  if (x <= mid)
    ...
    ins(d << 1 | 1, mid + 1, r, x, y);
  mxr[d] = get_max(mxr[d << 1], mxr[d << 1 | 1]);
}
int query(int d, int l, int r, int x, int y) {
  ...
  if (y <= mid)
    ...
    return get_max(query(d << 1, l, mid, x, y),
...
int main() {
  ...
  for (int i = 1; i < n; i++) {
    bz[i][0] = query(1, 1, a[n].r, a[i].l, a[i].r);
