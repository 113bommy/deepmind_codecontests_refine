...
void build(int id, int v, int l, int r) {
  while (v > cnt[id].size()) cnt[id].push_back(0);
  if (l == r) {
    return;
  }
  int mid = (l + r) >> 1;
...
int find_cnt(int id, int v, int tl, int tr, int l, int r) {
  ...
  int mid = (tl + tr) >> 1;
  return find_cnt(id, v * 2, tl, mid, l, min(r, mid)) +
         find_cnt(id, v * 2 + 1, mid + 1, tr, max(l, mid + 1), r);
}
int main() {
  scanf("%d", &m);
  for (int i = 0; i < m; i++) {
  ...
  n = li.size();
  for (int i = 0; i < m; i++) {
    int pos = lower_bound(li.begin(), li.end(), x[i]) - li.begin();
    g[pos].push_back(tt[i]);
  }
  ...
  for (int i = 0; i < m; i++) {
    ...
    if (a[i] == 1) {
      int pp =
          lower_bound(g[pos].begin(), g[pos].end(), tt[i]) - g[pos].begin();
