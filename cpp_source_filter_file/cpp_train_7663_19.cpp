...
int main() {
  ...
  sort(w + 1, w + n + 1, cmp);
  for (i = 2; i <= n; i++)
    if (w[i].x == w[i - 1].x) unit(w[i].p, w[i - 1].p);
  sort(w + 1, w + n + 1, cmp2);
  for (i = 2; i <= n; i++)
    if (w[i].y == w[i - 1].y) unit(w[i].p, w[i - 1].p);
  t[0] = 1;
  for (i = 1; i <= 2 * n; i++) t[i] = t[i - 1] * 2 % q;
  p = 1;
  for (i = 1; i <= n; i++)
    if (fa(i) == i)
      if (y[i] == x[i] - 1)
        p = (long long)p * (t[x[i] + 1] - 1) % q;
      else
        p = (long long)p * t[x[i] * 2 - y[i]] % q;
  printf("%d\n", p);
  return 0;
}
