...
int dp[20010];
struct Segment_tree {
  int maxj[20010], lazy[20010];
  void pushup(int now) { maxj[now] = max(maxj[now << 1], maxj[now << 1 | 1]); }
...
int main() {
  ...
  for (int j = 1; j <= m - k + 1; j++) {
    dp[j] =
        sum[0][j + k - 1] - sum[0][j - 1] + sum[1][j + k - 1] - sum[1][j - 1];
  }
  for (int i = 1; i < n; i++) {
    seg.Build(1, 1, m - k + 1);
    for (int j = 1; j <= k; j++) {
      seg.Update(1, 1, m - k + 1, max(1, j - k + 1), min(m - k + 1, j),
                 -a[i][j]);
    }
    for (int j = 1; j <= m - k + 1; j++) {
      dp[j] = sum[i][j + k - 1] - sum[i][j - 1] + sum[i + 1][j + k - 1] -
              sum[i + 1][j - 1];
      dp[j] += seg.Query();
      if (j == m - k + 1) break;
      seg.Update(1, 1, m - k + 1, max(1, j - k + 1), min(m - k + 1, j),
                 a[i][j]);
      seg.Update(1, 1, m - k + 1, max(1, j + k - k + 1), min(m - k + 1, j + k),
                 -a[i][j + k]);
    }
  }
  int ans = 0;
