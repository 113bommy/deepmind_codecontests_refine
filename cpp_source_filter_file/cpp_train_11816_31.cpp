...
long long inv[100010], fac[100010], f[2010];
int main() {
  int i, j, x, y;
  for (i = 2, inv[0] = inv[1] = 1; i <= 100000; i++)
    inv[i] = (1000000007 - 1000000007 / i) * inv[1000000007 % i] % 1000000007;
  for (i = 2; i <= 100000; i++) inv[i] = inv[i] * inv[i - 1] % 1000000007;
  for (i = 2, fac[0] = fac[1] = 1; i <= 100000; i++)
