...
int main() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    ;
    for (int j = 0; j < 2; j++) {
      if (check(ring[i].x, ring[i].y, ring[i].r[j], ring[other].x,
                ring[other].y, ring[other].r[1]) == INTERSECT ||
          check(ring[i].x, ring[i].y, ring[i].r[j], ring[other].x,
                ring[other].y, ring[other].r[0]) == INTERSECT)
        continue;
      if (check(ring[i].x, ring[i].y, ring[i].r[j], ring[other].x,
                ring[other].y, ring[other].r[1]) == INSIDE ||
          check(ring[i].x, ring[i].y, ring[i].r[j], ring[other].x,
                ring[other].y, ring[other].r[0]) != INSIDE)
        continue;
      ans++;
    }
  }
