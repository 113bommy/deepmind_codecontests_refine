...
inline void up(int k) {
  if (tree[k << 1].sum > tree[k << 1 | 1].sum)
    tree[k].sum = tree[k << 1].sum, tree[k].r = tree[k << 1].r;
  else
    tree[k].sum = tree[k << 1 | 1].sum, tree[k].r = tree[k << 1 | 1].r;
}
...
int main() {
  ...
  for (int i = 1; i <= 300001; i++) {
    ...
    for (int j = 0; j < (int)v1[i].size(); j++)
      add(1, 1, 3 * 100010, v1[i][j].first, v1[i][j].second, 1);
    for (int j = 0; j < (int)v2[i].size(); j++)
      add(1, 1, 3 * 100010, v2[i][j].first, v2[i][j].second, -1);
