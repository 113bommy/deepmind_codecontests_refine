...
void solve() {
  ...
  function<int(int, int, int, int, vector<vector<int>> &)> get =
      [&](int ax, int ay, int bx, int by, vector<vector<int>> &A) {
        int sum = A[bx][by];
        if (ax > 0) sum -= A[ax - 1][by];
        if (ay > 0) sum -= A[bx][ay - 1];
        if (ax > 0 && bx > 0) sum += A[ax - 1][ay - 1];
        return sum;
      };
  ...
  for (int i = 0; i < n - 1; i++) {
    for (int j = 0; j < m - 1; j++) {
      ...
      for (int k = 0; k <= min(min(rr, gg), min(bb, yy)); k++) {
        ...
        int panjang = (k + 1) * 2;
        if (panjang < batas)
          pref[panjang][ax][ay] += 1;
        else
          daftar[panjang].push_back({ax, ay});
      }
    }
  }
  for (int k = 0; k < batas; k++) {
    for (int i = 0; i < n; i++)
      for (int j = 1; j < m; j++) pref[k][i][j] += pref[k][i][j - 1];
    for (int i = 1; i < n; i++)
      for (int j = 0; j < m; j++) pref[k][i][j] += pref[k][i - 1][j];
  }
  while (q--) {
    int ax, bx, ay, by;
    cin >> ax >> ay >> bx >> by;
    ...
    for (int panjang = 2; panjang <= n; panjang += 2) {
      ...
      for (auto x : daftar[panjang]) {
        if (AX <= x.first && x.first <= BX && AY <= x.second &&
            x.second <= BY) {
          ans = panjang;
          break;
        }
