...
int S, T, head[1100], dep[1100], q[1100], l, r, esz, cur[1100];
...
int bfs() {
  ...
  while (l < r) {
    ...
    for (int t = head[u]; t; t = e[t].nxt)
      if (e[t].w && !dep[e[t].r]) dep[q[r++] = e[t].r] = dep[u] + 1;
  }
  return dep[T] != 0;
}
...
int dinic() {
  int ans = 0, a;
  while (bfs())
    while (a = find(S, 1831311)) ans += a;
  return ans;
}
};  // namespace WXHAKIOI2018
int f[1100][1100], v, e, n, k;
int a[1100];
bool check(int ans) {
  WXHAKIOI2018::init(0, n + v + 1);
  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= v; ++j)
      if (f[a[i]][j] <= ans) WXHAKIOI2018::addedge(i, j + n, 1831311);
...
int main() {
  ...
  for (int k = 1; k <= v; ++k)
    for (int i = 1; i <= v; ++i)
      for (int j = 1; j <= v; ++j) f[i][j] = min(f[i][j], f[i][k] + f[k][j]);
  int l = 0, r = 1831311, ans = -1;
  while (l <= r) {
    int mid = l + r >> 1;
    if (check(mid))
      ans = mid, r = mid - 1;
