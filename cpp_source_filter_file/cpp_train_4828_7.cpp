...
long long p_pow[nhash][maxn], h[5][nhash][maxn];
void init() {
  memset(h, 0, sizeof(h));
...
vector<string> A(5);
int orden[3] = {0, 1, 2};
vector<long long> sub_h(int lo, int hi, int id) {
  vector<long long> ans(nhash, 0);
  int n = A[id].size();
...
vector<long long> hash_s(string &s) {
  vector<long long> ans(nhash, 0);
  for (int j = 0; j < nhash; ++j) {
...
string check(int i, int j) {
  int leni = A[i].size();
  int lenj = A[j].size();
  vector<long long> sub_j, sub_i;
  if (lenj <= leni) {
    sub_j = sub_h(0, lenj - 1, j);
    if (lenj < leni)
      for (int ii = 0; ii < nhash; ++ii)
        sub_j[ii] = (sub_j[ii] * p_pow[ii][leni - 1]) % mod[ii];
  ...
  for (int ii = 1; ii <= lenj and leni - ii >= 0; ++ii) {
    ...
    if (leni < lenj) {
      for (int jj = 0; jj < nhash; ++jj)
        sub_i[jj] = (sub_i[jj] * p_pow[jj][lenj - leni]) % mod[jj];
    } else if (lenj < leni) {
      for (int jj = 0; jj < nhash; ++jj)
        sub_j[jj] = (sub_j[jj] * p_pow[jj][leni - lenj]) % mod[jj];
    }
    if (sub_i[0] == sub_j[0] and sub_i[1] == sub_j[1]) {
      id_best = ii;
    }
