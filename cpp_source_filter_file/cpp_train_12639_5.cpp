...
unsigned long long basepow[maxhashsize + 1];
void calc_basepow() {
  basepow[0] = 1;
...
int hash_lcp(hashstr &hs1, hashstr &hs2, int off1 = 0, int off2 = 0) {
  ...
  while (l < r) {
    int m = (l + r + 1) / 2;
    if ((hs1.h[off1 + m] - hs1.h[off1]) * basepow[off2] ==
        (hs2.h[off2 + m] - hs2.h[off2]) * basepow[off1])
      l = m;
    else
      r = m - 1;
  }
...
int one_diff_pos(hashstr &hs1, hashstr &hs2, int size, int off1 = 0,
                 int off2 = 0) {
  if (size < max(hs1.size() - off1, hs2.size() - off2)) return -1;
  int l = hash_lcp(hs1, hs2, off1, off2);
  if (l >= size) return -1;
  if (l + 1 + hash_lcp(hs1, hs2, off1 + l + 1, off2 + l + 1) != size) return -1;
  return l;
}
long long delta[26][100000 + 1];
bool isgray[16 + 1][100000];
int lmask[16 + 1][100000];
int main(int argc, char **argv) {
  ...
  for (int d = 2; (1 << d) - 1 <= n; d++) {
    const long long size = (1 << d) - 1;
    const long long subsize = (1 << (d - 1)) - 1;
    for (int i = 0; i < n; i++) {
      if (i + size <= n) {
        ...
        if (isgray[d][i]) lmask[d][i] = m0 | (1 << midc);
