...
int mul(int a, int b) { return (int)((long long)a * b % 998244353); }
int fa[300000 + 5][30];
int dep[300000 + 5];
int a[300000 + 5];
int n, k;
...
int dfs(int root) {
  ...
  if (a[root] == 0 && val.empty()) {
    ...
    for (int i = 0; i < (int)now_c.size(); i++) {
      f[root][1] =
          add(f[root][1], mul(mul(pre[i], bac[i + 1]), f[now_t[i]][1]));
    }
    f[root][0] = add(f[root][1], pre[(int)now_c.size()]);
    return -1;
  }
  f[root][1] = 1;
  for (int i = 0; i < (int)now_c.size(); i++) {
