...
void check(int chan, int video, int left, int right) {
  ...
  long long val = (right - left) * 1ll * viewers[chan];
  if (val > bst) {
    bst = val;
    c = chan;
    v = video;
...
int main() {
  ...
  for (int i = 0; i < V; i++) {
    ...
    videos[lc].push_back(i);
  }
  for (int i = 0; i < C; i++) channs[comp(cy[i])].push_back(i);
  for (int i = 1; i < allv.size(); i++)
    if (rm[i - 1].first > rm[i].first) rm[i] = rm[i - 1];
  for (int i = allv.size() - 2; i >= 0; i--)
    if (lm[i].first > lm[i + 1].first) lm[i] = lm[i + 1];
  for (int i = 0; i < C; i++) {
  ...
  for (int i = 0; i < M; i++) {
    ...
    for (int ch : channs[i]) {
      ...
      long long val = mlen.first * 1ll * viewers[ch];
      if (val > bst) {
        bst = val;
        c = ch;
        v = mlen.second;
