...
inline long long Inv() { return dfs(1, 0), fpw(ans % P, P - 2); }
...
void prework() {
  for (int i = 1; i <= m; i++) {
    for (auto t : vec[i]) st[++tp] = all ^ S(t, i);
  }
}
}  // namespace Deal
using Deal::Inv;
...
int main() {
  ...
  for (int i = 1, x, y; i < m; i++)
    read(x), read(y), ed[x][y] = ed[y][x] = 1, vec[x].push_back(y),
                      vec[y].push_back(x);
