...
int num[MAXN << 1], pre[MAXN], ST[21][MAXN << 1], logg2[MAXN << 1], fir[MAXN],
    fa[MAXN << 1];
int N;
deque<int> unused;
bool vis[MAXN];
...
void solve(int l, int r) {
  ...
  for (int i = r; i >= l; i = find(i - 1)) {
    while (cur.size() >= 2) {
      if ((num[cur[cur.size() - 2]] == num[i] ||
           (!num[i] || !num[cur[cur.size() - 2]])) &&
          (num[i] || num[cur[cur.size() - 2]]) && num[cur[cur.size() - 1]]) {
  ...
  deque<int> q;
  for (int i = 1; i + 1 < cur.size(); ++i) q.push_back(cur[i]);
  int rt = cur[0];
  while (!q.empty()) {
...
void work() {
  for (int i = 1; i < N << 1; ++i)
    if (num[i] && ST[0][i] != i) solve(find(ST[0][i]), i);
  if (!num[1]) solve(1, (N << 1) - 1);
}
int main() {
  init();
  work();
