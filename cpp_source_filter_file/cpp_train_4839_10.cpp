...
int main() {
  ...
  for (int i = 2; i < n; i++) {
    for (int j = 2; j < m; j++) p[j] &= ((a[i][j] == '#') ? false : true);
  ...
  for (int i = 1; i <= m; i++) d[n][i] = (d[n][i] == '#') ? false : true;
  for (int i = n - 1; i >= 1; i--)
    for (int j = 1; j <= m; j++)
      d[i][j] = (d[i + 1][j]) & ((a[i][j] == '#') ? false : true);
  for (int i = 2; i < n; i++) {
    L[i][2] = 1;
    for (int j = 3; j < m; j++) L[i][j] = (a[i][j] == '#') ? j : L[i][j - 1];
    R[i][m - 1] = m;
    for (int j = m - 2; j > 1; j--)
  ...
  for (int i = 2; i < m; i++) {
    U[2][i] = 1;
    for (int j = 3; j < n; j++) U[j][i] = (a[j][i] == '#') ? j : U[j - 1][i];
    D[n - 1][i] = n;
    for (int j = n - 2; j > 1; j--)
