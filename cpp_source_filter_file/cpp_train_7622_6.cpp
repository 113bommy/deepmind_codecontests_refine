...
struct static_mint {
  ...
  friend static_mint operator+(static_mint lhs, const static_mint &rhs) {
    lhs += rhs;
    return lhs;
  }
  friend static_mint operator-(static_mint lhs, const static_mint &rhs) {
...
struct dynamic_mint {
  ...
  friend dynamic_mint operator+(dynamic_mint lhs, const dynamic_mint &rhs) {
    lhs += rhs;
    return lhs;
  }
  friend dynamic_mint operator-(dynamic_mint lhs, const dynamic_mint &rhs) {
...
struct segment_tree {
  struct node {
    ...
    friend node operator+(const node &x, const node &y) {
      ...
      res.sum_r = x.sum_r + two[x.sz] * y.sum_l;
      return res;
    }
  };
  int n;
  ...
  node query(int ql, int qr) { return query_helper(ql, qr, 1, 0, n - 1); }
  node query_helper(int ql, int qr, int i, int l, int r) {
    if (r < ql || qr < l) return node();
    if (ql <= l && r <= qr) return t[i];
    int mid = (l + r) >> 1;
    node x = query_helper(ql, qr, i << 1, l, mid),
