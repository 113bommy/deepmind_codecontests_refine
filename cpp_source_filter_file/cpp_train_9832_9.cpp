#include <bits/stdc++.h>
using namespace std;
long long n, m, pa[111111], ans, dep[111111], an[111111][22], mx[111111][22], q,
    x, y, num;
...
long long get(long long x, long long y) {
  ...
  ans = max(ans, max(an[x][0], an[y][0]));
  return ans;
}
int main() {
  scanf("%I64d%I64d", &n, &m);
  for (long long i = 1; i <= m; i++) {
    scanf("%I64d%I64d%I64d", &ed[i].second.first, &ed[i].second.second,
          &ed[i].first);
  }
  sort(ed + 1, ed + m + 1);
  ...
  for (long long i = 1; i <= m; i++) {
    if (Union(ed[i].second.first, ed[i].second.second)) {
      ans += ed[i].first;
      g[ed[i].second.first].push_back(
          make_pair(ed[i].second.second, ed[i].first));
      g[ed[i].second.second].push_back(
  ...
  while (q--) {
    scanf("%I64d%I64d", &x, &y);
    if (num < n - 2 || (num == n - 2 && root(x) == root(y)))
      printf("-1\n");
    else if (num == n - 2)
      printf("%I64d\n", ans);
    else
