...
long long calculate(int c00, int c01, int c10, int c11, int head) {
  ...
  int L = c10 + c01 + 1;
  if (c01 == k || c01 == k - 1) {
    return candy((L + 1) / 2, c11 + 1) * candy(L / 2, c00) % MOD;
  }
  return 0;
}
...
int main() {
  ...
  cout << (check(s1, c00, c01, c10, c11) + solve(s1, c00, c01, c10, c11) -
           solve(s0, c00, c01, c10, c11) + 1LL * MOD * MOD) %
              MOD;
}
