...
long long dijkstra(int src, int dst) {
  ...
  static int last_v[MAXN], last_e[MAXN];
  ...
  dis[src] = 0;
  q.push(std::pair<long long, int>(0, src));
  static std::pair<long long, int> r;
  while (!q.empty()) {
    do {
      r = q.top();
      q.pop();
    } while (!q.empty() && dis[r.second] != r.first);
    ...
    for (int w = head[r.second]; w != -1; w = next[w])
      if (dis[dest[w]] > r.first + len[w]) {
        ...
        last_e[dest[w]] = w;
        q.push(std::pair<long long, int>(dis[dest[w]], dest[w]));
      }
  }
...
int main() {
  ...
  int u = 0;
  for (std::map<int, int>::iterator i = keypts.begin(); i != keypts.end();
       ++i, ++u)
    i->second = u;
  for (int i = 0; i < n; ++i) {
    t[i] += p[i];
  ...
  for (int i = 0; i < n; ++i)
    if (p[i] >= src) add_edge(p[i], d[i], t[i], i);
  std::map<int, int>::iterator i = keypts.begin();
  for (++i, u = 1; i != keypts.end(); ++i, ++u)
    if (u - 1 >= src) {
  ...
  printf("%I64d\n", dijkstra(src, dst));
  printf("%d\n", (int)path_tags.size());
  for (std::vector<int>::reverse_iterator i = path_tags.rbegin();
       i != path_tags.rend(); ++i)
    printf("%d%c", *i, (i + 1 == path_tags.rend()) ? '\n' : ' ');
  return 0;
}
