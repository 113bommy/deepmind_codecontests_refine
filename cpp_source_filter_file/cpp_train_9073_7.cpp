...
PT origPts[100005];
int n;
pair<double, double> v[200005];
bool almostEqual(double a, double b) { return fabs(a - b) < 1e-10; }
bool can(int allowed, double r) {
  ...
  sort(v, v + idx, cmp);
  {
    stack<pair<double, double> > clean;
    for (int i = 0; i < idx; i++) {
      pair<double, double> out = v[i];
      while (!clean.empty() && out.first >= clean.top().first &&
             out.second <= clean.top().second)
        clean.pop();
      clean.push(out);
    }
    idx = 0;
  ...
  {
    stack<pair<double, double> > superclean;
    int j = 0;
    for (int i = 0; i < idx; i++) {
      while (j < idx && v[j].first - v[i].first <= 2 * PI - 1e-10) j++;
      if (j < idx && almostEqual(v[j].first - v[i].first, 2 * PI) &&
          almostEqual(v[j].second - v[i].second, 2 * PI)) {
        superclean.push(v[i]);
      }
    }
    idx = 0;
  ...
  for (int i = 0; i < idx; i++) {
    ...
    while (lhs != rhs) {
      ...
      if (v[i].second >= v[j].first && v[i].second <= v[j].second) {
        lhs = mid;
      } else if (v[i].second >= v[j].first + 2 * PI &&
                 v[i].second <= v[j].second + 2 * PI) {
        lhs = mid;
      } else if (v[i].second >= v[j].first - 2 * PI &&
                 v[i].second <= v[j].second - 2 * PI) {
        lhs = mid;
  ...
  for (int d = 1; d < MAX_D; d++) {
    for (int i = 0; i < idx; i++) {
      inc[i][d] = inc[i][d - 1] + inc[(i + inc[d - 1][i]) % idx][d - 1];
    }
  }
  for (int i = 0; i < idx; i++) {
    int curr = i;
