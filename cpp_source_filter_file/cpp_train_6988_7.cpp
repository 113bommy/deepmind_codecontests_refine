...
int arr[20][N], mem[(1 << 16)][17], n, m, preMn[17][17], preBound[17][17];
int solve(int mask, int prv, int start) {
  if (mask + 1 == (1 << n)) return 1e9;
  int& ret = mem[mask][start];
  if (~ret) return ret;
  ret = 0;
  for (int i = 0; i < n; ++i) {
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      if (i == j) continue;
      int val = OO;
      for (int k = 0; k < m; ++k) val = min(val, abs(arr[i][k] - arr[j][k]));
      preMn[i][j] = val;
      val = OO;
      for (int k = 1; k < m; ++k)
  ...
  int ret = OO;
  if (n == 1) {
    for (int j = 1; j < m; ++j) ret = min(ret, abs(arr[0][j] - arr[0][j - 1]));
    cout << ret;
    return 0;
  }
  cout << solve(0, 0, n);
