...
inline bool cmp1ofQuery(Query const &lhs, Query const &rhs) {
  return lhs.l < rhs.l;
}
inline bool cmp2ofQuery(Query const &lhs, Query const &rhs) {
  return lhs.r > rhs.r;
}
Query q[maxn];
int cl[maxn], cr[maxn];
...
long long tri[maxn], tr0[maxn];
inline int lowbit(int x) { return x & -x; }
inline void add(long long *ar, int p, int v) {
  while (p <= n) {
    ar[p] += v;
    p += lowbit(p);
  }
}
inline long long query(long long *ar, int p) {
  int sum = 0;
  while (p > 0) {
    sum += ar[p];
    p -= lowbit(p);
...
inline long long query(long long *ar, int l, int r) {
  return query(ar, r) - query(ar, l - 1);
}
long long ans[maxn];
int main() {
  scanf("%d %d", &n, &m);
  ...
  for (int i = 1; i <= m; ++i) {
    ...
    long long v =
        query(tri, q[i].l, q[i].r) + query(tr0, q[i].l, q[i].r) * q[i].l;
    ans[q[i].id] -= v;
  }
  ...
  for (int i = 1; i <= m; ++i) {
    ...
    long long v =
        query(tri, q[i].l, q[i].r) + query(tr0, q[i].l, q[i].r) * q[i].r;
    ans[q[i].id] += v;
  }
