...
struct Paint_Board {
  ...
  Paint_Board(int n, int m)
      : n(n), m(m), a(n + 2, vector<int>(m + 2)), u(a), d(a), l(a), r(a) {}
  ...
  void set_line(int x, int clr) {
    for (int j = 1; j <= m; ++j) set_clr(x, j, clr);
  }
  void set_line(int x, int clr1, int clr2) {
    for (int j = 1; j <= m; ++j) set_clr(x, j, (j & 1) ? clr1 : clr2);
  }
  ...
  void print(void) {
    for (int i = 1; i <= n; ++i) {
      for (int j = 1; j <= m; ++j) printf("%s", uhalf[a[i][j]]);
      putchar('\n');
      for (int j = 1; j <= m; ++j) printf("%s", dhalf[a[i][j]]);
      putchar('\n');
    }
  }
};
int main(void) {
  ...
  board.set_line(0, 1);
  int bl = (b + m - 1) / m;
  for (int i = 1; i <= bl; ++i) board.set_line(i, 1);
  if (b % m) {
    for (int i = b % m + 1; i <= m; ++i) board.set_lu(bl, i);
    board.set_clr(bl + 1, 0, ((b % m) & 1) ? 1 : 2);
    for (int i = 1; i <= b % m; ++i) board.set_lu(bl + 1, i);
  } else
    board.set_line(bl + 1, 3, 4);
  for (int i = b % m ? bl + 1 : bl + 2; i <= n; ++i)
    for (int j = (i == bl + 1 ? b % m + 1 : 1); j <= m; ++j) {
      if (w)
        board.set_clr(i, j, 2), --w;
      else
        board.set_lu(i, j);
    }
  board.print();
