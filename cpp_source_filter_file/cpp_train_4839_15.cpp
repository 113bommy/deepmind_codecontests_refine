...
class fenwick {
 public:
  ...
  long long getRange(int i, int j) { return getSum(j) - getSum(i - 1); }
  void set(int i, int dif) {
    for (++i; i < size; i += (i & -i)) T[i] += dif;
  }
...
int main() {
  ...
  for (int j = 1; j < C; j++)
    for (int i = 0; i < R; i++) {
      if (A[i][j] == '#')
        ...
        blockLeft[i][j] = blockLeft[i][j - 1];
    }
  for (int i = 0; i < R; i++) {
    if (A[i][C - 1] == '#')
      blockRight[i][C - 1] = C - 1;
    else
      blockRight[i][C - 1] = C;
  }
  for (int j = (C - 2); j >= 0; j--)
    for (int i = 0; i < R; i++) {
      if (A[i][j] == '#')
        ...
        blockRight[i][j] = blockRight[i][j + 1];
    }
  for (int i = 0; i < R; i++) {
    for (int j = 0; j < C; j++) {
  ...
  for (int i = 1; i < (R - 1); i++) {
    for (int j = 1; j < (C - 1); j++) {
      ...
      if (bt == (R - 1)) bt--;
      if (bt < tp) continue;
      total += fColLeft[j].getRange(bt, tp);
    }
  }
  for (int i = 1; i < (R - 1); i++) {
    for (int j = 1; j < (C - 1); j++) {
      ...
      if (bt == (R - 1)) bt--;
      if (bt < tp) continue;
      total += fColRight[j].getRange(bt, tp);
    }
  }
  for (int i = 1; i < (R - 1); i++)
    if ((blockLeft[i][0] == -1) && (blockRight[i][0] == C)) total++;
  for (int j = 1; j < (C - 1); j++)
    if ((blockTop[0][j] == -1) && (blockBot[0][j] == R)) total++;
  cout << total << endl;
  return 0;
}
