...
int rightMost(int p, int l, int r, LL v) {
  if (r - l == 1) return t[p].minp <= v ? l : -1;
  int mid = (l + r) / 2;
  pushdown(p);
  if (t[2 * p + 1].minp <= v) return rightMost(2 * p + 1, mid, r, v);
  return rightMost(2 * p, l, mid, v);
}
int query(int p, int l, int r, LL tmp) {
  if (r - l == 1) return t[p].minp + tmp <= k ? l : -1;
  int mid = (l + r) / 2;
  pushdown(p);
  if (t[2 * p].mx >= tmp) {
    if (t[p].ans <= k)
    ...
    return max(rightMost(2 * p, l, mid, k - tmp),
               query(2 * p + 1, mid, r, tmp));
  }
}
void dfs(int u) {
  stk.push_back(u);
  if (nxt[u] != n) rangeAdd(1, 0, n, nxt[u] - 1, n, pre[u] - pre[nxt[u]]);
  if (u != n) {
  ...
  for (int v : e[u]) dfs(v);
  if (nxt[u] != n) rangeAdd(1, 0, n, nxt[u] - 1, n, pre[nxt[u]] - pre[u]);
  stk.pop_back();
}
int main() {
  ...
  for (int i = 1; i < n; ++i) pre[i] = pre[i - 1] + g[i - 1] - w[i - 1];
  for (int i = 1; i < n; ++i) suf[i] = suf[i - 1] + g[i] - w[i - 1];
  stk.push_back(n);
  pre[n] = -INF;
  nxt[n] = n;
  for (int i = n - 1; i >= 0; --i) {
    while (pre[i] <= pre[stk.back()]) stk.pop_back();
    nxt[i] = stk.back();
    e[nxt[i]].push_back(i);
    stk.push_back(i);
  }
