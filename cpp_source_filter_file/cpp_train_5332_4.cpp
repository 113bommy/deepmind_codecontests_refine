...
void AddEdge(int u, int v, int w, int c) {
  E[u].push_back(xb);
  edges[xb++] = (Edge){u, v, w, c};
  E[v].push_back(xb);
  edges[xb++] = (Edge){v, u, -w, 0};
}
...
bool BFS() {
  ...
  while (q.size()) {
    ...
    for (int e : E[u]) {
      int v, w, c;
      tie(v, w, c) = make_tuple(edges[e].v, edges[e].w, edges[e].c);
      if (c && ds[v] > ds[u] + w) {
        ds[v] = ds[u] + w;
        go[v] = e;
...
int main() {
  ...
  long long ans = MinCostMaxFlow() + (long long)cnt * inf;
  if (ans >= inf)
    printf("-1\n");
  else {
    printf("%lld\n", ans);
    for (int i = 1; i <= m; i++)
      printf("%c", edges[mao[i]].c == 0       ? 'R'
                   : edges[mao[i] + 2].c == 0 ? 'B'
                                              : 'U');
    printf("\n");
  }
