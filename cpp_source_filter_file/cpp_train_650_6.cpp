...
int n, Q, K, p3[200005], p3t, sz[100005], fa[100005];
struct node {
  int fg, x, y;
} q[100005];
vector<pair<int, int> > T[400005];
struct dsub {
  int x, y, sz;
} ds[100005];
int dt = 0;
...
inline void pop(int q) {
  while (dt > q) fa[ds[dt].x] = ds[dt].x, sz[ds[dt].y] -= sz[ds[dt].x], dt--;
}
inline void modif(int x, int l, int r, int dl, int dr, pair<int, int> dc) {
  if (l > dr || dl > r)
    return;
  else if (dl <= l && r <= dr)
    return T[x].push_back(dc);
  modif(x << 1, l, (l + r) >> 1, dl, dr, dc),
      modif(x << 1 | 1, ((l + r) >> 1) + 1, r, dl, dr, dc);
}
inline void solve(int x, int l, int r) {
  int ww = dt;
  for (auto z : T[x]) mrg(z.first, z.second);
  if (l == r)
    return (q[l].fg == 2 ? printf("%d\n", sz[getf(q[l].x)]) : 0), pop(ww);
  solve(x << 1, l, (l + r) >> 1), solve(x << 1 | 1, ((l + r) >> 1) + 1, r),
      pop(ww);
}
int main() {
  ...
  for (int i = 1; i <= Q; i++)
    read(q[i].fg), (q[i].fg < 3 ? read(q[i].x), 0 : 0),
        (q[i].fg < 2 ? read(q[i].y), 0 : 0);
  p3[++p3t] = 1;
  for (int i = 1; i <= Q; i++)
    if (q[i].fg == 3) p3[++p3t] = i;
