...
void plant(int v, int p = -1) {
  ...
  for (auto u : adj[v])
    if (u != p) {
      par[u][0] = v;
      for (int i = 1; i < maxb; i++) par[u][i] = par[par[u][i - 1]][i - 1];
      h[u] = h[v] + 1;
      plant(u, v);
    }
  ft[v] = now;
...
void upd(int p, int val) {
  for (p += 5; p < maxn; p += ((p) & (-(p)))) fen[p] += val;
}
int get(int p) {
  int res = 0;
  for (p += 5; p > 0; p -= ((p) & (-(p)))) res += fen[p];
  return res;
}
...
void handle() {
  ...
  dfs(0);
  for (int i = 0; i < maxn; i++) asko[i].clear(), askc[i].clear();
}
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    cin >> p;
    p--;
    if (p >= 0) adj[p].push_back(i), adj[i].push_back(p);
  }
  plant(0);
  cin >> q;
  for (int i = 0; i < q; i++) {
    ...
    if (type[i] == 1) {
      ...
      if (Lca[i] == a[i]) {
        ...
        askc[shit].push_back({y[i], i});
        asko[b[i]].push_back({y[i], i});
      } else if (Lca[i] == b[i]) {
        int shit = go(a[i], h[a[i]] - h[b[i]] - 1);
        askc[shit].push_back({y[i], i});
        asko[a[i]].push_back({y[i], i});
      } else {
        askc[Lca[i]].push_back({y[i], i});
        asko[a[i]].push_back({y[i], i});
      }
    }
  }
  ...
  for (int i = 0; i < q; i++)
    if (type[i] == 2) {
      ...
      if (a[i] == Lca[i])
        ...
        if (tmp[i] < k[i]) {
          k[i] -= tmp[i];
          int shit = go(b[i], h[Lca[i]] - h[b[i]] - 1);
          a[i] = shit;
        } else
          b[i] = Lca[i];
      }
  ...
  for (int i = maxb - 1; i >= 0; i--) {
    for (int j = 0; j < q; j++)
      if (type[j] == 2 && ans[j] != -1 && abs(h[b[j]] - h[a[j]]) > (1 << i)) {
        if (is_par(a[j], b[j])) {
          int shit = par[b[j]][i];
          askc[a[j]].push_back({y[j], j});
          asko[shit].push_back({y[j], j});
        } else {
          int shit = par[a[j]][i];
          askc[shit].push_back({y[j], j});
          asko[a[j]].push_back({y[j], j});
        }
      }
    handle();
    for (int j = 0; j < q; j++)
