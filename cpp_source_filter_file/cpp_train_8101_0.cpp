...
int N, l[maxN], e, prev[maxN], q1[maxN], q2[maxN], ed, q[maxN];
...
int dfs(int u, int fa, int& ans) {
	ans = u; int maxx = 0; prev[u] = 0;
	for(int i = l[u]; i != -1; i = G[i].fa) {
		int v = G[i].v;
		if(v == fa) continue; 
		int tmp, t;
		if((t = dfs(v, u, tmp)) + 1 > maxx) 
		maxx = t + 1, ans = tmp, prev[u] = v;
	}
	return maxx;
}
...
int main() {
	...
	dfs(1, -1, u); dfs(u, -1, v);
	//cout <<u<<' '<<v<<endl; 
	for(int i = u; i; i = prev[i]) caterpillar[i] = 1, q[++ed] = i;
	if(solve(u, q1) < N) {
		printf("-1\n");
		return 0;
	} 
	for(int i = 1; i * 2 <= N; ++i) swap(q[i], q[N - i + 1]);
	solve(v, q2);
	if(cmp()) {
