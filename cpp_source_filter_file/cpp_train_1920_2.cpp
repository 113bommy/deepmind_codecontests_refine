...
struct node {
  int id, v;
  inline bool operator<(const node &tmp) const { return v < tmp.v; }
} Node[M][3];
...
int main() {
  ...
  for (int i = 1, a, b; i ^ n; ++i)
    rd(a), rd(b), G[a].push_back(b), G[b].push_back(a);
  pret();
  for (cin >> m; m--;) {
    int u, v;
    rd(u), rd(v);
    if (d[u] < d[v]) swap(u, v);
    int w = lca(u, v), g = up(u, (d[u] + d[v] - d[w] * 2 - 1) >> 1);
    int res = max(dx[u] - d[u], q_down(u, d[u] - d[g]) + d[u]);
    Max(res, q_down(w, d[w]) + d[v]);
    if (w == v)
      Max(res, q_up(g, d[g] - d[v]) - d[v]);
    else {
      Max(res, dx[v] - d[v]);
      Max(res, q_up(g, d[g] - d[w] - 1) + d[v] - d[w] * 2);
      Max(res, q_down(u, d[v] - d[w] - 1) + d[v]);
