...
int n, m, k, n1, n2, n3, hd[N], cnt, col[N], num[N][2], p[N][N], node[N], ind,
    two[N];
...
int main() {
  ...
  read(n2);
  read(n3);
  for (int i = 0, i_end = n; i < i_end; ++i) hd[i] = -1;
  ...
  for (int i = 0, i_end = n; i < i_end; ++i)
    if (!vis[i]) {
      ...
      for (int j = 0, j_end = n; j < j_end; ++j) {
        f[k + 1][j] = false;
        if (j >= num[k][0] && f[k][j - num[k][0]]) f[k + 1][j] = true;
        if (j >= num[k][1] && f[k][j - num[k][1]])
          f[k + 1][j] = true, p[k + 1][j] = 1;
      }
      ++k;
    }
  ...
  for (int i = 0, j = 0, l; i < k; j += l, ++i) {
    l = num[i][0] + num[i][1];
    for (int p = j, p_end = j + l; p < p_end; ++p)
      if (col[node[p]] == two[i]) col[node[p]] = 2;
  }
  ...
  puts("YES");
  for (int i = 0, i_end = n; i < i_end; ++i) putchar('0' + col[i]);
  putchar('\n');
  return 0;
}
