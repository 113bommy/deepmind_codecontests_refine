...
struct Segment_Tree {
  ...
  void modify(int &p, int pos, int l, int r) {
    ...
    if (l == r) return;
    if (pos <= ((l + r) >> 1))
      modify(son[0][p], pos, l, ((l + r) >> 1));
    else
      modify(son[1][p], pos, ((l + r) >> 1) + 1, r);
  }
  int merge(int x, int y, int l, int r) {
    if (!x || !y || l == r) return x | y;
    int p = ++verx;
    son[0][p] = merge(son[0][x], son[0][y], l, ((l + r) >> 1));
    son[1][p] = merge(son[1][x], son[1][y], ((l + r) >> 1) + 1, r);
    return p;
  }
  bool query(int p, int lr, int rr, int l, int r) {
    if (lr > r || l > rr || !p) return false;
    if (lr <= l && r <= rr) return true;
    if (lr <= ((l + r) >> 1) && query(son[0][p], lr, rr, l, ((l + r) >> 1)))
      return true;
    if (rr > ((l + r) >> 1) && query(son[1][p], lr, rr, ((l + r) >> 1) + 1, r))
      return true;
    return false;
  }
} Q;
struct SAM {
  ...
  void insert(int c) {
    ...
    if (!p)
      ...
      if (len[q] == len[p] + 1)
        ...
        int clone = ++ver;
        len[clone] = len[p] + 1;
        for (register int i = 0; i < 26; ++i) nxt[i][clone] = nxt[i][q];
        link[clone] = link[q];
        while (p && nxt[c][p] == q) {
          nxt[c][p] = clone;
          p = link[p];
  ...
  void build() {
    for (register int i = 2; i <= ver; ++i) v[link[i]].push_back(i);
  }
  void dfs(int p) {
...
int main() {
  ...
  n = strlen(s + 1);
  T.reset();
  for (register int i = 1; i <= n; ++i) T.insert(s[i] - 'a');
  T.build();
  T.dfs(1);
  for (register int t = read(); t; --t) T.query();
  return 0;
}
