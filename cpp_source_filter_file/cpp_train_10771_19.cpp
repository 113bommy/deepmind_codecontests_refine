...
void update(int v, int l, int r, int pos, int num, int id) {
  if (l == r)
    ...
    int mid = (l + r) / 2;
    if (pos <= mid)
      update(2 * v, l, mid, pos, num, id);
    else
      update(2 * v + 1, mid + 1, r, pos, num, id);
...
int query(int v, int l, int r, int lo, int hi, int id) {
  ...
  int mid = (l + r) / 2;
  return query(2 * v, l, mid, lo, min(mid, hi), id) +
         query(2 * v + 1, mid + 1, r, max(mid + 1, lo), hi, id);
}
