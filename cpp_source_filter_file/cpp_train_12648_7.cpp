...
bool check(int i, int j, int k) {
  return (long long)(b[i] - b[j]) * (a[k] - a[j]) <
         (long long)(b[j] - b[k]) * (a[j] - a[i]);
}
void build(const std::vector<int> &order, std::deque<int> &block) {
  ...
  for (int i : order) {
    if (!used[i]) {
      ...
      while ((int)block.size() >= 2 &&
             !check(block[(int)block.size() - 2], block.back(), i)) {
        block.pop_back();
      }
      block.push_back(i);
    }
...
int main() {
  ...
  long long max = 0;
  for (int _ = 0; _ < n; ++_) {
    std::pair<long long, int> best(-(long long)1e18, 0);
    for (int i = 0; i < m; ++i) {
      int x = count[i];
      while ((int)block[i].size() >= 2) {
        int u = block[i][0];
        int v = block[i][1];
        if ((long long)a[u] * x + b[u] > (long long)a[v] * x + b[v]) {
          break;
        }
        block[i].pop_front();
      }
      if (!block[i].empty()) {
        int u = block[i].front();
        best = std::max(
            best, std::make_pair((long long)a[u] * x + b[u] + delta[i], u));
      }
    }
    sum += best.first;
    max = std::max(max, sum);
