...
int getpFast(int a) { return (a == p[a] ? a : p[a] = getpFast(p[a])); }
...
void rebuild(int l) {
  int r = min(m, l + P);
  for (int i = 0; i < int(n); i++) p[i] = i, rk[i] = 1;
  ...
  for (int i = l; i < r; ++i)
    if (q[i].t == 1) used[q[i].e0] = used[q[i].e1] = false;
  cur.clear();
  cnt = 0;
  for (int i = 0; i < int(l); i++)
    if (q[i].t == 1) {
...
int main() {
  ...
  for (int i = 0; i < int(m); i++) {
    ...
    if (q[i].t == 1) {
      ...
      for (auto e : cur) unite(edges[e].v, edges[e].u);
