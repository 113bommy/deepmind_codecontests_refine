...
struct LiChaoTree {
  ...
  LiChaoTree(const vector<T> &xs_, const T TINF, bool is_minimized = true)
      : xs(xs_), TINF(TINF), is_minimized(is_minimized) {
  ...
  void add(T a, T b, T left, T right) {
    int l = lower_bound((xs).begin(), (xs).end(), left) - xs.begin(),
        r = lower_bound((xs).begin(), (xs).end(), right) - xs.begin();
  ...
  T query(T x) {
    int node = lower_bound((xs).begin(), (xs).end(), x) - xs.begin();
    node += n;
    T res = dat[node].f(x);
    while (node >>= 1) {
      if (is_minimized) {
  ...
  function<void(int)> dfs = [&](int root) {
    ...
    for (int i = (0); i < (m); ++i) {
      ...
      for (auto pr : psum_b[i])
        chmax(ans, pr.first + 1LL * a[root] * pr.second);
    }
    if (!xs.empty()) {
      sort((xs).begin(), (xs).end());
      unique(xs);
      ...
      for (int i = (0); i < (m); ++i) {
        int sz = sum_a[i].size();
        for (int j = (0); j < (sz); ++j)
          chmax(ans, lct1.query(sum_a[i][j]) + psum_a[i][j]);
        for (int j = (0); j < (sz); ++j)
          lct1.add(psum_b[i][j].second, psum_b[i][j].first);
      }
      LiChaoTree<ll> lct2(xs, LINF, false);
      for (int i = m - 1; i >= 0; --i) {
        int sz = sum_a[i].size();
        for (int j = (0); j < (sz); ++j)
          chmax(ans, lct2.query(sum_a[i][j]) + psum_a[i][j]);
        for (int j = (0); j < (sz); ++j)
          lct2.add(psum_b[i][j].second, psum_b[i][j].first);
      }
    }
