...
struct node2 {
  int a, b;
  pair<int, int> w;
  node2(int x = 0, int y = 0, pair<int, int> z = make_pair(0, 0)) {
    a = x, b = y, w = z;
  }
};
bool operator<(node2 a, node2 b) { return a.w > b.w; }
...
pair<int, int> d2[MAXN][2];
void Bfs2() {
  for (int i = 1; i <= n; i++) d2[i][0] = d2[i][1] = make_pair(INF, INF);
  static bool vis[MAXN][2];
  d2[1][0] = make_pair(0, 0);
...
int main() {
  ...
  Bfs1();
  if (ans < INF && 0) return printf("%d\n", ans), 0;
  Bfs2();
  if (d2[n][0] < d2[n][1])
    ans = Fstpw(2, d2[n][0].first) + d2[n][0].second;
  else
    ans = Fstpw(2, d2[n][1].first) + d2[n][1].second;
  ans = (ans - 1) % Mod;
  printf("%d\n", ans);
  return 0;
}
