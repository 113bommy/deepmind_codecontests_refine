...
struct tree_t {
  ...
  int fe(int x) { return *e[x].begin(); }
  void prep() {
    for (int i = 1; i <= n; i++) {
      vs.insert(i);
      ds.insert(make_pair(d[i], i));
      if (d[i] == 1) lv.insert(make_pair(fe(i), i));
    }
  }
  int ojbk() {
  ...
  pair<int, int> find() {
    ...
    int nd = 2;
    auto it = lv.begin();
    while (nd--) ps.push_back(*it), it++;
    nd = 2;
    auto ti = lv.rbegin();
    while (nd--) ps.push_back(*ti), ti++;
    int u = -1;
    for (auto i : ps) {
      int j = i.first;
      if (d[j] > 2) {
    ...
    if (~u) {
      ...
      assert(ps.size() > 1);
      int a = ps[0].second, b = ps.back().second;
      return make_pair(a, b);
    }
  }
  void de(int x) {
    ...
    if (d[y] == 1) lv.insert(make_pair(fe(y), y));
  }
} A, B;
...
void solve0(int n) {
  ...
  do {
    ...
    if (ok) {
      for (int i = 0; i < n; i++) ans[vx[i]] = vy[i];
      return;
    }
...
void solve1(tree_t &A, tree_t &B) {
  ...
  vector<int> tt;
  for (auto x : B.vs)
    if (x != u_ && x != v_ && x != w_) tt.push_back(x);
  int t_ = 0;
  for (auto x : A.vs)
    if (x != u && x != v && x != w) ans[x] = tt[t_++];
}
void work() {
  int cur_n = A.vs.size();
  if (cur_n <= 5) {
  ...
  if (B.ojbk() == 1) {
    solve1(B, A);
    for (int i = 1; i <= n; i++) rev[ans[i]] = i;
    for (int i = 1; i <= n; i++) ans[i] = rev[i];
    return;
  }
  int a, b, c, d;
  tie(a, b) = A.find();
  ...
  work();
  ans[a] = d, ans[b] = c;
  if (ans[e] != g && ans[f] != h) swap(ans[a], ans[b]);
}
bool validate() {
  for (int i = 1; i <= n; i++)
    for (int j = 1; j < i; j++) {
