...
class Point {
 public:
  long long x;
  long long y;
  Point(long long _x = 0, long long _y = 0) : x(_x), y(_y) { return; }
  Point operator-(Point that) {
    return Point(this->x - that.x, this->y - that.y);
  }
  long long Cross(Point that) { return this->x * that.y - this->y * that.x; }
};
int a[200020];
long long s[200020];
Point f[200020];
long long BSFind0(int c, int v) {
  int l, m, r;
  if (c == 1) return (long long)v * f[0].x - f[0].y;
  for (l = 1, r = c; l + 1 < r;) {
    m = (l + r) >> 1;
...
int main(void) {
  ...
  for (i = n; i >= 1; i--) {
    ...
    while (c > 1 &&
           (Point(i, s[i - 1]) - f[c - 1]).Cross(f[c - 2] - f[c - 1]) >= 0)
      c--;
    f[c++] = Point(i, s[i - 1]);
  }
  for (i = 1; i <= n; i++) o += (long long)i * a[i];
  printf("%I64d\n", o);
  return 0;
}
