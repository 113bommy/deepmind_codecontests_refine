...
int main() {
  ...
  do {
    ...
    for (int i = 0; i < m; i++) {
      ...
      for (int j = 0; j < i; j++) {
        if (disallow[perm[i]][perm[j]]) color_forbidden[color[i]] = true;
      }
      int c = find(color_forbidden.begin(), color_forbidden.end(), false) -
              color_forbidden.begin();
      cnt = max(c + 1, cnt);
      color[i] = c;
    }
    mn = min(mn, cnt);
