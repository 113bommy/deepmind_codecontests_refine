...
void cleararr(long long *arr, int n) {
  for (int i = 0; i <= n; i++) arr[i] = 0;
}
bool solve(int n, int whattoadd, int scount) {
  ...
  cleararr(sum, vp2);
  cleararr(frec, vp2);
  for (int i = 1; i <= vp2; i++) sum[i] = sum[i - 1] + v2[i];
  long long result = 0, acc = 0;
  for (int i = vp2; 1 <= i; i--) {
    if (sum[i] <= i * (i - 1) + result) {
      acc += frec[i];
      result -= acc;
      frec[v2[i]]++;
      if (i <= v2[i]) acc++;
...
int main() {
  ...
  if (solve(n, 1, sadd) == 0) {
    ...
    for (int jump = (1 << 18); 1 < jump; jump /= 2) {
      if (x + jump <= sadd) {
        if (solve(n, 1, sadd - jump - x) == 1) x += jump;
      }
    }
    sadd -= x;
    if (solve(n, sadd, 1) == 0) {
      ...
      x = 0;
      for (int jump = (1 << 18); 1 < jump; jump /= 2) {
        if (solve(n, sadd + x + jump, 1) == 1) x += jump;
      }
      for (int i = 0; i <= x; i += 2) {
        cout << sadd + i << " ";
      }
