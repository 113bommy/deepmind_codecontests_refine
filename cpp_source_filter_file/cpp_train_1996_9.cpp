...
int n, t[100020], fac[100020], invfac[100020];
int C(int n, int m) { return mul(fac[n], mul(invfac[n - m], invfac[m])); }
int cv = 1;
long long T, lwb = 0, upb = 0, mb = 0;
int main() {
  read(n);
  read(T);
  fac[0] = 1;
  for (int i = 1; i <= 100010; i++) fac[i] = mul(i, fac[i - 1]);
  invfac[100010] = pow(fac[100010], mod - 2);
  for (int i = 100010 - 1; i >= 0; i--) invfac[i] = mul(i + 1, invfac[i + 1]);
