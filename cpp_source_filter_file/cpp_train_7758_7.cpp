...
using ll = long long;
template <const int MAXV, const int MAXE, class unit, const bool SCALING>
struct DinicMaxFlow {
  unit INF, EPS;
  DinicMaxFlow(unit INF, unit EPS) : INF(INF), EPS(EPS) {}
  struct Edge {
    int from, to;
    unit origCap, cap, maxCap;
    int rev;
    char isRev;
    Edge() {}
  ...
  int E, cur[MAXV], level[MAXV], q[MAXV], st[MAXV], deg[MAXV], ord[MAXE * 2],
      ind[MAXE * 2];
  bool cut[MAXV];
  Edge e[MAXE * 2];
  unit maxFlow, minCut, maxCap;
  void addEdge(int v, int w, unit vw, unit wv = 0) {
    e[E++] = Edge(v, w, vw, max(vw, wv), 0);
    e[E++] = Edge(w, v, wv, max(vw, wv), 1);
    e[E - 2].rev = E - 1;
    e[E - 1].rev = E - 2;
    deg[v]++;
    deg[w]++;
  ...
  bool bfs(int V, int s, int t, unit lim, char r) {
    ...
    while (front < back && level[t] == -1) {
      int v = q[front++];
      for (int i = st[v];
           i < st[v] + deg[v] && e[i].maxCap > lim && e[i].isRev <= r; i++) {
  ...
  void inferMinCutDfs(int v) {
    ...
    for (int i = st[v]; i < st[v] + deg[v]; i++)
      if (e[i].cap > EPS && !cut[e[i].to]) inferMinCutDfs(e[i].to);
  }
