...
int size1[400005], size2[400005], Max[400005], Min[400005], sum1[400005],
    sum2[400005], tag[2][400005];
...
int solve(int l, int r) {
  return Mod(Mod(query(1, 1, n, l, r, 0) - 1ll * sum(l, r) * r % mod + mod) +
             Mod(1ll * query(1, 1, n, l, r, 2) * r % mod -
                 query(1, 1, n, l, r, 1) + mod));
}
int main() {
  n = read();
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    change(1, 1, n, q1[tot1] + 1, i, pre[i] + 1, 0);
    q1[++tot1] = i;
    while (tot2 && suf[q2[tot2]] >= suf[i]) --tot2;
    change(1, 1, n, q2[tot2] + 1, i, suf[i] - 1, 1);
    q2[++tot2] = i;
    Max = max(Max, pre[i]);
    Add(ans, solve(Max + 1, i));
  }
  cout << ans << "\n";
  return 0;
}
