...
bool hit[8];
void bfs(int c) {
  for (int i = 0; i < N; i++) inque[i] = 0, d[c][i] = 1000000000;
  int len = 0;
  int cur = 0;
...
int main() {
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      ...
      for (int k = 0; k < lst[j].size(); k++)
        if (d[i][lst[j][k]] > low[i][j]) mask[lst[j][k]] |= (1 << i);
    }
  }
  for (int i = 0; i < N; i++) {
    cnt[A[i]][mask[i]]++;
  ...
  for (int i = 0; i < N; i++)
    for (int j = max(0, i - 16); j < i; j++) {
      int dist = 1000000000;
      for (int k = 0; k < 8; k++) dist = min(dist, d[k][i] + 1 + d[k][j]);
      ans[dist] -= 2;
      dist = min(dist, i - j);
      ans[dist] += 2;
    }
