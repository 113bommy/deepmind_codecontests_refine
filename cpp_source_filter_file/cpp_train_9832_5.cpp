...
pair<int, int> go_up(int v, int x) {
  int mx = 0;
  for (int b = 0; b < L; b++)
    if (x >> b & 1) mx = max(mx, up[v][b]), v = par[v][b];
  return {v, mx};
}
int lca(int u, int v) {
  if (h[u] < h[v]) swap(u, v);
  ...
  if (u == v) return v;
  for (int b = L - 1; ~b; --b)
    if (par[v][b] != par[u][b]) v = par[v][b], u = par[u][b];
  return par[v][0];
}
int main() {
  ...
  int p = 0;
  for (int i = 0; i < m; i++) {
    long long w = edge[i].first, id = edge[i].second;
    int u = s[id], v = t[id];
    if (unite(u, v)) {
      ans += w, p++;
      al[u].push_back({v, w});
  ...
  while (q--) {
    ...
    int lc = lca(u, v);
    cout << ans - max(go_up(u, h[u] - h[lc]).second,
                      go_up(v, h[v] - h[lc]).second)
         << '\n';
  }
  return 0;
}
