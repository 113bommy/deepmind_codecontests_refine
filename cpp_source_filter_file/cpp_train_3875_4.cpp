...
inline bool sCompare(const Lecture &a, const Lecture &b) { return a.sa < b.sa; }
inline bool eCompare(const Lecture &a, const Lecture &b) { return a.ea < b.ea; }
bool checkBad(int n) {
  for (int i = 1; i <= n; i++) {
    v1[i] = v[i];
    v2[i] = v[i];
  ...
  for (int i = 1, j = 1; i <= n; i++) {
    ...
    if (query(1, 1, m, v2[i].sb, v2[i].eb) > 0) {
      return true;
    }
  }
  return false;
