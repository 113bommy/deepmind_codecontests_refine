...
struct aho_corasic_automaton_unfixed {
  ...
  struct node {
    map<int, int> next;
    int len = 0;
    int link = -1, exit_link = -1;
    int cnt = 0, leafcnt = 0;
    long long val = 0;
  };
  vector<node> states = {{}};
  aho_corasic_automaton_unfixed(function<int(int)> convert =
                                    [](int c) { return tolower(c) - 'a'; })
      : convert(convert) {}
  template <class Str>
  aho_corasic_automaton_unfixed(
      const vector<Str> &a,
      function<int(int)> convert = [](int c) { return tolower(c) - 'a'; })
      : convert(convert) {
    insert(a), build();
  }
  void clear() { states = {{}}; }
  ...
  int insert(const Str &s) {
    ...
    for (auto C : s) {
      int c = convert(C);
      if (!states[u].next.count(c)) {
        states[u].next[c] = (int)states.size();
        states.emplace_back();
        states.back().len = states[u].len + 1;
      }
      u = states[u].next[c];
      ++states[u].cnt;
    }
  ...
  int next_state(int u, int c) {
    while (~u && !states[u].next.count(c)) u = states[u].exit_link;
    return ~u ? states[u].next[c] : 0;
  }
  void build() {
    deque<int> dq{0};
    while (!dq.empty()) {
      ...
      for (auto [c, v] : states[u].next) {
        states[v].link = next_state(states[u].link, c);
        states[v].exit_link = states[states[v].link].leafcnt
                                  ? states[v].link
                                  : states[states[v].link].exit_link;
        dq.push_back(v);
      }
    }
  }
...
struct matrix_base : vector<vector<T>> {
  using C = vector<vector<T>>;
  using C::assign;
  matrix_base &operator+=(const matrix_base &M) {
    assert(n == M.n && m == M.m);
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j) (*this)[i][j] += M[i][j];
    return *this;
  }
  ...
  matrix_base operator*(const matrix_base &M) const {
    ...
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j)
        for (auto k = 0; k < l; ++k) res[i][k] += (*this)[i][j] * M[j][k];
    return res;
  }
  matrix_base &operator*=(const matrix_base &M) { return *this = *this * M; }
  matrix_base &operator*=(T c) {
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j) (*this)[i][j] *= c;
    return *this;
  }
  ...
  matrix_base transposed() const {
    matrix_base res(m, n);
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j) res[j][i] = (*this)[i][j];
    return res;
  }
  matrix_base &transpose() { return *this = transposed(); }
  vector<T> maps_to(const vector<T> &v) const {
    assert(m == (int)v.size());
    vector<T> res(n, zero());
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j) res[i] += (*this)[i][j] * v[j];
    return res;
  }
  int n, m;
  matrix_base(int n, int m, T init_diagonal = zero(),
              T init_off_diagonal = zero())
      : n(n), m(m) {
    assign(n, vector<T>(m, zero()));
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j)
        (*this)[i][j] = i == j ? init_diagonal : init_off_diagonal;
  }
  matrix_base(const C &arr) : n((int)arr.size()), m((int)arr[0].size()) {
    assign(n, vector<T>(m, zero()));
    for (auto i = 0; i < n; ++i)
      for (auto j = 0; j < m; ++j) (*this)[i][j] = arr[i][j];
  }
};
template <class T, T (*zero)(), T (*one)()>
matrix_base<T, zero, one> operator*(T c, matrix_base<T, zero, one> M) {
  for (auto i = 0; i < M.n; ++i)
    for (auto j = 0; j < M.m; ++j) M[i][j] = c * M[i][j];
  return M;
}
template <class T>
T ZERO() {
...
template <class T>
using matrix = matrix_base<T, ZERO<T>, ONE<T>>;
struct X {
  long long data;
  ...
  X operator*(const X &otr) const { return {data + otr.data}; }
};
...
int main() {
  ...
  long long len;
  cin >> n >> len;
  aho_corasic_automaton_unfixed aut;
  vector<int> w(n);
  for (auto i = 0; i < n; ++i) {
    cin >> w[i];
  }
