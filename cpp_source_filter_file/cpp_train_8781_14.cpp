    ...
    while (true) {
      ...
      while (!Queue.empty()) {
        ...
        for (auto it = adj[u].begin(); it != adj[u].end(); ++it) {
          ...
          int newCost = Path[u] + (*it).cost;
          if (Path[(*it).vertice] > newCost) {
            Path[(*it).vertice] = newCost;
            Prev[(*it).vertice] = it;
            Delta[(*it).vertice] = min(Delta[u], (*it).lim - (*it).flow);
            if (!inQueue[(*it).vertice]) {
              inQueue[(*it).vertice] = 1;
