...
void unite(Node &res, const Node a, const Node &b) {
  ...
  for (int i = 1; i <= 3 * N; ++i) graph[i].clear(), comps[i] = 0;
  int last[2 * NMAX] = {0};
  for (int i = 1; i <= N; ++i) {
    if (last[a.comps[i][0]]) addEdge(i, last[a.comps[i][0]]);
    last[a.comps[i][0]] = i;
  }
  for (int i = 1; i <= N; ++i) {
    if (last[a.comps[i][1]]) addEdge(N + i, last[a.comps[i][1]]);
    last[a.comps[i][1]] = N + i;
  }
  for (int i = 1; i <= 2 * N; ++i) last[i] = 0;
  for (int i = 1; i <= N; ++i) {
    if (last[b.comps[i][0]]) addEdge(N + i, last[b.comps[i][0]]);
    last[b.comps[i][0]] = N + i;
  }
  for (int i = 1; i <= N; ++i) {
    if (last[b.comps[i][1]]) addEdge(2 * N + i, last[b.comps[i][1]]);
...
void build(int node, int st, int dr) {
  ...
  if (dr - st <= 1) {
    for (int i = 1; i <= 2 * N; ++i) graph[i].clear(), comps[i] = 0;
    int nd = 1;
    for (int j = st; j <= dr; ++j)
    ...
    for (int i = 1; i <= N; ++i) {
      ...
      if (st == dr) tree[node].comps[i][1] = comps[i];
    }
    if (st < dr) {
      for (int i = N + 1; i <= 2 * N; ++i) {
        if (!comps[i]) {
