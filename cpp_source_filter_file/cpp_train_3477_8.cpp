...
int main() {
  ...
  for (int i = 1; i < 200; i++) {
    for (int j = 0; j < i; j++) {
      mark[0][g[0][j] ^ g[0][n - j - 1]] = cnt;
      if (j > 0 && j < n - 1) mark[0][g[1][j - 1] ^ g[1][n - j - 2]] = cnt;
    }
    while (mark[0][g[0][i]] == cnt) g[0][i]++;
    for (int j = 0; j < i; j++) mark[1][g[1][j] ^ g[0][n - j - 1]] = cnt;
    while (mark[1][g[1][i]] == cnt) g[1][i]++;
    for (int j = 0; j < i; j++) {
      mark[2][g[2][j] ^ g[0][n - j - 1]] = cnt;
      if (j < n - 1) mark[2][g[2][j] ^ g[1][n - j - 2]] = cnt;
    }
    while (mark[2][g[2][i]] == cnt) g[2][i]++;
    for (int j = 0; j < i; j++) mark[3][g[2][j] ^ g[2][n - j - 1]] = cnt;
