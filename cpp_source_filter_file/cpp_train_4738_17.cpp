...
struct vec {
  ...
  vec operator-(const vec &k) const { return {x - k.x, y - k.y}; }
  double cross(const vec &k) const { return x * k.y - y * k.x; }
  double len() const { return sqrt(x * x + y * y); }
} gon[maxn], p, q;
int sign(const double &u) { return u < -eps ? -1 : u > eps; }
double inter(const vec &a, const vec &u, const vec &b, const vec &v) {
  return fabs(((a - b).cross(v)) / u.cross(v));
}
double solve(const vec &a, const vec &w) {
  ...
  for (int i = 0, u, v; i < n; ++i) {
    ...
    if (u == v) continue;
    it.push_back(std::make_pair(inter(a, w, gon[i], gon[(i + 1) % n] - gon[i]),
                                (u > v ? 1 : -1) * (u && v ? 2 : 1)));
  }
  ...
  for (int i = 0; i + 1 < it.size(); ++i) {
    inside += it[i].second;
    if (inside) ans += it[i + 1].first - it[i].first;
  }
  return ans * w.len();
}
int main() {
  scanf("%d%d", &n, &m);
  for (int i = 0; i < n; ++i) scanf("%lf%lf", &gon[i].x, &gon[i].y);
  for (int i = 0; i < m; ++i) {
