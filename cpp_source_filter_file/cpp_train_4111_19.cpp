...
int n, pos[Maxn], cnt[2 * Maxn], tmp_maxi[Maxn];
long long ans;
void work(int lt, int rt) {
  ...
  while (pnt2 < pnt1) cnt[tmp_maxi[pnt2] - pnt2 + n]--, pnt2++;
  maxi = tmp_maxi[mid + 1] = 0, mini = 0x3f3f3f3f;
  for (int i = mid; i >= lt; i--) tmp_maxi[i] = max(tmp_maxi[i + 1], pos[i]);
  pnt1 = pnt2 = mid;
  for (int i = mid + 1; i <= rt; i++) {
    mini = min(mini, pos[i]);
    maxi = max(maxi, pos[i]);
