...
def make_cpp_file(pid, index, code):
    with open(f'./cpp_file/cpp_test_{pid}_{index}.cpp', 'w') as f:
        f.write(code)
...
def run_test_cases(file, input_data_list, expected_output_list):
    ...
    if len(output_list) != len(input_data_list):
        with open(f'./cpp_result/exclude_test_gold.txt', "w", encoding='utf-8') as err_log_file:
...
def compile_and_test(pid_index_codes):
    ...
    compile_result = subprocess.run(
        ["g++", "-std=c++11", "-static", '-O2', "-o", compiled_file_path, f'./cpp_file/cpp_test_{pid}_{index}.cpp'],
        capture_output=True
    )
    ...
    success_check, error_rate = run_test_cases(f"cpp_test_{pid}_{index}", input_list, output_list)
    return (pid, index, success_check, error_rate, compiled_file_path)

def save_results(pid, results):
    try:
        with open(f'./cpp_test_result/cpp_test_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./cpp_test_result/cpp_test_check_{pid}.json', 'w') as f:
