...
def make_cpp_file(pid, index, code):
    with open(f'./cpp_gold_filter_file/cpp_train_{pid}_{index}.cpp', 'w') as f:
        f.write(code)
...
def compile_and_test(pid_index_codes):
    ...
    compile_result = subprocess.run(
        ["g++-7", "-std=c++11", "-static", '-O2', "-o", compiled_file_path, f'./cpp_gold_filter_file/cpp_train_{pid}_{index}.cpp'],
        capture_output=True
    )

    if compile_result.returncode != 0:
        with open(f'./cpp_result/exclude_train_gold.txt', "a", encoding='utf-8') as err_log_file:
...
def save_results(pid, results):
    try:
        with open(f'./cpp_train_gold_result/cpp_train_check_{pid}.json', 'r+') as f:
        ...
        with open(f'./cpp_train_gold_result/cpp_train_check_{pid}.json', 'w') as f:
...
if __name__ == "__main__":
    delete_all_files_in_folder('./cpp_train_gold_result')
    with open('./cpp_data/cpp_train_refine.json', 'r') as f:
